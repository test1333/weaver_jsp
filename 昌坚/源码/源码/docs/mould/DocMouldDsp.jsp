...
if(RecordSet.next()){
	if(!Util.null2String(RecordSet.getString("mouldType")).equals("0")){
		if(canEdit){
		request.getRequestDispatcher("/docs/mould/DocMouldEditExt.jsp?isdialog=2").forward(request,response);
		}else{
		request.getRequestDispatcher("/docs/mould/DocMouldDspExt.jsp?isdialog=2").forward(request,response);
		}
		return;
	}else{
	   if(canEdit){
		request.getRequestDispatcher("/docs/mould/DocMouldEdit.jsp?isdialog=2").forward(request,response);
		return;
		}
