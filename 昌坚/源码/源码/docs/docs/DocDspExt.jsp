...
%>
		    <script language=javascript>
					location="DocDsp.jsp?id=<%=docid%>&imagefileId=<%=imagefileId%>&isFromAccessory=true&isrequest=1&requestid=<%=requestid%>&desrequestid=<%=desrequestid%>&f_weaver_belongto_userid=<%=f_weaver_belongto_userid%>&f_weaver_belongto_usertype=<%=f_weaver_belongto_usertype+formmodeparamsStr%>";
		    </script> 
<%
    return ;
}
...
if(fromFlowDoc.equals("1")){
    ...
    if(!imagefileIdpdf.equals("")){
        response.sendRedirect("/docs/docs/DocDsp.jsp?id="+docid+"&requestid="+requestid+"&nodeId="+currentnodeid+"&isPrintNode="+isPrintNode+"&isSignatureNodes="+isSignatureNodes+"&from=pdfText");
		return ;
    }
...
menus = getMenuBars(onlyview, fromFlowDoc, isPersonalDoc, isrequest, canEditHis, docstatus,
		...
		OpenSendDoc, docid, cannewworkflow, canPublish, canInvalidate,
		 ...
		 isSignatureNodes, cantop, istop, accessorycount,countNum,hasUsedTemplet,RCMenu,RCMenuHeight,RCMenuHeightStep,iframeLoad,isFromAccessory,openFirstAss,isIE,fileExtendName,isToDocDspExt,islastversion,hasOpenEdition,isCustomer,imagefileName);
						 ...
						 <%if ((fileExtendName.isEmpty()||(!fileExtendName.isEmpty()&&isToDocDspExt))&&!sysremindflag){%>
							...
							<%}else{ %>
								<div style="display:<%=iframeLoad?"none":""%>">
									<OBJECT id="WebOffice" name="WebOffice" classid="<%=mClassId%>" style="POSITION:absolute;width:0;height:0;top:<%=fromFlowDoc.equals("1")?"-23px":"-23px" %>;" codebase="<%=mClientUrl%>" >
									</OBJECT>
								</div>
							<%} %>
