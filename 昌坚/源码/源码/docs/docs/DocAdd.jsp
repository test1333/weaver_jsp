...
<link type="text/css" href="/css/docie_wev8.css" rel="stylesheet"></link>
...
if("1".equals(isUseET)){
    menuBarToolsMap = new HashMap[]{new HashMap(),new HashMap(),new HashMap(),new HashMap(),new HashMap(),new HashMap()};
}else{
    menuBarToolsMap = new HashMap[]{new HashMap(),new HashMap(),new HashMap(),new HashMap(),new HashMap()};
}


RCMenu += "{DOC&nbsp;"+SystemEnv.getHtmlLabelName(58,user.getLanguage())+",javascript:onChangeDocTypeDoc(),_top} " ;
RCMenuHeight += RCMenuHeightStep ;
menuBarToolsMap[0].put("text","DOC&nbsp;"+SystemEnv.getHtmlLabelName(58,user.getLanguage()));
...
RCMenuHeight += RCMenuHeightStep ;
menuBarToolsMap[1].put("text","DOCX&nbsp;"+SystemEnv.getHtmlLabelName(58,user.getLanguage()));
menuBarToolsMap[1].put("iconCls","btn_word");
menuBarToolsMap[1].put("handler","onChangeDocType('DocAddExt.jsp','.docx');");

RCMenu += "{XLS&nbsp;"+SystemEnv.getHtmlLabelName(58,user.getLanguage())+",javascript:onChangeDocTypeXls(),_top} " ;
RCMenuHeight += RCMenuHeightStep ;
menuBarToolsMap[2].put("text","XLS&nbsp;"+SystemEnv.getHtmlLabelName(58,user.getLanguage()));
menuBarToolsMap[2].put("iconCls","btn_excel");
menuBarToolsMap[2].put("handler","onChangeDocType('DocAddExt.jsp','.xls');");

RCMenu += "{XLSX&nbsp;"+SystemEnv.getHtmlLabelName(58,user.getLanguage())+",javascript:onChangeDocTypeXlsx(),_top} " ;
RCMenuHeight += RCMenuHeightStep ;
menuBarToolsMap[3].put("text","XLSX&nbsp;"+SystemEnv.getHtmlLabelName(58,user.getLanguage()));
menuBarToolsMap[3].put("iconCls","btn_excel");
menuBarToolsMap[3].put("handler","onChangeDocType('DocAddExt.jsp','.xlsx');");

RCMenu += "{WPS&nbsp;"+SystemEnv.getHtmlLabelName(58,user.getLanguage())+",javascript:onChangeDocTypeWps(),_top} " ;
RCMenuHeight += RCMenuHeightStep ;
menuBarToolsMap[4].put("text","WPS&nbsp;"+SystemEnv.getHtmlLabelName(58,user.getLanguage()));
menuBarToolsMap[4].put("iconCls","btn_wps");
menuBarToolsMap[4].put("handler","onChangeDocType('DocAddExt.jsp','.wps');");
if("1".equals(isUseET)){
	RCMenu += "{ET&nbsp;"+SystemEnv.getHtmlLabelName(58,user.getLanguage())+",javascript:onChangeDocTypeEt(),_top} " ;
	RCMenuHeight += RCMenuHeightStep ;
    menuBarToolsMap[5].put("text","ET&nbsp;"+SystemEnv.getHtmlLabelName(58,user.getLanguage()));
    menuBarToolsMap[5].put("iconCls","btn_et");
    menuBarToolsMap[5].put("handler","onChangeDocType('DocAddExt.jsp','.et');");
}
...
<table id="topTitle" cellpadding="0" cellspacing="0">
	...
	<jsp:include page="/systeminfo/commonTabHead.jsp">
	   ...
	   <jsp:param name="docsubject" value='<%=URLEncoder.encode(URLEncoder.encode(docsubject,"UTF-8"),"UTF-8") %>'/>
...
function adjustContentHeight(type){
	...
	try{
		...
		//var divContentHeight=pageHeight-propTabHeight-55;
		...
		if(type=="load"){
			...
			//FCKEditorExt.initEditor('weaver','doccontent','<%=user.getLanguage()%>','',divContentHeight-28,"<%=maxUploadImageSize==0?"base":"basedoc"%>")
    ...
    ue = UE.getEditor('doccontent',{toolbars: [[
     ...
     ue = UE.getEditor('doccontent');
            ...
            ue.addListener( 'ready', function( editor ) {
					...
					<%if(secid>0){%>
						loadPropAndAcc(<%=secid%>,null,{invalidationdate:"<%=invalidationdate%>"});
					<%}%>
			} );
		} else {			
			    var editorheight=divContentHeight+15;
		...
		for(Iterator mbit = menuBars.iterator();mbit.hasNext();){
			...
			if(menuBarMap.size()>0) {
				...
				try{
					...
					if(window.console)console.log(e+"--->adjustContentHeight-->DocAdd.jsp");
				}
				<%
				}
			}
		...
		if(window.console)console.log(e+"--->adjustContentHeight end-->DocAdd.jsp");
	}
}
// 切换到HTML模式
