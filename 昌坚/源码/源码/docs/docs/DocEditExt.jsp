...
if("false".equals(isIE)){
	response.sendRedirect("/docs/docs/DocDspExt.jsp?fromFlowDoc="+fromFlowDoc+"&isFromAccessory="+isFromAccessory+"&isAppendTypeField="+isAppendTypeField+"&id="+docid+"&versionId="+versionId+"&f_weaver_belongto_userid="+f_weaver_belongto_userid+"&f_weaver_belongto_usertype="+f_weaver_belongto_usertype) ;
	return ;	
}
...
%>
	    ...
	    if(document.getElementById("WebOffice").WebSavePDF()){
			...
			try{
				weaver.SignatureAPI.InitSignatureItems();  //褰撶绔犳暟鎹彂鐢熷彉鍖栨椂锛岃閲嶆柊鎵ц璇ユ柟娉?
...
<body class="ext-ie ext-ie8 x-border-layout-ct" id="mybody" scroll="no" onUnload="onUnLoad()" <%if(!fromFlowDocsubm.equals("1")){%>onbeforeunload="protectDoc()"<%}%>>
...
<div id="divContentTab" style="width:100%;height:100%;">
	...
	<jsp:include page="/systeminfo/commonTabHead.jsp">
	  ...
	  <jsp:param name="docid" value="<%=docid %>"/>
	   ...
	   <jsp:param name="fileName" value='<%=URLEncoder.encode(URLEncoder.encode(fileName,"UTF-8"),"UTF-8") %>'/>
	 </jsp:include>
	<%-- 文档标题 start --%>
	...
	<%-- iWebOffice编辑控件 start --%>
		...
		<table cellpadding="0" cellspacing="0" style="width:100%;height:100%;">
			...
			<div id="objectDiv">
				<div>
				<OBJECT id="WebOffice" classid="<%=mClassId%>" style="POSITION:absolute;width:0;height:0;top:-23px;" codebase="<%=mClientUrl%>" >
				</OBJECT>
				</div>
