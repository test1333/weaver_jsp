				...
				<wea:item>
					<%if(!onlyRead){ %>
						<wea:required id="showOrderImage" required="true" value='<%=""+showOrder%>'>
							<INPUT style="width:50px;" temptitle="<%=SystemEnv.getHtmlLabelName(88,user.getLanguage())%>" class=InputStyle name=showOrder value="<%=showOrder%>" size=4 maxlength=4  onKeyPress="ItemCount_KeyPress()" onBlur='checknumber("showOrder");checkinput("showOrder","showOrderImage")' onchange='checkinput("showOrder","showOrderImage")' >
						</wea:required>
					<%}else{ %>
						<%= showOrder%>
					<%} %>
...
function callBackFun(data) {
	...
	else top.Dialog.alert('<%=SystemEnv.getHtmlLabelName(22943,user.getLanguage())%>!');
}
