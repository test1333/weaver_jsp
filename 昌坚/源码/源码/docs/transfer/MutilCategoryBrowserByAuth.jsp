...
<jsp:useBean id="MultiAclManager4Transfer" class="weaver.docs.transfer.MultiAclManager4Transfer" scope="page" />			
...
<%
    ...
    if(codeName.startsWith("auth")) codeName=codeName.substring(4);
		...
		//默认共享
		if("T144".equals(codeName)||"C144".equals(codeName)||"D124".equals(codeName)){//人员
			permissiontype=1;
		} else if("T224".equals(codeName)||"C244".equals(codeName)||"D224".equals(codeName)){//部门
			permissiontype=3;
		} else if("T324".equals(codeName)||"C344".equals(codeName)||"D324".equals(codeName)){//分部
			permissiontype=2;
		} else if("T414".equals(codeName)||"C434".equals(codeName)||"D414".equals(codeName)){//角色
			permissiontype=4;
		}
		...
		//目录维护权限
		if("T146".equals(codeName)||"C146".equals(codeName)||"D126".equals(codeName)){//人员
			operationcode=1;permissiontype=5;
		} else if("T225".equals(codeName)||"C245".equals(codeName)||"D225".equals(codeName)){//部门
			operationcode=1;permissiontype=1;
		} else if("T325".equals(codeName)||"C345".equals(codeName)||"D325".equals(codeName)){//分部
			operationcode=1;permissiontype=6;
		} else if("T415".equals(codeName)||"C435".equals(codeName)||"D415".equals(codeName)){//角色
			operationcode=1;permissiontype=2;
		}
		
		MultiCategoryTree tree = MultiAclManager4Transfer.getPermittedTree(fromid, operationcode, permissiontype, categoryname, -1, null);
    String searchSql=tree.getSearchSql();
    ...
    if(isAll){
    	...
    	if(!"".equals(selectids)) selectids=selectids.substring(1);
			...
			if (mjson.exsit(_type)) {
				mjson.updateArrayValue(_type, searchSql);
			} else {
				mjson.putArrayValue(_type, searchSql);
			}
