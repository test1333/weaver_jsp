 ...
 * 预付+月份+费用
 */
public class AbstractMainFl06WorkflowAction implements Action {
    BaseBean log = new BaseBean();

    public String execute(RequestInfo info) {
        log.writeLog("进入Fl-06供应商预付款申请表单AbstractMainFl06WorkflowAction——————");
        ...
        if (!"".equals(tableName)) {
            ...
            abstract_main = "预付" + applyDate + "费用";
            if (abstract_main.length() > 50) {
                abstract_main = abstract_main.substring(0, 50);
            }
            sql = " update " + tableName + " set QKLY='" + abstract_main + "' where requestid= " + requestid;
            rs.executeSql(sql);
