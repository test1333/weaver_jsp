...
String sql_condition="select t1.*,t2.* from CptSearchDefinition t1,cptDefineField t2 where t1.fieldname=t2.fieldname and t1.isconditions='1' and t2.issystem is null  order by t1.displayorder";
...
operatorInfo.put("operator_val", popedomOtherpara);
                        ...
                        if (CptSearchComInfo.getIsData().equals("1")){
                            ...
                            fromSql  = " from CptCapital  t1  ";
							if(userId != 1){
                          String sql1="select subcompanyid1 from hrmresource where id="+userId;
						  rs.executeSql(sql1);
						  ...
						  if(!"".equals(subcompanyid1)){
							  sqlWhere += " where t1.blongsubcompany='"+subcompanyid1+"' and ";
							   tempsearchsql = tempsearchsql.split("where")[1];
						  }
						} 
                        ...
                        rs.executeSql("select t1.*,t2.id as fieldid,t2.fieldname,t2.fieldlabel,t2.fieldhtmltype,t2.type,t2.issystem from CptSearchDefinition t1,cptDefineField t2 where t1.fieldname=t2.fieldname and t1.istitle = 1 and t1.mouldid = -1 order by t1.displayorder ");
                        ...
                        while(rs.next()){
                        	...
                        	if("name".equals(fieldname)) {
                                if ("2".equals(isdata)) {
                                    fieldhtmltype = 3;
                                    fieldtype = 23;
                                    temptableString += "<col width=\"5%\"  text=\"" + SystemEnv.getHtmlLabelName(fieldlabel, user.getLanguage()) + "\" column=\"" + "id" + "\" orderkey=\"" + "id" + "\" otherpara=\"" + user.getUID() + "+" + fieldid + "+" + fieldhtmltype + "+" + fieldtype + "\" transmethod='weaver.cpt.util.CptFieldManager.getBrowserFieldvalue' />";
                                } else {
...
function onLogData1(id){
	if(id){
		//var url="/systeminfo/SysMaintenanceLog.jsp?isdialog=1&sqlwhere=where operateitem=51 and relatedid="+id;
