...
<%
	...
	url = "CptAssortmentManage.jsp?paraid="+parentid+(isRoot?"&isroot=1":isTop?"&istop=1":"");
		...
		<div class="e8_boxhead">
		    ...
		    <ul class="tab_menu">
				...
				<%}else if(isTop){//1级资产组
			        %>
	       			<li <%if(!"1".equals(cptdelfromAss)){ %>class="current"<%} %>>
						<a target="tabcontentframe" href="CptAssortmentManage.jsp?istop=1&paraid=<%=parentid %>"><%=SystemEnv.getHtmlLabelName(1361 ,user.getLanguage()) %></a>
					</li>
				<%
				if(data1count<=0){
					%>
