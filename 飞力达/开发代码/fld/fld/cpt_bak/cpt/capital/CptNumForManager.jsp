...
if(user == null){
	...
	if(!"".equals(poststr)){
		...
		for(int i=0;i<s.length;i++){
			...
			if(ss.length>=2){
				String sql2="select capitalnum,frozennum from cptcapital where id="+ss[0];
				...
				if(rs.next()){
					...
					if((available_num+Util.getDoubleValue((String)selfMap.get(ss[0]),0) )<Util.getDoubleValue(""+tmpMap.get(ss[0]) ,0)){
