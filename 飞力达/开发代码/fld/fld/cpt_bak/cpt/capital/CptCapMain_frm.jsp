...
if(rs.next()){
    detachable=Util.getIntValue( rs.getString("cptdetachable"),0);
    session.setAttribute("cptdetachable",String.valueOf(detachable));
}
if(detachable==0){
    response.sendRedirect("CptCapMain_ass.jsp?from=tree");
    return;
}
%>
...
<IFRAME name=leftframe id=leftframe src="/cpt/capital/CptCapMain_left.jsp" width="100%" height="100%" frameborder=no scrolling=no>
浏览器不支持嵌入式框架，或被配置为不显示嵌入式框架。</IFRAME>
</td>

<td height=100% id=oTd2 name=oTd2 width="*" style=’padding:0px’>
<IFRAME name=contentframe id=contentframe src="/cpt/capital/CptCapMain_ass.jsp" width="100%" height="100%" frameborder=no scrolling=yes>
浏览器不支持嵌入式框架，或被配置为不显示嵌入式框架。</IFRAME>
</td>
</tr>
