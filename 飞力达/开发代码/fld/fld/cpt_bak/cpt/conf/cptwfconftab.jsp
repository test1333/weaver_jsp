...
//不能选的wf
String notwfid="";
RecordSet.executeSql("select wfid from cpt_cptwfconf where wftype!='"+wftype+"' ");
...
<FORM id=frmain name=frmain method=post action="/cpt/conf/cptwfconfop.jsp"    >
...
function initNodeSelect(container,tr,entry){
	...
	CptDwrUtil.getCptWfNodeItem(wfid,function(data){
     ...
     var option= {
    		 ...
    		 //canDrag:true,
    		 useajax:true,
    		 ajaxurl:'/cpt/conf/cptwfconfop.jsp',
...
1 select the need to realize the flow of assets functions;
2 selection process must be assets through fields and number fields, the main table or a detailed table all can (asset purchase process is asset information browsing field);
3 after selecting the process, bind the related business logic fields, configure the node or export binding business actions;
4 note that the asset browse field and the number field must be paired (as in the main form or in a detail form);	
	<%
}else if(user.getLanguage()==9){
	%>
配置說明:
1.選擇需要實現資産流轉功能的流程;
2.被選流程必須要有資産浏覽字段和數量字段,主表或明細表皆可(資産申購流程是資産資料浏覽字段);
3.選擇流程後,請綁定相關業務邏輯字段,配置節點或出口綁定業務動作;
4,注意,資産浏覽字段和數量字段必須配對(如同是在主表單裏或者同是在明細表單裏);	
	<%
}else{
	%>
配置说明:
1.选择需要实现资产流转功能的流程;
2.被选流程必须要有资产浏览字段和数量字段,主表或明细表皆可(资产申购流程是资产资料浏览字段);
3.选择流程后,请绑定相关业务逻辑字段,配置节点或出口绑定业务动作;
4,注意,资产浏览字段和数量字段必须配对(如同是在主表单里或者同是在明细表单里);
	<%
}
...
function getWfBrowserUrl(data){
	...
	var sqlwhere="where isbill=1 and exists ( select 1 from workflow_billfield t2 where t2.billid=workflow_base.formid and t2.fieldhtmltype=3 and t2.type="+type+" and workflow_base.id not in("+notwfid+") and formid not in(14,18,19,201,220,221,222,224) )";
	var URL = "/cpt/conf/CptUrlTrans.jsp?srctype=trantxss&type="+type+"&notwfid="+notwfid;
	var returnValue="";
    jQuery.ajax({
      url: URL,
      type: "post",
...
function getBrowserUrlFn(data){
	...
	switch(fieldname){
		case "sqr":
			...
			break;
		case "zczl":
			url+="%26htmltype=3%26type=179";
			break;
		case "zc":
			url+="%26htmltype=3%26type=23";
			break;
		case "sl":
