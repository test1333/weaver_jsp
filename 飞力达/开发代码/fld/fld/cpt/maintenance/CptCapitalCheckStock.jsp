...
String checkstatus = Util.null2String(request.getParameter("checkstatus"));     //状态:0:未审批;1:审批
...
<FORM id=weaver name=frmain action="CptCapitalCheckStock.jsp" method=post>
...
<BUTTON class=Btn id=button1 accessKey=A onclick='window.location.href="CptCapitalCheckStockAdd1.jsp"' name=button1><U>A</U>-<%=SystemEnv.getHtmlLabelName(1421,user.getLanguage())%></BUTTON>
<% } %>
<table class=form>
  <colgroup> 
  <col width="7%"><col width="18%"><col width="7%"><col width="18%">
  <col width="7%">  <col width="18%"><col width="7%">  <col width="18%">
  <tbody> 
   <TR class=Section>
   <tr> 
   ...
   <td class=Field> 
    <input class=saveHistory maxlength=20 name="checkstockno" size=17 value="<%=checkstockno%>">
    </td>
    <td><%=SystemEnv.getHtmlLabelName(1387,user.getLanguage())%></td>
     <td class=Field> 
    <input class=saveHistory maxlength=200 name="location" size=17 value="<%=location%>">
    </td>
   ...
   <td><%=SystemEnv.getHtmlLabelName(602,user.getLanguage())%></td>
    <td class=Field>
     <select name=checkstatus class=saveHistory>
     <option value="" <% if(checkstatus.equals("")) {%>selected<%}%>></OPTION>
     <option value="0" <% if(checkstatus.equals("0")) {%>selected<%}%>><%=SystemEnv.getHtmlLabelName(1422,user.getLanguage())%></OPTION>
    <option value="1" <% if(checkstatus.equals("1")) {%>selected<%}%>><%=SystemEnv.getHtmlLabelName(1423,user.getLanguage())%></OPTION>
     </select>
    </td>
  </tr>
   <tr> 
   ...
   <td><%=SystemEnv.getHtmlLabelName(1424,user.getLanguage())%></td>
    <td class=Field colspan=3><button class=calendar id=SelectDate onClick="gettheDate(fromdate,fromdatespan)"></button>&nbsp; 
      <span id=fromdatespan ><%=fromdate%></span> -&nbsp;&nbsp;<button class=calendar 
...
if(rs.getDBType().equals("oracle")){
	sql = "create table "+temptable+"  as select * from (select * from CptCheckStock "+sql+" order by createdate desc ) where rownum<"+ (pagenum*perpage+2);
}else{
	sql = "select top "+(pagenum*perpage+1)+" * into "+temptable+" from CptCheckStock "+sql+" order by createdate desc ";
}
rs.execute(sql);   
...
if(RecordSetCounts>pagenum*perpage){
	...
	String sqltemp="";
if(rs.getDBType().equals("oracle")){
	sqltemp="select * from (select * from  "+temptable+" order by createdate) where rownum< "+(RecordSetCounts-(pagenum-1)*perpage+1);
}else{
	sqltemp="select top "+(RecordSetCounts-(pagenum-1)*perpage)+" * from "+temptable+"  order by createdate ";
}
...
%>
  ...
  <%  	}
  %>
      <TD><A HREF="CptCapitalCheckStockEdit.jsp?checkstockid=<%=""+tempid%>"><%=tempcheckstockno%></A></TD>
      <TD><%=tempstockdesc%></TD>
      <TD>
