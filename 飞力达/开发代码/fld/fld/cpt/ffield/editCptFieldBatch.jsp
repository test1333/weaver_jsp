...
<jsp:useBean id="DeptFieldManager0" class="weaver.cpt.util.CptFieldManager" scope="page"/>
...
<jsp:useBean id="CptCardGroupComInfo1" class="weaver.cpt.util.CptCardGroupComInfo" scope="page" />
...
//System.out.println("groupOptionStr:\n"+groupOptionStr);
...
String sql = "select * from cptDefineField where viewtype=0 order by dsporder,id";
...
String titlename = SystemEnv.getHtmlLabelName(16503, user.getLanguage());
String needfav ="1";
String needhelp ="";
%>
...
function showdetaildata(){
	...
	jQuery.ajax({
		url : "/cpt/ffield/editCptFieldBatchDetail.jsp?qname=<%=qname%>&fieldname_kwd=<%=fieldname_kwd%>&fieldlabel_kwd=<%=fieldlabel_kwd%>",
		type : "get",
		async : true,
		cache:false,
		processData : false,
...
%>
<FORM id="frmMain" name="frmMain" action="/cpt/ffield/editCptFieldBatch.jsp" method="post" >
	...
	for(j=0; j<ncol; j++) {
		...
		switch(j){
			case 0:
				...
				var sHtml = "<input  type='checkbox' name='check_select' value='0_"+rowindex+"'><img moveimg src='/proj/img/move_wev8.png'   title='<%=SystemEnv.getHtmlLabelNames("82783",user.getLanguage())%>' />";
            ...
            case 5:
                var oDiv = document.createElement("div");
                var sHtml = "<input type='checkbox'  class=inputstyle onClick='linkageIsmand("+rowindex+")' name='isopen_"+rowindex+"' value='1' checked>";
                oDiv.innerHTML = sHtml;
                oCell.appendChild(oDiv);
                break;
            case 6:
                var oDiv = document.createElement("div");
                var sHtml = "<input type='checkbox'  class=inputstyle onClick='linkageIsopen("+rowindex+")' name='ismand_"+rowindex+"' value='1' >";
                oDiv.innerHTML = sHtml;
                oCell.appendChild(oDiv);
                break;
			case 7:
	...
	for (loop=copyedRowArray.length-1; loop >=0 ;loop--){
	  ...
	  for(i=0; i<ncol; i++) {
		...
		switch(i) {
			...
			case 5:
				var oDiv = document.createElement("div");
				var sHtml = "<input type='checkbox'  class=inputstyle onClick='linkageIsmand("+rowindex+")' name='isopen_"+rowindex+"' value='"+isopen+"' "+checkedFlag+" >";	   
				oDiv.innerHTML = sHtml;
				oCell.appendChild(oDiv);
				break;					
			case 6:
				var oDiv = document.createElement("div");
				var sHtml = "<input type='checkbox'  class=inputstyle onClick='linkageIsopen("+rowindex+")' name='ismand_"+rowindex+"' value='"+ismand+"' "+checkedFlag2+" >";	   
				oDiv.innerHTML = sHtml;
				oCell.appendChild(oDiv);
				break;					
			case 7:
...
function onSave(obj){
	frmMain.action="/cpt/ffield/cptFieldOperation.jsp" 
	changeRows = 0;
...
function linkageIsopen(index){
	var ismand= jQuery("input[name='ismand_"+index+"']:checkbox"); //必填
	var isopen= jQuery("input[name='isopen_"+index+"']:checkbox"); //启用
	if(isopen&&ismand&&ismand.attr("checked")==true){
		isopen.attr("checked", true).next("span").addClass("jNiceChecked");
	}
}

//启用联动
function linkageIsmand(index){
	var ismand= jQuery("input[name='ismand_"+index+"']:checkbox");
	var isopen= jQuery("input[name='isopen_"+index+"']:checkbox");
	if(isopen&&ismand&&isopen.attr("checked")==false){
		ismand.attr("checked", false).next("span").removeClass("jNiceChecked");
	}
}
