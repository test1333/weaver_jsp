...
String grouptablename="cpt_cptwfconf";
...
if(wftype.length()>1){
	...
	actname="Cpt"+wftype.substring(0, 1).toUpperCase()+wftype.substring(1)+"Action";
}
  ...
  if(src.equals("editgroupbatch")){//批量编辑分组
	...
	rs1.executeSql("select * from workflow_billfield where billid in(select formid from workflow_base where id in(select wfid from cpt_cptwfconf where wftype='"+wftype+"' ))");
	while(rs1.next()){
