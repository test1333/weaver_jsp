...
if(!isBatch){
	...
	if(isfromView){
		...
		if("-2".equals(checkstate)){//已收回的入库单,跳转到编辑入库单页面
			response.sendRedirect("/cpt/capital/CapitalBlankTab.jsp?title="+SystemEnv.getHtmlLabelNames("83575",user.getLanguage())+"&url=/cpt/capital/CptCapitalInstock1tab.jsp?isdialog=1%26id="+id);
			return;
		}
	}
}
...
<%
if(isfromView){
	if("-1".equals(checkstate)){//已作废
		RCMenu += "{"+SystemEnv.getHtmlLabelName(82690,user.getLanguage())+",javascript:onReApply(this),_self} " ;
		RCMenuHeight += RCMenuHeightStep ;
	}else if(!"1".equals(checkstate) ){//收回
		RCMenu += "{"+SystemEnv.getHtmlLabelName(18666,user.getLanguage())+",javascript:onReject(this),_self} " ;
		RCMenuHeight += RCMenuHeightStep ;
	}
}else{
...
if(isfromView){
	if("-1".equals(checkstate)){//已作废
		%>
			<input type="button" id="btn_reapply" value="<%=SystemEnv.getHtmlLabelName(82690,user.getLanguage()) %>" class="e8_btn_top"  onclick="onReApply(this)"/>
		<%
	}else if(!"1".equals(checkstate)){//收回
		%>
			<input type="button" id="btn_reject" value="<%=SystemEnv.getHtmlLabelName(18666,user.getLanguage()) %>" class="e8_btn_top"  onclick="onReject(this)"/>
		<%
	}
}else{
	%>
...
if(!isBatch){
	%>
		...
		<wea:item>
			<BUTTON type="button" class=calendar style="display:none;" id=SelectDate onclick="getdiscardDate(StockInDateSpan,StockInDate)"></BUTTON>
						  <SPAN id=StockInDateSpan ><%=Util.null2String( RecordSet.getString("stockindate")) %></SPAN>
						  <input type="hidden" name="StockInDate" value="<%=Util.null2String( RecordSet.getString("stockindate")) %>" onpropertychange="compare_date()">
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("83579",user.getLanguage())%>' attributes="" >
		<wea:item attributes="{'isTableList':'true'}">
			...
			if(!isBatch){
				...
				String sql="select m.id as mainid,d.* from CptStockInDetail d,CptStockInMain m where m.id=d.cptstockinid and m.id in("+id+") and m.ischecked=0 order by m.id,d.id ";
