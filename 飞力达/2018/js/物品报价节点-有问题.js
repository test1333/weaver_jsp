...
var smsj = "#field23083_"; //税码税金
var smsj2 = "#field13706_"; //税金(修改)
var bhsje = "#field14264_"; //不含税金额
var hsje = "#field23063_"; //含税金额
//20180605 不含税金额计算
// function bind() {
...
// function bindchange2(numx) {
	// jQuery(smsj2 + numx).bindPropertyChange(function () {
		...
		// var hsje_val = jQuery(hsje + numx).val();
		// var diff = Number(hsje_val) - Number(smsj2_val);
		// diff = diff.toFixed(2);
    ...
    checkCustomize = function() {
		...
		for (var index = 0; index < indexnum1; index++) {
			if($("#field13259span a").text() == "固定资产"){	
				if(isNaN($(dj+index).val())||parseFloat($(dj+index).val()) < 2000 ){
					alert("金额小于2000，不属于固定资产，请退回重新提交。");
					return false;
				}
			}
