    ...
    <%@ page import="weaver.general.Util" %>
      ...
      if(reqid.length()>0){
		String tmp_reqid = reqid+",0";
		  String sql = " select wm_concat(id) as ids,max(v.ZCBH) as ZCBH,max(v.YHZH) as YHZH,max(v.YHZHMS) as YHZHMS,  max(v.shybm) as shybm ,v.kmid ,v.gys1id ,sum(v.HSZJ) as JE ,count(*) as cnt,max(v.cptname) cptname, "
		  +" v.cbzx,max(v.zcms) zcms,max(v.ywfw) ywfw,max(v.ywfwmc) ywfwmc,max(v.sm) sm,max(v.shlv) shlv,max(v.shj) shj,max(v.nbdd) nbdd,max(v.nbddms) nbddms, "
		  +" max(v.requestid) requestid, max(v.exetype) as type1 ,max(v.syrcbzxmc) as syrcbzxmc , max(v.kq) as kq ,max(v.qsyyf) as qsyyf , max(v.jsyyf) as jsyyf"
	 	...
	 	while(rs.next()) {
			...
			dataBuff.append("###");
			dataBuff.append(Util.null2String(rs.getString("ZCBH")));
			dataBuff.append("###");
			...
			dataBuff.append("###");
			dataBuff.append(Util.null2String(rs.getString("YHZH")));
			dataBuff.append("###");
			dataBuff.append(Util.null2String(rs.getString("YHZHMS")));
			dataBuff.append("###");
			dataBuff.append(Util.null2String(rs.getString("kq")));
			dataBuff.append("###");
			dataBuff.append(Util.null2String(rs.getString("qsyyf")));
			dataBuff.append("###");
			dataBuff.append(Util.null2String(rs.getString("jsyyf")));
			dataBuff.append("###");
			dataBuff.append(Util.null2String(rs.getString("ids")));
			dataBuff.append("@@@");
			}
		}
