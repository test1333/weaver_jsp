...
“是否有超预算项”逻辑判端，明细表中只要有一条明细中“报销金额”大于“预算金额”即判断“是否有超预算项”值为“Y”没有这为“N”
    ...
    */
	...
	function val_all(index){
		...
		//alert(account_all);alert(money_all);
		if(Number(account_all)>Number(money_all)){
			var _percent_val = jQuery("#"+_percent).val();
			var _percent_now = Number(account_all)/Number(money_all)*100;
			if(Number(_percent_now)>Number(_percent_val)){
				jQuery("#"+_percent).val(_percent_now);
				jQuery("#"+_percent+"span").text(_percent_now);
			}
			alert("申请物品金额超预算!");
			//return false;
		}
...
jQuery(document).ready(function () {
	var applyworkflow = "#field14103"; // 物品申请流程
...
var reqid_dt = "field14141_";
     var wpbm = "field14181_";
	var exetype = "field14182_";
      var useDept = "#field9315"; //使用部门
        var GSBER = "#field14042";//业务范围
    	var ysly = "#field14041";//预算来源
	  //明细
