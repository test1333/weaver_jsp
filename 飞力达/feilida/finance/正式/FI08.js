...
<script type="text/javascript">
    ...
    var id_cph = "field14281"; //车牌号
    var id_zzkm = "field10374"; //总账科目
    var list_zzkm = ["6601540201",
                    "6601540202",
                    "6601550100",
                    "6601560100",
                    "6601600100",
    ...
    var handleZFFS_CHK = function () {
        ...
        if (isSuccess == true) {
            //判断科目，控制车牌必填
            $("#oTable0 input[name*='" + id_zzkm + "_']").each(function () {
                if (isSuccess == true) {
                    //alert("CHK:" + isSuccess);
                    var idx = $(this).attr("name").replace(id_zzkm+"_", "");
                    //alert(idx);
                    var v_zzkm = $(this).val();
                    var ii = 0;
                    for (ii = 0; ii < list_zzkm.length; ii++) {
                        if (list_zzkm[ii] == v_zzkm) {
                            if ($("#" + id_cph + "_" + idx).val() == "") {
                                window.top.Dialog.alert("总账科目【" + v_zzkm + "】对应车牌号必填！");
                                isSuccess = false;
                                return false;
                            }
                        }
    ...
    */
    ...
    jQuery("button[name=addbutton0]").live("click", function () {
        ...
        for (var index = indexnum_dt1; index < indexnum0; index++) {
            ...
            jQuery(bhsje+index).attr("readonly", "readonly");
        }
    ...
    function bindchange2(numx) {
        jQuery(smsj2 + numx).bindPropertyChange(function () {
            ...
            jQuery(bhsje + numx).val(diff.toFixed(2));
            //jQuery(bhsje + numx + "span").text(diff.toFixed(2));
        })
    }
    ...
    var getlx = function () {
    		var d_zbbxlx_val=jQuery(d_zbbxlx).val();
    	if(d_zbbxlx_val == '1'){
        jQuery("button[name=delbutton0]").hide();
        jQuery("button[name=addbutton0]").hide();
       }else{
       	    jQuery("button[name=delbutton0]").show();
                jQuery("button[name=addbutton0]").show();
       }
    ...
    jQuery(document).ready(function () {
        ...
        checkCustomize = function () {
            ...
            var _account = "field10645_";
                 var ysqje = "field15923_"; // 原申请金额
                var exType = "#field11171"; // 报销类型	
                	 var kmid1 = "field13882_"; 
            var indexnum00 = jQuery("#indexnum0").val();
            	 var exType_val = jQuery(exType).val();
            	var flag1="";
            	flag1="";
            for (var index001 = 0; index001 < indexnum00; index001++) {
                ...
                var diff = Number(bxje_val) - Number(smsj2_val);
                jQuery(bhsje + index001).val(diff.toFixed(2));
                //jQuery(bhsje + index001 + "span").text(diff.toFixed(2));
                if(exType_val == '1'){
                  if(jQuery("#" + ysqje + index001).length > 0){
                  
                	var kmid1_val=jQuery("#" + kmid1 + index001).val().length;
     
                	if(kmid1_val == '10'){
                		
                		if(flag1 == '1'){
                		 alert("科目ID8位和10位不能在一张单子上,请检查");
                		  return false;
                		}else{
                		 flag1="2";
                		}
                  ...
                  var ysqje_val=jQuery("#" + ysqje + index001).val();
                  var bxje_val=jQuery(bxje + index001).val();
                  if(Number(bxje_val)>Number(ysqje_val)){
              	   alert("报销金额必须小于原申请金额，请查看");
              	   return false;
              	}
                }	
            }
    ...
    jQuery(document).ready(function () {
        ...
        var _gys1id="field13782_"; //供应商编号
        var _gys1name="field13783_"; //供应商名称
        var _yhzh="field14481_";//银行账号
        var _yhzhms="field14482_";//银行账号描述
        	 var _mxids="field15941_";//明细ids
        	          var zzkm11 = "field10374_";
        ...
        if (nodesnum0 > 0) {
            for (var index = 0; index < indexnum0; index++) {
                ...
                if(expenseType_val == '1'){
                 jQuery("#" + _gys1id + index).attr("readonly", "readonly");
                 jQuery("#" + _gys1id + index + "_browserbtn").hide();
                jQuery("#" + _gys1name+ index).attr("readonly", "readonly");
                  jQuery("#" + _deptName + index).attr("readonly", "readonly");
                  jQuery("#" + _dept + index).attr("readonly", "readonly");
                  jQuery("#" + _dept + index+"_browserbtn").attr("hidden", "hidden");                 
                   jQuery("#" + _cptCard + index).attr("readonly", "readonly");
                    jQuery("#" + _cptDescribe + index).attr("readonly", "readonly");
                     jQuery("#" + zzkm11+ index).attr("readonly", "readonly");
        ...
        function fl_dtChange(tmpObj, nowRow) {
            ...
            jQuery("#" + _cptKSTAR + nowRow).bindPropertyChange(function () {
                ...
                get_cpt_info(tmp_cptNumber, oadate, tmp_budget, tmp_money, tmp_KSTAR, tmp_keyNum, tmp_dept, useDept, '', tmp_businessRange);
            });
        ...
        function get_cpt_info(tmp_cptNumber, oadate, tmp_budget, tmp_money, tmp_KSTAR, tmp_keyNum, tmp_dept, tmp_usedept, tmp_exetype, tmp_businessRange) {
            ...
            //alert("OADATE"+oadate_val+","+"ANLKL"+tmp_KSTAR_val+","+"AMOUNT"+amount+","+"DEPTID"+useDept_val+","+"EXECTYPE"+exectype+","+"KSTAR"+tmp_KSTAR_val+","+"KOSTL"+GSBER_val);
            //alert("GSBER"+tmp_businessRange_val+","+"KOSTL"+tmp_businessRange_val);
            if (tmp_cptNumber.length > 0 && oadate.length > 0 && useDept_val.length > 0) {
                ...
                jQuery.post("/feilida/js/cpt/fl_getCptInfoNew.jsp", {
                    ...
                    var GPKEY = obj.GPKEY;
                     ...
                     // eval('var obj1 =' + EXTSTRING);
                     // var KSTAR_TYPE = obj1.KSTAR_TYPE;
        ...
        function fl_change(_rowdtid) {
            ...
            var zzkm1 = "field10374_";
                	      ...
                	      var text_arr = text.split("@@@");
                            if (text_arr.length > 1) {
                                ...
                                for (var i = _rowdtid; i < length_dt; i++) {
                                    ...
                                    addRow0(0);
                                    getlx();
                                    //alert( text_arr[i - _rowdtid]);
                                    var tmp_arr = text_arr[i - _rowdtid].split("###");

                                    jQuery("#" + _deptName + i).val(tmp_arr[0]); //存放id
                                    jQuery("#" + _deptName + i).attr("readonly", "readonly");

                                    jQuery("#" + _dept + i).val(tmp_arr[1]); //存放id
                                    	jQuery("#" + _dept + i + "span").html(tmp_arr[1]); //存放名称
                                        jQuery("#" + _dept + i).attr("readonly", "readonly");
                                     jQuery("#" + _dept + i+"_browserbtn").attr("hidden", "hidden");

                                    jQuery("#" + _cptCard + i).val(tmp_arr[2]); //存放id
					     jQuery("#" + _cptCard + i).attr("readonly", "readonly");
                                    jQuery("#" + _cptDescribe + i).val(tmp_arr[3]); //存放id
                                   jQuery("#" + _cptDescribe + i).attr("readonly", "readonly");

                                    jQuery("#" + _budgetMoney + i).val(tmp_arr[4]); //存放id
                                    jQuery("#" + _budgetMoney + i).attr("readonly", "readonly");
                                    jQuery("#" + _budget + i).val(tmp_arr[5]); //存放id

                                    jQuery("#" + _budget + i).attr("readonly", "readonly");
                                    ...
                                    jQuery("#" + zzkm + i).val(tmp_arr[13]); //存放id
                                     ...
                                     jQuery("#" + zzkm1+ i).attr("readonly", "readonly");
                                    //alert("134"+tmp_arr[14]); 
                                    jQuery("#" + _keyNum + i).val(tmp_arr[14]); //存放id

                                    jQuery("#" + _keyNum + i).attr("readonly", "readonly");

                                    jQuery("#" + _expense + i).val(tmp_arr[15]); //存放id
                                    	  jQuery("#" + _ysqje + i).val(tmp_arr[15]); //存放id
                                    	  jQuery("#" + _ysqje + i+ "span").html(tmp_arr[15]); //存放id
                                    ...
                                    jQuery("#" + exetype + i + "span").html(tmp_arr[18]); //存放名称
                                    	  jQuery("#" + _gys1id + i).val(tmp_arr[19]); //存放id
                                    jQuery("#" + _gys1id + i + "span").html(tmp_arr[19]); //存放名称
                                    jQuery("#" + _gys1name+ i).val(tmp_arr[20]); //存放id
                                   // jQuery("#" + _gys1name + i + "span").html(tmp_arr[20]); //存放名称
                                    jQuery("#" + _gys1id + i).attr("readonly", "readonly");
                                     jQuery("#" + _gys1id + i + "_browserbtn").hide();
                                    jQuery("#" + _gys1name+ i).attr("readonly", "readonly");
                                    ...
                                    jQuery("#" + _yhzhms+ i).val(tmp_arr[22]); 
                                     jQuery("#" + _mxids+ i).val(tmp_arr[23]); 
                                    ...
                                    get_cpt_info(tmp_cptNumber, oadate, tmp_budget, tmp_money, tmp_KSTAR, tmp_keyNum, tmp_dept, tmp_usedept, tmp_arr[18], tmp_businessRange);
             ...
             var indexnum11 = jQuery("#nodesnum2").val();
            if (indexnum11 > 0) {
                jQuery("[name = check_node_2]:checkbox").attr("checked", true);
                deleteRow2(2);
            }
