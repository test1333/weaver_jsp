    ...
    <%@ page import="weaver.general.Util" %>
      ...
      if(reqid.length()>0){
		String tmp_reqid = reqid+",0";
		  String sql = " select wm_concat(id) as ids,max(v.ZCBH) as ZCBH,max(v.YHZH) as YHZH,max(v.YHZHMS) as YHZHMS,  max(v.shybm) as shybm ,v.kmid ,v.gys1id ,sum(v.je) as JE ,count(*) as cnt,max(v.cptname) cptname, "
		  ...
		  +"  from ( select dtl.id,'' as a ,dtl.ZCBH,dtl.YHZH,dtl.YHZHMS,t.shybm,dtl.kmid,dtl.gys1id,case when cpa.assortmentname ='固定资产' then sys_guid()||'' else ' ' end as assortmentname ,dtl.JE,dtl.ysxx,dtl.guige "
