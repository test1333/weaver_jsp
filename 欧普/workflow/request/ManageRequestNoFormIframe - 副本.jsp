	...
	String sql_del = " Select tablename From Workflow_bill Where id in ("
		...
		if(!"".equals(tableName_del)){
			...
			if(!"".equals(fwzh)){
				...
				if(nodetype.equals("0")&&isremark != 1&&isremark != 9&&isremark != 7&&isremark != 5&&!"1".equals(session.getAttribute("istest"))){
					canCancel = "1";
				}
			}
		}
...
if("1".equals(canCancel)){
	RCMenu += "{"+SystemEnv.getHtmlLabelName(16210,user.getLanguage())+",javascript:doCancel1(this),_self}" ;
	RCMenuHeight += RCMenuHeightStep ;
	strBar += "{text: '"+SystemEnv.getHtmlLabelName(16210,user.getLanguage())+"',iconCls:'btn_backSubscrible',handler: function(){bodyiframe.doCancel1(this);}},";
}
if(haveCancelright)
{
...
<script>
function doCancel1(obj){
	//您确定要撤销当前流程吗?
	if(confirm("<%=SystemEnv.getHtmlLabelName(26157,user.getLanguage())%>?")){
		enableAllmenu();
		document.location.href="/workflow/workflow/wfFunctionManageLink.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&flag=cancel&requestid=<%=requestid%>&canCancel=1" //xwj for td3665 20060224
	}
	else
	{
		displayAllmenu();
