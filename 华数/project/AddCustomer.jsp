	  ...
	  <TABLE class=ViewForm>
         ...
         <%  rs.executeSql(" select gs from formtable_main_"+gsformid+"_dt2 ");  
          ...
          %>
        	<select name=firsttype id="firstype_<%=gsid %>" onchange="getdynlboptions(this,<%=gsid %>)" disabled=true style="display:none" > 
             <option value=-1 > </option> 
             	<% 
        	    rs2.executeSql(" select distinct fm.firsttype,ft.dsporder from formtable_main_"+lxlzwhformid+"_dt2  fm left join firsttype ft on fm.firsttype=ft.typename where ssgs="+gsid+" order by ft.dsporder asc ");	
        	 	while(rs2.next()){  
        	    %>
        ...
        <TR>
         ...
         <TR>
          <TD>项目投资情况</TD>
          <TD class=Field>
          	 投资预算金额:<INPUT class=InputStyle size=6 name="xmtzysje" value=0.0>万 （其中软件金额：<INPUT class=InputStyle  size=6 name="xmrjje" value=0.0>万 硬件金额:<INPUT class=InputStyle  size=6 name="xmfyzyyjje" value=0.0>万 项目其他经费：<INPUT class=InputStyle size=6 name="xmqtjf" value=0.0>万）云资源金额：<INPUT class=InputStyle size=6 name="xmyzyje"  value=0.0 >万
