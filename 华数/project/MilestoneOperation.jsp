...
if(method.equals("add"))
{
	...
	String isover=Util.null2o(request.getParameter("isover"));
	 
	RecordSet.executeSql("insert into wasu_projstone(projid,stoneorder,stonename,stonedate,isover) values("+projid+","+stoneorder+",'"+stonename+"','"+stonedate+"',"+isover+")");
...
if(method.equals("saveisover")){   
	String isoverids[]=request.getParameterValues("isoverids");  
	if(isoverids != null)
	{
		String isovers[]=request.getParameterValues("isover");
		for(int i=0;i<isoverids.length;i++)
		{
			ProcPara = isoverids[i]; 
            if (!ProcPara.equals("")) {
//             	System.out.println(" update wasu_projstone set isover="+isovers[i]+" where id="+ProcPara);
                RecordSet.executeSql(" update wasu_projstone set isover="+isovers[i]+" where id="+ProcPara);  
            }  
		}
	} 
	response.sendRedirect("/project/Milestonelist.jsp?projid="+projid);
...
{   
	...
	RecordSet.executeSql(" update wasu_projstone set stoneorder="+stoneorder+",stonename='"+stonename+"',stonedate='"+stonedate+"',isover="+isover+" where id="+id); 
	response.sendRedirect("/project/Milestonelist.jsp?projid="+projid); 
	return;
}
%>