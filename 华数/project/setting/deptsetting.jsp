...
<input type=hidden name=method value="deptsetting"/>
		...
		<table class=ViewForm>
         ...
         <table class=ViewForm id="catalogTable">
			...
			<TR class=Spacing>
             ...
             <%
             int i = -1; 
             rs.executeSql("Select * From formtable_main_244_dt1  order by id asc ");
             while(rs.next()) {  
            	 i++;
             ...
             <tr>
				...
				<td width="30%"> 
					   <button class=browser onClick="onShowDepartment('departmentspan_<%=i%>','departmentid_<%=i%>')"></button>
						<span id=departmentspan_<%=i%> ><%=DepartmentComInfo.getDepartmentname(bm)%></span>
						<input id=departmentid_<%=i%>  name=departmentid type=hidden  value="<%=bm%>">  
				</td>  
				<%-- <td  width="30%">
...
<tr>
	...
	<td width="50%"> 
		<BUTTON class=Browser onClick="onShowDepartment('departmentspan_##row##', 'departmentid_##row##')"></BUTTON>
		<span id="departmentspan_##row##"></span> 
		<INPUT type=hidden id="departmentid_##row##" name=departmentid value="">
	</td>  
	<!-- <td width="30%">
