...
if(!projid.equals("")){
	...
	xmcode=rs.getString("xmcode"); 
	xmjlname=rs.getString("xmjlname");
	xmgs=rs.getString("xmgs");
	xmbm=rs.getString("xmbm");
	lxdh=rs.getString("lxdh");
	sqrq=rs.getString("sqrq");
	xmzl=rs.getString("xmzl");
	xmlb=rs.getString("xmlb");
	xmjlemail=rs.getString("xmjlemail");
	sfndys=rs.getString("sfndys");
	lxlzxs=rs.getString("lxlzxs");
	xmtzysje= rs.getString("xmtzysje");
	xmrjje= rs.getString("xmrjje");
	xmfyzyyjje= rs.getString("xmfyzyyjje");
	xmyzyje= rs.getString("xmyzyje");  
	firsttype= rs.getString("firsttype");  
	xmqtjf= rs.getString("xmqtjf");  
}
String lxlzwhformid= Util.null2o(weaver.file.Prop.getPropValue("projconfig", "lxlzwh")); 
 
String imagefilename = "/images/hdMaintenance.gif";
String titlename ="编辑:项目基本信息";   
String needfav ="1";
String needhelp ="";
...
<FORM id=weaver name= weaver action="/project/CustomerOperation.jsp" method=post onsubmit='return check_form(this,"Name")' enctype="multipart/form-data">
  ...
  <TR>
	...
	<TD vAlign=top>
	  ...
	  <TABLE class=ViewForm>
        ...
        <TR>
          <TD>项目编码</TD>
          <TD class=Field><INPUT class=InputStyle maxLength=50 name="xmcode" STYLE="width:95%"  value="<%=xmcode %>"></TD>
        </TR><tr><td class=Line colspan=2></td></tr>
         <TR>
          <TD>项目经理</TD>
          <TD class=Field>
           <!-- <BUTTON class=Browser id=xmjlid onClick="onShowResourceID()"></BUTTON> 
           <span  id=xmjlidspan> </span>  -->
              <INPUT class=InputStyle type=text name=xmjlname value="<%=xmjlname %>">
        </TD>
        </TR><tr><td class=Line colspan=2></td></tr> 
         <TR> 
          <TD>所属公司</TD>
          <TD class=Field> 
              <select name=xmgs id=xmgs onchange="changeyijifl(this)" > 
                <%  
         ...
         <TR>
          ...
          <TD class=Field>  
               <BUTTON class="Calendar" id="selectBeginDate" onclick="onshowPlanDate(sqrq, selectBeginDateSpan)"></BUTTON> 
				...
				<INPUT type="hidden" name="sqrq" value="<%=sqrq%>">   
        </TD>
        </TR><tr><td class=Line colspan=2></td></tr>     
        <TR>
          <TD>项目经理邮箱	</TD>
          <TD class=Field><INPUT class=InputStyle maxLength=120 name="xmjlemail" value="<%=xmjlemail %>"   STYLE="width:95%">
        ...
        <TR>
          ...
          <TD class=Field>
          	<select name=xmzl>
				<option value=0 <%if(xmzl.equals("0")){ %>selected<%} %>>客户化项目</option>
				<option value=1 <%if(xmzl.equals("1")){ %>selected<%} %>>一般投资项目</option>
				<option value=2 <%if(xmzl.equals("2")){ %>selected<%} %>>重大投资项目</option> 
			</select>
          </TD>
        </TR><tr><td class=Line colspan=2></td></tr>  
        <TR>
        	...
        	<select name=firsttype id="firstype_<%=gsid %>" onchange="getdynlboptions(this,<%=gsid %>)" disabled=true style="display:none" > 
             <option value=-1 > </option> 
        	    ...
        	    rs2.executeSql(" select distinct firsttype from formtable_main_"+lxlzwhformid+"_dt2 where ssgs="+gsid+" order by firsttype asc ");	
        	 	while(rs2.next()){  
        	    %>
				<option value=<%=rs2.getString("firsttype") %> <%if(firsttype.equals(rs2.getString("firsttype"))){%>selected<%} %>   ><%=rs2.getString("firsttype") %></option>
			   <%} %> 
			 </select>
			 <% }  %> 
          ...
          <TD class=Field>
	        ...
	        <option value=-1 > </option> 
			        <% 
		          	...
		          	rs2.executeSql(" select * from formtable_main_"+lxlzwhformid+"_dt2 where ssgs="+xmgs+"  and firsttype='"+firsttype+"' order by sx asc ");	
		      	 	while(rs2.next()){     
		      	    %>
					<option value=<%=rs2.getString("lbzm") %> <%if(xmlb.equals(rs2.getString("lbzm"))){ %>selected<%} %> ><%=rs2.getString("lbmc") %></option>
				   <%}  %> 
			 </select>
			 </td>
        </TR><tr><td class=Line colspan=2></td></tr>     
        <TR>
          <TD>联系电话</TD>
          <TD class=Field><INPUT class=InputStyle maxLength=50 name="lxdh" STYLE="width:95%"  value="<%=lxdh%>"></TD>
        </TR><tr><td class=Line colspan=2></td></tr>        
        <TR>
          <TD>是否年度预算内项目</TD>
          <TD class=Field>
          	<select name=sfndys>
				<option value=0 <%if(sfndys.equals("0")){ %>selected<%} %>>是</option>
				<option value=1 <%if(sfndys.equals("1")){ %>selected<%} %>>否</option>
			</select>
          </TD>
        </TR><tr><td class=Line colspan=2></td></tr>     
        <TR>
          ...
          <TD class=Field>
          	...
          	<% 
          	 ...
          	 while(rs.next()){   
          	%>
				<option value=<%=rs.getString("px") %> <%if(lxlzxs.equals(rs.getString("px"))){ %>selected<%} %>><%=rs.getString("lzmc") %></option>
			   <%}  %> 
			</select>  
          </TD>
        </TR><tr><td class=Line colspan=2></td></tr>     
        <TR>
          <TD>项目投资情况</TD>
          <TD class=Field> 
            投资预算金额:<INPUT class=InputStyle size=6 name="xmtzysje" value="<%=xmtzysje%>">万 （其中软件金额：<INPUT class=InputStyle  size=6 name="xmrjje" value="<%=xmrjje%>">万 硬件金额:<INPUT class=InputStyle  size=6 name="xmfyzyyjje"  value="<%=xmfyzyyjje%>">万 ）云资源金额：<INPUT class=InputStyle size=6 name="xmyzyje"  value="<%=xmyzyje%>">万
          </TD>
        </TR><tr><td class=Line colspan=2></td></tr>
        <TR>
        <TD>项目其他经费</TD>
        <TD class=Field> 
         <INPUT class=InputStyle size=6 name="xmqtjf" value="<%=xmqtjf%>">  
        </TD>
      </TR><tr><td class=Line colspan=2></td></tr>     
...
<script language=javascript>
 ...
 jQuery(document).ready(function(){
	 //alert(jQuery("#xmgs").val()); 
	 var obj=document.getElementById("xmgs");
	 changeyijifl(obj); 
	 
	 //var fr=document.getElementById("firstype_"+obj.value); 
	 //var index = fr.selectedIndex; //选中索引 
	 //var firsttype = fr.options[index].value;//  
	 //getdynlboptions(obj,firsttype);
 });
 ...
 function changefl(obj){    
	 ...
	 jQuery("#xmlb_"+obj.value).attr("disabled",false);     
	 jQuery("#xmlb_"+obj.value).css("display","block");   
 }
