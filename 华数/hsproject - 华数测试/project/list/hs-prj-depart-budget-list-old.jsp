...
<link rel="stylesheet" type="text/css" href="/js/jquery-autocomplete/browser_wev8.css" />
	...
	<BODY>
		...
		<FORM id=report name=report action="/hsproject/project/list/hs-prj-depart-budget-list.jsp" method=post>
		...
		<%
		String backfields = "id,year,department,budget,(select isnull(sum(isnull(t2.usedmoney,0)),0) from hs_projectinfo t,uf_prj_usedmoney t1,uf_prj_usedmoney_dt1 t2 where t.id=t1.prjid and t1.id=t2.mainid and t2.year=a.year and t.belongdepart=a.department) as amount";
		String fromSql  =  " from (select max(id) as id,year,department,isnull(sum(isnull(budget,0)),0) as budget from uf_prj_depbudget group by department,year) a";
		String sqlWhere =  " 1=1  ";
		if(userid !=1){
		...
		"			<head>";
				tableString +="<col width=\"20%\" text=\"年份\" column=\"year\" orderkey=\"year\" />"+ 
						"<col width=\"20%\" text=\"部门\" column=\"department\" orderkey=\"department\" transmethod=\"weaver.hrm.company.DepartmentComInfo.getDepartmentname\" />"+
                		"<col width=\"20%\" text=\"预算总额\" column=\"budget\" orderkey=\"budget\" />"+ 
                		"<col width=\"20%\" text=\"实际发生额\" column=\"amount\" orderkey=\"amount\" linkvaluecolumn=\"id\" linkkey=\"ysid\" href= \"/hsproject/project/list/hs-prj-depart-budget-list-detail-Url.jsp\" target=\"_fullwindow\"/>"+                               
						"</head>"+
		 "</table>";
	//showExpExcel="false"
	%>
