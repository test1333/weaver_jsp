...
<link rel="stylesheet" type="text/css" href="/js/jquery-autocomplete/browser_wev8.css" />
	...
	String out_pageId="hapl";
	String userdp = "";
	String superdp = "";
	if(userid != 1){
	 userdp = projectUtil.getDepartmentId(String.valueOf(userid));
	 superdp = projectUtil.getSupperDepartmentId(userdp);
	}
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(18375,user.getLanguage())%>' attributes="{'groupDisplay':'none'}">
		...
		%>
			<table class="ViewForm" style="margin-left:10px!important;margin-right:10px!important;">
				...
				<tr class=field>
					<%
						...
						List<Map<String,String>> subList = ptd.getPrjTypeCompany();
			...
			<div class="listbox2" >
						<div class='titleitem'>
							<div class="titlecontent" style="border-bottom:2px solid <%=color[0] %>;"><label><%=sub.get("subcompanyname")%></label></div>								
						</div>
						<div class="mainItem">
						<%
							
							List<ProjectTypeBean> ptblist = ptd.getUsedProjectType(sub.get("subcompanyid"));
							if(userid != 1){
								String alldp ="";
								if(!"".equals(userdp)){
									alldp = alldp+userdp;
								...
								ptblist = ptd.getUsedProjectType(alldp,sub.get("subcompanyid"));
							}
							for(ProjectTypeBean ptb:ptblist){
						%>	
						<div class="centerItem" >
							<div class="fontItem" >
							<img name="esymbol" src="/images/ecology8/request/workflowTitle_wev8.png" style="vertical-align: middle;">
								<a href="javascript:onNewPrj2('<%=ptb.getId() %>');"><%=ptb.getTypename()%></a>
							</div>
	...
	function onNewPrj(prjid){
		openFullWindowForXtable("/hsproject/project/view/hs-add-project-url.jsp?prjtype="+prjid);
	}
	function onNewPrj2(prjid){
	var url="/hsproject/project/view/hs-add-project-url.jsp?prjtype="+prjid;
