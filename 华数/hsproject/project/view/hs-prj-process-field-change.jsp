...
<HTML>
	...
	String processname = pid.getProcessName(processid);
	...
	String fieldvalue = pid.getFieldValue(processid,fieldid);
	...
	<BODY>
		...
		<FORM id=report name=report action="/hsproject/project/aciton/save-processfield-change.jsp" method=post>
		...
		<div >
				...
				<wea:group context="修改阶段字段">
				<wea:item>阶段名称</wea:item>
				<wea:item>
				<%=processname%>
                 <input type="hidden" name="processname" id="processname" class="InputStyle" type="text" value="<%=processname%>" />
                </wea:item>
			...
			<%	
					...
					<%
							...
							if("processtype".equals(pfb.getFieldname())){
								fieldValue2 = new ProcessTypeDao().getTypeName(processtype);
							}else{
								fieldValue2 = etm.doProcessEditTrans(pfb,fieldValue2);
