		...
		<FORM id=report name=report action="/hsproject/project/report/hs-show-prj-all-report-list.jsp" method=post>
			...
			<table id="topTitle" cellpadding="0" cellspacing="0">
				<tr>
					...
					<input type="button" value="导出" class="e8_btn_top_first" style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis; max-width: 100px;" onclick="/weaver/weaver.file.ExcelOut;"/>
                ...
                <tbody>
					 <tr>
					 	...
					 	for(PrjAllReportDtBean prdb:prjlist){
							...
							er.addStringValue(showname,"Header");
						%>
					 ...
					 <%
						for(ProcessTypeBean ptb:prblist){
						    ...
						    if(dtMap.get(processid) != null){
						    	...
						    	er.addStringValue(processname+"/"+showname,"Header");	
						  %>
                ...
                <tbody>
					...
					<%
							for(PrjAllReportDtBean prdb:prjlist){
									...
									if("0".equals(iscommon)){
									String fieldtransvalue = pri.prjReportTransMethod(Util.null2String(rs.getString(fieldname)),fieldid);
						...
						%>
											<td class="fvalue"><%=fieldtransvalue%></td>
						<%				
										}
									er.addStringValue(pri.removeHtmlTag(fieldtransvalue));	
									}else{
								    	...
								    	if(rs_dt.next()){
								    		for(PrjAllReportDtBean prdb:dtlist){
								    	    	String fieldname =prdb.getFieldname();
										 		...
										 		String fieldid=prdb.getFieldid();
										 		if("0".equals(iscommon)){
										 			String fieldtransvalue=pri.processReportTransMethod(Util.null2String(rs_dt.getString(fieldname)),fieldid);
										%>
													<td class="fvalue"><%=fieldtransvalue%></td>
										<%			
													er.addStringValue(pri.removeHtmlTag(fieldtransvalue));	
