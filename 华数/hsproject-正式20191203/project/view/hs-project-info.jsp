...
<link rel="stylesheet" type="text/css" href="/js/jquery-autocomplete/browser_wev8.css" />
	...
	if(userid != 1){
	 ...
	 String supdp = projectUtil.getSupperDepartmentId(userdp);
	 String prjdp = projectUtil.getPrjTypeDepart(prjtype);
	  if(!"".equals(userdp) && !"".equals(prjdp)){
	  		if(userdp.equals(prjdp)||supdp.equals(prjdp)){
	  			canedit = "1";
	  		}
	  }
	}else{
	...
	String xmjl = projectUtil.checkisProjectManager(String.valueOf(userid),prjid);
	String allattachs = projectUtil.getProjectAllAttachs(prjid);
	...
	<BODY>
		...
		RCMenu += "{项目文档打包下载,javascript:downloadAllDocs(),_top} " ; 
		...
		<FORM id=report name=report action="/hsproject/project/list/hs-project-list.jsp" method=post>
			...
			<table id="topTitle" cellpadding="0" cellspacing="0">
				<tr>
					...
					<%
						}else if("1".equals(xmjl)){
					%>
						<input type="button" value="编辑" class="e8_btn_top_first" style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis; max-width: 100px;" onclick="editprj2();"/>
					<%		
						}
					%>
            ...
            %>
            		<wea:item><%=vtm.doTrans(pfb.getId(),fieldValue, "0")%>  元	    	
					</wea:item>		
			<%
	...
	function onshowPlanDate1(inputname,spanname,isMustInput){
		...
		function editprj2(){
			window.location.href="/hsproject/project/view/hs-edit-project-djr.jsp?id=<%=prjid%>&prjtype=<%=prjtype%>&isxmjl=1";
		}
		function downloadAllDocs(){ 
			window.location="/weaver/weaver.file.FileDownload?fieldvalue=<%=allattachs%>&download=1&downloadBatch=1&requestid="; 
		}
