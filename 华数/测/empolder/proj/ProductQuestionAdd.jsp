...
<%
  String sql = "select * from produceQuestion where productVersionID = '"+productVersionID+"' and isShow = '0' order by id ";
  //out.println(sql);
  rs.executeSql(sql);
%>
	<TR>
...
<%-- 				<input class=inputstyle type='text' name='isOK' id='isOK_<%=rownum %>' value=<%=isOK %>></TD>								 --%>
			  <TD class=Field width=35%>
				 <input class=inputstyle type='text' name='terms' id='terms_<%=rownum %>' value=<%=terms %>>
...
function doSubmit(){
	weaver.action="ProductQuestionOption.jsp";
	weaver.submit();
}
