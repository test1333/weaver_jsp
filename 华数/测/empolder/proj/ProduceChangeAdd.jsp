...
<%
  String sql = "select * from changeInfo where ProjID = '"+ProjID+"' and productVersionID = '"+productVersionID+"' order by id ";
  //out.println(sql);
  rs.executeSql(sql);
%>
	<TR>
		  ...
		  <tr class=header>
	       ...
	       <tr id = "Row_<%= ++rownum %>"  class = 'opera' >
			  ...
			  <TD class=Field width=15%>
				<input class=inputstyle type='text' name='content' id='content_<%=rownum %>' value=<%=content %> size="50"></TD>	
			  <TD class=Field width=10%>
...
function doSubmit(){
	weaver.action="ProduceChangeOption.jsp";
	weaver.submit();
}
...
function addRow(obj){
	...
	if(obj=='oTable'){
		...
		trStr += "  <INPUT class=inputstyle id='content_"+rownum+"' type=text  name='content' size='50'>";
