...
<TH width=35%>名称</TH>
<TH width=35%>描述</TH>
<TH width=35%>所属业务线</TH>
</tr><TR class=Line style="height: 1px"><TH colspan="4" ></TH></TR>
<%
int i=0;
sql = "select t1.*,t2.name as cname from productProject t1 left join serviceLine t2 on t1.forserverline = t2.id"+
     " where t1.isShow = '0' and t1.forserverline in (select id from serviceLine where personnel_1 like '%,"+user.getUID()+",%' or personnel_1 like '%,"+user.getUID()+"' or personnel_1 like '%"+user.getUID()+",%' or personnel_1 = '"+user.getUID()+"')"+sqlwhere+" order by t1.order_o,t1.id";
