...
if(!"".equals(formTime)) andSql+=" and t1.formTime >= '"+formTime+"'";
if(!"".equals(toTime)) andSql+=" and t1.toTime <= '"+toTime+"'";
...
<input class=inputstyle type=hidden name=operation value="add">
                         ...
                         <!--搜索部分-->
                          ...
                          <%
                                ...
                                //设置好搜索条件
                                String backFields =" t1.id,t1.num,t1.name,t1.version, case t1.priority when '0' then '高' when '1' then '中' when '2' then '低' end as priority,t1.timeLine, case t1.execution when '0' then '正常' when '1' then '延期' when '2' then '暂缓' when '3' then '完成' end as execution ,t1.productManager,t1.projectManager,t2.dutynameid ";
                                String fromSql = " projectInfo t1";
                                       fromSql += " left join ProductDictionary t2 on t1.name = t2.id";
//                                 String sqlWhere = " where t1.isShow = '0' and t1.projectManager = '"+userID+"' "+andSql;
								String sqlWhere = "";
								if(isAll){
									sqlWhere = "where t1.isShow = '0'";
								}else{
									sqlWhere = " where t1.isShow = '0' and (t2.createPerson = '"+userID+"' or t2.dutynameid = '"+userID+"' or t1.projectManager = '"+userID+"' or t1.productManager = '"+userID+"' or (t1.members like '%,"+userID+",%' or t1.members like '%,"+userID+"' or t1.members like '"+userID+",%' or t1.members = '"+userID+"')"
											+"or t1.forline in (select id from serviceLine where ','||personnel_1||',' like '%,"+user.getUID()+",%')) "+andSql;
								}
...
//                                              "<col width=\"10%\"  text=\"编号\" column=\"num\" orderkey=\"num\" />"+
                                       if(isAll){
                                    	   tableString += "<col width=\"10%\"  text=\"产品线名称\" column=\"name\" orderkey=\"name\" />";
                                       }else{
                                    	   tableString += "<col width=\"10%\"  text=\"产品线名称\" column=\"name\" orderkey=\"name\" href=\"ProductAdd.jsp\" linkkey=\"id\" linkvaluecolumn=\"id\" target=\"_self\" transmethod=\"tool.tools.getProductLineName\"/>";
                                       }
                                       tableString += "<col width=\"10%\"  text=\"产品版本\" column=\"version\" orderkey=\"version\" />"+
                                             "<col width=\"10%\"  text=\"优先级\" column=\"priority\" orderkey=\"priority\" />"+
                                             "<col width=\"10%\"  text=\"计划上线时间\" column=\"timeLine\" orderkey=\"timeLine\" />"+
...
function onSearch(){
        document.frmmain.action="ProductSearchList.jsp";
...
if NOT isempty(id) then
	...
	frmmain.productManager.value=id(0)
	else
	productManagerspan.innerHtml = ""
	frmmain.productManager.value=""
	end if
end if
