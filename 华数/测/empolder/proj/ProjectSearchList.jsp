...
String needhelp ="";
    ...
    //构建where语句
    String andSql="";
    if(!"".equals(procode)) andSql+=" and t1.procode like '%"+procode+"%'";
                          ...
                          <%
                                ...
                                String sqlWhere = ""; 
                                		if(isAll){
                                			...
                                			sqlWhere = " where (t1.id in (select t1.id  from Prj_ProjectInfo t1 left join (select t1.id,t1.members from projectInfo t1 left join ProductDictionary t2 on t1.name = t2.id where t1.isShow = '0' and t2.dutynameid != '"+userID+"') t2 on t1.productversionid = t2.id"+
												  " where t1.businessleader = '"+userID+"' or t1.producemanager = '"+userID+"' or t1.projectmanager = '"+userID+"' or technicalmanager = '"+userID+"'"+ 
                                                  " or (t1.members = '"+userID+"' or t1.members like '"+userID+",%' or t1.members like '%,"+userID+",%' or t1.members like '%,"+userID+"'))"+
//                                                   " or (t2.members = '"+userID+"' or t2.members like '"+userID+",%' or t2.members like '%,"+userID+",%' or t2.members like '%,"+userID+"')) "+
                                                  " or t1.id in (select t1.id from Prj_ProjectInfo t1 join (select t1.id from projectInfo t1 left join ProductDictionary t2 on t1.name = t2.id where t1.isShow = '0' and t2.dutynameid = '"+userID+"') t2 on t1.productprojectid = t2 .id)) "+andSql;
                                		}
...
//                                 out.print(backFields+fromSql+sqlWhere);
                                ...
                                String tableString=""+
                                       ...
                                       "<head>";
                                       if(isAll){
                                    	   tableString += "<col width=\"10%\"  text=\"子项目编号\" column=\"procode\" orderkey=\"procode\"/>";
                                       }else{
                                    	   tableString += "<col width=\"10%\"  text=\"子项目编号\" column=\"procode\" orderkey=\"procode\" href=\"/proj/data/ViewProject.jsp\" linkkey=\"ProjID\" linkvaluecolumn=\"id\" target=\"_self\"/>";  
                                       }
                                       tableString += "<col width=\"10%\"  text=\"产品开发项目\" column=\"productprojectID\" orderkey=\"productprojectID\" transmethod=\"tool.tools.getProductPorjectName\"/>"+
