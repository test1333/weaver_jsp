...
if(!id.equals("")){
	...
	if(RecordSet.next()){
		...
		isover=Util.null2String(RecordSet.getString("isover"));
    }
}
...
RCMenu += "{"+SystemEnv.getHtmlLabelName(86,user.getLanguage())+",javascript:submitData2(),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
    }else{
  ...
  <TR class=Sapcing>
     ...
     <TR><TD class=Line colSpan=2></TD></TR>
        ...
        <tr class="Spacing"><td colspan=2 class="Line"></td></tr>
          ...
          <TD>里程碑日期</TD>
          <TD class=Field>
            <button type="button" class=Calendar id="selectBeginDate" onclick="onshowPlanDate('stonedate','selectBeginDateSpan')"></BUTTON> 
              	<SPAN id=selectBeginDateSpan ><%=stonedate%></SPAN> 
              	<INPUT type="hidden" name="stonedate" id="stonedate" value="<%=stonedate%>">  
        </TR>
        <tr class="Spacing"><td colspan=2 class="Line"></td></tr>
           <TR>
          <TD>是否已完成</TD>
          <TD class=Field>
			   <select name=isover> 
			      <option value=0  <%if(isover.equals("0")){ %>selected<%} %> >否</option>
			      <option  value=1 <%if(isover.equals("1")){ %>selected<%} %>>是</option>
			   </select>
		  </TD>
        </TR>
        <TR><TD class=Line colSpan=2></TD></TR>
...
<input class=inputstyle type="hidden" name="method"  id=method value="delete"> 
<input class=inputstyle type="hidden" name="projid" id=projid value="<%=projid%>"> 
<TABLE class=form>
  <COLGROUP>
  <COL width="20%">
  <COL width=80%>
...
<%		}%>
			...
			<th width=10><%if(canedit){%><input class=inputstyle type=checkbox name=stoneids value="<%=RecordSet.getString("id")%>"><%}%>
			<input class=inputstyle type=hidden name=isoverids value="<%=RecordSet.getString("id")%>"> 
			</th>
			<td>
			...
			<td>
					<select name=isover  > 
					      <option value=0  <%if(RecordSet.getString("isover").equals("0")){ %>selected<%} %> >否</option>
					      <option  value=1 <%if(RecordSet.getString("isover").equals("1")){ %>selected<%} %>>是</option>
					 </select>
			</td>
    </tr>
<%
...
function submitData2(){  
	document.getElementById("method").value="saveisover";
	weaverD.method.value="saveisover"; 
	weaverD.submit(); 
}
