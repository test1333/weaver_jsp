...
<link rel="stylesheet" type="text/css" href="/js/jquery-autocomplete/browser_wev8.css" />
	...
	String out_pageId = "bmys01-cpmpany_"+subcompany;
	...
	<BODY>
		...
		<FORM id=report name=report action="/hsproject/project/list/hs-prj-depart-budget-list-company.jsp" method=post>
		...
		String fromSql  =  " from (select max(id) as id,year,department,nvl(sum(nvl(budget,0)),0) as budget,nvl(sum(nvl(amount,0)),0) as amount from uf_prj_depbudget where company='"+subcompany+"' group by department,year order by id desc) a";
		String sqlWhere =  " 1=1  ";
		...
		String operateString= "";
		  tableString =" <table tabletype=\"none\" pagesize=\""+ PageIdConst.getPageSize(out_pageId,user.getUID(),PageIdConst.HRM)+"\" pageId=\""+out_pageId+"\" >"+         
				   "	   <sql backfields=\""+backfields+"\" sqlform=\""+fromSql+"\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlWhere)+"\"  sqlorderby=\""+orderby+"\"  sqlprimarykey=\"id\" sqlsortway=\"desc\" sqlisdistinct=\"false\" sumColumns=\"budget,amount,amounthq\" showCountColumn=\"true\" decimalFormat=\"%,.2f|%,.2f|%,.2f\"  />"+
		operateString+
		"			<head>";
				tableString +="<col width=\"6%\" text=\"序号\" column=\"rownum\" orderkey=\"rownum\" />"+
						"<col width=\"10%\" text=\"年份\" column=\"year\" orderkey=\"year\" />"+ 
						"<col width=\"14%\" text=\"部门\" column=\"department\" orderkey=\"department\" transmethod=\"weaver.hrm.company.DepartmentComInfo.getDepartmentname\" />"+
                		"<col width=\"20%\" text=\"预算总额\" column=\"budget\" orderkey=\"budget\" />"+ 
                		"<col width=\"20%\" text=\"实际发生额\" column=\"amount\" orderkey=\"amount\" />"+                               
						"<col width=\"10%\" text=\"发生率\" column=\"fsl\" orderkey=\"fsl\" />"+   
						"<col width=\"20%\" text=\"实际发生额(获取)\" column=\"amounthq\" orderkey=\"amounthq\" linkvaluecolumn=\"id\" linkkey=\"ysid\" href= \"/hsproject/project/list/hs-prj-depart-budget-list-detail-Url.jsp\" target=\"_fullwindow\"/>"+
						"</head>"+
		 "</table>";
	//showExpExcel="false"
	%>
