...
<%
	...
	String titleFName = "传媒-部门预算使用情况（单位：万元）";
	...
	int minnum = 100;
	int maxnum = 2000;
	String sql = "select max(budget) as maxbudget from (select sum(nvl(budget,0)) as budget from uf_prj_depbudget where year='"+currentyear+"' and company=6 group by department)";
	rs.executeSql(sql);
	if(rs.next()){
	...
	sql = " select department,(select departmentname from hrmdepartment where id=department) as departmentname,sum(nvl(budget,0)) as budget,sum(nvl(amount,0)) as amount from uf_prj_depbudget where year='"+currentyear+"' and company=6 group by department";
	//out.println("sql:"+sql);
	rs.executeSql(sql);
...
<!-- 代码 开始 -->
    <div>
        <div id="jqChart" style="width: 900px; height: 350px;"></div>
    </div>
<!-- 代码 结束 -->
</div>
