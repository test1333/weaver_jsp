...
<link rel="stylesheet" type="text/css" href="/js/jquery-autocomplete/browser_wev8.css" />
    ...
    String out_pageId = "hsxmgc-inner3";
    String canedit="0";
	if(userid != 1){
	...
	<BODY>
		...
		<%
		String backfields = "rownum,id,prjid,processname,prjtype,fzrid,processtype,begindate,enddate,status,case when isused='1' then '停用' else '启用' end as isused";
		String fromSql  =  " from (select * from hs_prj_process where  prjid='"+prjid+"' order by id asc) t1";
		...
		if("1".equals(canedit)){
           operateString = "<operates width=\"20%\">"+
							...
							"     <operate isalwaysshow=\"true\" href=\"javascript:updatestatus();\" linkkey=\"id\" linkvaluecolumn=\"id\" text=\"启用/停用\" index=\"2\"/> "+
		...
		"			<head>";
				tableString +="<col width=\"6%\" text=\"序号\" column=\"rownum\" orderkey=\"rownum\" />"+
						"<col width=\"12%\" text=\"阶段名称\" column=\"processname\" orderkey=\"processname\" linkvaluecolumn=\"id\" linkkey=\"id\" href= \"/hsproject/project/view/hs-process-info-url.jsp\" target=\"_fullwindow\"/>"+ 
						...
						"<col width=\"12%\" text=\"状态\" column=\"status\" orderkey=\"status\"/>"+
						"<col width=\"12%\" text=\"启用/停用\" column=\"isused\" orderkey=\"isused\"/>"+
	...
	<script type="text/javascript">
		...
		function updatestatus(id){
			...
			jQuery.ajax({
				type: "POST",
				url: "/hsproject/project/aciton/update-process-status.jsp",
				data: {'processid':id, 'type':'check'},
				dataType: "text",
			...
			if(result == "0"){
				if(confirm("是否确定要停用")){
					...
					jQuery.ajax({
						type: "POST",
						url: "/hsproject/project/aciton/update-process-status.jsp",
						data: {'processid':id, 'type':'update', 'updatetype':'0'},
						dataType: "text",
						async:false,//同步   true异步
						success: function(data){
							data=data.replace(/^(\s|\xA0)+|(\s|\xA0)+$/g, '');
				...
				if(confirm("是否确定要启用")){
					jQuery.ajax({
						type: "POST",
						url: "/hsproject/project/aciton/update-process-status.jsp",
						data: {'processid':id, 'type':'update', 'updatetype':'2'},
						dataType: "text",
						async:false,//同步   true异步
						success: function(data){
							data=data.replace(/^(\s|\xA0)+|(\s|\xA0)+$/g, '');
