...
<script type="text/javascript" src="/js/swfupload/handlers_wev8.js"></script>
	...
	ProjectTypeDao ptd = new ProjectTypeDao();
	ProjectTypeBean ptb = ptd.getProjectType(prjtype);

	String subcompany = ptb.getSubcompany();
	BaseBean log = new BaseBean();
	Map<String, String> fieldsmap = new ProjectFieldDao().getMustFieldNames(prjtype);
	String fieldnames =fieldsmap.get("fieldnames");
	//out.print("fieldnames"+fieldnames);
	String isattachmust = fieldsmap.get("isattachmust");;
	//out.print("isattachmust"+isattachmust);
	String accsize ="100";
	String accsec="5801";//目录测5701 正5801
	String attach="";
	ProjectUtil pu = new ProjectUtil();
	String filetyps=pu.getFileTypes();
	%>
	<BODY>
		<div id="tabDiv">
		...
		<FORM id=report name=report action="/hsproject/project/aciton/submit-project-info.jsp" method=post enctype="multipart/form-data">
			...
			<input type="hidden" name="submitType_mt" id="submitType_mt"  value="add">
			...
			<table id="topTitle" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td class="rightSearchSpan" style="text-align:right;">
						<input type="button" value="保存" class="e8_btn_top_first" style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis; max-width: 100px;" onclick="saveprj(this);"/>
				...
				function uploadComplete(fileObj) {
					try {
						...
						if (this.getStats().files_queued === 0) {
							...
							if(isattachmust == "1" && jQuery("#attach").val()==""){
			...
			%>
                        ...
                        <INPUT type="hidden" name="<%=showfieldname%>" id="<%=showfieldname%>" value="">
			...
			<%	
					...
					<%
							...
							if("prjtype".equals(pfb.getFieldname())){
								fieldValue = new ProjectTypeDao().getTypeName(prjtype);
							}else if("belongCompany".equals(pfb.getFieldname())){
								fieldValue = etm.doProjectEditTrans(pfb,subcompany);
							}else{
								fieldValue = etm.doProjectEditTrans(pfb,"");
				...
				%>
						...
						<wea:item><%=pfb.getShowname()%></wea:item>
				<%
					if("prjbudget".equals(pfb.getFieldname()) && "0".equals(pfb.getIscommon())){
			%>
					<wea:item><%=fieldValue%>  万元	    	
					</wea:item>	
			<%
				    }else if("softwareamount".equals(pfb.getFieldname()) && "0".equals(pfb.getIscommon())){
            %>
            		<wea:item><%=vtm.doTrans(pfb.getId(),fieldValue, "0")%>  万元	    	
					</wea:item>	
			<%
				    }else if("hardwareamount".equals(pfb.getFieldname()) && "0".equals(pfb.getIscommon())){
            %>
            		<wea:item><%=vtm.doTrans(pfb.getId(),fieldValue, "0")%>  万元	    	
					</wea:item>	
			<%
				    }else if("otheramount".equals(pfb.getFieldname()) && "0".equals(pfb.getIscommon())){
            %>
            		<wea:item><%=vtm.doTrans(pfb.getId(),fieldValue, "0")%>  万元	    	
					</wea:item>	

			<%
				    }else if("cloudamount".equals(pfb.getFieldname()) && "0".equals(pfb.getIscommon())){
            %>
            		<wea:item><%=vtm.doTrans(pfb.getId(),fieldValue, "0")%>  万元	    	
					</wea:item>	
			
			<%
				    }else if("prjamount".equals(pfb.getFieldname()) && "0".equals(pfb.getIscommon())){
            %>
            		<wea:item><%=fieldValue%>  元	    	
					</wea:item>		 
			<%
				    }else if("purchaseamount".equals(pfb.getFieldname()) && "0".equals(pfb.getIscommon())){
            %>
            		<wea:item><%=fieldValue%>  元	    	
					</wea:item>	   
            <%
