	  ...
	  <TABLE id=AccessoryTable class="ViewForm" style="margin-top:10px;">
		...
		<COL width="15%">
          ...
          <TR>
          	<TD >会议纪要名称</TD>
          	<td class=field>
          		<input type=text class=inputStyle id="meetingname" name="meetingname" onchange='checkinput("meetingname","meetingnamemage")' style="width:40%"><SPAN id=meetingnamemage><IMG src='/images/BacoError.gif' align=absMiddle></SPAN>
          	</td>
         </TR>
         <TR class=spacing>
          	<TD class=line colspan=2></TD></TR>
          	...
          	<TD >会议时间</TD>
          	<td class=field>
          		 <button type="button" class=Calendar id="selectBeginDate" onclick="onshowPlanDate('meetingtime','selectBeginDateSpan')"></BUTTON> 
              	<SPAN id=selectBeginDateSpan ><IMG src='/images/BacoError.gif' align=absMiddle></SPAN> 
          	...
          	<TD >相关项目</TD> 
          	<td class=field>
          		 <BUTTON class=browser id="selectProjids" onclick="onshowProject('meetingprojidsSpan','meetingprojids')"></BUTTON> 
				<SPAN id="meetingprojidsSpan" name=meetingprojidsSpan> <IMG src='/images/BacoError.gif' align=absMiddle>
				</SPAN> 
				<INPUT type="hidden" name="meetingprojids"  > 
          	</td>
         </TR>
...
function doSave(obj){
	if(check_form(weaver,"meetingname,meetingtime,meetingprojids")){ 
		obj.disabled = true; 
...
sub onshowProject(spanname,inputename)
		tmpids = document.all(inputename).value
		id1 = window.showModalDialog("/systeminfo/BrowserMain.jsp?url=/interface/MultiCommonBrowser.jsp?type=browser.xgxm&beanids=&splitflag=,"&tmpids)
