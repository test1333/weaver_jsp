...
<FORM id=weaver name= weaver action="/project/ProjTaskOperation.jsp" method=post  enctype="multipart/form-data">
          ...
          <TD class=Field>  
                 <select name="wdfl"  id="wdfl" >
                   ...
                   <option value=<%=rs.getString("id") %>  ><%=rs.getString("flmc") %></option>
               		 <%}  %>  
      			 </select>  <IMG  id="wdflerror" src="/images/BacoError.gif" align=absMiddle>
        </TD>
        </TR><tr><td class=Line colspan=4></td></tr>   
       	<%  
...
function doSave(obj){
	var wdfl_val=jQuery("#wdfl").val();
	if(wdfl_val=='' || wdfl_val =="-1"){
	 alert("文档分类必填");
	 return;	
       }
	if(check_form(document.weaver,"taskname,sqrq"))
	{
	   obj.disabled=true;
