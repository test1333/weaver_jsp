...
<HTML>
	...
	out.print("prjstatus_x"+prjstatus_x);
		...
		<FORM id=report name=report action="/CRM/report/getProjectReportAll.jsp" method=post>
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;">
				...
				<option value="" <%if("".equals(status)){%> selected<%} %>>
					<%=""%></option>
				<option value="0" <%if("0".equals(status)){%> selected<%} %>>
					<%="草稿"%></option>
				<option value="1" <%if("1".equals(status)){%> selected<%} %>>
					<%="評估取消"%></option>
				<option value="2" <%if("2".equals(status)){%> selected<%} %>>
					<%="專案凍結"%></option>
				<option value="3" <%if("3".equals(status)){%> selected<%} %>>
					<%="專案完成"%></option>
				<option value="4" <%if("4".equals(status)){%> selected<%} %>>
					<%="凍結"%></option>
				<option value="5" <%if("5".equals(status)){%> selected<%} %>>
					<%="評估中"%></option>
				<option value="6" <%if("6".equals(status)){%> selected<%} %>>
					<%="---"%></option>
		    	<option value="7" <%if("7".equals(status)){%> selected<%} %>>
					<%="審批退回"%></option>
				<option value="8" <%if("8".equals(status)){%> selected<%} %>>
					<%="制令段-工程"%></option>
				<option value="9" <%if("9".equals(status)){%> selected<%} %>>
					<%="業務段-規劃中"%></option>
				<option value="10" <%if("10".equals(status)){%> selected<%} %>>
					<%="業務段-已報價"%></option>
				<option value="11" <%if("11".equals(status)){%> selected<%} %>>
					<%="制令段-驗收"%></option>
				<option value="12" <%if("12".equals(status)){%> selected<%} %>>
					<%="製令段-資材"%></option>
				<option value="13" <%if("13".equals(status)){%> selected<%} %>>
					<%="制令段-製造"%></option>
				<option value="14"<%if("14".equals(status)){%> selected<%} %>>
					<%="制令段-裝機"%></option>
				</select>
				</wea:item>
