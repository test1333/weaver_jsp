...
<HTML>
  ...
  <link type="text/css" rel="stylesheet" href="/wui/theme/ecology8/skins/default/wui_wev8.css" />
	<BODY >
		...
		<FORM id=report name=report action="/tjj/document/report/doc-report-yf.jsp" method=post>
            ...
            <table class="ViewForm1  maintable" scrolling="auto">
                ...
                <tbody>
					 <tr>
						...
						<td class="fname"  align="center" rowspan="2">月份</td>
            ...
            <table class="ViewForm1  maintable" scrolling="auto">
                ...
                <tbody>
					<%
						int seq = 1;
						if(!"".equals(year)){
							sql = "select * from (select yf,dbo.f_get_documentcount('',yf,"+year+",'',2,0) as gzdt,dbo.f_get_documentcount('',yf,"+year+",'',2,1) as jjxx,dbo.f_get_documentcount('',yf,"+year+",'',2,0)+dbo.f_get_documentcount('',yf,"+year+",'',2,1) as hj1,dbo.f_get_documentcount('',yf,"+year+",'',2,2) as tjfx,dbo.f_get_documentcount('',yf,"+year+",'',2,3) as tjnc,dbo.f_get_documentcount('',yf,"+year+",'',2,4) as djwg,dbo.f_get_documentcount('',yf,"+year+",'',2,2)+dbo.f_get_documentcount('',yf,"+year+",'',2,3)+dbo.f_get_documentcount('',yf,"+year+",'',2,4) as hj2 from uf_Documentreport where 1=1 and nf="+year+"  and (yf<>'' and yf is not null) group by yf) a order by yf asc";
						}else{
							sql = "select * from (select yf,dbo.f_get_documentcount('',yf,'','',2,0) as gzdt,dbo.f_get_documentcount('',yf,'','',2,1) as jjxx,dbo.f_get_documentcount('',yf,'','',2,0)+dbo.f_get_documentcount('',yf,'','',2,1) as hj1,dbo.f_get_documentcount('',yf,'','',2,2) as tjfx,dbo.f_get_documentcount('',yf,'','',2,3) as tjnc,dbo.f_get_documentcount('',yf,'','',2,4) as djwg,dbo.f_get_documentcount('',yf,'','',2,2)+dbo.f_get_documentcount('',yf,'','',2,3)+dbo.f_get_documentcount('',yf,'','',2,4) as hj2 from uf_Documentreport where 1=1 and (yf<>'' and yf is not null) group by yf) a order by yf asc";	
						}
						rs.execute(sql);
						while(rs.next()){
					...
					<tr >
						...
						<td class="fvalue" align="center"><%=yf%>月</td>
	...
	window.onload=function(){
		...
		function refersh() {
	     ...
	     function daochu(){
		   var year="<%=year%>";
		   window.open("/tjj/document/report/exportexcel.jsp?type=2&year="+year);
		}
