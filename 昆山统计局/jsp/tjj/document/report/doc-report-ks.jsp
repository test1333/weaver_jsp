...
<HTML>
  ...
  <link type="text/css" rel="stylesheet" href="/wui/theme/ecology8/skins/default/wui_wev8.css" />
	<BODY >
		...
		<FORM id=report name=report action="/tjj/document/report/doc-report-ks.jsp" method=post>
            ...
            <table class="ViewForm1  maintable" scrolling="auto">
                ...
                <tbody>
					 <tr>
						...
						<td class="fname"  align="center" rowspan="2">科室</td>
            ...
            <table class="ViewForm1  maintable" scrolling="auto">
                ...
                <tbody>
					<%
						int seq = 1;
						if(!"".equals(year)){
							sql = "select (select departmentname from hrmdepartment where id=ks) as ks,dbo.f_get_documentcount('','',"+year+",ks,1,0) as gzdt,dbo.f_get_documentcount('','',"+year+",ks,1,1) as jjxx,dbo.f_get_documentcount('','',"+year+",ks,1,0)+dbo.f_get_documentcount('','',"+year+",ks,1,1) as hj1,dbo.f_get_documentcount('','',"+year+",ks,1,2) as tjfx,dbo.f_get_documentcount('','',"+year+",ks,1,3) as tjnc,dbo.f_get_documentcount('','',"+year+",ks,1,4) as djwg,dbo.f_get_documentcount('','',"+year+",ks,1,2)+dbo.f_get_documentcount('','',"+year+",ks,1,3)+dbo.f_get_documentcount('','',"+year+",ks,1,4) as hj2 from uf_Documentreport where 1=1 and nf="+year+"  and (ks<>'' and ks is not null) group by ks";
						}else{
							sql = "select (select departmentname from hrmdepartment where id=ks) as ks,dbo.f_get_documentcount('','','',ks,1,0) as gzdt,dbo.f_get_documentcount('','','',ks,1,1) as jjxx,dbo.f_get_documentcount('','','',ks,1,0)+dbo.f_get_documentcount('','','',ks,1,1) as hj1,dbo.f_get_documentcount('','','',ks,1,2) as tjfx,dbo.f_get_documentcount('','','',ks,1,3) as tjnc,dbo.f_get_documentcount('','','',ks,1,4) as djwg,dbo.f_get_documentcount('','','',ks,1,2)+dbo.f_get_documentcount('','','',ks,1,3)+dbo.f_get_documentcount('','','',ks,1,4) as hj2 from uf_Documentreport where 1=1  and (ks<>'' and ks is not null) group by ks";
						}
						rs.execute(sql);
						while(rs.next()){
					...
					<tr >
						...
						<td class="fvalue" align="center"><%=ks%></td>
	...
	window.onload=function(){
		...
		function refersh() {
	     ...
	     function daochu(){
		   var year="<%=year%>";
		   window.open("/tjj/document/report/exportexcel.jsp?type=1&year="+year);
		}
