...
//	String tmp_val = "@"+java.net.URLEncoder.encode(ResourceComInfo.getResourcename(empID))+"%20%20";
	...
	if("addRecord".equals(type)){
		response.sendRedirect("/formmode/view/AddFormMode.jsp?modeId=2041&formId=-541&type=1");  
	}
	if("editRecord".equals(type)){
		response.sendRedirect("/formmode/view/AddFormMode.jsp?modeId=2041&formId=-541&type=2&viewfrom=fromsearchlist&billid="+customID);  
	}
	if("daoru".equals(type)){
		response.sendRedirect("/gvo/check/ExcelIn.jsp");  
	}
