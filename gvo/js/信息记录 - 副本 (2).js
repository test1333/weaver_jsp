...
<script type="text/javascript">
   ...
   jQuery(document).ready(function(){
	 ...
	 dodetail3();
	  jQuery(lclx).bindPropertyChange(function () {
        ...
        jQuery("button[name=addbutton3]").live("click", function () {
        ...
        checkCustomize = function () {
        	var indexnum3 = jQuery("#indexnum3").val();
        	 for(var index=0;index<indexnum3;index++){
        	   	 if (jQuery("#"+wzxxjl_dt4 + index).length > 0) {
        	   	 	 var wzxxjl_dt4_val = jQuery("#"+wzxxjl_dt4 + index).val();
        	   	 	 var cgzzsj_dt4_val = jQuery("#"+cgzzsj_dt4 + index).val();
        	   	 	 if(wzxxjl_dt4_val == '' && cgzzsj_dt4_val==''){
        	   	 	 	 alert("完整信息记录与采购组织数据必须填写一个请检查");
        	   	 	 	 return false;
        	   	        }
        	       }
        	 }
        ...
        for( var index=0;index<indexnum0;index++){
            if( jQuery("#"+xxjlbh_dt1+index).length>0){
            	 if(lclx_val == '1'){
            	       ...
            	       addcheck(gc_dt1+index,'1');
            	       addcheck(cgzz_dt1+index,'1');
            	       addcheck(xxjlxx_dt1+index,'1');
            	       addcheck(gysbh_dt1+index,'1');
            	       addcheck(wlbh_dt1+index,'1');
            	       addcheck(jj_dt1+index,'0');
   ...
   function bindchangefzfm(index){
   	   ...
   	   jQuery("#"+gjmytk_dt1+index).bindPropertyChange(function () {
   	     ...
   	     jQuery("#"+sfjtj_dt1+index).bindPropertyChange(function () {
   	    	var sfjtj_dt1_val = jQuery("#"+sfjtj_dt1+index).val();
   	    	 if(sfjtj_dt1_val != '' ){
   	    	 	addcheck(jtyjsl_dt1+index,'0');
   	    	 	addcheck(yjdj_dt1+index,'0');
   	    	 	addcheck(jtejsl_dt1+index,'0');
   	    	 	addcheck(ejdj_dt1+index,'0');
   	    	}else{
   	         removecheck(jtyjsl_dt1+index,'0');
   	         removecheck(yjdj_dt1+index,'0');
   	         removecheck(jtejsl_dt1+index,'0');
   	         removecheck(ejdj_dt1+index,'0');
   	       }
   	    }) 
   }
   function bindchangedw(index){
   	   jQuery("#"+jbdw_dt1+index).bindPropertyChange(function () {
   	         var jbdw_dt1_val= jQuery("#"+jbdw_dt1+index).val();
  ...
  function setxxjlbhall(){
  	   ...
  	   var indexnum0 =jQuery("#indexnum0").val();
  	    for( var index=0;index<indexnum0;index++){
  	       if( jQuery("#"+xxjlbh_dt1+index).length>0){
  	            ...
  	            if(xxjlbh_dt1_val !=""){
  	               xxhlbhall_val=	xxhlbhall_val+flag1+flag2+"#"+xxjlbh_dt1_val+"#";
  	               flag1="";
  	               flag2=",";
  	            }
  	       }
       ...
       var lclx_val = jQuery(lclx).val();
        for( var index=0;index<indexnum1;index++){
            if( jQuery("#"+xxjlbh_dt2+index).length>0){
   	    	 ...
   	    	 if(sfjtj_dt2_val != '' ){
   	    	 	addcheck(jtyjsl_dt2+index,'0');
   	    	 	addcheck(yjdj_dt2+index,'0');
   	    	 	addcheck(jtejsl_dt2+index,'0');
   	    	 	addcheck(ejdj_dt2+index,'0');
   	    	}
            	         bindchangejt2(index);
            	  }
            }
   ...
   function bindchangejt2(index){
	jQuery("#"+sfjtj_dt2+index).bindPropertyChange(function () {
   	    	var sfjtj_dt2_val = jQuery("#"+sfjtj_dt2+index).val();
   	         ...
   	         removecheck(jtyjsl_dt2+index,'0');
   	         removecheck(yjdj_dt2+index,'0');
   	         removecheck(jtejsl_dt2+index,'0');
   	         removecheck(ejdj_dt2+index,'0');
   	       }
   	    }) 
   }
   ...
   function showhideButton(){
      ...
      if(lclx_val == '1' ){
      	 jQuery("button[name=sapmulbutton1]").css('display','none');
      }
       //if(lclx_val == '2' ){
