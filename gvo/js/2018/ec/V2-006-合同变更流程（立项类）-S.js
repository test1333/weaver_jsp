...
<script type="text/javascript">
    ...
    var bghsfktj="#field8361";//变更后收付款条件
    var sfktj="#field8323";//收付款条件
    var bg4="#field8347";//变更4
    var bghhtje="#field8362";//变更后合同金额
    var yhtje="#field8324";//原合同金额
	
	var sfwlx="#field15928";//是否为立项
    jQuery(document).ready(function(){
	    var sfwlx_val=jQuery(sfwlx).val();
	    if(sfwlx_val == "0"){
		   dolx();
		   jQuery(sfwlx+"_browserbtn").attr('disabled',true);
		}else if(sfwlx_val == "1"){
			jQuery("#mxdt1").hide();
			jQuery("#mxdt2").hide();
			jQuery(sfwlx+"_browserbtn").attr('disabled',true);
		}
	   jQuery(sfwlx).bindPropertyChange(function () {
			var sfwlx_val=jQuery(sfwlx).val();
			if(sfwlx_val == "0"){
				dolx();
				jQuery(sfwlx+"_browserbtn").attr('disabled',true);
	   ...
	   checkCustomize = function () {
			...
			if(sfwlx_val == "0"){
				...
				if(sfbg_val==""||sfbg_val=="1"){
					var bghhtje_val=jQuery(bghhtje).val();
					var yhtje_val=jQuery(yhtje).val();
					if(Number(bghhtje_val) != Number(yhtje_val)){
					 Dialog.alert("变更后合同/预算金额不等于原合同/预算金额，请检查");
					 return false;	
					}
				}	  
			}
	...
	function dolx(){
	  ...
	  var sfbg_val = jQuery(sfbg).val();
	  dodetail1(sfbg_val);	 
	  setTimeout('showorhidebutton()',500);
	  jQuery(sfktj).bind("change",function () {
         var sfktj_val= jQuery(sfktj).val();
          if(sfktj_val=="1"){
               	dodetail1("0");
             }else{
               dodetail1("1");
       ...
       jQuery(bghsfktj).bind("change",function () {
         var bghsfktj_val= jQuery(bghsfktj).val();
          if(bghsfktj_val=="1"){
               	dodetail1("0");
             }else{
               dodetail1("1");
	  ...
	  jQuery(bg4).bindPropertyChange(function (){ 
     	 //alert("1");
      	var bghsfktj_val=jQuery(bghsfktj).val();
        var sfktj_val=jQuery(sfktj).val();
		var bg4_val = jQuery(bg4).val();
	    if(bg4_val=="0" && bghsfktj_val=="1"){
	     	 dodetail1("0");
	    }else if((bg4_val==""||bg4_val=="1") && sfktj_val=="1"){
	     	 dodetail1("0");
	    }else{
	     	var sfbg_val = jQuery(sfbg).val();
	    	dodetail1(sfbg_val);
