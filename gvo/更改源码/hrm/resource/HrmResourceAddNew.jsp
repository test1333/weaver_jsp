...
<%
  ...
  int socopeId=-1;  
	...
	//  GVO编号生成策略
  String gvo_workcode = "000000";
  sql = "select gvo_get_workcode('3000') as workcode from dual";
  rs.executeSql(sql);
  if(rs.next()){
  	 gvo_workcode = Util.null2String(rs.getString("workcode"));
  }
%>
...
<%
	while(HrmFieldManager.next()){
		...
		if(fieldName.equalsIgnoreCase("workcode")){
		%>
			<wea:item><%=SystemEnv.getHtmlLabelName(714,user.getLanguage())%></wea:item>
			<wea:item><%=gvo_workcode%>
				<input class="inputstyle" type="hidden" name="workcode" value="<%=gvo_workcode%>">
			</wea:item>
		<%
		}
...
//2004-6-16 Edit by Evan :传sql参数给部门浏览页面
    url=encode("/hrm/company/DepartmentBrowserByDec.jsp?isedit=1&rightStr=HrmResourceAdd:Add&sqlwhere=<%=xssUtil.put(getDepartmentSql(user))%>&selectedids="+jQuery("input[name=departmentid]").val());
...
function doSave(obj){
  ...
  var workcode_obj = document.getElementsByName("workcode");
  var _workcode = workcode_obj[0].value;
  var max_workcode = "";
  //alert("_workcode="+_workcode);
  jQuery.ajaxSettings.async = false; //同步传值
  jQuery.post("/hrm/resource/gvo_check_workcode.jsp", {}, function (data) {
    //alert(data);
    data = data.replace(/\n/g, "").replace(/\r/g, "");
    //alert(data);
    //eval('var obj =' + data);
 ...
 if (_workcode !== max_workcode) {
    alert("编号已存在,请刷新页面重新提交!");
    return false;
}
...
jQuery(document).ready(function(){
		jQuery("#workcode").attr("readonly","readonly");
