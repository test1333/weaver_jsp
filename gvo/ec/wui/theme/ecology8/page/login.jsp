...
if (form1.loginid.value=="") {
	if(languageid==8){
		errMessage=errMessage+"<%=SystemEnv.getHtmlLabelName(16647,8)%>\n";i=i+1;
	}else if(languageid==9){
		errMessage=errMessage+"<%=SystemEnv.getHtmlLabelName(16647,9)%>\n";i=i+1;
	}else{
		errMessage=errMessage+"<%=SystemEnv.getHtmlLabelName(16647,7)%>\n";i=i+1;
	}
}
if (form1.userpassword.value=="") {
	if(languageid==8){
		errMessage=errMessage+"<%=SystemEnv.getHtmlLabelName(16648,8)%>\n";j=j+1;
	}else if(languageid==9){
		errMessage=errMessage+"<%=SystemEnv.getHtmlLabelName(16648,9)%>\n";j=j+1;
	}else{
		errMessage=errMessage+"<%=SystemEnv.getHtmlLabelName(16648,7)%>\n";j=j+1;
	}
}
if (i>0){
...
function changeMsg(msg, ele)
{
    ...
    if(msg==0){
		...
		<%}%>
        if(document.getElementById("validatecode").value=='<%=SystemEnv.getHtmlLabelName(84270,7)%>'||document.all.validatecode.value=='<%=SystemEnv.getHtmlLabelName(84270,8)%>'||document.all.validatecode.value=='<%=SystemEnv.getHtmlLabelName(84270,9)%>') { 
        ...
        if(document.getElementById("validatecode").value=='') { 
			if(languageid==8){
				document.getElementById("validatecode").value='<%=SystemEnv.getHtmlLabelName(84270,8)%>';
			}else if(languageid==9){
				document.getElementById("validatecode").value='<%=SystemEnv.getHtmlLabelName(84270,9)%>';
			}else{
				document.getElementById("validatecode").value='<%=SystemEnv.getHtmlLabelName(84270,7)%>';
			}
        }
    }
}
...
function forgotPassword(){
	...
	var thisTitle = "";
	if(languageid==8){
		thisTitle = "<%=SystemEnv.getHtmlLabelName(127899,8)%>";
	}else if(languageid==9){
		thisTitle = "<%=SystemEnv.getHtmlLabelName(127899,9)%>";
	}else{
		thisTitle = "<%=SystemEnv.getHtmlLabelName(127899,7)%>";
	}
    ...
    <script language="JavaScript">
        function checkusb1(){ 
         ...
         <script language="JavaScript">
             ...
             var languageid = document.getElementById("islanguid").value; 
               //需要输入动态口令
			   if(jQuery("#tokenAuthKey").val()==""||jQuery("#tokenAuthKey").val()=="<%=SystemEnv.getHtmlLabelName(127901,7)%>"||jQuery("#tokenAuthKey").val()=="<%=SystemEnv.getHtmlLabelName(127901,8)%>"||jQuery("#tokenAuthKey").val()=="<%=SystemEnv.getHtmlLabelName(127901,9)%>"){
				if(languageid==8){
						window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(127901,8)%>!");
					}else if(languageid==9){
						window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(127901,9)%>！");
					}else{
						window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(127901,7)%>！");
					}
			        jQuery("#tokenAuthKey").focus();
			        return false;
			   }else if(!isdigit(jQuery("#tokenAuthKey").val())){
					if(languageid==8){
						window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(127902,8)%>");
					}else if(languageid==9){
						window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(127902,9)%>");
					}else{
						window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(127902,7)%>");
					}
					jQuery("#tokenAuthKey").focus();
					return false;
			   }else if(jQuery("#tokenAuthKey").val().length!=6){
				   if(languageid==8){
						window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(127903,8)%>");
					}else if(languageid==9){
						window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(127903,9)%>");
					}else{
						window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(127903,7)%>");
					}
					jQuery("#tokenAuthKey").focus();
					return false; 
			   }
		...
		<div id="normalLogin" class="p-l-5">
			...
			&nbsp;
				...
				<%} else if(message0.equals("122")){ //
					focusflg = 1;
				%>
				    <%=SystemEnv.getHtmlLabelName(84268,languageid)%>&nbsp;<a href='/login/syncTokenKey.jsp'><%=SystemEnv.getHtmlLabelName(84269,languageid)%></a> 
				<%}else {%>
...
function setSyslangulChange(langlid){
	if(langlid==8){
		jQuery("#for_loginid").html("<%=SystemEnv.getHtmlLabelName(83594,8)%>");
		jQuery("#for_userpassword").html("<%=SystemEnv.getHtmlLabelName(409,8)%>");
		jQuery("#forgotpassword").html("<%=SystemEnv.getHtmlLabelName(81614,8)%>");
		jQuery("#login").val("<%=SystemEnv.getHtmlLabelName(674,8)%>");
		jQuery("#for_validatecode").html("<%=SystemEnv.getHtmlLabelName(84270,8)%>");
		jQuery("#for_tokenAuthKey").html("<%=SystemEnv.getHtmlLabelName(84271,8)%>");
		
		jQuery("#forgotpassword").css("left","15px");
		jQuery("#login").css("letter-spacing","1px");
		
	}else if(langlid==9){
		jQuery("#for_loginid").html("<%=SystemEnv.getHtmlLabelName(83594,9)%>");
		jQuery("#for_userpassword").html("<%=SystemEnv.getHtmlLabelName(409,9)%>");
		jQuery("#forgotpassword").html("<%=SystemEnv.getHtmlLabelName(81614,9)%>");
		jQuery("#login").val("<%=SystemEnv.getHtmlLabelName(674,9)%>");
		jQuery("#for_validatecode").html("<%=SystemEnv.getHtmlLabelName(84270,9)%>");
		jQuery("#for_tokenAuthKey").html("<%=SystemEnv.getHtmlLabelName(84271,9)%>");
		
		jQuery("#forgotpassword").css("left","50px");
		jQuery("#login").css("letter-spacing","2px");
		
	}else{
		jQuery("#for_loginid").html("<%=SystemEnv.getHtmlLabelName(83594,7)%>");
		jQuery("#for_userpassword").html("<%=SystemEnv.getHtmlLabelName(409,7)%>");
		jQuery("#forgotpassword").html("<%=SystemEnv.getHtmlLabelName(81614,7)%>");
		jQuery("#login").val("<%=SystemEnv.getHtmlLabelName(674,7)%>");
		jQuery("#for_validatecode").html("<%=SystemEnv.getHtmlLabelName(84270,7)%>");
		jQuery("#for_tokenAuthKey").html("<%=SystemEnv.getHtmlLabelName(84271,7)%>");
