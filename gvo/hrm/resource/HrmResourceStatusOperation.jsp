...
String type = "0";
// 添加 支付工资 离职类型 离职原因
String issalary = Util.fromScreen(request.getParameter("issalary"),user.getLanguage());
String dismissreason = Util.fromScreen(request.getParameter("dismissreason"),user.getLanguage());
String dismisstype = Util.fromScreen(request.getParameter("dismisstype"),user.getLanguage());
// 添加 离职日期  开始停止工作日期  离职原因
    ...
    if(operation.equalsIgnoreCase("fire")){  //解聘
      ...
      rs.executeProc("HrmResource_Extend",para);
        ...
        for(int i=0 ; i<resourceids.length; i++) {
            ...
            //  添加离职原因  离职类型  支付工资   20170516  lizheng  修改
             int count1=0;
            String sql2="select count(1) as count  from cus_fielddata where id='"+tempresourceid+"' and scopeid='1'";
              rs.executeSql(sql2);
              if(rs.next()){
                  count1 = rs.getInt("count");
              }
              if(count1>0){
                sql2  = "update cus_fielddata set field22='"+issalary+"',field23='"+dismissreason+"',field24='"
                    +dismisstype+"' where id ='"+tempresourceid+"' and SCOPEID=1";
                     new BaseBean().writeLog("sql = " + sql2);	
                     rs.executeSql(sql2);
              }else{
                sql2="insert into cus_fielddata(SCOPE,SCOPEID,ID,FIELD22,FIELD23,FIELD24) values ('HrmCustomFieldByInfoType',1,'"+tempresourceid+"','"+issalary+"','"+dismissreason+"','"+dismisstype+"' )";
                new BaseBean().writeLog("sql = " + sql2);
                rs.executeSql(sql2);
              }      
            //离职时离职者参与的所有协作全部标记为已读 myq 修改 2008.2.20 结束
            //  添加离职日期  离职原因   开始停止工作日期
            if(tempresourceid.startsWith(","))
                tempresourceid = tempresourceid.substring(1);
            if(tempresourceid.endsWith(","))
                tempresourceid = tempresourceid.substring(0,tempresourceid.length()-1);
            String sql  = "update hrmresource set datefield2='"+changedate+"',datefield3='"+begindate+"',textfield5='"
                    +changereason+"' where id in("+tempresourceid+")";
            rs.executeSql(sql);
