...
<FORM id=weaver name=weaver STYLE="margin-bottom:0" action="" method="post">
 ...
 -->
<TABLE width="100%" class="ListStyle_xg">
	<colgroup>
<col width="40%"></col>
<col width="20%"></col>
<col width="20%"></col>	
...
<%
	...
	int num = 0;
	while(rs.next()){
	    String tsmc = Util.null2String(rs.getString("tsmc"));
		String zz = Util.null2String(rs.getString("zz"));
		String num_cc = Util.null2String(rs.getString("num_cc"));
		String bookzt = Util.null2String(rs.getString("bookzt"));
		String tsmcid = Util.null2String(rs.getString("tsmcid"));
		num++;
		if(num == 0){
			num = 1;
			%>
		...
		%>
		<Td><a href="/formmode/view/AddFormMode.jsp?type=0&modeId=142&formId=-91&billid=<%=tsmcid%>&opentype=0&customid=123&viewfrom=fromsearchlist" target=_blank><span style="border:0px solid #000000;overflow:hidden;" title="<%=tsmc%>"><%=tsmc%></span></a></Td>
		<Td><span style="border:0px solid #000000;overflow:hidden;" title="<%=zz%>"><%=zz%></span></Td>
		<Td><%=num_cc%></Td>
		<Td><%=bookzt%></Td>
		</Tr>
		<%
