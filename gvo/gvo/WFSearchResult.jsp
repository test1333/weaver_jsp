...
if(user.getUID() > 1){
	...
	while(RecordSet.next()){
		...
		sql_dt="select count(1) as count from  ofs_todo_data a where   a.userid ='"+tmp_uid+"' and a.isremark='0'  and islasttimes=1";
		rs_dt.executeSql(sql_dt);
		if(rs_dt.next()){
			tmp_num = String.valueOf(Integer.valueOf(tmp_num)+rs_dt.getInt("count"));
		}
...
String sql_ec="select pcurl,requestname from ofs_todo_data where userid in("+accounts+") and isremark='0' and islasttimes=1";
RecordSet.executeSql(sql_ec);
while(RecordSet.next()){
	i++;
	String pcurl= Util.null2String(RecordSet.getString("pcurl"));
	String rname = Util.null2String(RecordSet.getString("requestname")); 
%>
<tr style="cursor:pointer;" onClick="winopen1('<%=pcurl%>');" bgColor='<%if((i%2)!=0){%>#f6f8f7<%}else{%>#FFFFFF<%}%>' onMouseOver="ch_color(this);" onMouseOut="ch_color_out(this);" bordercolorlight="#ffffff" bordercolordark="#ffffff" class='normal_text' id="content_line_0">
<td  class='td_content'><span style="border:0px solid #000000;width:190px;text-overflow:ellipsis; white-space:nowrap; overflow:hidden;" title="<%=rname%>"><%=rname%></span></td>
<%
}
...
function winopen1(url) {
	wopener = window.open('http://222.92.108.195:1816'+url);
	checkOpener();
}
