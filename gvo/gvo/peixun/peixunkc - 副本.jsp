								...
								<FORM id=weaver name=weaver STYLE="margin-bottom:0" action="" method="post">
									...
									<TABLE width="100%">
										<tr>
											...
											String sql = " select jsmc,kcmc,pxrq,ks from formtable_main_193 where jsmc = " +id + " order by pxrq desc ";
											rs.executeSql(sql);
											...
											num++;
											%>
											<Td width="25%"><a href="/formmode/view/AddFormMode.jsp?type=0&modeId=542&formId=-193&billid=<%=id%>&opentype=0&customid=522&viewfrom=fromsearchlist" target=_blank><%=kcmc%></a>   <font size="1px">(<%=pxrq%>)</font></Td>
											<%if(num%4==0){%>
										</tr><tr>
										<%}%>
										<%
