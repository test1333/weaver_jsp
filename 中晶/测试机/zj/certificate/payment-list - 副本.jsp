...
<link rel="stylesheet" type="text/css" href="/js/jquery-autocomplete/browser_wev8.css" />
	...
	String fklx = Util.null2String(request.getParameter("fklx"));
	int cansee=0;
	...
	<BODY>
		...
		<FORM id=report name=report action="/zj/certificate/payment-list.jsp" method=post>
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;">
				...
				<wea:item>付款类型</wea:item>
				<wea:item>
				 <select class="e8_btn_top middle" name="fklx" id="fklx">
                <option value="" <%if("".equals(fklx)){%> selected<%} %>>
                    <%=""%></option>
                <option value="0" <%if("0".equals(fklx)){%> selected<%} %>>
                    <%="单笔付款"%></option>
                <option value="1" <%if("1".equals(fklx)){%> selected<%} %>>
                    <%="合并付款"%></option>
		...
		<%
		String backfields = "fkseq,sqr,isnull(SUM(ISNULL(fkje,0)),0) as fkje,fkrq,fksj,case when fklx='0' then '单笔付款' when fklx='1' then '合并付款' else '' end as fklx";                 
		...
		if(!"".equals(beginDate)){
				sqlWhere +=  " and fkrq >='"+beginDate+"'";
		}
		if(!"".equals(endDate)){
			sqlWhere +=  " and fkrq <='"+endDate+"'";
		}
		if(!"".equals(fklx)){
			sqlWhere +=" and fklx='"+fklx+"'";
		}
		 sqlWhere +=" group by fkseq ,sqr,fkrq,fksj,fklx";
		...
		operateString+
		"			<head>";
			tableString+= " <col width=\"20%\" text=\"收款人\" column=\"sqr\" otherpara=\"column:sqr\" transmethod=\"weaver.general.WorkFlowTransMethod.getWFSearchResultName\"/>"+

			"               <col width=\"20%\" text=\"金额\" column=\"fkje\"  />"+
			"               <col width=\"20%\" text=\"日期\" column=\"fkrq\"  />"+
			"               <col width=\"20%\" text=\"时间\" column=\"fksj\"  />"+
			"               <col width=\"20%\" text=\"付款类型\" column=\"fklx\"  />"+
