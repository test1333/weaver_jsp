...
setFJ();
        ...
        jQuery(fylx).bindPropertyChange(function () {
            ...
            if (fylx_val == '25') {
                jQuery(fq1).val('')
                jQuery(fq1 + 'span').text('')
                jQuery(fq2).val('')
                jQuery(fq2 + 'span').text('')
                jQuery(fq1 + 'browser').attr('disabled', 'disabled')
                jQuery(fq2 + 'browser').attr('disabled', 'disabled')
            } else {
                jQuery(fq1 + 'browser').removeAttr('disabled')
                jQuery(fq2 + 'browser').removeAttr('disabled')
            }
        })
...
setFJ();
        ...
        function changeFYLX () {
            for (var i = 0; i < jQuery('#indexnum3').val(); i++) {
                (function (j) {
                    jQuery(fylxDT4 + j).bindPropertyChange(function () {
                        setFYLX()
                    })
                })(i)
            }
    ...
    function setWSJE (index) {
        ...
        if (jeheValue > 0.00 && kdkjeValue > 0.00) {
            if (fplxValue === '3' || fplxValue === '4' || fplxValue === '5') {
                jQuery('#field6596_' + index).val(jeheValue - kdkjeValue)
			...
			if(jQuery(fylx4+i).length>0){
				...
				if(fy4 =='29'){//办公用品
					if (fj_val == "null" || fj_val == '' || fj_val.length<1) {
						...
						removeInputCheckField(fj, 'field_6523span');
					}
					break;
				}else{
					removeInputCheckField(fj, 'field_6523span');
