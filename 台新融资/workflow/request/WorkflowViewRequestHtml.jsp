...
<%@ page import="txrz.util.ParseHtmlUtil" %>
...
ParseHtmlUtil htmlUtil = new ParseHtmlUtil();
	wfformhtml = htmlUtil.parseHtml(wfformhtml);
	// new BaseBean().writeLog("********:" + wfformhtml);
wfformhtml = wfformhtml.replace("<div id=\"splitPage\"></div>","</tbody></table></td></tr></tbody></table>" +
		"<div style=\"page-break-after: always;\"></div><table class=\"excelOuterTable tablefixed\"><tbody><tr><td align=\"center\">" +
		"<table class=\"excelMainTable tablefixed\" style=\" width:720px; \" _hasPercent=\"false\" >\n" +
		"<tbody>\n" +
		...
		"</tr>");
if(isprint==1){
	String wfid = Util.null2o(weaver.file.Prop.getPropValue("txrzqc", "wfid"));
	if((","+wfid+",").indexOf(","+workflowid+",")>=0){
			String showcqh = "";
			String tableName = "";
        ...
        if (rs.next()) {
			...
			String sqlpr = "select zzqch from "+tableName+" where requestid="+requestid;
			rs.execute(sqlpr);
			if(rs.next()){
					showcqh = Util.null2String(rs.getString("zzqch"));

			}
			wfformhtml = "<style  type=\"text/css\">@media screen {div.divFooter {display: none;}}@media print {div.divFooter {position: fixed;top: 0;right:0px; color:#808080a8;font-size:14pt;font-family:KaiTi!important;}}</style><div class=\"divFooter\">签呈号："+showcqh+"</div>"+wfformhtml;
	}
