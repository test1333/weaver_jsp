...
<table border="0" cellpadding="1" cellspacing="1" width="488px" id="tab_list" >
	<colgroup><col width="40px"></col><col width="308px"></col><col width="width:45px"></col><col width="width:85px"></col></colgroup>
 	<tr><th>程度</th><th>标题</th><th>创建人</th><th>创建日期</th></tr>
<thead>
	<!--
...
if(user.getUID() > 1){
//	String tmp_sql = "select t2.userid,count(*) as c_num from workflow_requestbase t1,workflow_currentoperator t2 "
...
//		    +" and t1.workflowid !=1 "
		  //  +" and t1.requestid not in (select requestid from workflow_requestbase wr where wr.requestname like '会议%' and wr.workflowid=1) "
//	        +" group by t2.userid ";
...
//	}
	String tmp_sql = "select id,lastname,belongto,decode(trim(belongto),null,'主账号','0','主账号','-1','主账号','次账号') as z_type from hrmresource where id in("+accounts+") order by id";
	RecordSet.executeSql(tmp_sql);
	while(RecordSet.next()){
...
RecordSet.executeSql("select "+backfields+fromSql+sqlWhere+orderby);
//out.println("select "+backfields+fromSql+sqlWhere+orderby);
int i = 0;
int tmc_flag = 0;
while(RecordSet.next()) {
	i++;
...
<td  class='td_content' align=center><span style="border:0px solid #000000;width:50px;text-overflow:ellipsis; white-space:nowrap; overflow:hidden;"><%=ResourceComInfo.getResourcename(RecordSet.getString("creater"))%></span></td>
	<%
	String nodename = "";
...
function winopen(rid,uid) {
	wopener = window.open('/fx/SSOForward.jsp?fromUserId='+uid+'&type=WFL&requestid='+rid,'newwin'+rid);
	checkOpener();
}
