...
int isgoveproj = Util.getIntValue(IsGovProj.getPath(),0);//0:非政务系统，1：政务系统
	...
	String sql_lxr = "";
	if("3".equals(type_n)){
		sql_lxr="select * from contact_record_person where workplanid='"+workID+"'";
		rs.executeSql(sql_lxr);
		if(rs.next()){
			lxrname = Util.null2String(rs.getString("name"));
			lxrtel = Util.null2String(rs.getString("tel"));
		}
	}
	...
	<wea:layout type="2col">
		...
		<!--================ 紧急程度  ================-->
		   ...
		   <%}%>
			...
			<!--modify start by tangjianyong 2017-11-01-->
		<% if("3".equals(type_n)){ %>
			<wea:item>联系人</wea:item>
          	<wea:item >
          		<%=lxrname%>
          	</wea:item>
			<wea:item>联系电话</wea:item>
          	<wea:item >
          		<%=lxrtel%>
          	</wea:item>
