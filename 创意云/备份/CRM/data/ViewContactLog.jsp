...
<div id="rightinfo" style="background:#fff;">
	...
	<!--modify by tangjianyong 2017-11-01 start -->
		<table style="width: 100%;height: auto;margin-top:5px;margin-bottom:5px;overflow: hidden;" cellpadding="0" cellspacing="0" border="0">
		<tr>
			<td width="*" valign="top" align="center" style="padding-left:10px;padding-right:10px;"  colspan="2">
		...
		<tr>
			<td  width="200px" valign="top" style="padding-left:10px;">
			联系人: <input  id="lxrname" type="text"  class="InputStyle" value="" name="wcbl"/><span id="lxrnamespan"><img align='absmiddle' src='/images/BacoError_wev8.gif'></span>
			</td>	
			<td valign="top"  style="padding-right:10px;">
			联系电话: <input  id="lxrtel" type="text"   class="InputStyle" value="" name="lxrtel"/><span id="lxrtelspan"><img align='absmiddle' src='/images/BacoError_wev8.gif'></span>
			</td>
		</tr>
	<!--modify by tangjianyong 2017-11-01 end -->
		<tr>
	...
	<div id="maininfo" style="padding-right:5px;<%=from.equals("rdViewCrm")?"overflow:hidden;":""%>" class="scroll1" >
		  ...
		  <%
			...
			//modify start by tangjianyong 2017-11-01
			backfields+=",(select name from contact_record_person where workplanid=id) as lxrname,(select tel from contact_record_person where workplanid=id) as lxrtel ";
			//modify end by tangjianyong 2017-11-01
					...
					if(labelid.equals("my")){  //我的客户
						if(viewtype.equals("0")){ //仅本人客户
							...
							if(!subResourceid.equals(""))
								searchstr+=" and (t2.manager="+resourceid+" or t2.manager in("+subResourceid+"))";
			...
			while(rs.next()){
				...
				String fileid = rs.getString("relateddoc");
					//modify start by tangjianyong 2017-11-01
				String lxrname = Util.null2String(rs.getString("lxrname"));
				String lxrtel =  Util.null2String(rs.getString("lxrtel"));
				//modify end by tangjianyong 2017-11-01
				String info= "";
				if(!"".equals(fileid)){
				...
				htmlStr+="<div class='feedbackshow'>"+
					   "<div class='feedbackinfo' >"+
							...
							"<div class='left' style='max-width:90%;word-break:break-all;'>"+
					   		"	<div><a href='/hrm/resource/HrmResource.jsp?id="+createrid+"' target='_blank'>"+ResourceComInfo.getResourcename(createrid)+"</a>"+
							" 	"+createdate+" "+createtime+"</div>"+
							"	<div class='feedbackrelate'>"+
								   "<div>"+description+"</div>"+
								   //modify by tangjianyong 2017-11-01
								    "<div> 联系人："+lxrname+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;联系人电话："+lxrtel+"</div>"+
...
//add by jianyong tang 2017-11-2 start
jQuery(document).ready(function(){
  jQuery("#lxrname").bind("change",function(){
	  var lxrname_val=jQuery("#lxrname").val();
	  if(lxrname_val == ""){
		  jQuery("#lxrnamespan").html("<img align='absmiddle' src='/images/BacoError_wev8.gif'>");
	  }else{
		  jQuery("#lxrnamespan").html("");
	  }

  })
  jQuery("#lxrtel").bind("change",function(){
	  var lxrtel_val=jQuery("#lxrtel").val();
	  if(lxrtel_val == ""){
		  jQuery("#lxrtelspan").html("<img align='absmiddle' src='/images/BacoError_wev8.gif'>");
	  }else{
		  jQuery("#lxrtelspan").html("");
	  }

  })
})
//add by jianyong tang 2017-11-2 end
var parentWin = null;
if("<%=isfromtab%>" == "true"){
	parentWin = parent;
}else{
	...
	function doFeedback(){
			//modify start by tangjianyong 2017-11-01
		var lxrname = jQuery("#lxrname").val();
		var lxrtel = jQuery("#lxrtel").val();
		if(lxrname == "" || lxrtel == ""){
			window.top.Dialog.alert("联系人和联系电话不能为空，请填写");
			return;
		}
		//modify start by tangjianyong 2017-11-01
		ContactInfo = $.trim($("#ContactInfo").val());
		//modify start by tangjianyong 2017-10-31
		//alert(ContactInfo.length);
		if(ContactInfo.length<=30){
			window.top.Dialog.alert("联系记录的长度必须大于30个汉字，请重新填写");
			return;
		}
		//modify end by tangjianyong 2017-10-31
		if(ContactInfo==""){
