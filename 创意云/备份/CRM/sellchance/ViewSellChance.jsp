...
String chanceCreater="";
String sql_new = "select creater from CRM_SellChance where id="+chanceid;
rs.executeSql(sql_new);
if(rs.next()){
  chanceCreater = Util.null2String(rs.getString("creater"));
}
if(useridcheck.equals(chanceCreater)){
	canedit = true;
}else{
	canedit = false; 
}
...
<wea:layout type="4col" attributes="{'expandAllGroup':'true'}">
	...
	<%
		rst.execute("select * from CRM_CustomerDefinFieldGroup where usetable = 'CRM_SellChance' order by dsporder asc");
		while(rst.next()){
			String groupid = rst.getString("id");
			CrmFieldComInfo comInfo = new CrmFieldComInfo("CRM_SellChance",groupid) ;
			if(0 == comInfo.getArraySize()){
				continue;
			}
		%>
