...
<%
	...
	RCMenu += "{更新数据,javascript:onCheckup1(),_self} " ;
	RCMenuHeight += RCMenuHeightStep ;

%>
	<%@ include file="/systeminfo/RightClickMenu_wev8.jsp" %>
	<FORM id=report name=report action=/cx/kq/updatekqdata.jsp method=post>
		...
		if("1".equals(multiIds)){
			UpdateKqStatusAction aa = new UpdateKqStatusAction();
			String sql = "select * from (select convert(varchar(10),dateadd(dd,number,'"+beginDate+"'),120) as day from master..spt_values where type='P' and dateadd(dd,number,'"+beginDate+"')<=dateadd(dd,0,'"+endDate+"')) a";
			rs.execute(sql);
			while(rs.next()){
				String kqdate = Util.null2String(rs.getString("day"));
				aa.updatekqDate(kqdate);
			}
			boolean isOver = true;
			if(isOver){
				out.println("<font size=\"5px\" color=\"red\">执行完成，更新成功!</font>");
			}else{
				out.println("<font size=\"5px\" color=\"red\">执行完成，更新失败，请联系管理员或者重新同步!</font>");
			}
		}
...
<wea:group context="更新考勤数据" attributes="test">
...
<wea:item>
	...
	<wea:item>&nbsp;&nbsp;</wea:item>
		...
		<table width="30%">
			...
			<tr>
				<td><input name="hi1" type="button" value="更新数据" class="e8_btn_top_first" style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis;  max-width: 100px;width:100px;height:30px;font-size:15px;" onClick="onCheckup()"></td>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
