...
<link rel="stylesheet" type="text/css" href="/js/jquery-autocomplete/browser_wev8.css" />
    ...
    String out_pageId = "jxdfej_xm2_all";
	...
	<BODY>
		...
		<FORM id=report name=report action="/cx/jx/xmwd/df/jxdf-ej-xm-list-all.jsp" method=post>
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;">
				...
				<wea:item>
					...
					browserValue='<%=sjbm%>'
					isMustInput='1'
					browserSpanValue='<%=Util.toScreen(DepartmentComInfo.getDepartmentNames(sjbm),user.getLanguage())%>'
					hasInput='true'
		...
		<%
		String backfields = " ryid,lastname,workcode,sjbm,jobtitle,(select jobtitlename from hrmjobtitles where id=a.jobtitle) as titlename,erjm,status,convert(varchar(20),year)+'年'+zq as zqspan,year,zq,zqid,requestid,keyid,bdbh,khzgdf,khpjdj,khpjdjname,convert(varchar(20),ygkhqz)+'%' as ygkhqz,xmmc,(select xmmc from uf_xmxx where id=a.xmmc) as xmmcname,(select cmssbm from uf_xmxx where id=a.xmmc) as cmssbm,(select field1 from cus_fielddata where id=a.ryid and scopeid=-1) as yglx,(select (select tc.selectname from workflow_billfield ta, workflow_bill tb,workflow_selectitem tc where ta.billid=tb.id and tc.fieldid=ta.id  and tb.tablename='uf_xmjxkhjg' and ta.fieldname='xmkhdj' and tc.selectvalue=t.xmkhdj) as xmkhdj from uf_xmjxkhjg t where xmmc=a.xmmc and khzqnf=a.year and khzq=a.zqid) as xmkhdj,(select xmkhxs from uf_xmjxkhjg t where xmmc=a.xmmc and khzqnf=a.year and khzq=a.zqid) as xmkhxs,(select zzsjxs from uf_ygzzsjxs t where gh=(select workcode from hrmresource where id=a.ryid) and khzqnf=a.year and khzq=a.zqid) as zzsjxs,[dbo].[get_jxjlxs](a.year,a.zqid,a.xmmc,a.ryid) as tsjlxs,cast((select xmkhxs from uf_xmjxkhjg t where xmmc=a.xmmc and khzqnf=a.year and khzq=a.zqid) *(select xs from uf_yggrjxkhxs where khzqnf=a.year and khzq=a.zqid and ygkhdj=a.khpjdj )*a.ygkhqz/100 as numeric(10,2)) as jdjjxs,cast((select zzsjxs from uf_ygzzsjxs t where gh=(select workcode from hrmresource where id=a.ryid) and khzqnf=a.year and khzq=a.zqid) *[dbo].[get_jxjlxs](a.year,a.zqid,a.xmmc,a.ryid)*a.ygkhqz/100 as numeric(10,4)) as jdtbxs,(select xs from uf_yggrjxkhxs where khzqnf=a.year and khzq=a.zqid and ygkhdj=a.khpjdj ) as grjxkkxs "+
		",cast((select zzsjxs from uf_ygzzsjxs t where gh=(select workcode from hrmresource where id=a.ryid) and khzqnf=a.year and khzq=a.zqid)*(select xmkhxs from uf_xmjxkhjg t where xmmc=a.xmmc and khzqnf=a.year and khzq=a.zqid) *(select xs from uf_yggrjxkhxs where khzqnf=a.year and khzq=a.zqid and ygkhdj=a.khpjdj )*a.ygkhqz/100 as numeric(10,4)) as jdjjjsxs";
		String fromSql  =  " from (select d.id as ryid,d.lastname,d.workcode,b.sjbm,d.jobtitle,b.erjm,case when c.currentnodetype='3' then '已打分' else '打分中' end as status,b.khzqnf as year,(select tc.selectname from workflow_billfield ta, workflow_bill tb,workflow_selectitem tc where ta.billid=tb.id and tc.fieldid=ta.id  and tb.tablename='"+tablename+"' and ta.fieldname='khzq' and tc.selectvalue=b.khzq) as zq,b.khzq as zqid,b.requestid,convert(varchar(20),b.requestid) as keyid,b.bdbh,b.khzgdf,b.khpjdj,(select tc.selectname from workflow_billfield ta, workflow_bill tb,workflow_selectitem tc where ta.billid=tb.id and tc.fieldid=ta.id  and tb.tablename='"+tablename+"' and ta.fieldname='khpjdj' and tc.selectvalue=b.khpjdj) as khpjdjname,b.ygkhqz,b.xmmc,d.departmentid as dpid from "+tablename+" b,workflow_requestbase c,hrmresource d  where  b.requestid=c.requestid and b.fqr=d.id  and d.status&lt;5 and b.khzqnf='"+year+"' and b.khzq='"+zq+"' "+								
								" ) a"; 
		String sqlWhere =  " 1=1 ";
		if("".equals(zq)){
			sqlWhere += " and 1=2 ";
		...
		if(!"".equals(sjbm)){
			sqlWhere += " and a.dpid in("+sjbm+")";
		}
		if(!"".equals(khpjdj)){
			sqlWhere += " and a.khpjdj='"+khpjdj+"' ";
		}
		if(!"".equals(xmmc)){
			sqlWhere += " and a.xmmc in ("+xmmc+")";
		}
		...
		tableString +="<col width=\"100px\" text=\"项目所在部门\" column=\"cmssbm\" orderkey=\"cmssbm\"  transmethod=\"weaver.hrm.company.DepartmentComInfo.getDepartmentname\"/>"+ 
			...
			"<col width=\"120px\"  text=\"考核周期\" column=\"zqspan\" orderkey=\"zqspan\"  />"+
			...
			"<col width=\"150px\"  text=\"单据编号\" column=\"bdbh\" orderkey=\"bdbh\" linkkey=\"requestid\" linkvaluecolumn=\"requestid\" href=\"/workflow/request/ViewRequest.jsp\" target=\"_fullwindow\" />"+
