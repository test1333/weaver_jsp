...
<link rel="stylesheet" type="text/css" href="/js/jquery-autocomplete/browser_wev8.css" />
	...
	<BODY>
		...
		<FORM id=report name=report action="/cx/jx/khjg/jxkhjghz-ej-list.jsp" method=post>
				...
				<wea:item>评价等级</wea:item>
					<wea:item>
						<select class="e8_btn_top middle" name="khpjdj" id="khpjdj">
							...
							<option value="S(杰出)" <%if("S(杰出)".equals(khpjdj)){%> selected<%} %>>
								<%="S(杰出)"%></option>
		...
		<%
		String backfields = " departmentid,id,lastname,workcode,jobtitle, titlename,ejbm,sjbm,year,zq,zzzhdf,zhkhdj,khzq";
		String fromSql  =  "  from (select departmentid,id,lastname,workcode,jobtitle,(select jobtitlename from hrmjobtitles where id=a.jobtitle) as titlename,ejbm,sjbm,year,zq,zzzhdf,(select djmc from uf_jxlevel_map where ks&lt;a.zzzhdf and js>=a.zzzhdf) as zhkhdj,'"+year+"'+'_'+'"+zq+"' as khzq from (select id,lastname,workcode,jobtitle,case when (select supdepid from hrmdepartment where id=t.departmentid)=0 then  t.departmentid else (select supdepid from hrmdepartment where id=t.departmentid) end as ejbm,case when (select supdepid from hrmdepartment where id=t.departmentid)=0 then null else t.departmentid end as sjbm,'"+year+"' as year,'"+month+"' as zq,dbo.get_jxzzdf('"+year+"','"+zq+"',t.id) as zzzhdf,departmentid from hrmresource t where status&lt;5 and t.departmentid in ("+deptid+") and exists(select 1 from "+tablename+" a,workflow_requestbase b where a.requestId=b.requestid and b.currentnodetype=3  and khzqnf='"+year+"' and khzq='"+zq+"' and fqr=t.id)) a)t"; 
		String sqlWhere =  " 1=1 ";


		if(!"".equals(ry)){
			sqlWhere += " and a.id='"+ry+"' ";
		}
		
		if(!"".equals(khpjdj)){
			sqlWhere += " and a.zhkhdj='"+khpjdj+"' ";
		}
		...
		String orderby =  "departmentid asc"  ;
		String tableString = "";
		...
		operateString+
		"			<head>";
		tableString +="<col width=\"10%\" text=\"二级部门\" column=\"ejbm\" orderkey=\"ejbm\"  transmethod=\"weaver.hrm.company.DepartmentComInfo.getDepartmentname\"/>"+ 
			...
			"<col width=\"10%\" text=\"姓名\" column=\"id\" orderkey=\"id\"  transmethod='weaver.proj.util.ProjectTransUtil.getResourceNamesWithLink'/> "+ 
			...
			"<col width=\"10%\"  text=\"综合考核等级\" column=\"zhkhdj\" orderkey=\"zhkhdj\"  />"+
			"<col width=\"10%\"  text=\"单据编号\" column=\"id\" orderkey=\"id\" otherpara=\"column:khzq\" transmethod=\"morningcore.jx.TransUtil.getFlowHref\" />"+
