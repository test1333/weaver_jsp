...
<link rel="stylesheet" type="text/css" href="/js/jquery-autocomplete/browser_wev8.css" />
	...
	<BODY>
		...
		<FORM id=report name=report action="/cx/jx/xmwd/qzhz/qzhz-xm-list-all.jsp" method=post>
		...
		<%
		String backfields = " keyid,xm,xm1,nf,jd,nbdd,convert(varchar(20),rltrbl)+'%' as rltrbl,ys,convert(varchar(20),pjqz)+'%' as pjqz,(select xmmc from uf_xmxx where id=a.xm) as xmmcname,(select xmbh from uf_xmxx where id=a.xm) as xmbh,(select workcode from hrmresource where id=a.xm1) as workcode,(select field1 from cus_fielddata where id=a.xm1 and scopeid=-1) as yglx,case when jd='1' then '第一季度'  when jd='2' then '第二季度' when jd='3' then '第三季度' else '第四季度' end as jdspan,(select count(distinct yf) from v_jx_ygtrxx where xm1=a.xm1 and nf=a.nf and jd=a.jd) as yfs,cast(a.rltrbl/(select count(distinct yf) from v_jx_ygtrxx where xm1=a.xm1 and nf=a.nf and jd=a.jd) as numeric(10,1)) as pjtrqz,dpid";
		String fromSql  =  " from (select max(id) as keyid,xm,xm1,nf,jd,max(nbdd) as nbdd,isnull(sum(isnull(rltrbl,0)),0) as rltrbl,sum(ys) as ys ,cast(isnull(sum(isnull(rltrbl,0)),0)/sum(ys) as numeric(10,1))  as pjqz,(select departmentid from hrmresource where id=t.xm1) as dpid  from v_jx_ygtrxx t group by xm,xm1,nf,jd) a"; 
		String sqlWhere =  " 1=1 ";
		if("".equals(year)){
			sqlWhere += " and 1=2 ";
		}else{
		...
		if(!"".equals(xmmc)){
			sqlWhere += " and xm in ("+xmmc+")";
		}
		if(!"".equals(sjbm)){
			sqlWhere += " and dpid in("+sjbm+")";
		}
