...
<link rel="stylesheet" type="text/css" href="/js/jquery-autocomplete/browser_wev8.css" />
    ...
    String out_pageId = "jxkhjghzej_xm_all";
	...
	<BODY>
		...
		<FORM id=report name=report action="/cx/jx/xmwd/khjg/jxkhjghz-ej-xm-list-all.jsp" method=post>
		...
		<%
		String backfields = " departmentid,id,lastname,workcode,jobtitle, titlename,ejbm,sjbm,year,zq,zzzhdf,zhkhdj,khzq,(select field1 from cus_fielddata where id=t.id and scopeid=-1) as yglx,'"+year+"年"+month+"' as zqspan,convert(varchar(20),khqz)+'%' as khqz,(select zzsjxs from uf_ygzzsjxs  where gh=(select workcode from hrmresource where id=t.id) and khzqnf='"+year+"' and khzq='"+zq+"') as zzsjxs,[dbo].[get_tbjlxs_hz_xmwd]('"+year+"','"+zq+"',t.id) as tbjlxs,[dbo].[get_ygjxzhxs_hz_xmwd]('"+year+"','"+zq+"',t.id) as ygjxzhxs";
		String fromSql  =  "  from (select departmentid,id,lastname,workcode,jobtitle,(select jobtitlename from hrmjobtitles where id=a.jobtitle) as titlename,ejbm,sjbm,year,zq,zzzhdf,(select djmc from uf_jxlevel_xm_map where ks&lt;a.zzzhdf and js>=a.zzzhdf) as zhkhdj,'"+year+"'+'_'+'"+zq+"' as khzq,khqz from"+
		" (select t.departmentid,id,lastname,workcode,jobtitle,case when (select supdepid from hrmdepartment where id=t.departmentid)=0 then  t.departmentid else (select supdepid from hrmdepartment where id=t.departmentid) end as ejbm,case when (select supdepid from hrmdepartment where id=t.departmentid)=0 then null else t.departmentid end as sjbm,'"+year+"' as year,'"+month+"' as zq,dbo.get_jxzzdf_xmwd('"+year+"','"+zq+"',t.id) as zzzhdf,(select isnull(sum(isnull(a.ygkhqz,0)),0) from "+tablename+" a,workflow_requestbase b where a.requestId=b.requestid and isnull(a.khzgdf,0)>0  and khzqnf='"+year+"' and khzq='"+zq+"' and fqr=t.id) as khqz from hrmresource t where status&lt;5  and exists(select 1 from "+tablename+" a,workflow_requestbase b where a.requestId=b.requestid and isnull(a.khzgdf,0)>0   and khzqnf='"+year+"' and khzq='"+zq+"' and fqr=t.id)) a)t"; 
		String sqlWhere =  " 1=1 ";
		if("".equals(zq)){
			sqlWhere += " and 1=2 ";
		}
		...
		if(!"".equals(sjbm)){
			sqlWhere += " and t.departmentid in("+sjbm+")";
		}
		...
		tableString +="<col width=\"8%\" text=\"工号\" column=\"workcode\" orderkey=\"workcode\" />"+ 
			...
			"<col width=\"10%\"  text=\"特别激励系数\" column=\"tbjlxs\" orderkey=\"tbjlxs\"  />"+
