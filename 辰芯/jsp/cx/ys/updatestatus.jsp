...
%>
	...
	<FORM id=report name=report action=/cx/ys/updatestatus.jsp method=post>
		...
		if("1".equals(multiIds)){	
			...
			if(count == 0){
				...
				sql = "update uf_yssjdr set zt='1' where zt='0'";
				rs.execute(sql);
				sql = "update uf_yssjdr set zt='0' where nf='"+year+"' and bb='"+version+"'";
				rs.execute(sql);
			}
			
			if(isOver){
				out.println("<font size=\"5px\" color=\"red\">执行完成，启用成功!</font>");
			}else{
				out.println("<font size=\"5px\" color=\"red\">执行完成，启用失败，该年份、版本预算不存在</font>");
			}
		}
...
<wea:group context="启用预算版本" attributes="test">
...
<wea:item>年份</wea:item>
	...
	<wea:item>&nbsp;&nbsp;</wea:item>
		...
		<table width="30%">
			...
			<tr>
				<td><input name="hi1" type="button" value="启用" class="e8_btn_top_first" style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis;  max-width: 100px;width:100px;height:30px;font-size:15px;" onClick="onCheckup()"></td>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			...
			<script type="text/javascript">	
				...
				function onCheckup(){
					...
					if(year == "" || version==""){
					 ...
					 top.Dialog.confirm('是否启用预算版本？',function(){
       					 document.report.multiIds.value = "1";
						document.report.submit();
   					 });
