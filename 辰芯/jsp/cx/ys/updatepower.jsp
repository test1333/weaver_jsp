...
<script language=javascript src="/js/ecology8/hrm/HrmSearchInit_wev8.js"></script>
	<script type="text/javascript">	
		...
		function onCheckup1(){
			if(window.confirm('是否重构改版本数据权限？（重构权限需要较长时间）')){
...
<%
	...
	RCMenu += "{重构权限,javascript:onCheckup1(),_self} " ;
	RCMenuHeight += RCMenuHeightStep ;

%>
	<%@ include file="/systeminfo/RightClickMenu_wev8.jsp" %>
	<FORM id=report name=report action=/cx/ys/updatepower.jsp method=post>
		...
		if("1".equals(multiIds)){	
			...
			isOver=ei.setPower2(version,year);
			
			if(isOver){
				out.println("<font size=\"5px\" color=\"red\">权限重构完成!</font>");
			}else{
				out.println("<font size=\"5px\" color=\"red\">权限重构失败，该年份、版本预算不存在</font>");
			}
		}
...
<wea:group context="预算数据权限重构" attributes="test">
...
<wea:item>年份</wea:item>
	...
	<wea:item>&nbsp;&nbsp;</wea:item>
		...
		<table width="30%">
			...
			<tr>
				<td><input name="hi1" type="button" value="重构权限" class="e8_btn_top_first" style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis;  max-width: 100px;width:100px;height:30px;font-size:15px;" onClick="onCheckup()"></td>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			...
			<script type="text/javascript">	
				...
				function onCheckup(){
					...
					if(year == "" || version==""){
						window.top.Dialog.alert("年份和版本必填");
						return;
					}
					 top.Dialog.confirm('是否重构改版本数据权限？（重构权限需要较长时间）？',function(){
       					 document.report.multiIds.value = "1";
						document.report.submit();
   					 });
