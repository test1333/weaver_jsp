...
<HTML>
	...
	String indexid =  Util.null2String(request.getParameter("indexid"));
	...
	String sql = "select "+fieldname+",GFMC from uf_fyszsj where id="+customid;
	rs.executeSql(sql);
	if(rs.next()){
		khmc = Util.null2String(rs.getString("GFMC"));
	...
	<BODY>
		...
		<FORM id=report name=report action="/cx/contract/supplier/save-field-change.jsp" method=post>
		...
		<div >
			...
			<%	
					...
					<%
							}else if("5".equals(cfb.getFieldtype())){
								String showfieldname = cfb.getFieldname();
								String llanbs = "/systeminfo/BrowserMain.jsp?url=/interface/CommonBrowser.jsp?type=browser."+cfb.getZdyanbs();
								%>
								<wea:item>
								<brow:browser name='<%=showfieldname%>'
								viewType='0'
								browserValue='<%=fieldValue2%>'
								isMustInput='1'
								browserSpanValue="<%=biu.getMulZdyValues(cfb,fieldValue2)%>"
								hasInput='true'
								linkUrl='' 
								completeUrl=''
								width='20%'
					...
					<%
							}else if("6".equals(cfb.getFieldtype())){
								String showfieldname = cfb.getFieldname();
								String llanbs = "/systeminfo/BrowserMain.jsp?url=/interface/MultiCommonBrowser.jsp?type=browser."+cfb.getZdyanbs()+"&beanids=&splitflag=,&tmpids";
								%>
								<wea:item>
								<brow:browser name='<%=showfieldname%>'
								viewType='0'
	 ...
	 jQuery(document).ready(function () {
		...
		if(issuccess == "1"){
			...
			//var parentWin = window.parent;
       		// parentWin.getchangevalue(seqno,fieldid,indexid);
