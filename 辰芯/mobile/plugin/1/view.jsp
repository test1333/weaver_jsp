...
String fromofs = Util.null2String(request.getParameter("fromofs"));
...
WorkflowServiceImpl workflowWebService = new WorkflowServiceImpl();
workflowWebService.setUser(user);
...
if ("create".equals(method)) {
	...
	workflowWebService.writeWorkflowReadFlag(String.valueOf(requestid), String.valueOf(userid), (workflowRequestInfo==null?0:workflowRequestInfo.getIsremark()));
...
String sql_isreject = " select a.nodeid lastnodeid, a.isSubmitDirect lastIsSubmitDirect, a.logtype from workflow_requestlog a, workflow_nownode b where a.requestid = b.requestid and a.destnodeid = b.nownodeid "
...
while(crs.next()) {
	...
	if("3".equals(logtype)) {
		...
		lastIsSubmitDirect = Util.null2String(crs.getString("lastIsSubmitDirect"));
		break;
	}
...
if(!"".equals(lastnodeid)) {
	...
	if(crs.next()) {
		...
		if("2".equals(isSubmitDirectNode) && "1".equals(lastIsSubmitDirect)) { // 操作者选择体会会后再提交直达本节点
			isSubmitDirectNode = "1";
		}
	}
}
...
String userNewBrow = Util.null2String(carRs.getPropValue("mobileBrow","userNewBrow"));//是否启用全屏浏览按钮
%>
<html>
<head>
 <style type="text/css">
...
.message-box{font-family:微软雅黑!important};
	...
	<script type="text/javascript" src="/mobile/plugin/1/js/copyrow_wev8.js"></script>
	<script type="text/javascript" src="/js/workflow/BigDecimal.js"></script>
			...
			try{
			 ...
			 $("button[name^='addbutton']").click(function(){
			  ...
			  $("button[name^='delbutton']").click(function(){
				 var tableindex=$(this).attr("name").replace("delbutton","");
				 var trrowindex = (new Number($("#nodenum"+tableindex).val())-1);
				 delformuaAttrShowDetail(trrowindex);
			 });
		  }catch(e){}
    ...
    */
                ...
                if (data.error) {
                    ...
                    if (data.logs != undefined && data.logs != null && count != "0") {
                        ...
                        function(i, item) {
                            ...
                            viewsignHtml += "<div class='signRow_operate'><div class=\"signRow_operatorname\">" + operatorName + "&nbsp;&nbsp;"+operatorDept+"</div> <div class=\"signRow_operateTime\">" + operateType + " 于 " + operateDate + "&nbsp;" + operateTime + "" + "</div></div>";
	...
	function showDialog(url, data) {
		...
		if(data.indexOf("method=listFnaBudgetFeeType") === -1 ){
			var selectedids = jQuery("#"+returnIdField).val();
			data += "&selectedids="+selectedids;
			showDialog2(url, data,'<%=userNewBrow%>');
		}else{
	...
	function getIsSubmitDirectNode() {
		return "<%=workflowRequestInfo.getWorkflowBaseInfo().getIsSubmitDirectNode() %>";
	}
...
<body >
<div id="browDiv" style="display:none;z-index: 10000;position:fixed;width: 100%;height: 100%;overflow: auto;">
	<iframe id="browFrame" frameborder="0"  scrolling="auto" src=""  style="width: 100%;height: 100%"></iframe>
</div>
<%--
...
<form id="workflowfrm" action="/mobile/plugin/1/RequestOperation.jsp?module=<%=module%>&scope=<%=scope%>&fromofs=<%=fromofs %>&f_weaver_belongto_userid=<%=f_weaver_belongto_userid%>&f_weaver_belongto_usertype=<%=f_weaver_belongto_usertype%>" method="post" enctype="multipart/form-data" autocomplete="off">
...
<%} %>
		...
		<div style="height:10px;overflow:hidden;"></div>
			...
			if ((workflowRequestInfo.getSubmitButtonName() != null && !workflowRequestInfo.getSubmitButtonName().equals(""))
				    ...
				    ||(workflowRequestInfo.getDeleteName() != null && !workflowRequestInfo.getDeleteName().equals(""))
			...
			if ((workflowRequestInfo.getSubmitButtonName() != null && !workflowRequestInfo.getSubmitButtonName().equals(""))
				...
				if((workflowRequestInfo.getForwardButtonName()!=null&&!workflowRequestInfo.getForwardButtonName().equals("")) 
				  ...
				  if(isCanEditRemark){
					   ...
					   if(workflowRequestInfo.getDeleteName()!=null&&!workflowRequestInfo.getDeleteName().equals("")){
                            onlyforwardButton =false;
					   }
				  }
			...
			<div class="signlistbtn">
			   ...
			   <div class="signbtnforwardopration" style="display:none;">
				...
				if(cntDisplayBtn > 0 || "1".equals(ismonitor)){ %>
                  ...
                  <%}%>
					...
					%>
					  ...
					  <%}
					   ...
					   <%}%>
					   <input type="hidden" name="isSubmitDirect" id="isSubmitDirect" value="0" />
                  ...
                  <%}%>
                  <%if(workflowRequestInfo.getDeleteName() != null && !workflowRequestInfo.getDeleteName().equals("")) { %>
					 <div class="listbtnhandler" id="doDelete" onclick="doDelete(this);">
					   <div class="listbtnimg"  style="background-image:url(/mobile/plugin/1/images/approval.png);background-size: contain;"></div>
				       <div class="listbtntext">
					        <%=workflowRequestInfo.getDeleteName()%>
					   </div>
					  </div>
				   <%} %>
				<%}%>
