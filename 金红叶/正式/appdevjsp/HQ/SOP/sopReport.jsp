...
String xgryxm = Util.null2String(request.getParameter("xgryxm"));
...
sql="select count(1) as count from  HrmRoleMembers where roleid=406 and resourceid ="+user_id;	
}
rs.executeSql(sql);
if(rs.next()){
...
<HTML>
  ...
  <link type="text/css" rel="stylesheet" href="/wui/theme/ecology8/skins/default/wui_wev8.css" />
	<BODY >
		...
		<FORM id=report name=report action="/appdevjsp/HQ/SOP/sopReport.jsp" method=post>
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;">
				...
				<wea:item>相关人员名称</wea:item>	
				<wea:item>
				 <input name="xgryxm" id="xgryxm" class="InputStyle" type="text" value="<%=xgryxm%>"/>
				</wea:item>
				...
				browserUrl="/systeminfo/BrowserMain.jsp?url=/interface/CommonBrowser.jsp?type=browser.ITZL_CMOTEAM"
			  ...
			  hasInput="true"  hasBrowser = "true" isMustInput='1' isSingle="false"
                ...
                <wea:item>SOP阶段</wea:item>
				<wea:item>
				<select class="e8_btn_top middle" name="sopstatus" id="sopstatus"> 
				<option value="" <%if("".equals(sopstatus)){%> selected<%} %>>
					<%=""%></option>
				<option value="草案" <%if("草案".equals(sopstatus)){%> selected<%} %>>
					<%="草案"%></option>
				<option value="与各部门讨论" <%if("与各部门讨论".equals(sopstatus)){%> selected<%} %>>
					<%="与各部门讨论"%></option>
				<option value="向股东报告" <%if("向股东报告".equals(sopstatus)){%> selected<%} %>>
					<%="向股东报告"%></option>
				<option value="完成完整SOP" <%if("完成完整SOP".equals(sopstatus)){%> selected<%} %>>
					<%="完成完整SOP"%></option>
				<option value="书面呈准" <%if("书面呈准".equals(sopstatus)){%> selected<%} %>>
					<%="书面呈准"%></option>
				</select>
				</wea:item>
	...
	<div class="table-head" style="width:3200px; ">
			<table class="ViewForm1  outertable" scrolling="auto">
    <tbody>		
        <tr>			
            <td>
            <table class="ViewForm1  maintable" scrolling="auto">
                <colgroup>  <col width="80px"></col><col width="100px"></col><col width="140px"></col><col width="180px"></col><col width="100px"></col><col width="100px"></col><col width="100px"></col><col width="100px"></col>
                ...
                <tbody>
					 <tr>
						...
						<td class="fname" rowspan="2" align="center">SOP阶段</td>
	...
	<div class="table-body" id="div22" style="width:3220px; ">
	<table class="ViewForm1  outertable" scrolling="auto">
    <tbody>		
        <tr>			
            <td>
            <table class="ViewForm1  maintable" scrolling="auto">
                ...
                <tbody>
					...
					String sop_status = "";
					int colorflag = -1;
					String backfields = " f_get_taskinfo(a.id,'','4') as color,f_get_taskinfo(a.id,'','5') as color1,  f_getsopstatus(a.id,'1') as sop_status,f_getsopstatus(a.id,'2')  as colorflag,a.id,a.procode,a.name,f_get_HRSName(b.sop_sop_contact,'1','') as sop_sop_contact,"+
					...
					String sqlWhere = " where a.id=b.id  and b.scopeid='1' and a.parentid=1";
					if(!"".equals(sopname)){
					...
					if(!"".equals(xgryxm)){
						sqlWhere +=" and f_checksop_name(a.id,'"+xgryxm+"') = 'S' ";
					}
					if(!"".equals(sopstatus)){
						sqlWhere +=" and f_getsopstatus(a.id,'1') = '"+sopstatus+"' ";
					}
					if(!"".equals(dhcolor)){
						if("1".equals(dhcolor)){
							dhcolor = "1,-1";
						}
						sqlWhere +=" and f_get_taskinfo(a.id,'','4') in("+dhcolor+") ";
					}
					if(!"".equals(xgry)){
					    sqlWhere +=" and (a.members like '%"+xgry+"%' or a.manager = '"+xgry+"' or "+
 									" b.sop_cmo_team_leader = '"+xgry+"' or "+
      								...
      								" b.sop_flow_contact = '"+xgry+"' or "+
       								...
       								" exists(select 1 from prj_taskprocess c where c.prjid=a.id and c.hrmid like '%"+xgry+"%')) ";
					}
					if(!"".equals(xgry1)){
					    sqlWhere +=" and (a.members like '%"+xgry1+"%' or a.manager = '"+xgry1+"' or "+
 									" b.sop_cmo_team_leader = '"+xgry1+"' or "+
      								" b.sop_sop_contact = '"+xgry1+"' or b.sop_cmo_contact = '"+xgry1+"' or "+
      								" b.sop_flow_contact = '"+xgry1+"' or "+
       								" b.sop_sub_flow_contact = '"+xgry1+"' or "+
       								" exists(select 1 from prj_taskprocess c where c.prjid=a.id and c.hrmid like '%"+xgry1+"%')) ";
					}
				...
				String bgcolor1="";
                    ...
                    <tr>
						...
						<%}else if(colorflag == 0){%>
						<td class="fvalue" bgcolor="<%=bgcolor1%>">&nbsp;&nbsp;<%=yj1%></td>
						<td class="fvalue" bgcolor="<%=bgcolor1%>">&nbsp;&nbsp;<%=sj1%></td>
						<td class="fvalue" bgcolor="<%=bgcolor1%>">&nbsp;&nbsp;<%=mqwc1%></td>
                        <%}else{%>
						...
						<%}else if(colorflag == 1){%>
						<td class="fvalue" bgcolor="<%=bgcolor1%>">&nbsp;&nbsp;<%=yj2%></td>
						<td class="fvalue" bgcolor="<%=bgcolor1%>">&nbsp;&nbsp;<%=sj2%></td>
						<td class="fvalue" bgcolor="<%=bgcolor1%>">&nbsp;&nbsp;<%=mqwc2%></td>
						<%}else{%>
						...
						<%}else if(colorflag == 2){%>
						<td class="fvalue" bgcolor="<%=bgcolor1%>">&nbsp;&nbsp;<%=yj3%></td>
						<td class="fvalue" bgcolor="<%=bgcolor1%>">&nbsp;&nbsp;<%=sj3%></td>
						<td class="fvalue" bgcolor="<%=bgcolor1%>">&nbsp;&nbsp;<%=mqwc3%></td>
						<%}else{%>
						...
						<%}else if(colorflag == 3){%>
						<td class="fvalue" bgcolor="<%=bgcolor1%>">&nbsp;&nbsp;<%=yj4%></td>
						<td class="fvalue" bgcolor="<%=bgcolor1%>">&nbsp;&nbsp;<%=sj4%></td>
						<td class="fvalue" bgcolor="<%=bgcolor1%>">&nbsp;&nbsp;<%=mqwc4%></td>
						<%}else{%>
						...
						<%}else if(colorflag == 4){%>
						<td class="fvalue" bgcolor="<%=bgcolor1%>">&nbsp;&nbsp;<%=yj5%></td>
						<td class="fvalue" bgcolor="<%=bgcolor1%>">&nbsp;&nbsp;<%=sj5%></td>
						<td class="fvalue" bgcolor="<%=bgcolor1%>">&nbsp;&nbsp;<%=mqwc5%></td>
						<%}else{%>
...
tableString =" <table tabletype=\"none\" pagesize=\""+ PageIdConst.getPageSize(tmc_pageId,user.getUID(),PageIdConst.HRM)+"\" >"+	
    ...
    operateString+
    "			<head>";
		tableString+="<col width=\"10%\"  text=\"灯号\" column=\"color1\" orderkey=\"color1\" hide=\"true\"/>"+
		"<col width=\"10%\"  text=\"SOP阶段\" column=\"sop_status\" orderkey=\"sop_status\" hide=\"true\"/>"+
