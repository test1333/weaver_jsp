	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(1361,user.getLanguage()) %>' attributes="" >
		...
		<wea:item>
			<brow:browser viewType="0" name="approvewfid" browserValue="" 
						...
						completeUrl="/data.jsp?type=workflowBrowser&from=prjwf&sqlwhere=isbill=1 and ( formid=74 or exists(select 1 from prj_prjwfconf where prj_prjwfconf.isopen='1' and prj_prjwfconf.wftype=2 and prj_prjwfconf.wfid=workflow_base.id))" />
...
function getWfBrowserUrl(data){
	var sqlwhere="where isbill=1 and ( formid=74 or exists(select 1 from prj_prjwfconf where prj_prjwfconf.isopen='1' and prj_prjwfconf.wftype=2 and prj_prjwfconf.wfid=workflow_base.id))";
