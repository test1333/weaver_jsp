...
if(!HrmUserVarify.checkUserRight("DeptDefineInfo1:DeptMaintain1", user)){
  ...
  if(src.equals("addfieldbatch")){//批量添加字段
	  		...
	  		if(!new_OR_modify.equals("")){//不为空表示是编辑字段，为空表示新添加字段。
	            ...
	            int imgheight = Util.getIntValue(request.getParameter("imgheight_"+index),0);
				  ...
				  if(!new_OR_modify.equals("")){//不为空表示是编辑字段，为空表示新添加字段。
					  ...
					  //插入字段信息
					  RecordSetTrans.executeSql("INSERT INTO prjtskDefineField(groupid,isopen,ismand,billid,fieldname,fieldlabel,fielddbtype,fieldhtmltype,type,dsporder,viewtype,detailtable,textheight,childfieldid,imgwidth,imgheight) "+
	  ...
	  response.sendRedirect("/proj/ffield/editprjtskfieldbatch.jsp?ajax=1");
