...
String sql_preTask= "select finish,subject from  Prj_TaskProcess where taskindex in ( " +pretaskid+") AND prjid="+Util.getIntValue(ProjID)+"";
...
%>			
			<button type="button" class=e8_browflow onclick="onShowMTask('taskids02span','taskids02','ProjID','taskrecordid')" id="taskids02"></button>
...
<%
			...
			if(fieldhtmltype==3&&(type==161||type==162)){
				String showname=PrjTskFieldManager.getBrowserFieldvalue(user,fieldValue, type, fielddbtype,false);
				String andChar="?";
				...
				String urls=BrowserComInfo.getBrowserurl(""+type)+"?type="+fielddbtype;     // 浏览按钮弹出页面的
...
String partentid_define="";
String temp_isactived="";
String sup_temp_begin="";
String sup_temp_end="";
String xl="";
rs.executeSql(" select xl,parentid,isactived from prj_taskprocess where id = "+taskrecordid+"");
if(rs.next()){
	partentid_define = Util.null2String(rs.getString("parentid"));
	temp_isactived = Util.null2String(rs.getString("isactived"));
	xl = Util.null2String(rs.getString("xl"));
}
rs.executeSql(" select beginDate||' '||begintime as temp_begin,enddate||' '||endtime as temp_end from prj_taskprocess where id = "+partentid_define+"");
if(rs.next()){
	sup_temp_begin = Util.null2String(rs.getString("temp_begin"));
	sup_temp_end = Util.null2String(rs.getString("temp_end"));
}
...
//任务类型
var task_type="field21";//类型
var task_name="subject";//任务名称
var task_fzr="hrmid";var task_fzr_span="hrmid_browserbtn";//负责人
var task_date="Calendar";
var task_time="Clock";
var task_days="workday";//工期
var task_sdays="realmandays";//实际工期
var task_cost="fixedcost";//项目预算
//var task_status="field22";//任务状态
var task_filenum="field23";//交付物数量
var task_filedesc="field24";//交付物说明
var task_pro="finish";//进度
var task_main="islandmark";//是否里程碑
var task_subtask="taskids02";//前置任务
var task_desc="content";//任务说明

var task_type_val="<%=xl%>";
var temp_isactived="<%=temp_isactived%>";

//任务只读
if(temp_isactived=="2"){
  jQuery("select[name="+task_type+"]").show();
  jQuery("select[name="+task_type+"]").attr('disabled',true);
  jQuery(".weatable_"+task_type).hide();
}
  
if(task_type_val=="1"&&temp_isactived=="2"){
  //alert(0);
  jQuery("input[name="+task_name+"]").attr("readonly","readonly");
  jQuery("input[name="+task_fzr+"]").attr("readonly","readonly");
  jQuery("#"+task_fzr_span).attr('disabled',true);
  jQuery("."+task_date).attr('disabled',true);
  jQuery("."+task_time).attr('disabled',true);
  jQuery("input[name="+task_days+"]").attr("readonly","readonly");
  jQuery("input[name="+task_sdays+"]").attr("readonly","readonly");
  //jQuery("input[name="+task_cost+"]").attr("readonly","readonly");

  jQuery("#"+task_pro).attr("readonly","readonly");
  jQuery("#"+task_pro).attr("class", "");

  //jQuery("select[name="+task_status+"]").show();
  //jQuery("select[name="+task_status+"]").attr('disabled',true);
  //jQuery(".weatable_"+task_status).hide();

  jQuery("#"+task_filenum).attr("readonly","readonly");
  jQuery("#"+task_filedesc).attr("readonly","readonly");
  jQuery("input[name="+task_main+"]").attr("readonly","readonly");
  jQuery("#"+task_subtask).attr('disabled',true);
  jQuery("#"+task_desc).attr("readonly","readonly");
}
...
//计划时间必填
bindchange("#begindate", prj_date);
bindchange("#enddate", prj_date);
prj_date();

if(jQuery("#actualBeginDate").length>0){
//任务进度绑定
bindchange("#finish", task_proc);
bindchange("#actualBeginDate", task_proc2);
bindchange("#actualEndDate", task_proc3);
} 
/*
//状态给进度赋值
 jQuery("select[name="+task_status+"]").bind('change',function(){ 
    var obj = jQuery("select[name="+task_status+"]").val(); 
    if(obj == 2){
		jQuery("#"+task_pro).val(100); 
	}else if(obj == 1){
		jQuery("#"+task_pro).val(50); 
	}else{
		jQuery("#"+task_pro).val(0); 
	}
  }); */
}); 
...
function check_jfw(){
	cansubmit = "0";
	var taskid1="<%=taskrecordid%>";
jQuery.post("/appdevjsp/HQ/ITProject/CheckJFW.jsp", {
				"taskid": taskid1
           }, function (data) {
				data = data.replace(/\n/g, "").replace(/\r/g, "");
				var result1=Number(data);
				if(result1 == 1){
				jQuery("#field21span").text("上阶段未完成或者上阶段交付物未提交完毕");
				jQuery("#field21span").css("color","red");
				cansubmit = "1";
				}
			});
}
...
function prj_date(){
	...
	if(begindate_val==""){
		jQuery("#begindatespan").html("<img src='/images/BacoError_wev8.gif' align='absmiddle'>");
		jQuery("#begindate").val("");
	}
	if(enddate_val==""){
		jQuery("#enddatespan").html("<img src='/images/BacoError_wev8.gif' align='absmiddle'>");
		jQuery("#enddate").val("");
	}
}
...
function task_proc(){
	var finish_val=eval(toFloat(document.all("finish").value,0));
	var actualBeginDate_val=jQuery("#actualBeginDate").val();
	var actualEndDate_val=jQuery("#actualEndDate").val();
	if(finish_val>0&&finish_val<100){
		if(actualBeginDate_val==""){
			jQuery("#actualBeginDatespan").html("<img src='/images/BacoError_wev8.gif' align='absmiddle'>");
		}
			jQuery("#actualEndDate").val("");
			jQuery("#actualEndDatespan").html("");

	}else if(finish_val>=100){
			if(actualBeginDate_val==""){
				jQuery("#actualBeginDatespan").html("<img src='/images/BacoError_wev8.gif' align='absmiddle'>");
		    }
		    if(actualEndDate_val==""){
		    	jQuery("#actualEndDatespan").html("<img src='/images/BacoError_wev8.gif' align='absmiddle'>");
		    }	
			
	}else{
			jQuery("#actualBeginDate").val("");
			jQuery("#actualEndDate").val("");
			jQuery("#actualBeginDatespan").html("");
			jQuery("#actualEndDatespan").html("");
	}
}
...
if(finish_val==100){
var flag =0;
	if(jQuery("#SWFUpload_0_0").length >0){
 		flag=1;
	}else{
  		for (var i=0;i<=20;i++){
  			if(jQuery("#span_id_"+i).length >0){
    			if(jQuery("#span_id_"+i).css("visibility")=="hidden"){
      				flag=1;
    			}
  			}else{
     			i = 21;
	...
	if(flag == 0){
  		alert(" 项目完成请上传附件./Please upload accessory ");
  		return 1;
	}
}
}
...
<script language="javascript">
function typeRelease(){
	//移除下拉框disabled
 	var task_type="field21";//类型
	//var task_status="field22";//任务状态
  	jQuery("select[name="+task_type+"]").removeAttr("disabled");
  	//jQuery("select[name="+task_status+"]").removeAttr("disabled");
}
function submitData(obj){
	if(cansubmit == "1"){
		alert("上阶段未完成或者上阶段交付物未提交完毕");
		return false;
	}
if(jQuery("#actualBeginDate").length>0){
	/*//附件验证
    ...
    var actualBeginDate_val = jQuery("#actualBeginDate").val();
    var actualEndDate_val = jQuery("#actualEndDate").val();
    if(finish_val>0&&actualBeginDate_val==""||finish_val==100&&actualEndDate_val==""){
    	check_form(frmain,"actualBeginDate,actualEndDate");
    	return false;
    }
} 

  	//时间验证
  	var partentid_define="<%=partentid_define%>";
  	if(partentid_define!="0"){
  		var partentid_define="<%=partentid_define%>";
  		var sup_temp_begin="<%=sup_temp_begin%>";
  		var sup_temp_end="<%=sup_temp_end%>";
  		var temp_begin =jQuery("#begindate").val()+" "+jQuery("#begintime").val();
  		var temp_end =jQuery("#enddate").val()+" "+jQuery("#endtime").val();
  		//alert("sup_temp_begin:"+sup_temp_begin+"sup_temp_end:"+sup_temp_end+"temp_begin:"+temp_begin+"temp_end:"+temp_end);
  		var is_legal=true;
  		if(sup_temp_begin<=temp_begin&&sup_temp_end>=temp_end){
  			is_legal=false;
  		}
  		if(is_legal){
  			alert("任务时间区间超出阶段或上级任务时间区间");
  			return false;
  		}
  	}
