...
if(!"1".equals(isGeneratedTemplateFile)){
	response.sendRedirect("/proj/imp/prjimpopt.jsp?generateTemplateFile=1&isdata=2&isdialog="+isDialog) ;
	return;
}
...
<FORM id=cms name=cms action="prjimpopt.jsp?isdata=2" method=post enctype="multipart/form-data">
...
<wea:layout attributes="{'expandAllGroup':'true'}">
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("101,1332,18596",user.getLanguage()) %>' attributes="{'groupDisplay':''}" >
		<wea:item><%=SystemEnv.getHtmlLabelNames("172,25106",user.getLanguage())%></wea:item>
		<wea:item>
			<brow:browser viewType="0" name="parentprj" 
				browserValue="" 
				browserSpanValue=""
				browserUrl="/systeminfo/BrowserMain.jsp?url=/proj/data/ProjectBrowser.jsp?from=prjtskimp"
				hasInput="true" isSingle="true" hasBrowser = "true" isMustInput='2' width="200px"
				completeUrl="/data.jsp?type=8&from=prjtskimp"  />
		</wea:item>
...
%>
	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(19010,user.getLanguage())%>'>
		<wea:item attributes="{'colspan':'full'}">
			<div>
				<br>
				<%=SystemEnv.getHtmlLabelName(28447,user.getLanguage())%><a style="color: #1d98f8;cursor: pointer;" href="prjtskimp_xlsnew.xls"><%=SystemEnv.getHtmlLabelName(28446,user.getLanguage())%></a>.
				<br><br>
		...
		<wea:item attributes="{'colspan':'full'}">
			<div>
				...
				<span style="color:#FF0000;">
					...
					<%=SystemEnv.getHtmlLabelName(1352,user.getLanguage())%>&nbsp;
					<%=SystemEnv.getHtmlLabelName(2097,user.getLanguage())%>&nbsp;
				</span>	
...
function onSave(obj){
	if($GetEle('parentprj').value=="" ){
		window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(15859,user.getLanguage())%>");
	}else if ($GetEle('filename').value=="" || $GetEle('filename').value.toLowerCase().indexOf(".xls")<0){
...
$(function(){
	try{
		parent.setTabObjName("<%=SystemEnv.getHtmlLabelNames("1332,18596",user.getLanguage()) %>");
	}catch(e){}
});
</script>
</body>
