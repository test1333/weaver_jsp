...
String xl = Util.null2String(request.getParameter("xl"));
...
<wea:layout type="4col">
	<wea:group context='<%=SystemEnv.getHtmlLabelNames("15774",user.getLanguage())%>'>
		...
		<wea:item><%=SystemEnv.getHtmlLabelNames("22256",user.getLanguage())%></wea:item>
		<wea:item>
			<select name="xl">
				<option value=""></option>
				<option value="0" <%="0".equals(xl)?"selected":"" %> ><%=SystemEnv.getHtmlLabelNames("1332",user.getLanguage())%></option>
				<option value="1" <%="1".equals(xl)?"selected":"" %> ><%=SystemEnv.getHtmlLabelNames("24902",user.getLanguage())%></option>
			</select>
		</wea:item>
...
String sqlWhere = " where t1.prjid=t2.id and t1.isdelete =0 and t1.finish<100  and "+tonullsql+"  and "+
...
if(!"".equals(xl)){
	if(Util.getIntValue(xl,0)==0){
		sqlWhere+=" and t1.xl =0 ";
	}else{
		sqlWhere+=" and t1.xl =1 ";
	}
}
...
String backfields = " t1.id,t1.subject,t1.prjid,t1.begindate,t1.enddate,t1.actualbegindate,t1.finish,t1.status,t1.islandmark, "+
					" case t1.xl when 0 then '任务' when 1 then '阶段' else '' end as type ";
...
String tableString=""+
        ...
        "<head>"+
        	  ...
        	  "<col width=\"10%\"  text=\""+SystemEnv.getHtmlLabelName(22256,user.getLanguage())+"\" column=\"type\" orderkey=\"type\" />"+
              "<col width=\"12%\"  text=\""+SystemEnv.getHtmlLabelName(1352,user.getLanguage())+"\" column=\"id\" orderkey=\"subject\" otherpara=\"column:subject+column:status+"+user.getLanguage()+"+column:islandmark+column:prjid+column:begindate+column:enddate\"  transmethod='weaver.proj.util.ProjectTransUtil.getPrjTaskNameByStatus' />"+
              "<col width=\"13%\"  text=\""+SystemEnv.getHtmlLabelName(22168,user.getLanguage())+"\" column=\"begindate\"  orderkey=\"begindate\"/>"+
              "<col width=\"12%\"  text=\""+SystemEnv.getHtmlLabelName(22170,user.getLanguage())+"\" column=\"enddate\" orderkey=\"enddate\"/>"+
              "<col width=\"12%\"  text=\""+SystemEnv.getHtmlLabelNames("33351",user.getLanguage())+"\" column=\"actualbegindate\" orderkey=\"actualbegindate\" />"+
              "<col width=\"21%\"  text=\""+SystemEnv.getHtmlLabelName(847,user.getLanguage())+"\" column=\"finish\" orderkey=\"finish\" otherpara='column:enddate' showaspercent=\"true\" transmethod='weaver.proj.util.ProjectTransUtil.getPrjTaskProgressbar' />"+                             
        "</head>"+
        "<operates width=\"5%\">"+
