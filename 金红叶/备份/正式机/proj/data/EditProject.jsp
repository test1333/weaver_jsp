...
if(!openfieldMap.isEmpty()){
	...
	<wea:item attributes='<%=hideField %>'>
		...
		<%}else	if("protemplateid".equals(fieldName)){
			...
			if(fieldhtmltype==3&&(type==161||type==162)){
				String showname=PrjFieldManager.getBrowserFieldvalue(user,fieldValue, type, fielddbtype,false);
				String andChar="?";
	...
	<%
	if(fieldcount==fieldsize && groupcount==1 && PrjSettingsComInfo.getPrj_acc() ){//附件位置
...
//字段联动 add by wxp 20161219
function datainput(){
    var peopleId = $("#hrmids02").val();
	var URL = "/proj/data/GetBranchesById.jsp?peopleId="+peopleId;
	 URL = encodeURI(URL);
	 var flag=false;
	 jQuery.ajax({
       url: URL,
       ...
       success: function(data){ 
            ...
            var temp1 =  "#field"+"<%=branchesid%>";
            var temp2 =  "#field"+"<%=branchesid%>"+"span";
            
            $(temp1).attr("value",tempValue);    
            $("#hrmids02").attr("onpropertychange","datainput()");//删除项目成员时删除对应的分部
            
             var htmlStr = "";
              for(var i=0;i<temp.length/2;i++){
                htmlStr += "<span class='e8_showNameClass'>" +
			               "<a href='/hrm/company/HrmSubCompanyDsp.jsp?id="+temp[i*2]+"' target='_blank' title="+temp[i*2+1]+">"+temp[i*2+1]+"</a>" +
			               "<span id="+temp[i*2]+" class='e8_delClass' style='visibility: hidden; opacity: 1;'>x</span>" +
			               "</span>";
              }            
              $(temp2).html(htmlStr);                 
         }
    ...
    //20161230 sop字段联动
    function sopmanager(){
    	var managerid  = jQuery("#customfield25").val();
    	var url = "/appdevjsp/HQ/SOP/SopLinkage.jsp?type=2&managerid="+managerid;
    	url = encodeURI(url);
    		jQuery.ajax({
       			url: url,
       			type: "post",
       			...
       			var temp = new Array();
   				 temp = data.toString().split(","); 
   				 var lfgcval = "#customfield19";
   				 var lfgcspan = "#customfield19span";
			     	var lfgcspan_url = "<span class='e8_showNameClass'>" +
			               			"<a href='/hrm/company/HrmSubCompanyDsp.jsp?id="+temp[0]+"' target='_blank' title="+temp[1]+">"+temp[1]+"</a>" +
			               			"<span class='e8_delClass' style='visibility: hidden; opacity: 1;'>x</span>" +
			               			"</span>";
			            $(lfgcval).val(temp[0]);
   						$(lfgcspan).html(lfgcspan_url); 

			        if(temp[1]==undefined){
			        	$(lfgcspan).html(""); 
   						$(lfgcspan).html("<img src='/images/BacoError_wev8.gif' align='absmiddle'>");    
			        }           
         }        
   		});
    }
    ...
    function comuser(userid){
    	var url = "/appdevjsp/HQ/SOP/SopLinkage.jsp?type=1&userid="+userid;
    	url = encodeURI(url);
    		jQuery.ajax({
       			url: url,
       			type: "post",
       			...
       			var temp = new Array();
   				temp = data.toString().split(","); 
   				var ckr = "#customfield12";
   				var ckrspan = "#customfield12span";
   				var url1 = "<span class='e8_showNameClass'>" +
			               			"<a href='/hrm/HrmTab.jsp?_fromURL=HrmResource&id="+temp[0]+"' target='_blank' title="+temp[1]+">"+temp[1]+"</a>" +
			               			"<span class='e8_delClass' style='visibility: hidden; opacity: 1;'>x</span>" +
			               			"</span>";
			    	$(ckr).val(temp[0]);
   					$(ckrspan).html(url1); 
   	...
   	function comtype(userid){
    	...
    	url = encodeURI(url);
    		jQuery.ajax({
       			...
       			var temp = new Array();
   				temp = data.toString().split(","); 
   				//alert(temp[2]);alert(temp[3]);
   				var zb = "#customfield24";
   				var zbspan = "#customfield24span";
   				var url2 = "<span class='e8_showNameClass'>" +
			               			"<a>"+temp[3]+"</a>" +
									"<span class='e8_delClass' style='visibility: hidden; opacity: 1;'>x</span> " +
			               			"</span>";
			    	$(zb).val(temp[2]);
    ...
    function comtype_cg(){
    	var comuserid = jQuery("#customfield12").val();
		comtype(comuserid);
    }

    function commanager(){
    	var cateid = jQuery("#customfield24").val();
    	//alert("cateid="+cateid);
    	var url = "/appdevjsp/HQ/SOP/SopLinkage.jsp?type=0&cateid="+cateid;
    	url = encodeURI(url);
    		jQuery.ajax({
       			url: url,
       			type: "post",
       			...
       			var temp = new Array();
   				temp = data.toString().split(","); 
   				var manger = "#customfield11";
   				var mangerspan = "#customfield11span";
   				var url3 = "<span class='e8_showNameClass'>" +
			               			"<a href='/hrm/HrmTab.jsp?_fromURL=HrmResource&id="+temp[0]+"' target='_blank' title="+temp[1]+">"+temp[1]+"</a>" +
									"<span class='e8_delClass' style='visibility: hidden; opacity: 1;'>x</span> " +
			               			"</span>";
			    	$(manger).val(temp[0]);
   					$(mangerspan).html(url3);
         }        
   		});
    }
    ...
    //add by wxp 20161219 &金光 20121230
    jQuery(document).ready(function(){
	       $("#hrmids02").attr("onpropertychange","datainput()");
	       var type = $("input[name='prjtype']").val()
	       var userid = <%=userid%> 
	       if(type=="1"){
	       	    /*CMO*/
	       		//comuser(userid); //CMO窗口人
	       		//comtype(userid);  //CMO组别
	       		//commanager();     //CMO负责人
	       		bindchange("#customfield12", comtype_cg);
	       		bindchange("#customfield24", commanager);

	       		/*SOP*/
	       		bindchange("#customfield25", sopmanager);
	       	}
    });
