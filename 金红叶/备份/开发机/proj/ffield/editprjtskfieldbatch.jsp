...
<jsp:useBean id="DeptFieldManager0" class="weaver.proj.util.PrjTskFieldManager" scope="page"/>
...
String sql = "select * from prjtskDefineField where viewtype=0 order by dsporder,id";
...
function showdetaildata(){
	...
	jQuery.ajax({
		url : "/proj/ffield/editprjtskfieldbatchdetail.jsp?qname=<%=qname%>&fieldname_kwd=<%=fieldname_kwd%>&fieldlabel_kwd=<%=fieldlabel_kwd%>",
		type : "get",
		async : true,
		cache:false,
		processData : false,
...
%>
<FORM id="frmMain" name="frmMain" action="/proj/ffield/editprjtskfieldbatch.jsp" method="post" >
...
function checkmaxlength(maxlen,elementname){
    ...
    if(tmpvalue < maxlen){
    	window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(23548,user.getLanguage())%>");
...
function onSave(obj){
	frmMain.action="/proj/ffield/prjtskfieldoperation.jsp" 
	changeRows = 0;
