...
String tablename = "prj_projectinfo";
String tablename2 = "cus_fielddata";//项目类型自定义字段
...
<thead>
          <tr class=header>
            ...
            <td style="display:none;" NOWRAP><%=SystemEnv.getHtmlLabelName(88,user.getLanguage())%></td>
          </tr>
          </thead>
</table>
...
<tr style='display:none;'>
	...
	<td></td>
		...
		String issystem="";
		String allowhide="";
		String fieldtype="";//1,项目自定义字段;2,项目类型自定义字段;
		int textheight = 0;
		String sql ="";
		
		if(fromType&&proTypeId>0){//项目类型(包括项目通用自定义字段,和项目类型自己的自定义字段)
			sql =" select * from ( ";
			sql+=" select '1' as fieldtype,t1.id,t1.isopen,t1.ismand,t1.isused,t1.groupid,t1.issystem,t1.allowhide,t1.fieldname,t1.fieldlabel,t1.fielddbtype,t1.fieldhtmltype,t1.type,t1.dsporder,t1.textheight,t1.imgwidth,t1.imgheight,t1.childfieldid from prjDefineField t1 where t1.prjtype="+proTypeId+" and t1.viewtype=0 "; 
			sql+=" union ";
			...
			sql+=" and t1.scopeid='"+proTypeId+"' ";
			sql+=" ) tt ";
		}else{//通用项目自定义字段
			sql= "select '1' as fieldtype,t1.id,t1.isopen,t1.ismand,t1.isused,t1.groupid,t1.issystem,t1.allowhide,t1.fieldname,t1.fieldlabel,t1.fielddbtype,t1.fieldhtmltype,t1.type,t1.dsporder,t1.textheight,t1.imgwidth,t1.imgheight,t1.childfieldid from prjDefineField t1 where t1.prjtype=-1 and t1.viewtype=0  ";
		}
		...
		//System.out.println("sql:==\n"+sql);		
			RecordSet.executeSql(sql);
				while(RecordSet.next()){
					...
					fieldtype = Util.null2String( RecordSet.getString("fieldtype"));
					...
					//已使用过的字段和项目类型中的项目自定义字段都不允许删除
					//boolean isFieldUsed="1".equals(isused)||PrjFieldManager.isFieldUsed(""+proTypeId,fieldname,fieldtype)||(fromType && "1".equals(fieldtype));
					boolean isFieldUsed="1".equals(issystem)||PrjFieldManager.isFieldUsed(""+proTypeId,fieldname,fieldtype)||(fromType && "1".equals(fieldtype));
%>
          <TR class=<%=trClass%>>
			<td  height="23" NOWRAP >
			...
			<td NOWRAP >
			  ...
			  <input type="hidden" name="fieldtype_<%=rowsum%>" value="<%=fieldtype %>" >
			</td>
			<td NOWRAP>
				<%
				if(sysField){
			...
			<td NOWRAP>
				  ...
				  <div id="div1_<%=rowsum%>" <%if(fieldhtmltype.equals("1")){%>style="display:inline"<%}else{%>style="display:none"<%}%>>
					...
					<div id="div1_1_<%=rowsum%>" <%if(fieldhtmltype.equals("1")&&type.equals("1")){%>style="display:inline"<%}else{%>style="display:none"<%}%>>
					  <%
					      int maxlength = 0;
					      if(isoracle){
					          rs.executeSql("select max(lengthb("+fieldname+")) from "+("1".equals(fieldtype)? tablename:tablename2));
					          if(rs.next()) maxlength = Util.getIntValue(rs.getString(1),0);
					      }else{
					          rs.executeSql("select max(datalength("+fieldname+")) from "+("1".equals(fieldtype)? tablename:tablename2));
					          if(rs.next()) maxlength = Util.getIntValue(rs.getString(1),0);
					      }
					  %>
				  ...
				  <div id="div5_<%=rowsum%>" <%if(fieldhtmltype.equals("5")&&!"1".equals(issystem)&&(proTypeId==-1||(proTypeId>0&&"2".equals(fieldtype)))){%>style="display:inline"<%}else{%>style="display:none"<%}%> >
					...
					<span style='display:none;' id='childfieldidSpan_<%=rowsum%>'></span>
					<input type='hidden' value='' name='childfieldid_<%=rowsum%>' id='childfieldid_<%=rowsum%>'>
					</div>
					<div id="div5_5_<%=rowsum%>" <%if(fieldhtmltype.equals("5")&&!"1".equals(issystem)&&(proTypeId==-1||(proTypeId>0&&"2".equals(fieldtype)))){%>style="display:inline"<%}else{%>style="display:none"<%}%>>
				  	<table class='ViewForm' id='choiceTable_<%=rowsum%>' cols=7 border=0>
						...
						<col width="0%">
				  		...
				  		String selecttablename="1".equals(fieldtype)?"prj_SelectItem":"2".equals(fieldtype)?"cus_selectitem":"prj_SelectItem";
				  		String backfields=" * ";
				  		if("2".equals( fieldtype)){
				  			backfields=" * ";
				  		}
				  		RecordSet1.executeSql("select "+backfields+" from "+selecttablename+" where  fieldid="+fieldid+" order by selectvalue ");
				  		...
				  		<td style="display:none;"><input type="hidden" id="selectvalue<%=rowsum%>_<%=recordchoicerowindex%>" name="selectvalue<%=rowsum%>_<%=recordchoicerowindex%>" value="<%=RecordSet1.getString("selectvalue")%>">
							<input type='checkbox' name='isAccordToSubCom<%=rowsum%>_<%=recordchoicerowindex%>' value='1' <%="" %>><%=SystemEnv.getHtmlLabelName(22878,user.getLanguage())%>&nbsp;&nbsp;
						  ...
						  <td style="display:none;">
							...
							<input type="hidden" id="childItem_<%=rowsum%>_<%=recordchoicerowindex%>" name="childItem_<%=rowsum%>_<%=recordchoicerowindex%>" value="<%="" %>" >
							<span id="childItemSpan_<%=rowsum%>_<%=recordchoicerowindex%>" name="childItemSpan_<%=rowsum%>_<%=recordchoicerowindex%>"><%="" %></span>
						  </td>
          ...
          <div id="div6_<%=rowsum%>" <%if(fieldhtmltype.equals("6")){%>style="display:inline"<%}else{%>style="display:none"<%}%>>
				...
				<div id="div7_<%=rowsum%>" <%if(fieldhtmltype.equals("7")){%>style="display:inline"<%}else{%>style="display:none"<%}%>>
				  ...
				  if(fieldhtmltype.equals("7")){
				     rs.executeSql("select * from prj_specialfield where fieldid = " + fieldid + "  ");
				     rs.next();
