...
int passnoworktime = Util.getIntValue(request.getParameter("passnoworktime"),0);
...
RCMenu += "{"+SystemEnv.getHtmlLabelName(91,user.getLanguage())+",javascript:delRows(),_self}";
RCMenuHeight += RCMenuHeightStep;

RCMenu += "{"+SystemEnv.getHtmlLabelName(86,user.getLanguage())+",javascript:parent.submitData(this),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
...
<!------------------任务列表---------------------开始-->
   ...
   <input type="hidden" name="prjmanager" id="prjmanager" value='<%=manager %>'>
   ...
   <TABLE CLASS="ListStyle" valign="top" cellspacing=1 id="tblTask" >
    	  ...
    	  <thead>
    	  	<TR class="header">
	           ...
	           <TH	width="16%"><%=SystemEnv.getHtmlLabelName(742,user.getLanguage())%></TH>     
	           <TH	width="16%"><%=SystemEnv.getHtmlLabelName(743,user.getLanguage())%></TH>
	           ...
	           <TH  style="display:none"><%=SystemEnv.getHtmlLabelName(386,user.getLanguage())%></TH>
...
<div style="display:none;height:100px!important;"></div>
...
//加载任务
$(function(){
	var taskDataUrl="/proj/task/PrjTaskTreeJSONData.jsp?loadAll=1&src=project&ProjID=<%=ProjID %>";
	var taskDataLazyUrl="/proj/task/PrjTaskTreeJSONData.jsp?loadAll=0&src=project&ProjID=<%=ProjID %>";
	loadTaskData("tblTask",taskDataUrl,taskDataLazyUrl,"true");
