...
String pageId=Util.null2String(PropUtil.getPageId("prj_myprojecttab"));
%>
...
String sqlWhere = " where  t1.status not in (0,3,4,6,7) and ( "+CommonShareManager.getPrjShareWhereByUser(user) +" )  ";
if(!"".equals(paraid)){
...
String sqlWhere2=SQLUtil.filteSql(RecordSetV.getDBType(),  sqlWhere);

String statusid=Util.null2String( request.getParameter("statusid"));
if(!"".equalsIgnoreCase(src)&&!"".equals(statusid) ){
	sqlWhere+=" and t1.status='"+statusid+"' ";
}
...
String orderby =" t1.procode,enddate,t1.id ";
		...
		jQuery.post(
			...
			function(data){
				window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelNames("16746",user.getLanguage())%>",function(){
					_table.reLoad();
					getTabNum('prjexec',"<%=src %>","<%=XssUtil.put(sqlWhere) %>","<%=XssUtil.put(sqlWhere2) %>","<%=statusid %>");
				});
			}
		);
	}
...
function getTabNum(type,src,sqlwhere,sqlwhere2,statusid){
	jQuery.ajax({
		url : "/proj/data/PrjGetTabNumAjax.jsp",
		type : "post",
		async : true,
		processData : true,
		data : {type:type,src:src,sqlwhere:sqlwhere,sqlwhere2:sqlwhere2,statusid:statusid},
		dataType : "json",
		success: function do4Success(data){
			if(data.totalCount1){
				...
				try{//自定义状态
					if(data.cusStatusNum){
						var arr=data.cusStatusNum;
						for(var o in arr){
							if(arr[o].status){
								var cusspan="cusstatus_"+arr[o].status+"_span";
								var cusnum=arr[o].totalcount;
								if(cusnum>0){
									parent.$(".e8_box").find("#"+cusspan).html("("+cusnum+")");
								}
							}
						}
					}
...
$(function(){
	...
	if(cptgroupname==''){
		cptgroupname='<%=SystemEnv.getHtmlLabelName(16408,user.getLanguage()) %>';
	}
	try{
