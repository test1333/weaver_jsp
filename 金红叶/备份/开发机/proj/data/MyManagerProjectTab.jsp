...
<%
String search_resourceid = Util.null2String(request.getParameter("search_resourceid"));//从人力资源卡片过来
...
String crm = Util.null2String(request.getParameter("crm"));
    ...
    String pageId=Util.null2String(PropUtil.getPageId("prj_mymanagerprojecttab"));
%>
...
<%
if("frozen".equalsIgnoreCase(src)||"complete".equalsIgnoreCase(src)){
	...
	%>
		...
		<wea:item>
			<Select class="InputStyle" name="prjstatus">
				...
				<%
				String defaultStatusIds=",0,1,2,5,6,7,";
				while(ProjectStatusComInfo.next()){
					if("".equals(src)){
						if(defaultStatusIds.indexOf(","+ProjectStatusComInfo.getProjectStatusid()+",")<=-1){
							continue;
						}
					}
				%>
...
if(!"".equals( search_resourceid)){
	userid=search_resourceid;
}

String sqlWhere = " where t1.manager='"+userid+"' and ( "+CommonShareManager.getPrjShareWhereByUser(user) +" )  ";
if(!"".equals(paraid)){
...
if("all".equalsIgnoreCase(src)){
	...
	sqlWhere+=" and t1.status not in(0,3,4,6,7) ";//未完成,指正常的项目(包括正常,自定义,延期的状态)
}
...
String tableString=""+
        ...
        "<sql backfields=\""+backfields+"\" sqlform=\""+fromSql+"\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlWhere)+"\" sqlprimarykey=\"t1.id\" sqlorderby=\""+orderby+"\" sqlsortway=\"desc\" sqldistinct=\"true\" />"+
        "<head>"+                             
      ...
      if("frozen".equals(src)){
        ...
        tableString+=""+      
              "<col width=\"25%\"  text=\""+SystemEnv.getHtmlLabelName(18628,user.getLanguage())+"\" column=\"members\" orderkey=\"members\" transmethod='weaver.proj.util.ProjectTransUtil.getResourceNamesWithLink'/>"+
        "</head>"+
        "<operates width=\"5%\">"+
			...
			jQuery.post(
				...
				function(data){
					window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelNames("83472",user.getLanguage())%>",function(){
						_table.reLoad();
						getTabNum('myprj',"<%=src %>","<%=XssUtil.put(sqlWhere) %>","<%=XssUtil.put(sqlWhere2) %>");
...
$(function(){
	...
	if('<%=""+user.getUID() %>'!='<%=userid %>'){
		cptgroupname='<%=ResourceComInfo.getResourcename(userid) %>'+' <%=SystemEnv.getHtmlLabelNames("83812",user.getLanguage())%>';
	}
	
	try{
		parent.setTabObjName(cptgroupname);
		var searchresourceid='<%=search_resourceid %>';
	}catch(e){}
});
//列表数据刷新后触发
