...
String[] lxs = fu.getParameterValues("txtlx");       //负责人
//4E8
...
String alltskaccids="";
    ...
    if (rowIndexs!=null){
        for (int i=0 ;i<rowIndexs.length;i++){
			...
			String lx =  Util.null2String(lxs[i]);
			...
			RecordSet.executeSql("update Prj_TaskProcess set xl='"+lx+"' where id="+TaskIDinsert);
	...
	boolean bNeedUpdate=false;
        ...
        if (rowIndexs!=null)
                                for (int i=0 ;i<rowIndexs.length;i++){
                                    ...
                                    String begintime =  Util.null2String(beginTimes[i]);
                                    String endtime=  Util.null2String(endTimes[i]);
                                    String lx1 =  Util.null2String(lxs[i]);
                                    if(endDate.compareTo(maxEndDate)>0){
                                    	maxEndDate=endDate;
                                    }
                                    int beforeTask = Util.getIntValue(beforeTasks[i],0);
                                    ...
                                    **/
                                    ...
                                    if(taskrecordid>0){
										...
										String strsqlForTask="update Prj_TaskProcess set subject='"+taskName+"',hrmid='"+manager+"',begintime='"+begintime+"',endtime='"+endtime+"',begindate='"+beginDate+"',enddate='"+endDate+"',workday="+workLong+",fixedcost="+budget+",prefinish="+beforeTask+",parentid="+parentid+",level_n="+level+",dsporder="+i+",status="+status+",parentids='"+parentids+"',parenthrmids='"+parenthrmids+"',xl='"+lx1+"' where prjid="+ProjID+" and taskIndex="+rowIndex;
								      ...
								      //父任务更新时间和工期
                                        ...
                                        if (RecordSet.next())   TaskID = RecordSet.getString("maxid_1");
											RecordSet.executeSql("update Prj_TaskProcess set xl='"+lx1+"' where id="+TaskID);
                                      //将存储过程中的parentids和parenthrmids两个字段的值重新赋值  因为hrmid改为多人力资源  Start
