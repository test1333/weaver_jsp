...
String accdocids=Util.null2String(request.getParameter("accdocids"));
    ...
    String[] lxs = request.getParameterValues("txtlx");       //负责人
          ...
          //附件上传开始
            ...
            if(taskNames!=null&&taskNames.length>0){
                ...
                for (int i=0 ;i<taskNames.length;i++){
                    ...
                    String strsqlForTask = "insert into Prj_TemplateTask (templetId,templetTaskId,taskName,taskManager,begindate,enddate,workday,budget,parentTaskId,befTaskId,xl) values("+newProjId+","+rowIndex+",'"+taskName+"','"+manager+"','"+beginDate+"','"+endDate+"',"+workLong+","+budget+","+parentTaskId+","+beforeTask+",'"+lx+"')";  
                    blog.writeLog("tangtest任务:"+strsqlForTask);              
            ...
            if(rowIndexs!=null&&rowIndexs.length>0){
                ...
                for (int i=0 ;i<rowIndexs.length;i++){
                  ...
                  //4E8
                    ...
                    if (taskid>0){
                        strsqlForTask="update Prj_TemplateTask set templetTaskId='"+rowIndex+"',taskName='"+taskName+"',taskManager='"+manager+"',begindate='"+beginDate+"',enddate='"+endDate+"',workday="+workLong+",budget="+budget+",parentTaskId="+parentTaskId+","
                        +"befTaskId="+beforeTask+",xl='"+lx+"' where templetid="+templetId+" and id="+taskid;
                    } else {
                        strsqlForTask = "insert into Prj_TemplateTask (templetId,templetTaskId,taskName,taskManager,begindate,enddate,workday,budget,parentTaskId,befTaskId,xl) values("+templetId+","+rowIndex+",'"+taskName+"','"+manager+"','"+beginDate+"','"+endDate+"',"+workLong+","+budget+","+parentTaskId+","+beforeTask+",'"+lx+"')"; 
                    }
                     blog.writeLog("tangtest任务:"+strsqlForTask);    
                    rs.executeSql(strsqlForTask); 
                 }
    	...
    	for(int i=0;i<arr.length;i++){
    		...
    		if (!"".equals(id1)) {
               ...
               //删除除任务表中的数据
                ...
                rs.executeSql(strDelTask);
               //删除自定义表中的数据
                String strDelCdTemplet = "delete cus_fielddata where scope='ProjCustomField'  and id = "+id1;
                rs.executeSql(strDelCdTemplet);
