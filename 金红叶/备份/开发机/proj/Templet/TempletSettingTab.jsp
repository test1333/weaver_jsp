	...
	<wea:group context='<%=SystemEnv.getHtmlLabelName(1361,user.getLanguage())%>' attributes="{'groupDisplay':''}" >
		...
		<wea:item>
			<brow:browser viewType="0"  width="200px" name="relatingFlow" browserValue='<%=wfid %>' browserSpanValue='<%=wfComInfo.getWorkflowname(""+wfid) %>'
						...
						completeUrl="/data.jsp?type=workflowBrowser&from=prjwf&sqlwhere=isbill=1 and (formid=152 or formid in(select formid from prj_prjwfconf where isopen='1' and wftype=3))"
...
function getWfBrowserUrl(data){
	var sqlwhere="where isbill=1 and (formid=152 or formid in(select formid from prj_prjwfconf where isopen='1' and wftype=3))";
