...
function addRow(){
	var select_id_max='0';
	$("#tblTask").find("[name=txtlx1]").each(function(){
	//	alert($(this).val());
		var txtlx_val=  $(this).val();		
		var select_stmp = $(this).attr("id")
		var select_id = select_stmp.substring(7,select_stmp.length);
		if(Number(select_id_max)<Number(select_id)){
			select_id_max = select_id;
		}
    ...
    childNode.editStart();
    lossFlag=true;
	select_id_max=Number(select_id_max)+1;
	$("#txtlx1_"+select_id_max).removeAttr("disabled");
}
function delRows(){
...
function reloadTaskTree(){
	...
	//	alert($(this).val());
		var txtlx_val=  $(this).val();		
		var select_stmp = $(this).attr("id")
		var select_id = "#txtlx_"+select_stmp.substring(7,select_stmp.length);
	    $(select_id).attr("value",txtlx_val);
...
function loadTaskData(tableId,url,lazyurl,isedit,istemplate){
	...
	$("#"+tableId).fancytree({
		...
		renderColumns: function(event, data) {
            ...
            var budgetHtml="<input type='text' style='display:none' name='txtBudget' size='8' id='txtBudget_"+iRowIndex+"' onKeyPress='ItemNum_KeyPress(this)' value='"+budget+"'>";                
			...
			var lxHtml='';	
			if(isedit=="true" && "true" !=istemplate){
				lxHtml="<select  style = 'width:100px;' name='txtlx1' id='txtlx1_"+iRowIndex+"' disabled='disabled'> <option value='' ";
				if(lx !='0'&&lx !='1'){
					lxHtml = lxHtml +" selected ";
				}
				lxHtml = lxHtml+"></option> <option value='0'";
				if(lx=='0'){
					lxHtml = lxHtml +" selected ";
				}
				lxHtml = lxHtml+">任务</option><option value='1' ";
				if(lx=='1'){
					lxHtml = lxHtml +" selected ";
				}
				lxHtml = lxHtml+">阶段</option></select>";
			}else{	  
				lxHtml="<select  style = 'width:100px;' name='txtlx1' id='txtlx1_"+iRowIndex+"'> <option value='' ";
				if(lx !='0'&&lx !='1'){
					lxHtml = lxHtml +" selected ";
				}
				lxHtml = lxHtml+"></option> <option value='0'";
              ...
              var lxHtml1="<input type='text'  style='display:none' name='txtlx' size='8' id='txtlx_"+iRowIndex+"' onKeyPress='ItemNum_KeyPress(this)' value='"+lx+"'>";                
            if(lossFlag){
            	$tdList.eq(4).find("input[name=txtTaskName]").val(tasknameval);
			}else{
				
				$tdList.eq(3).html(lxHtml);
				$tdList.eq(4).html(realidHtml+parentindexHtml+levelHtml+templatetaskidHtml+rowindexHtml+tasknameHtml+worklongHtml);
				$tdList.eq(5).html(begindateHtml+" "+begintimeHtml);
				$tdList.eq(6).html(enddateHtml+" "+endtimeHtml);
				$tdList.eq(7).html(pretaskHtml);				
				$tdList.eq(8).html(managerHtml);
				$tdList.eq(9).html(budgetHtml);
				$tdList.eq(10).html(lxHtml1);
			}
