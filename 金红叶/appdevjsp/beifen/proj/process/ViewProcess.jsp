...
int perpage=16;                                 
String backfields = " t1.parentids,t1.hrmid,t1.id,t1.subject,t1.prjid,t1.begindate,t1.enddate,t1.begintime,t1.endtime,t1.actualbegintime,t1.actualendtime,t1.actualbegindate,t1.actualenddate,t1.finish,t1.islandmark,t1.workday,t1.realManDays,t1.status,t1.level_n,case t1.xl when 0 then '任务' when 1 then '阶段' else '' end as type";
...
if(!"".equals(version)){
	fromSql=" (select a.*,b.xl from Prj_TaskInfo a left join Prj_TaskProcess b on a.taskid=b.id) t1 ";
...
String tableString=""+
        ...
        "<head>"+                             
              "<col width=\"17%\"  text=\""+SystemEnv.getHtmlLabelName(1352,user.getLanguage())+"\" column=\"id\" orderkey=\"subject\" otherpara=\"column:status+"+user.getLanguage()+"+"+level+"+"+version+"\"  transmethod='weaver.proj.util.ProjectTransUtil.getPrjTaskNameByLevel' target='_fullwindow' />"+
              "<col width=\"12%\"  text=\""+SystemEnv.getHtmlLabelName(22256,user.getLanguage())+"\" column=\"type\" orderkey=\"type\" />"+
              "<col width=\"13%\"  text=\""+SystemEnv.getHtmlLabelName(15285,user.getLanguage())+"\" column=\"hrmid\" orderkey=\"hrmid\" transmethod='weaver.cpt.util.CommonTransUtil.getHrmNamesWithCard' />"+
              ...
              "<col width=\"15%\"  text=\""+SystemEnv.getHtmlLabelName(742,user.getLanguage())+"("+SystemEnv.getHtmlLabelName(84364,user.getLanguage())+")\" column=\"begindate\" orderkey=\"begindate\" otherpara='column:begintime+column:actualbegindate+column:actualbegintime+\"\"' transmethod='weaver.proj.util.ProjectTransUtil.getPrjTask2Date' />"+
              "<col width=\"15%\"  text=\""+SystemEnv.getHtmlLabelName(743,user.getLanguage())+"("+SystemEnv.getHtmlLabelName(84364,user.getLanguage())+")\" column=\"enddate\" orderkey=\"enddate\" otherpara='column:endtime+column:actualenddate+column:actualendtime+\"\"' transmethod='weaver.proj.util.ProjectTransUtil.getPrjTask2Date' />"+
              "<col width=\"20%\"  text=\""+SystemEnv.getHtmlLabelName(847,user.getLanguage())+"\" column=\"finish\" orderkey=\"finish\" otherpara='column:enddate' showaspercent=\"true\" transmethod='weaver.proj.util.ProjectTransUtil.getPrjTaskProgressbar' />"+                             
        "</head>";
if("".equals(version)){
        tableString+=      
        "<operates width=\"5%\">"+
	...
	if(totalCount>0){
        ...
        success: function(data){
	    	returnValue = jQuery.trim(data);
           	firsttr.find("td").eq(1).next().next().next().html(returnValue);
        	}
	    });
	}
	//initProgressbar();
