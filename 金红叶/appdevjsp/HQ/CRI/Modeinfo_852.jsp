...
<script type="text/javascript">
	...
	jQuery(document).ready(function(){		
		...
		checkCustomize = function(){
			...
			var needchecks = jQuery("#needcheck").val();
			var fieldids = 	needchecks.split(",");
			for(var i=0;i<fieldids.length;i++){
				if(fieldids[i] != ""){
					var fieldvalue = jQuery("#"+fieldids[i]).val();
					if(fieldvalue == ""){
						window.top.Dialog.alert("必填字段未填写");
						return false;
					}
				}
			}	
			...
			if(!(gdgz=="")){
				...
				jQuery.ajax({
					...
					success:function(data){
						...
						if(result == "0"){
							flag = "success";
							alert("<%=SystemEnv.getHtmlLabelName(-11701, usertemp.getLanguage())%>");//开放流程已创建成功！
							jQuery("#disfield<%=zt%>").val("4");
							jQuery("#field<%=zt%>").val("4");
						}else if(result == "1"){
							alert("<%=SystemEnv.getHtmlLabelName(-11704, usertemp.getLanguage())%>");//结案流程已创建成功！
							flag = "success";
							jQuery("#disfield<%=zt%>").val("5");
							jQuery("#field<%=zt%>").val("5");
						}else if(result == "2"){
							alert("<%=SystemEnv.getHtmlLabelName(-11702, usertemp.getLanguage())%>");//搁置流程已创建成功！
							flag = "success";
							jQuery("#disfield<%=zt%>").val("6");
							jQuery("#field<%=zt%>").val("6");
						}else if(result == "3"){
							alert("<%=SystemEnv.getHtmlLabelName(-11703, usertemp.getLanguage())%>");//取消流程已创建成功！
							flag = "success";
							jQuery("#disfield<%=zt%>").val("7");
							jQuery("#field<%=zt%>").val("7");
						}else if(result == "100"){
							alert("<%=SystemEnv.getHtmlLabelName(-11705, usertemp.getLanguage())%>");//该通知流程创建失败！
							flaga = "stop";
						}else if(result == "500"){
							flaga = "stop";
						}
	...
	function CheckZt(){
		...
		if(zt=="4" || zt=="5" || zt=="6" || zt=="7"){
			alert("<%=SystemEnv.getHtmlLabelName(-11696, usertemp.getLanguage())%>");//只能选择前面4种状态的值，请重新选择！
			jQuery("#field<%=zt%>").val("");
		}
	}	
	function Detail_1(){
