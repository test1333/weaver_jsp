...
if(operate.equals("add") || operate.equals("addAndDetail")){
	...
	while(rs.next()){
		...
		insq="insert into zoa_wffield_map (Tid,FieldName,SAPFieldName,ChangeType,ChangeInfo,Remark,IsActive) values ('"+zhuid+"',UPPER('"+fieldnames+"'),UPPER('"+fieldnames+"'),'0','','','1')";
	...
	for(int i=0;i<listname.size();i++){
		...
		while(rs.next()){
			fieldnames=rs.getString("fieldname"); 
			insq="insert into zoa_wffield_map (Tid,FieldName,SAPFieldName,ChangeType,ChangeInfo,Remark,IsActive) values ('"+list.get(i)+"',UPPER('"+fieldnames+"'),UPPER('"+fieldnames+"'),'0','','','1')";
			RecordSet.executeSql(insq);	
		}
...
//
    ...
    for(int i=1;i<=fieldscount;i++){
		...
		///listMing
		String cssql = "insert into zoa_wffield_map(Tid,FieldName,SAPFieldName,ChangeType,ChangeInfo,Remark,IsActive) values(?,?,?,?,?,?,?)"; 
		...
		if(i<=zhus) {
			...
			if(rs.next()){
				...
				"' where  tid= '"+zhuid+"' and FieldName=UPPER('"+FieldName+"')";
			...
			//System.out.println("-------------"+n+"-------------");
				if(m==0&&i<=(n+zhus)){
					...
					if(rs.next()){
						...
						"' where  tid= '"+listMing1.get(j)+"' and FieldName=UPPER('"+FieldName+"')";
