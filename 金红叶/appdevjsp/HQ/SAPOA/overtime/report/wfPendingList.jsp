...
<wea:layout type="4col">
	...
	String backfields = " id,getwfcreater(requestid) as creator,requestid,receivedate||' '||receivetime as tDateTime,userid, getwfNower(requestid) as users1, "
       				+" nvl(isreminded,0) as isreminded, 'N' as isprocessed,"
       				+" OPERATEDATE||' '||OPERATETIME as opDateTime,getAddDateTime(receivedate,substr(receivetime,0,5),lasttime) as adt ,"
    					+ "ROUND ((lasttime-diffTodayTime(f.receivedate,substr(f.receivetime,0,5)) )/60/8,1) as adds  "; 
	String fromSql  = " from WfPendingOverTimeView f ";
	...
	if(searchType == 0 ){
		fromSql = " from WorkflowPendingMattersView f ";
	}
	
	if(searchType == 1){
		sqlWhere = sqlWhere + " and f.optime > f.lasttime";
	}else if(searchType == 2){
		sqlWhere = sqlWhere + " and optime < lasttime  AND lasttime - optime   < 480 ";
	}else if(searchType == 3){
		sqlWhere = sqlWhere + " and optime < lasttime  AND lasttime - optime   >= 480 ";
	}else if(searchType == 4){
	//	sqlWhere = sqlWhere + " and optime < lasttime  AND lasttime - optime   >= 960 ";
		sqlWhere = sqlWhere + " and optime < lasttime  AND lasttime - optime   >= 480 ";
	}
...
//	  " <col width=\"12%\" text=\"审批完成时间\" column=\"opDateTime\" orderkey=\"opDateTime\"   />"+
	  " <col width=\"7%\" text=\"剩余时间(天)\" column=\"adds\" orderkey=\"adds\"   />"+
	" <col width=\"6%\" text=\"自动流转标识\" column=\"isprocessed\" orderkey=\"isprocessed\"   />"+
	" <col width=\"7%\" text=\"当前处理人\" column=\"users1\" orderkey=\"users1\"  />"+
      " </head>"+
      " </table>";
%>
