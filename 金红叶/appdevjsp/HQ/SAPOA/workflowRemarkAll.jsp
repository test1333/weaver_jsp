...
<%
    ...
    if (!"".equals(requestid)) {
        sql = "select lc_no from zoat00020 where lc_type='" + lc_type + "' and ref_no ='" + ref_no + "' order by lc_no asc";
        rs.executeSql(sql);
        while (rs.next()) {
    ...
    if ("".equals(requestid)) {
        ...
        if (!"".equals(requestid)) {
            sql = "select lc_no from zoat00030 where lc_type='" + lc_type + "' and ref_no ='" + ref_no + "' order by lc_no asc";
            rs.executeSql(sql);
            while (rs.next()) {
                requestids = requestids + flag + Util.null2String(rs.getString("lc_no"));
                flag = ",";
    ...
    request.getSession().setAttribute("weaver_user@bean", User.getUser(Integer.valueOf("52001991"), 0));
...
<wea:layout type="table" attributes="{'cols':'7','cws':'10%,15%,10%,15%,15%,15%,20%'}">
