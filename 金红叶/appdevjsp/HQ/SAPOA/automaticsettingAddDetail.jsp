				...
				<table class=ListStyle cellspacing=1>
					...
					if(!formID.equals("")){
					    if(isbill.equals("0")){//表单
					        ...
					        RecordSet.executeSql("select * from workflow_billfield where viewtype=0 and billid="+formID+" order by id ");
					        while(RecordSet.next()){
					...
					<%if(i%2==0){%><tr class=DataLight><%}%>
						...
						<%
						  String yanzh="select SAPFIELDNAME,CHANGETYPE,CHANGEINFO,REMARK ,ISACTIVE,ismust  from  zoa_wffield_map where TID= (select ID from zoa_wftable_map where fid ="+viewid+"   and TYPE='M'  ) and FIELDNAME = UPPER('"+SapFieldlable+"')";
						  ...
						  if(rss.next()){
							  ...
							  String ismust = Util.null2String(rss.getString("ismust"));
							...
							<select id="zhaunhuan_<%=fieldscount%>" name="zhaunhuan_<%=fieldscount%>" onchange="onSelect(<%=fieldscount%>)" >
								...
								<option value="9" <% if(CHANGETYPE.equals("9")) out.print("selected");%>  >长数字</option>
							</select>    						
						</td>	
							<%}else{%>
								<td>
							<select id="zhaunhuan_<%=fieldscount%>" name="zhaunhuan_<%=fieldscount%>" onchange="onSelect(<%=fieldscount%>)" >
								...
								<option value="9"  >长数字</option>
							</select>    						
						</td>
							<%}
							if(!CHANGEINFO.equals("")){%>
								<td>
							<textarea  id="zhuanhuanneirong_<%=fieldscount%>" name="zhuanhuanneirong_<%=fieldscount%>" rows=1 style="width:90%" ><%=CHANGEINFO%></textarea>
							...
							<%	
								...
								<td>
							<textarea  id="zhuanhuanneirong_<%=fieldscount%>" name="zhuanhuanneirong_<%=fieldscount%>" rows=1   style="width:90%"></textarea>
							
								</td>
							<%} 
							if(!"".equals(ismust)){%>
							 <td>
							<input type="checkbox" tzCheckbox="true" class=InputStyle id="ismust_<%=fieldscount%>" name="ismust_<%=fieldscount%>" value="<%=ismust%>" onclick="aa1(<%=fieldscount%>)"   <%if("1".equals(ismust)) {%> checked <%}%>>
							...
							<td>
							<input type="checkbox" tzCheckbox="true" class=InputStyle id="ismust_<%=fieldscount%>" name="ismust_<%=fieldscount%>" value="0" onclick="aa1(<%=fieldscount%>)" >
							...
							<%}  	  	  
							 if(!REMARK.equals("")){%>
							 <td>
							<textarea   id="neirongmiaoshu_<%=fieldscount%>" name="neirongmiaoshu_<%=fieldscount%>" rows=1  style="width:90%"><%=REMARK%></textarea>
								</td> 
							 <% }else{%>
							  <td>
							<textarea   id="neirongmiaoshu_<%=fieldscount%>" name="neirongmiaoshu_<%=fieldscount%>" rows=1  style="width:90%"></textarea>
								</td> 
							 <%}  
							 if(!ISACTIVE.equals("")){%>
							 <td>
				...
				<table class=ListStyle cellspacing=1>
				  ...
				  while(rs1.next()){
					...
					<%}%>
						...
						<%
						  String yanzh="select SAPFIELDNAME,CHANGETYPE,CHANGEINFO,REMARK ,ISACTIVE,ismust  from  zoa_wffield_map where TID= (select ID from zoa_wftable_map where fid ="+viewid+"   and TYPE not in ('M') and  tbname= '"+RecordSet.getString("tablename")+"' and  ISACTIVE=1 ) and FIELDNAME = UPPER('"+SapDetailfield+"')";
						  rss.executeSql(yanzh);
						  if(rss.next()){
							  String  SAPFIELDNAME =Util.null2String(rss.getString("SAPFIELDNAME"));
							  String  CHANGETYPE =Util.null2String(rss.getString("CHANGETYPE"));
							...
							<select id="zhaunhuan_<%=fieldscount%>" name="zhaunhuan_<%=fieldscount%>" onchange="onSelect(<%=fieldscount%>)" >
								...
								<option value="7" <% if(CHANGETYPE.equals("7")) out.print("selected");%>  >MANDT</option>
								<option value="8" <% if(CHANGETYPE.equals("8")) out.print("selected");%>  >附件ABI</option>
								<option value="9" <% if(CHANGETYPE.equals("9")) out.print("selected");%>  >长数字</option>
							</select>    						
						</td>	
							<%}else{%>
...
function aa1(index){
	...
	var shifou_val=jQuery("#ismust_"+index).val();
	if(shifou_val==0){
		jQuery("#ismust_"+index).val("1");
	}else{
		jQuery("#ismust_"+index).val("0");
	}
