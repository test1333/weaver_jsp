...
<%
	...
	if(filename.lastIndexOf(".")>=0){
		...
		if("jpg".equals(type.toLowerCase())){
			...
			response.setHeader("Content-Disposition", "attachment;filename=\"temple."+type+"\"");
		}else if("pdf".equals(type.toLowerCase())){
			response.setContentType("application/pdf");
			response.setHeader("Content-Disposition", "inline;filename=\"temple."+type+"\"");
		}else{
			response.setContentType("APPLICATION/OCTET-STREAM");
			response.setHeader("Content-Disposition", "attachment;filename=\"temple."+type+"\"");
		}
		...
		response.setContentType("APPLICATION/OCTET-STREAM");
		response.setHeader("Content-Disposition", "attachment;filename=\"temple\"");
	}
	//response.setContentType("APPLICATION/OCTET-STREAM");
	//response.setContentType("image/jpeg");
	//response.setContentType("application/pdf");
