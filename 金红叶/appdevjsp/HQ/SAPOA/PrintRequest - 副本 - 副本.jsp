...
request.getSession().setAttribute("weaver_user@bean", User.getUser(Integer.valueOf("52001990"), 0));
...
//}
%>
<%@ include file="/appdevjsp/HQ/SAPOA/init_wev8.jsp" %>
...
response.sendRedirect("/workflow/request/ViewRequestDoc.jsp?f_weaver_belongto_userid="+userid+"&f_weaver_belongto_usertype="+usertype+"&requestid="+requestid+"&isrequest="+isrequest+"&isovertime="+isovertime+"&isaffirmance="+isaffirmance+"&reEdit="+reEdit+"&wflinkno="+wflinkno);
return;
}
}
...
if(istoManagePage && !isprint && !wfmonitor){
    ...
    String actionPage = "/workflow/request/ManageRequestNoForm.jsp?fromFlowDoc="+fromFlowDoc ;
...
%>
        ...
        if( overtime!="1")
        {
        	...
        	<%if("1".equals(isShowChart)){%>
				window.location.href="/workflow/request/WorkflowDirection.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&requestid=<%=requestid%>&workflowid=<%=workflowid%>&isbill=<%=isbill%>&formid=<%=formid%>";
				<%}else{%>
					window.location.href = "/workflow/request/RequestView.jsp";
				<%}%>
        	}
        	try{
   				parent.window.taskCallBack(2,<%=preisremark%>); 
...
<%}%>
    <form name="frmmain" method="post" action="/workflow/request/RequestOperation.jsp" <%if (!fromPDA.equals("1")) {%> enctype="multipart/form-data" <%}%>>
<%
...
<form name="frmmain" method="post" action="/workflow/request/RequestOperation.jsp">
    <%
    request.setAttribute("requestname",requestname);
    request.setAttribute("workflowname",workflowname);
	session.setAttribute(userid+"_"+requestid+"requestname",requestname);
	...
	%>
	   ...
	   <%} %>
            <jsp:include page="/workflow/request/WorkflowViewRequestHtml.jsp" flush="true">
				<jsp:param name="modeid" value="<%=modeid%>" />
                <jsp:param name="workflowid" value="<%=workflowid%>" />
                <jsp:param name="languageid" value="<%=user.getLanguage()%>" />
                <jsp:param name="canactive" value="<%=canactive%>" />
	...
	<%}else{%>
            <jsp:include page="/workflow/request/WorkflowViewRequestBodyAction.jsp" flush="true">
                <jsp:param name="workflowid" value="<%=workflowid%>" />
                <jsp:param name="languageid" value="<%=user.getLanguage()%>" />
                <jsp:param name="canactive" value="<%=canactive%>" />
                <jsp:param name="deleted" value="<%=deleted%>" />
            ...
            <jsp:include page="/workflow/request/WorkflowViewSignAction.jsp" flush="true">
                <jsp:param name="workflowid" value="<%=workflowid%>" />
                <jsp:param name="languageid" value="<%=user.getLanguage()%>" />
                <jsp:param name="requestid" value="<%=requestid%>" />
                <jsp:param name="userid" value="<%=userid%>" />
...
function openSignPrint1() {
    var redirectUrl = "/workflow/request/PrintMode.jsp?isprintlog=true&f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&requestid=<%=requestid%>&isbill=<%=isbill%>&workflowid=<%=workflowid%>&formid=<%=formid%>&nodeid=<%=nodeid%>&billid=<%=billid%>&fromFlowDoc=1&urger=<%=urger%>&ismonitor=<%=ismonitor%>" ;
...
function openSignPrint() {
  var redirectUrl = "/workflow/request/PrintRequest.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&requestid=<%=requestid%>&isprint=1&fromFlowDoc=1&urger=<%=urger%>&ismonitor=<%=ismonitor%>" ;
  <%//解决相关流程打印权限问题  
  if(wflinkno>=0){
  %>
  redirectUrl = "/workflow/request/PrintRequest.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&requestid=<%=requestid%>&isprint=1&fromFlowDoc=1&isrequest=1&wflinkno=<%=wflinkno%>&urger=<%=urger%>&ismonitor=<%=ismonitor%>";
  <%}%>  
...
function onNewRequest(wfid,requestid,agent){
	var redirectUrl =  "/workflow/request/AddRequest.jsp?f_weaver_belongto_userid=<%=userid%>&f_weaver_belongto_usertype=<%=usertype%>&workflowid="+wfid+"&isagent="+agent+"&reqid="+requestid;
	var width = screen.availWidth-10 ;
	var height = screen.availHeight-50 ;
	var szFeatures = "top=0," ;
	szFeatures +="left=0," ;
