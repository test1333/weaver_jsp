...
<%
String operateFrom = StringUtils.trimToEmpty(request.getParameter("operateFrom"));
String operateTH= StringUtils.trimToEmpty(request.getParameter(TableTextUtil.OPERATE_TH));
String operateQZ= StringUtils.trimToEmpty(request.getParameter(TableTextUtil.OPERATE_QZ));
...
//根据文档id判断该目录下是否开启分享功能
String canEnjoy="";
String sqlForEnjoy="select isSetEnjoy from docseccategory where id in(select seccategory from DocDetail where id="+docid+")";
RecordSet rss=new RecordSet();
rss.execute(sqlForEnjoy);
while(rss.next()){
	canEnjoy=rss.getInt("isSetEnjoy")+"";
	
}
if(("1").equals(canEnjoy)){
	canEnjoy="true";
}else{
	canEnjoy="false";
}
...
boolean isNotShow = "1".equals(fromFlowDoc) && !"1".equals(operateTH) && !"1".equals(operateQZ);
...
//lmc 20180827 查询流程的相关信息
String requestInfo = RequestDoc.getSignaturOpinions(requestid+"",workflowid+"",
							languageId);
requestInfo = requestInfo.replaceAll("%nbsp;", "");
rs.writeLog(" lmc requestInfo="+requestInfo);
...
//TD 10368 End
	...
	if(toWarmPage){
		toWarmPage = !"false".equals(request.getParameter("toWarmPage"));
	}

	if(toWarmPage && !canDownload){
		%>
		    <script language=javascript>
				var _agent = navigator.userAgent;
				if(navigator.userAgent.indexOf("MSIE 7.0") > -1 || navigator.userAgent.indexOf("MSIE 8.0") > -1){
					location="/docs/pdfview/web/sysRemind.jsp?labelid=998";
				}else{
					var _href = location.href;
					var _prehref = _href.substring(0,_href.indexOf("?"));
					var _params = _href.substring(_href.indexOf("?") + 1);
					location.href = _prehref + "?toWarmPage=false&" + _params;
				}
		    </script> 
		<%
		 return ;
...
RecordSet.writeLog("===========================1259"+isApplyMould);
RecordSet.writeLog("===========================1259"+wordmouldid);
RecordSet.writeLog("===========================1259"+isUseTempletNode);
...
if(!(imagefileName.toLowerCase().endsWith(".doc") || 
	...
	if(requestid > 0 && "".equals(isAppendTypeField)){
		//BaseBeanOfDocDspExt.writeLog("test aaa requestid:"+requestid+" isAppendTypeField:"+isAppendTypeField+" workflowid:"+workflowid+" nodeid:"+nodeid+" currentnodeid:"+currentnodeid+" isCurrentOperator:"+isCurrentOperator);
		int templenodeid = 0;
		String isstart = "";
		String sqldoc = "select useTempletNode from workflow_createdoc where status=1 and workflowid="+workflowid; 
		rs.executeSql(sqldoc);
		if(rs.next()){
			templenodeid = Util.getIntValue(rs.getString("useTempletNode"),0);
		}
		if(templenodeid > 0){
			if(currentnodeid == nodeid){
				sqldoc = "select isstart from workflow_nodebase where id="+currentnodeid;
				rs.executeSql(sqldoc);
				if(rs.next()){
					isstart = Util.null2String(rs.getString("isstart"));
				}
				if(!"1".equals(isstart)){
					if(nodeid != templenodeid){
						iframeLoad = true;
					}
				}
...
menus = getMenuBars(onlyview, fromFlowDoc, isPersonalDoc, isrequest, canEditHis, docstatus,
		...
		OpenSendDoc, docid, cannewworkflow, canPublish, canInvalidate,
		 ...
		 isSignatureNodes, cantop, istop, accessorycount,countNum,hasUsedTemplet,RCMenu,RCMenuHeight,RCMenuHeightStep,iframeLoad,isFromAccessory,openFirstAss,isIE,fileExtendName,isToDocDspExt,islastversion,hasOpenEdition,isCustomer,imagefileName,docsubject,isUseEMessager,canEnjoy);
...
%>
	...
	<jsp:include page="/docs/docs/DocDspExtLoad.jsp" flush="true">
		...
		<jsp:param name="isFromTable" value="<%=TableTextUtil.OPERATE_FROM.equals(operateFrom) %>" />
		<jsp:param name="isTextInForm" value="<%=isTextInForm %>" />
        <jsp:param name="requestInfo" value="<%=requestInfo %>" />        
	</jsp:include>
...
<iframe id="DocCheckInOutUtilIframe" name="DocCheckInOutUtilIframe" frameborder=0 scrolling=no src=""  style="display:none"></iframe>
...
<div id="divContentTab" style="width:100%;height:100%;">
       <table id="topTitle" cellpadding="0" cellspacing="0">
			<tr>
				...
				<%if(!onlyview){
					...
					<%} %>
				<%}%>
				<% if(!onlyview && "1".equals(fromFlowDoc)) { %>

						<%if (istoManagePage) { %>
							<%if (isUseTempletNode) { %>
						<% if(!TableTextUtil.OPERATE_FROM.equals(operateFrom)||isSignatureNodes) { %>
				...
				<%}%>
								<%if(countNum > 1 && isUseTempletNode && isremark==0&&!TableTextUtil.OPERATE_FROM.equals(operateFrom)){ %>
					...
					<% if(!TableTextUtil.OPERATE_FROM.equals(operateFrom)) { %>
				...
				<%}%>
				  <%if((docstatus.equals("1")||docstatus.equals("2")||docstatus.equals("5")) && "1".equals(user.getLogintype())  && ( !isFromAccessory || "1".equals(isrequest)) && isUseEMessager==1){ %>
					<input type=button class="e8_btn_top" style='<%= canEnjoy.equals("true") ? "" : "display:none;" %>' onclick="javascript:onDocShare(<%=docid %>,'<%=docsubject%>')" value="<%=SystemEnv.getHtmlLabelName(126091, user.getLanguage())%>"></input>
				  <%} %>
	...
	<div class="e8_box demo2">
    	<div class="e8_boxhead" style='<%=isNotShow ? "display:none" : "" %>'>
						 ...
						 <%if ((fileExtendName.isEmpty()||(!fileExtendName.isEmpty()&&isToDocDspExt))&&!sysremindflag){%>
							...
							<%}else{ %>
								<div style="display:<%=iframeLoad?"none":""%>">
									<OBJECT id="WebOffice" name="WebOffice" classid="<%=mClassId%>" style="POSITION:absolute;width:0;height:0;<%if("1".equals(isTextInForm)){%>top:-20px<%}else{%>top:<%=fromFlowDoc.equals("1")?"-23px":"-23px" %><%}%>;" codebase="<%=mClientUrl%>" >
									</OBJECT>
								</div>
							<%} %>
...
function useTempletCancel(){
	...
	function adjustContentHeight(type,deltaHeight){
		...
		try{
			...
			<%if("1".equals(isTextInForm)){%>
				jQuery("#WebOffice").height(divContentHeight-60);
			<%}else{%>
...
function doNothing(){  
    ...
    <%if(filetype.equals(".doc")||filetype.equals(".xls")||filetype.equals(".wps")||filetype.equals(".et")){%>
    	try{
...
private Map<String,Object> getMenuBars(
		...
		if (!onlyview) {
			if (!fromFlowDoc.equals("1")) {
					...
					**/
					...
					if (canEdit || (defaultDocLocked != 1 && !docstatus.equals("5")) || hasRightOfViewHisVersion) {
						if ((filetype.equals(".doc")||filetype.equals(".docx")|| filetype.equals(".wps"))&&!iframeLoad) {
							if(!isUseTempletNode || !"1".equals(fromFlowDoc)){
				...
				if ((canEdit || (defaultDocLocked != 1 && !docstatus.equals("5")) || hasRightOfViewHisVersion) && (!isUseTempletNode || !"1".equals(fromFlowDoc))) {
					if ((filetype.equals(".doc") ||filetype.equals(".docx")|| filetype.equals(".wps"))&&!iframeLoad) {
						RCMenu += "{"+SystemEnv.getHtmlLabelName(16385,user.getLanguage())+",javascript:webOfficeMenuClickNew(36),_top} " ;
						RCMenuHeight += RCMenuHeightStep ;
					}
		...
		if(canEnjoy.equals("true")&&(docstatus.equals("1")||docstatus.equals("2")||docstatus.equals("5")) && "1".equals(user.getLogintype()) && ( !isFromAccessory || "1".equals(isrequest)) && isUseEMessager==1){
