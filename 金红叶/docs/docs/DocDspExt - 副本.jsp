						 ...
						 <%if ((fileExtendName.isEmpty()||(!fileExtendName.isEmpty()&&isToDocDspExt))&&!sysremindflag){%>
							...
							<%}else{ %>
								<div style="display:<%=iframeLoad?"none":""%>">
									<OBJECT id="WebOffice" name="WebOffice" classid="<%=mClassId%>" style="POSITION:absolute;width:0;height:0;<%if("1".equals(isTextInForm)){%>top:-15px<%}else{%>top:<%=fromFlowDoc.equals("1")?"-23px":"-23px" %><%}%>;" codebase="<%=mClientUrl%>" >
									</OBJECT>
								</div>
							<%} %>
