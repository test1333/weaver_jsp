...
<link href="/wui/theme/<%=session.getAttribute("SESSION_TEMP_CURRENT_THEME") %>/jquery/plugin/passwordStrength/password_strength_wev8.css" rel="stylesheet" type="text/css" />
    <script src="/wui/theme/<%=session.getAttribute("SESSION_TEMP_CURRENT_THEME") %>/jquery/plugin/passwordStrength/jquery.passwordStrength_wev8.js" type="text/javascript"></script>
		...
		$(document).ready(function(){
			var $pwd = $('input[name="passwordxmm"]');					   
...
String passwordjmm = Util.null2String(request.getParameter("passwordjmm"));
...
String passwordComplexity ="0"; settings.getPasswordComplexity();
...
<FORM id=password name=frmMain style="MARGIN-TOP: 3px" action=HrmResourceOperationGZMM.jsp method=post>
...
<wea:item>
	<INPUT class=inputstyle id=passwordjmm type=password style="width: 150px;"  name=passwordjmm onchange='checkinput("passwordjmm","passwordoldimage")'/>
...
<wea:item><%=SystemEnv.getHtmlLabelName(27303,user.getLanguage())%></wea:item>
    ...
    <wea:item>
		<INPUT class=inputstyle id=passwordxmm type=password style="width: 150px"  name=passwordxmm > 
		<span id=passwordnewimage><img src="/images/BacoError_wev8.gif" align=absMiddle></span>
		<%
			String title = "";
    ...
    <wea:item>
		<INPUT class=inputstyle id=confirmpassword type=password style="width: 150px"  name=confirmpassword onchange='checkinput("confirmpassword","confirmpasswordimage")'>
...
jQuery(document).ready(function(){
	jQuery("#passwordxmm").bind("keyup",function(){
		var newpwd=this.value;
		var reg = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}$/;
		if(reg.test(newpwd)){
			jQuery("#passwordnewimage").html("<span style='color:green;'>满足条件！</span>");
		}else{
			jQuery("#passwordnewimage").html("<span style='color:red;'>长度大于或等于6位,包含大写字母、小写字母、数字!</span>");
		}
	});
});
...
function submitData() {
	...
	var newpwd=jQuery("#passwordxmm").val();
	if(newpwd != ""){
		var reg = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}$/; 
		if(!reg.test(newpwd)){
			jQuery("#passwordnewimage").html("<span style='color:red;'>不满足条件！</span>");
			window.top.Dialog.alert("新密码不满足条件！");
			return false;
		}
	}
	...
	if(checkpassword ()){
		var flag = true;
		<%if("0".equals(usbstate) && "2".equals(userUsbType)){%>
...
{
	var ins = document.getElementById("passwordxmm");
...
if(!check_form(password,"passwordjmm,passwordxmm,confirmpassword"))//检查必填 
    return false;
if(password.passwordxmm.value.length<<%=minpasslen%>){
    window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(20172,user.getLanguage())+minpasslen%>");
	return false;
    }
if(password.passwordxmm.value != password.confirmpassword.value) {//两次密码不一致验证
...
return true;
		...
		<%if("0".equals(usbstate) && "2".equals(usbType)){%>
		<script language=javascript>  
		...
		<script language=VBScript>
			function changeKeyPW()
				...
				If Err.number<>0 or hCard = 0 then
					'alert("<%=SystemEnv.getHtmlLabelName(83717, user.getLanguage())%>")
					ret = 0
					Exit function
				End if
				htactx.VerifyUserPin hCard, CStr(password.passwordjmm.value) '校验口令
				If Err.number<>0 Then
					htactx.CloseDevice hCard
					ret = 0
					Exit function
				End if
				htactx.ChangeUserPin hCard, CStr(password.passwordjmm.value), CStr(password.passwordxmm.value)
				If Err.number<>0 Then
					'alert ("<%=SystemEnv.getHtmlLabelName(83720, user.getLanguage())%>")
					ret = 0
					htactx.CloseDevice hCard
					Exit function
				End If
