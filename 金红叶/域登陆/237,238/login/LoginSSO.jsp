...
try{
	...
	String RedirectFile="/portal/plugin/main.jsp"; 
	...
	if(isSyadmin){
		if(ismutilangua){
				islanguid = Util.getIntValue(request.getParameter("islanguid"),0);
					...
					//单点登录时取该用户默认系统使用语言
					RecordSet1.executeSql("select systemlanguage from HrmResource where loginid='"+names[1]+"'");
					if(RecordSet1.next()){
	...
	if(gopage.length()>0){
		RedirectFile = "/portal/plugin/main.jsp?gopage="+gopage ;
	}
	if (loginfile.equals("")){
	...
	if(usercheck.equals("15") || usercheck.equals("16")|| usercheck.equals("57") || usercheck.equals("17")|| usercheck.equals("45")|| usercheck.equals("46")|| usercheck.equals("47")|| usercheck.equals("52")|| usercheck.equals("55") || usercheck.equals("60") || usercheck.equals("61")){
	   ...
	   return;
			...
			String field139="";//
		    String sql="select b.field139 from hrmresource a,cus_fielddata b where a.id=b.id and a.loginid='"+names[1]+"' and b.scopeid=-1 and b.scope='HrmCustomFieldByInfoType'";
			rs.executeSql(sql);
			if(rs.next()){
				field139 = Util.null2String(rs.getString("field139"));
			}
			if("1".equals(field139)){
				 response.sendRedirect("/login/Login.jsp?logintype=1&formmethod=get&message=-110");
	   			 return;
			}
