...
function numberChangeToChinese(currencyDigits) {
	//Constants:
    ...
    if (isNaN(currencyDigits) || currencyDigits > Math.pow(10, 12)) {
        return "";
    }
...
function checkLengthbrow(elementname,spanname,len,fieldname,msg,msg1,demand)
{
	...
	}else if(len1 == 0 && str == '' && jQuery("#" + elementname + "spanimg").length > 0){
            if (demand==1)
            {
                jQuery("#" + elementname + "spanimg").html("<img src=\"/images/BacoError_wev8.gif\" align=absmiddle>");
            }
            else
            {
                jQuery("#" + elementname + "spanimg").html("");
            }
	}
}
            ...
            if(returnvalues!=""){
                ...
                for(n=0;n<fieldarray.length;n++){
                    ...
                    if(rownum<0 && jQuery("input#edesign_layout").length>0){
                    	//if(!triByLoad)		//页面加载也需要触发，兼容节点前附加操作给选择框赋值情况
                            ...
                            for(j=0;j<fieldidarray.length;j++){
                                ...
                                if (tfieldidarray[1]==isdetail){
                                    if(rownum>-1){  // 明细字段
										...
										try{
											jQuery('#fieldCodediv').css("display","");
										}catch(e){}
                            ...
                            for(j=0;j<fieldidarray.length;j++){
                                ...
                                if (tfieldidarray[1]==isdetail){
                                    if(rownum>-1){  //明细字段
										...
										try{
											jQuery('#fieldCodediv').css("display","none");
										}catch(e){}
                                        if(document.forms[0].elements[i].name=='field'+tfieldidarray[0]&&$GetEle('oldfieldview'+tfieldidarray[0])){
                                            isedit=$GetEle('oldfieldview'+tfieldidarray[0]).value;
                                            if(isedit>1&&$GetEle('field_'+tfieldidarray[0]+"span")){
...
function setBrowserReadOnly(fieldid,flag){
	...
	if(jQuery("#_areaselect_field"+filedid)){
        if(jQuery("#"+fieldid) && jQuery("#"+fieldid).val()!=''&&jQuery("#"+fieldid).val()!='0'){
			var divobj = jQuery("#_areaselect_field"+fieldid);
			if(jQuery("span[key='"+fieldid+"areawflabel']").length>0){jQuery("span[key='"+fieldid+"areawflabel']").remove();}
...
//联动改变显示属性联动生成的只读对象值
function syncUpdateReadOnlyText(vthis){
	var fieldInputId = jQuery(vthis).attr("name");
	var fieldReadOnlyObj = jQuery("span#"+fieldInputId+"_readonlytext");
	if(fieldReadOnlyObj.length > 0)
		fieldReadOnlyObj.text(jQuery(vthis).val());
}
