		  ...
		  <tr <%if(isLight){%> class=datalight <%} else {%> class=datadark  <%}%> onmouseover="showQuote('<%="quote"+i%>',true,this)" onmouseout="showQuote('<%="quote"+i%>',false,this)">
			...
			<table style="background:#fff"  width="100%" border="0" cellspacing="0" cellpadding="0" class="singleSignTbl" onmouseover="showquoteandtransto(this)" onmouseout="hidequoteandtransto(this)">
			  <tr style=" vertical-align: top;">
			    ...
			    <td width="160px" align="left" style="color:#099cff;border-bottom:1px solid #e7e7e7;word-wrap:break-all;word-break:break-all;">
			    	<div style="word-wrap:break-word;word-break:break-all;width:150px;">
			    	<!-- 人员姓名 START -->
					<span>
					<%
						String jobtitlenamesign="";
						String subcompanyname="";
					    String sql_sign="select b.jobtitlename from hrmresource a,hrmjobtitles b where a.jobtitle=b.id and a.id="+log_operator;
						rssign.executeSql(sql_sign);
						if(rssign.next()){
							jobtitlenamesign = Util.null2String(rssign.getString("jobtitlename"));
						}
						sql_sign="select b.subcompanyname from hrmresource a,hrmsubcompany b where a.subcompanyid1=b.id and a.id="+log_operator;
						rssign.executeSql(sql_sign);
						if(rssign.next()){
							subcompanyname = Util.null2String(rssign.getString("subcompanyname"));
						}
					...
					<%
							       ...
							       if (log_operatortype.equals("0")) {
										...
										if (!"".equals(displayid)){
										   ...
										   if("".equals(signfieldids)||signfieldids.contains("1")){
										     if (isinneruser) { %>
												<font  color="#AAAAAA"><%=jobtitlenamesign+" -"%></font><a id="user_<%=log_iframeId%>" href="javaScript:openhrm(<%=displayid %>);" onclick='pointerXY(event);' style='color:#099cff'>
													<%=displayname %>
												</a>
							...
							<!-- 操作人部门信息 -->
									<%if("".equals(signfieldids)||signfieldids.contains("3")){%>
										...
										%>
											<p><font  color="#AAAAAA"><%=subcompanyname+"/"%></font><a href="/hrm/company/HrmDepartmentDsp.jsp?id=<%=displaydepid %>" target="_Blank" class="department_href"><%=displaydepname %></a></p>
										<%
										} else {%>
											<%=subcompanyname+"/"%><%=displaydepname %>
										<%
										} %>
										<%=
										    operatorDepartInfo
										...
										%>
												...
												while(RecordSetlog3.next()){
												 ...
												 if(accessoryCount>1){
												  ...
												  if(accessoryCount==1 &&!isprint&&((!fileExtendName.equalsIgnoreCase("xls")&&!fileExtendName.equalsIgnoreCase("doc"))||!nodownload)){%>
												      ...
												      &nbsp;&nbsp;<BUTTON class="wffbtn" accessKey=1 style="color:#123885;border:0px;line-height:20px;font-size:12px;padding:3px;background: #fff"  onclick="addDocReadTag('<%=showid%>');top.location='/weaver/weaver.file.FileDownload?fileid=<%=docImagefileid%>&download=1&requestid=<%=requestid%>&desrequestid=<%=desrequestid%>'; return false;" type="button">
								          ...
								          <%
													...
													if (showimg == 1 && rssign.next() && true) {
														//System.out.println("showimg:"+showimg);
														// 获取签章图片并显示
