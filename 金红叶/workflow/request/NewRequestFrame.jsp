...
<jsp:useBean id="WfTextUtil" class="weaver.workflow.request.WfTextUtil" scope="page" />
<!--以下是显示定制组件所需的js -->
...
<%
	...
	String rqids = "";
	int count = 0;
	String sql2 = "select count(c.id) as count from workflow_requestbase a ,workflow_base b,workflow_billfield c where a.workflowid=b.id and b.formid=c.billid and a.requestid="+requestid+" and c.detailtable is null and Upper(c.fieldname)='RQIDS'";
	RecordSet.executeSql(sql2);
	if(RecordSet.next()){
		count = RecordSet.getInt("count");
	}
	if(count > 0){
		sql2 = "select c.tablename from workflow_requestbase a ,workflow_base b,workflow_bill c where a.workflowid=b.id and b.formid=c.id and a.requestid="+requestid;
		RecordSet.executeSql(sql2);
		if(RecordSet.next()){
			tablename = Util.null2String(RecordSet.getString("tablename"));
		}

		sql2 = "select rqids from "+tablename + " where requestid="+requestid;
		RecordSet.executeSql(sql2);
		if(RecordSet.next()){
			rqids = Util.null2String(RecordSet.getString("rqids"));
		}
		if(!"".equals(rqids)){
			cansee = "1";
		}
...
<DIV id=mainRequestFrame class=" x-panel-body x-panel-body-noheader" style="left:0px;top:0px;">
	      ...
	      <div class="e8_boxhead">
	         ...
	         <div>
	             <ul class="tab_menu">
			        ...
			        <%if(isworkflowdoc.equals("1") && WfTextUtil.isKeepTextBtn(workflowid+"")){ %>
					...
					<% if("1".equals(cansee)){%>
			        <li>
			            <a onclick="javascript:changeTabToWfsign('WfMoreSign',this);"  style="cursor:pointer;">
			             	<span id="test">相关审批意见</span>
			            </a>
			        </li>  
					<% }%>
	          	</ul>
	...
	<%} %>
	<% if("1".equals(cansee)){ %>
		<div id="divWfMoreSign" class='requestContentBody _synergyBox' style="height:80%;display:none">
			<IFRAME src='' id="WfMoreSignframe" name="WfMoreSignframe" style='width:100%;height:100%' BORDER=0 FRAMEBORDER=no NORESIZE=NORESIZE scrolling='auto'></IFRAME>
		</div>
	<% } %>
		<!-- 
...
<script>
	...
	function changeTabToWfsign(newTab, o) {
	var rqids = "<%=rqids%>";	
	...
	if (newTab == 'WfMoreSign') {
		try {
			if (jQuery("#WfMoreSignframe").attr("src") == "") {
				jQuery("#loading").css("display", "");
				jQuery("#WfMoreSignframe").attr("src", "/appdevjsp/HQ/WORKFLOW/workflowRemarkAll.jsp?rqids="+rqids);
				eventPush(jQuery("#WfMoreSignframe").get(0), 'load', loadcomplete);
			}
		} catch(e) {}
	}
