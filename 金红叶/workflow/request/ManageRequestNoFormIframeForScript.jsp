...
boolean isTextInForm = Util.null2String(request.getParameter("isTextInForm")).equals("true") ? true: false;;
...
var wfbtnobj ;
function doWfsubmitorsaveFun(funcName){
	eval(funcName+"(wfbtnobj,1)");
}

function onWfSaveODoc(funcName){
	<%
	if(isTextInForm){
	%>
	try{
		ODocIframe.onSaveODoc(funcName);
	}catch(e){
		alert("ODocIframe.onSaveODoc 调用失败！");
		//doWfsubmitorsaveFun(funcName);
	}
	<%}else{%>
	doWfsubmitorsaveFun(funcName);
	<%}%>
...
function doSubmitBack(obj,isfrom){
	
	if (typeof(isfrom) == "undefined") {
		wfbtnobj = obj;	
		onWfSaveODoc("doSubmitBack");
		return;
	}
...
function doSubmitNoBack(obj,isfrom){
	if (typeof(isfrom) == "undefined") {
		wfbtnobj = obj;	
		onWfSaveODoc("doSubmitNoBack");
		return;
	}
	if(<%=isCptwf %>){//自定义资产流程
		cptcusifover(function(){
...
function doRemark_nBack(obj,isfrom){
    <%
	if(IsCanModify){
	%>
	if (typeof(isfrom) == "undefined") {
		wfbtnobj = obj;	
		onWfSaveODoc("doRemark_nBack");
		return;
	}
	<%}%>
	$G("needwfback").value = "1";
...
function doRemark_nNoBack(obj,isfrom){
	<%
	if(IsCanModify){
	%>
	if (typeof(isfrom) == "undefined") {
		wfbtnobj = obj;	
		onWfSaveODoc("doRemark_nNoBack");
		return;
	}
	<%}%>
	$G("needwfback").value = "0";
...
function doAffirmanceBack(obj,isfrom){
	if (typeof(isfrom) == "undefined") {
		wfbtnobj = obj;	
		onWfSaveODoc("doAffirmanceBack");
		return;
	}
	<% if("1".equals(isSubmitDirectNode)) { %>
		if($G("SubmitToNodeid")) {
...
function doAffirmanceNoBack(obj,isfrom){
	if (typeof(isfrom) == "undefined") {
		wfbtnobj = obj;	
		onWfSaveODoc("doAffirmanceNoBack");
		return;
	}
	<% if("1".equals(isSubmitDirectNode)) { %>
		if($G("SubmitToNodeid")) {
...
function doSave_nNew(obj,isfrom){
	if (typeof(isfrom) == "undefined") {
		wfbtnobj = obj;	
		onWfSaveODoc("doSave_nNew");
		return;
	}
	getRemarkText_log();
	doSave_n(obj);
}
function doReject_New(obj,isfrom){
	if (typeof(isfrom) == "undefined") {
		wfbtnobj = obj;	
		onWfSaveODoc("doReject_New");
		return;
	}
	getRemarkText_log();
	doReject();
}
function doSubmit_Pre(obj,isfrom){
	if (typeof(isfrom) == "undefined") {
		wfbtnobj = obj;	
		onWfSaveODoc("doSubmit_Pre");
		return;
	}
...
function doJujue(){
	var reamrk = "";
	if(window._myCheckSignInput) {  // oops
		getRemarkText_log();
		if(!_myCheckSignInput()) {
			return false;
		}
	}
	reamrk = $G("remarkText10404").value;  // 强制必填时获取签字意见的内容
	var info = "<%=SystemEnv.getHtmlLabelName(382093,user.getLanguage())%>";
	top.Dialog.confirm(info,function(){
		enableAllmenu();
		jQuery.ajax({
			url : "/workflow/request/JujueOperation.jsp",
			type : "post",
			async : true,
			data:{
				requestid:"<%=requestid%>",
			...
			success: function do4Success(data){
				if(data=="1"){ 
					...
					window.top.Dialog.alert('<%=SystemEnv.getHtmlLabelNames("25659,498",user.getLanguage())%>');
					displayAllmenu();
				}
			}
		});
