...
userlist.add(Belongtoid + "");
		...
		boolean hasrequestlevel = true;//2017-10-30修改 原值false
									...
									<TABLE width="100%" cellspacing="0" cellpadding="0">
										<tr>
											<td valign="top" class="_xTableOuter"><%
													...
													// 处理已办排序 end
													//-------------------------------------------
													String backfields = " t1.requestid,t1.requestmark,t1.createdate, t1.createtime,t1.creater, t1.creatertype, t1.workflowid, t1.requestname, t1.requestnamenew, t1.status,t1.requestlevel,t1.currentnodeid,t2.viewtype,t2.userid,t2.receivedate,t2.receivetime,t2.isremark,t2.nodeid,t2.agentorbyagentid,t2.agenttype,t2.isprocessed " + operateDateTimeFieldSql;
													String fromSql = " from workflow_requestbase t1,workflow_currentoperator t2 ";//xxxxx
													String sqlWhere = sqlwhere;
													String para2 = "column:requestid+column:workflowid+column:viewtype+" + isovertime + "+" + user.getLanguage() + "+column:nodeid+column:isremark+" + user.getUID() + "+column:agentorbyagentid+column:agenttype+column:isprocessed+column:userid";
					...
					if(!userIDAll.equals(String.valueOf(user.getUID()))){
													...
													if (isMultiSubmit && iswaitdo == 1) {
														...
														tableString = " <table instanceid=\"workflowRequestListTable\" pageId=\""+pageId+"\"   tabletype=\"checkbox\" pagesize=\""
																...
																+ "\"  sqlprimarykey=\"t1.requestid\" sqlsortway=\"Desc\" sqlisdistinct=\"false\" />" + operateString + "			<head>";
														...
														tableString = " <table instanceid=\"workflowRequestListTable\" pageId=\""+pageId+"\"   tabletype=\"checkbox\" pagesize=\""
																...
																+ " <checkboxpopedom  id=\"checkbox\"  popedompara=\"column:workflowid+column:isremark+column:requestid+column:nodeid+column:userid\" showmethod=\"weaver.general.WorkFlowTransMethod.getWFSearchResultCheckBox\" />"
																+ "	   <sql backfields=\"" + backfields + "\" sqlform=\"" + fromSql + "\" sqlwhere=\"" + Util.toHtmlForSplitPage(sqlWhere) + "\"  sqlorderby=\"" + orderby
																+ "\"  sqlprimarykey=\"t1.requestid\" sqlsortway=\"Desc\" sqlisdistinct=\"true\" />" + operateString + "			<head>";
														...
														tableString += "<col width=\"10%\" display=\""+hasreceivetime +"\"  text=\""
																	...
																	+ "\" column=\"receivedate\" orderkey=\"t2.receivedate,t2.receivetime\" otherpara=\"column:receivetime\" transmethod=\"weaver.general.WorkFlowTransMethod.getWFSearchResultCreateTime\" />";
														...
														tableString = " <table instanceid=\"workflowRequestListTable\" tabletype=\"none\" pageId=\""+pageId+"\" cssHandler=\"com.weaver.cssRenderHandler.request.CheckboxColorRender\" pagesize=\"" + PageIdConst.getPageSize(PageIdConst.getWFPageId(urlType),user.getUID()) + "\" >"
																...
																+ "\" sqlwhere=\"" + Util.toHtmlForSplitPage(sqlWhere) + "\"  sqlorderby=\" t1.createdate, t1.createtime \"  sqlprimarykey=\"t1.requestid\" sqlsortway=\"Desc\" sqlisdistinct=\"false\" />"
																+ operateString
																+ "			<head>";
														...
														tableString = " <table instanceid=\"workflowRequestListTable\" tabletype=\"none\" pageId=\""+pageId+"\" cssHandler=\"com.weaver.cssRenderHandler.request.CheckboxColorRender\" pagesize=\"" + PageIdConst.getPageSize(PageIdConst.getWFPageId(urlType),user.getUID()) + "\" >"
																+ "	   <sql backfields=\"" + backfields + "\" sqlform=\"" + fromSql
																+ "\" sqlwhere=\"" + Util.toHtmlForSplitPage(sqlWhere) + "\"  sqlorderby=\" t1.createdate, t1.createtime \"  sqlprimarykey=\"t1.requestid\" sqlsortway=\"Desc\" sqlisdistinct=\"true\" />"
																+ operateString
																+ "			<head>";
																	tableString += "<col width=\"19%\" display=\""+hasrequestname+"\" text=\""
																	+ SystemEnv.getHtmlLabelName(1334, user.getLanguage())
														...
														tableString = " <table instanceid=\"workflowRequestListTable\" tabletype=\"none\" pageId=\""+pageId+"\" cssHandler=\"com.weaver.cssRenderHandler.request.CheckboxColorRender\" pagesize=\"" + PageIdConst.getPageSize(PageIdConst.getWFPageId(urlType),user.getUID()) + "\" >"
																+ "	   <sql backfields=\"" + backfields + "\" sqlform=\"" + fromSql
																+ "\" sqlwhere=\"" + Util.toHtmlForSplitPage(sqlWhere) + "\"  sqlorderby=\"" + orderby + "\"  sqlprimarykey=\"t1.requestid\" sqlsortway=\"Desc\" sqlisdistinct=\"false\" />"
																+ operateString
																+ "			<head>";
														...
														tableString += "<col width=\"19%\" display=\""+hasrequestname+"\" text=\""
																...
																tableString = " <table instanceid=\"workflowRequestListTable\" tabletype=\"none\" pageId=\""+pageId+"\" cssHandler=\"com.weaver.cssRenderHandler.request.CheckboxColorRender\" pagesize=\"" + PageIdConst.getPageSize(PageIdConst.getWFPageId(urlType),user.getUID()) + "\" >"
																+ "	   <sql backfields=\"" + backfields + "\" sqlform=\"" + fromSql
																+ "\" sqlwhere=\"" + Util.toHtmlForSplitPage(sqlWhere) + "\"  sqlorderby=\"" + orderby + "\"  sqlprimarykey=\"t1.requestid\" sqlsortway=\"Desc\" sqlisdistinct=\"true\" />"
																+ operateString
																+ "			<head>";
...
//System.out.println("-1421--belongtoshow-"+belongtoshow);
		...
		function OnMultiSubmitNew(obj){
		    ...
		    if(_reqIds!=""){
				...
				//先检查批量提交的流程中是否有费控流程（E8后台设置的费控流程，系统表单），并做相应提交校验
				jQuery.ajax({
					url : "/fna/budget/FnaBatchSubmitControl.jsp",
					type : "post",
					processData : false,
					data : "_reqIds="+_reqIds,
					dataType : "json",
					success: function do4Success(fnainfo){
						try{
							parent.jQuery("#submitloaddingdiv").hide();
							parent.jQuery("#submitloaddingdiv_out").hide();
						}catch(ex1){}
						if(fnainfo.flag){
		...
		function getFlowWindowUrl(){
			return "/systeminfo/BrowserMain.jsp?url=/workflow/workflow/WorkflowTypeBrowser.jsp?typeid=" + $G("workflowtype").value;
		}
		</script>
