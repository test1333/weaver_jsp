...
<wea:layout attributes="{'expandAllGroup':'true'}">
	<div id="TimeDate">
		<wea:item attributes="{'isTableList':'true'}">
		    <wea:layout type="table" attributes="{'cols':'13','cws':'120px,200px,200px,100px,100px,100px,100px,100px,100px,100px,120px,100px,100px'}">
				<wea:group context="" attributes="{'groupDisplay':'none'}">
				<wea:item type="thead">項目狀態</wea:item>
				
			    <wea:item type="thead">客戶</wea:item>
			    <wea:item type="thead">設備名稱</wea:item>
				<wea:item type="thead">實際單價</wea:item>
			    <wea:item type="thead">實際台數</wea:item>
				<wea:item type="thead">實際總價</wea:item>
				<wea:item type="thead">實際接單月份</wea:item>
				<wea:item type="thead">預計銷貨月份</wea:item>
				<wea:item type="thead">預計銷貨金額</wea:item>
				<wea:item type="thead">預計驗收月份</wea:item>
			    <wea:item type="thead">預計結案驗收金額</wea:item>
		...
		<% String sql="select "+backfields +" from "+fromSql+" "+sqlWhere+" order by "+orderby;
		 RecordSet rs1 = new RecordSet();
		   ...
		   String prjstatus="";
		   String persalemoney="";
		   String peracceptancemoney="";
		   
		  while(rs1.next()){
	        takeordermonth = Util.null2String(rs1.getString("takeordermonth"));
	        cusumer = Util.null2String(rs1.getString("cusumer"));
	        quity = Util.null2String(rs1.getString("realquity"));
	        singleprice = Util.null2String(rs1.getString("realsingleprice"));
			 itembuget = Util.null2String(rs1.getString("realitembuget"));
			...
			persalemoney = Util.null2String(rs1.getString("persalemoney"));
			peracceptancemoney = Util.null2String(rs1.getString("peracceptancemoney"));
			  ...
			  %>
			   <wea:item><%=prjstatus%></wea:item>
				 ...
				 <wea:item><div style="text-align:right;"><%=singleprice%></div></wea:item>
			     <wea:item><%=quity%></wea:item>			     
			     <wea:item><div style="text-align:right;"><%=itembuget%></div></wea:item>
				  <wea:item><%=takeordermonth%></wea:item>				  
				  <wea:item><%=persalemonth%></wea:item>
				  <wea:item><div style="text-align:right;"><%=persalemoney%></div></wea:item>
			     <wea:item><%=peracceptancemonth%></wea:item>
				  <wea:item><div style="text-align:right;"><%=peracceptancemoney%></div></wea:item>
			     <wea:item><%=pcentchange%></wea:item>
			     <wea:item><div style="text-align:right;"><%=pertotalmoney%></div></wea:item>
