...
String wjbh="";
String sql11=" select wjbh from cus_fielddata where scope='DocCustomFieldBySecCategory' and id="+docid
rs.executeSql(sql11);
if(rs.next()){
	wjbh = Util.null2String(rs.getString("wjbh"));	
}
if(!"".equals(wjbh)){
	sql111="update docdetail set doccode='"+wjbh+"' where id="+docid;
	rs.executeSql(sql11);
}
//文档弹出窗口次数结束
...
if(12==docType&&imagefileId<=0){
	rs.executeSql("select imagefileid from docimagefile where docid="+docid+" and (isextfile is null or isextfile<>'1') ");
	if(rs.next()){
...
if(isPDF&&isUseiWebPDF==1){
	...
	if(imagefileId==0){
		if(openFirstAss){
			...
			response.sendRedirect("/weaver/weaver.file.FileDownload?fileid="+imagefileId+"&coworkid="+coworkid+"&requestid="+requestid+"&desrequestid="+desrequestid+"&votingId="+votingId+"&workplanid="+workplanid+formmodeparamsStr);
			return ;
		} else {
			isPDF = false;
		}
...
if(isPDF_First){
       ...
       if(fileCount==1){
           rs.executeSql("select imagefileid from DocImageFile where isextfile = '1' and docid = "+docid+" and docfiletype <> '1'");
           if(rs.next()){
...
if(openFirstAss){
	...
	if(redUrl!="") {
		...
		response.sendRedirect(redUrl+"&votingId="+votingId);
		return;
	}
}
					...
					else
					{
						var index = parseInt(jQuery("#praiseCount").text());
		  				...
		  				if(index > 12)
		  				{
							...
							jQuery("#fuserFont").text(index-12);
		  				}
					}
				...
				jQuery("#praiseInfo").unbind("click");
					...
					if(index - 1 > 0)
		  			...
		  			if(index-1 > 12)
		  			{
							...
							jQuery("#fuserFont").text(index-13);
		  			}
					...
					<% } else { %>
					 ...
					 <% } %>
				  	...
				  	<% } if(userInfos.size() > 13 ) {%>
				  		...
				  		<span style="padding-left: 10px;"><%=SystemEnv.getHtmlLabelName(126387,user.getLanguage())%><font id="fuserFont"><%= userInfos.size()-13 %></font><%=SystemEnv.getHtmlLabelName(126388,user.getLanguage())%></span><div onclick="moreUserView();" class="moreUserView"></div>
				  	<% }%>
