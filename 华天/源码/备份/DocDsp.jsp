...
if(isPDF&&isUseiWebPDF==1&&"true".equalsIgnoreCase(isIE)){
	...
	if(imagefileId==0){
		if(!"sub".equals(pagestatus)&&openFirstAss){
			...
			if(!isIE8){
				response.sendRedirect("DocDspExt.jsp?imagefileId="+imagefileId+"&versionId="+versionId_fromAcc+"&isFromAccessory="+isFromAccessory+"&openFirstAss="+openFirstAss+"&fromFlowDoc="+fromFlowDoc+"&from="+from+"&userCategory="+userCategory+"&id="+docid+"&olddocid="+olddocid+"&isrequest="+isrequest+"&requestid="+requestid+"&votingId="+votingId+"&desrequestid="+desrequestid+"&blnOsp="+blnOsp+"&topage="+URLEncoder.encode(topageFromOther)+"&meetingid="+meetingid+"&f_weaver_belongto_userid="+f_weaver_belongto_userid+"&f_weaver_belongto_usertype="+f_weaver_belongto_usertype+"&workplanid="+workplanid+formmodeparamsStr);
			return ;
			}
			else
			{
		...
		if(!isIE8){
				response.sendRedirect("DocDspExt.jsp?imagefileId="+imagefileId+"&versionId="+versionId_fromAcc+"&isFromAccessory="+isFromAccessory+"&fromFlowDoc="+fromFlowDoc+"&from="+from+"&userCategory="+userCategory+"&id="+docid+"&olddocid="+olddocid+"&isrequest="+isrequest+"&requestid="+requestid+"&votingId="+votingId+"&desrequestid="+desrequestid+"&blnOsp="+blnOsp+"&topage="+URLEncoder.encode(topageFromOther)+"&meetingid="+meetingid+"&f_weaver_belongto_userid="+f_weaver_belongto_userid+"&f_weaver_belongto_usertype="+f_weaver_belongto_usertype+"&workplanid="+workplanid+formmodeparamsStr);
		return ;
		}
		else
		{
...
if(openFirstAss){
	...
	if(redUrl!="") {
		...
		if("1".equals(isAppendTypeField)){
			response.sendRedirect(redUrl+"&checkOutMessage="+URLEncoder.encode(URLEncoder.encode(checkOutMessage,"UTF-8"),"UTF-8")+"&votingId="+votingId+"&hasviewlog="+hasviewlog);
		}else{
			response.sendRedirect(redUrl+"&checkOutMessage="+URLEncoder.encode(URLEncoder.encode(checkOutMessage,"UTF-8"),"UTF-8")+"&openFirstAss=true&votingId="+votingId+"&hasviewlog="+hasviewlog);
		}
		return;
	}
}
