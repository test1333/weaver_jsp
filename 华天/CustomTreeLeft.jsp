...
<BODY>
	...
	<div style="width:100%;height:100%;overflow-y: hidden;">
					...
					<td style="display: table-cell;border-right:0px;height:0px;" class="leftTypeSearch">
						<div style="border-bottom-color: currentColor; border-bottom-width: medium; border-bottom-style: none;" class="topMenuTitle">
							<span class="leftType" onclick="showE8TypeOption();">
							    ...
							    <span>
									<div style="width: auto; height: auto; position: relative;" id="e8typeDiv">
										<span id="optionSpan"><%=treename %></span>								
									</div>
								</span>								
							</span>
		...
		<ul id="CustomTree" class="ztree" style="padding-bottom: 0px;"></ul>
	</div>
	...
	function setTreeField(treeNode) {
		var url = "/formmode/tree/CustomTreeHrefAjax.jsp?mainid=<%=id%>&pid="+treeNode.id+"&isRefreshTree=<%=isRefreshTree%>";
		jQuery.ajax({
			url : url,
			type : "post",
			processData : false,
...
function refreshTree(id,lastnodeid){
   ...
   if(search != ""){
    jQuery.ajax({
				...
				success:function(data){
				   zNodes = data;
			       $("#overFlowDivTree").after("<ul id='CustomTree' class='ztree' style='padding-bottom: 0px;'></ul>");
