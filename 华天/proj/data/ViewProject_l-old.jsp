...
if(!"".equals(ProjID)){
int count_other=0;
String sql_other="select count(1) as count from uf_project_other where xmid="+ProjID;
rs.executeSql(sql_other);
if(rs.next()){
count_other = rs.getInt("count");
}
String MONEY_other="";
String PROCESS_other="";
String PROCESSID_other="";
String PROER1_other="";
String PROTYPE_other="";
String XMZZ_other="";
String XSY_other="";
String CPLX_other="";
String SCYSYCS_other="";
String SCYSYPS_other="";
String KHMC_other="";
String KHDM_other="";
String KHLXR_other="";
String KHLXRDH_other="";
String KHLXRYX_other="";
sql_other="select MONEY,PROCESS,PROCESSID,PROER1,PROTYPE,XMZZ,XSY,CPLX,SCYSYCS,SCYSYPS,KHMC,KHDM,KHLXR,KHLXRDH,KHLXRYX from cus_fielddata where scope='ProjCustomFieldReal' and id="+ProjID;
rs.executeSql(sql_other);
if(rs.next()){
	MONEY_other=Util.null2String(rs.getString("MONEY"));
	PROCESS_other=Util.null2String(rs.getString("PROCESS"));
	PROCESSID_other=Util.null2String(rs.getString("PROCESSID"));
	PROER1_other=Util.null2String(rs.getString("PROER1"));
	PROTYPE_other=Util.null2String(rs.getString("PROTYPE"));
	XMZZ_other=Util.null2String(rs.getString("XMZZ"));
	XSY_other=Util.null2String(rs.getString("XSY"));
	CPLX_other=Util.null2String(rs.getString("CPLX"));
	SCYSYCS_other=Util.null2String(rs.getString("SCYSYCS"));
	SCYSYPS_other=Util.null2String(rs.getString("SCYSYPS"));
	KHMC_other=Util.null2String(rs.getString("KHMC"));
	KHDM_other=Util.null2String(rs.getString("KHDM"));
	KHLXR_other=Util.null2String(rs.getString("MONEY"));
	KHLXRDH_other=Util.null2String(rs.getString("KHLXR"));
	KHLXRYX_other=Util.null2String(rs.getString("KHLXRYX"));
}
if("".equals(KHMC_other)&&count_other>0){
	sql_other="update cus_fielddata a set (MONEY,PROCESS,PROCESSID,PROER1,PROTYPE,XMZZ,XSY,CPLX,SCYSYCS,SCYSYPS,KHMC,KHDM,KHLXR,KHLXRDH,KHLXRYX)=(select MONEY,PROCESS,PROCESSID,PROER1,PROTYPE,XMZZ,XSY,CPLX,SCYSYCS,SCYSYPS,KHMC,KHDM,KHLXR,KHLXRDH,KHLXRYX from uf_project_other b "+
	" where a.id=b.xmid) where a.id="+ProjID+" and a.scope='ProjCustomFieldReal'";
	rs.executeSql(sql_other);
}
if(!"".equals(KHMC_other)&&count_other<=0){
	sql_other="insert into uf_project_other(xmid,MONEY,PROCESS,PROCESSID,PROER1,PROTYPE,XMZZ,XSY,CPLX,SCYSYCS,SCYSYPS,KHMC,KHDM,KHLXR,KHLXRDH,KHLXRYX) "+
	" values('"+ProjID+"','"+MONEY_other+"','"+PROCESS_other+"','"+PROCESSID_other+"','"+PROER1_other+"','"+PROTYPE_other+"','"+XMZZ_other+"','"+XSY_other+"','"+CPLX_other+"', "+
	" '"+SCYSYCS_other+"','"+SCYSYPS_other+"','"+KHMC_other+"','"+KHDM_other+"','"+KHLXR_other+"','"+KHLXRDH_other+"','"+KHLXRYX_other+"')";
	rs.executeSql(sql_other);
}
if(!"".equals(KHMC_other)&&count_other>0){
	sql_other="update uf_project_other a set (MONEY,PROCESS,PROCESSID,PROER1,PROTYPE,XMZZ,XSY,CPLX,SCYSYCS,SCYSYPS,KHMC,KHDM,KHLXR,KHLXRDH,KHLXRYX)=(select MONEY,PROCESS,PROCESSID,PROER1,PROTYPE,XMZZ,XSY,CPLX,SCYSYCS,SCYSYPS,KHMC,KHDM,KHLXR,KHLXRDH,KHLXRYX from cus_fielddata b where a.xmid=b.id "+
	" and scope='ProjCustomFieldReal'  ) where a.xmid="+ProjID;
	rs.executeSql(sql_other);
}
}
/*查看项目成员*/
...
if(!openfieldMap.isEmpty()){
	...
	while(it.hasNext()){
		...
		if("2".equals(fieldkind)){
			fieldValue=CptFieldManager.getFieldvalue(user, Util.getIntValue( fieldid.replace("prjtype_", "") ), v.getInt("fieldhtmltype"), v.getInt("type"), Util.null2String( RecordSet_cus.getString(fieldName)) , 2,true);
		}else{
	...
	if("description".equals(fieldName)){//相关客户后面显示相关合同
		...
		<%
	}else if(fieldcount==fieldsize && groupcount==2){//第2个组最后显示项目进度
		if(isExecProject){
			%>
	...
	}else if(fieldcount==fieldsize && groupcount==1 && PrjSettingsComInfo.getPrj_acc() ){//附件位置
