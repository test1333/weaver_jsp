...
String proType = "标准项目";
String sql = "select * from cus_fielddata cf left join Crm_Customerinfo cc on khmc=cc.id left join crm_customercontacter t3"
+" on khlxr=to_char(t3.id) where scope='ProjCustomFieldReal' and cf.id="+projectID;
rs.executeSql(sql);
if(rs.next()){
	money = Util.null2String(rs.getString("money"));
	process = Util.null2String(rs.getString("process"));
	proer = Util.null2String(rs.getString("proer1"));
	String tmp_proType = Util.null2String(rs.getString("proType"));
	
	xmzz = Util.null2String(rs.getString("xmzz"));
	xsy = Util.null2String(rs.getString("xsy"));
	cplx = Util.null2String(rs.getString("cplx"));
	scysycs = Util.null2String(rs.getString("scysycs"));
	scysyps = Util.null2String(rs.getString("scysyps"));
	
	khmc = Util.null2String(rs.getString("khmc"));
	khdm = Util.null2String(rs.getString("khdm"));
	khlxr = Util.null2String(rs.getString("firstname"));
	cus_name = Util.null2String(rs.getString("name"));
	khlxrdh = Util.null2String(rs.getString("khlxrdh"));
	khlxryx = Util.null2String(rs.getString("khlxryx"));
	
	
	if("1".equals(tmp_proType)){
		proType = "非标项目";
	}
}
