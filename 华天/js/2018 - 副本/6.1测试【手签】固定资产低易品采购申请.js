...
jQuery(document).ready(function () {
    jQuery('#' + t0.bbzj).attr('readonly', 'readonly')
    jQuery('#' + t0.wbzj).attr('readonly', 'readonly')
    jQuery('#' + t0.bbdj01).attr('readonly', 'readonly')
    jQuery('#' + t0.bbdj02).attr('readonly', 'readonly')
    jQuery('#' + t0.bbdj03).attr('readonly', 'readonly')
    jQuery('#' + t0.bbdj04).attr('readonly', 'readonly')
    jQuery('#' + t0.bbdj05).attr('readonly', 'readonly')

    checkCustomize = function () {
        var result = true
        //_C.v(t0.bbzj, sumTotal('1'))
        //_C.v(t0.wbzj, sumTotal('2'))
        if (_C.v(t0.bbzj) == '0.0000'&&_C.v(t0.wbzj) == '0.0000') {
            Dialog.alert('请先填写物品单价')
            result = false
        }
	...
	jQuery("#"+t0.bbdj01).bindPropertyChange(function(){
		getSummoney();
	})
	jQuery("#"+t0.bbdj02).bindPropertyChange(function(){
		getSummoney();
	})
	jQuery("#"+t0.bbdj03).bindPropertyChange(function(){
		getSummoney();
	})
	jQuery("#"+t0.bbdj04).bindPropertyChange(function(){
		getSummoney();
	})
	jQuery("#"+t0.bbdj05).bindPropertyChange(function(){
		getSummoney();
	})
...
function getSummoney(){
	...
	var bbdj01_val = jQuery("#"+t0.bbdj01).val();
	var bbdj02_val = jQuery("#"+t0.bbdj02).val();
	var bbdj03_val = jQuery("#"+t0.bbdj03).val();
	var bbdj04_val = jQuery("#"+t0.bbdj04).val();
	var bbdj05_val = jQuery("#"+t0.bbdj05).val();
	if(sl1_val != "" && hl01_val != "" && bbdj01_val != ""){
		if(hl01_val == "4"){
			sumbb = accAdd(sumbb,accMul(sl1_val,bbdj01_val));
		}else{
			sumwb = accAdd(sumwb,accMul(sl1_val,bbdj01_val));
		}
	}
	if(sl2_val != "" && hl02_val != "" && bbdj02_val != ""){
		if(hl02_val == "4"){
			sumbb = accAdd(sumbb,accMul(sl2_val,bbdj02_val));
		}else{
			sumwb = accAdd(sumwb,accMul(sl2_val,bbdj02_val));
		}
	}
	if(sl3_val != "" && hl03_val != "" && bbdj03_val != ""){
		if(hl03_val == "4"){
			sumbb = accAdd(sumbb,accMul(sl3_val,bbdj03_val));
		}else{
			sumwb = accAdd(sumwb,accMul(sl3_val,bbdj03_val));
		}
	}
	if(sl4_val != "" && hl04_val != "" && bbdj04_val != ""){
		if(hl04_val == "4"){
			sumbb = accAdd(sumbb,accMul(sl4_val,bbdj04_val));
		}else{
			sumwb = accAdd(sumwb,accMul(sl4_val,bbdj04_val));
		}
	}
	if(sl5_val != "" && hl05_val != "" && bbdj05_val != ""){
		if(hl05_val == "4"){
			sumbb = accAdd(sumbb,accMul(sl5_val,bbdj05_val));
		}else{
			sumwb = accAdd(sumwb,accMul(sl5_val,bbdj05_val));
		}
	}
	jQuery("#"+ t0.bbzj).val(sumbb);
	jQuery("#"+ t0.wbzj).val(sumwb);
