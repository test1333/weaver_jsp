...
jQuery(document).ready(function () {
    ...
    jQuery('#xmmc').hide()
    jQuery(yfxmmc).hide()
    // var sqbm_val = jQuery(sqbm).val()
    jQuery(superDept).bindPropertyChange(function () {
        var superDeptVal = jQuery(superDept).val()
       // console.log('superDeptVal='+superDeptVal)
        // 如果是封装技术研究院，技术部，LED事业部的固定资产申请，需要在右上角显示研发项目名称
        if (superDeptVal === '642' || superDeptVal === '651' || superDeptVal === '656' ) {
            jQuery('#xmmc').show()
            jQuery(yfxmmc).show()
            document.all('needcheck').value += ',' + _yfxmmc
            document.getElementById(_yfxmmc + 'span').innerHTML = "<img src='/images/BacoError_wev8.gif' align='absMiddle' </img>"
        } else {
            jQuery('#xmmc').hide()
            jQuery(yfxmmc).hide()
        }
...
jQuery(document).ready(function () {
    ...
    function sumTotal (flag) {
        ...
        for (var i = 0; i < typeArr.length; i++) {
            if (_C.v(typeArr[i]) !== '') {
                if (flag === '1') {
                    var tmpSum = accMul(_C.v(amountArr[i]), _C.v(priceArr1[i]))
                    sumArr.push(Number(tmpSum))
                } else if (flag === '2') {
                    var tmpSum = accMul(_C.v(amountArr[i]), _C.v(priceArr2[i]))
                    sumArr.push(Number(tmpSum))
                }
            }
        }

        return sumArray(sumArr)
    }
