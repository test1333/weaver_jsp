...
public class getItTimeofS implements Action{
	...
	public String execute(RequestInfo info) {
		...
		//	rs.executeSql(sql);
			...
			if (!" ".equals(tableName)) {
				...
				sql="select  enddate  from "+tableName+"_dt1 where content=16 and mainid= "+id;
				rs.executeSql(sql);
				String enddate="";
				if(rs.next()){
				...
				if(!"".equals(enddate)){												
						...
						sql="update "+tableName+"_dt1 set startdate = '"+starttime+"',enddate = '"+end+"' where content=17 and mainid= "+id;
						log.writeLog("updatesql1 ---------" + sql);
						rs.execute(sql); 
