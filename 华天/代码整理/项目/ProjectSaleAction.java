...
public class ProjectSaleAction implements Action {

	public String execute(RequestInfo info) {
		BaseBean log = new BaseBean();
		
		log.writeLog("ProjectSaleAction____Start");
	...
	//	rs.executeSql(sql);
		...
		if (!" ".equals(tableName)) {
			...
			sql = "select * from "+tableName+"_dt1 where mainid in(Select id from "+tableName
						+" where  requestId="+requestid+") and fromWhat=1";
			log.writeLog("sql ---------" + sql);
			rs.executeSql(sql);
			String flag = "";
			while(rs.next()){			
			...
			sql = "update "+tableName+" set salem='"+emp_2 +"' where requestId="+requestid;
			log.writeLog("sql ---------" + sql);
			rs.executeSql(sql);
		}
		return SUCCESS;
