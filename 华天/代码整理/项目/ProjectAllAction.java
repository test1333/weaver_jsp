...
public class ProjectAllAction implements Action {

	public String execute(RequestInfo info) {
		BaseBean log = new BaseBean();
		
		log.writeLog("ProjectAllAction____Start");
	...
	//	rs.executeSql(sql);
		...
		if (!" ".equals(tableName)) {
			...
			sql = "update "+tableName+" set allm='"+emp_2 +"' where requestId="+requestid;
			log.writeLog("sql ---------" + sql);
			rs.executeSql(sql);
		}
		return SUCCESS;
