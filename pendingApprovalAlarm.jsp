...
<HTML>
	...
	String approvestatus = Util.null2String(request.getParameter("approvestatus"));
	...
	<BODY>
		...
		<FORM id=report name=report action="/CRM/report/getProjectReportAll.jsp" method=post>
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;">
				...
				<wea:item>
				 <select class="e8_btn_top middle" name="approvestatus" id="approvestatus"> 
					 <option value="" <%if("".equals(approvestatus)){%> selected<%} %>>
					<%=""%></option>
				<option value="未审批" <%if("未审批".equals(approvestatus)){%> selected<%} %>>
					<%="未审批"%></option>
				<option value="已审批" <%if("已审批".equals(approvestatus)){%> selected<%} %>>
					<%="已审批"%></option>
				 </select>
				</wea:item>
