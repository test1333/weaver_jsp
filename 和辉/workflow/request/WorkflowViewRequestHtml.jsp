...
int pagesize = 10; // 每页打印的明细行数
int startIndex = 0; // 起始数据索引，0代表第一条
...
String isprintpage = "";
String printnum = "";
rs_html.executeSql("select isFormSignature,pdfprint,isprintpage,printnum from workflow_flownode where workflowId="+workflowid+" and nodeId="+nodeid);
if(rs_html.next()){
	isFormSignature = Util.null2String(rs_html.getString("isFormSignature"));
	pdfprint = Util.null2String(rs_html.getString("pdfprint"));
	isprintpage = Util.null2String(rs_html.getString("isprintpage"));
	printnum = Util.null2String(rs_html.getString("printnum"));
}
...
if(isprint == 1){
    if(isprintpage.equals("1")) { // 如果设置了流程分页，明细表分页显示
        pagesize = Util.getIntValue(printnum);
        wfLayoutToHtml.setPageSize(pagesize);
        wfLayoutToHtml.setIsSplitPrint(true);
        wfLayoutToHtml.setStartIndex(startIndex); // 设置起始数据索引
    }
...
if(isprintpage.equals("1")) { // 如果设置了流程分页，明细表分页显示
    wfformhtml = ""+wfformhtml;
        int totalCount = otherPara_hs.get("totalCount") == null ? 0 : ((Integer) otherPara_hs.get("totalCount"));
        int size = otherPara_hs.get("size") == null ? 0 : ((Integer) otherPara_hs.get("size")); // 总页数
        for(int i = 1; i < size; i++) {
            startIndex += pagesize;
            wfLayoutToHtml.setStartIndex(startIndex); // 设置起始数据索引
            wfLayoutToHtml.setPageNum(i + 1); // 设置当前页码
            ret_hs = wfLayoutToHtml.analyzeLayout();
            otherPara_hs = wfLayoutToHtml.getOtherPara_hs();
            
            if(!Util.delHtml(Util.null2String((String)ret_hs.get("wfformhtml"))).equals("")){
               wfformhtml += "<div style=\"page-break-after:always;\" ></div><br><br>  " + Util.null2String((String)ret_hs.get("wfformhtml"));
            }
