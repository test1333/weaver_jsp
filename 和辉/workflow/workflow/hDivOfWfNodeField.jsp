...
if(rs.next()){
	...
	isprintpage=Util.getIntValue(Util.null2String(rs.getString("isprintpage")),0);
	printnum=Util.null2String(rs.getString("printnum"));
...
<wea:layout type="2col">
	<wea:group context='<%=SystemEnv.getHtmlLabelName(16367,user.getLanguage())%>' attributes="<%=hdiv %>">
		...
		<wea:item><%=SystemEnv.getHtmlLabelName(23825,user.getLanguage())+SystemEnv.getHtmlLabelName(257,user.getLanguage())  %></wea:item> 
        <wea:item>
       <div class="outerdiv">
            <input type="checkbox" tzCheckbox="true" name="isprintpage" id="isprintpage" value="1" <%if(isprintpage==1){%>checked<%}%>> 
            <span id="printpage"  style="margin-left:150px;vertical-align:middle;float:right;">
                <span style="vertical-align:middle;margin-right :25px;"><%=SystemEnv.getHtmlLabelName(384351,user.getLanguage()) %>:
                &nbsp;&nbsp;<input type='text' id='printnum' name='printnum' value='<%=printnum %>' style='width:50px'></span>
	            <span style="vertical-align:middle;"><%=SystemEnv.getHtmlLabelName(125555,user.getLanguage())%>&nbsp;&nbsp;
	            <span style="display:inline-block;vertical-align:middle;margin-right :96px;">         
	               <brow:browser name="syncPAGNumodes" viewType="0" hasBrowser="true" hasAdd="false" 
                        getBrowserUrlFn="getShowNodesUrl" getBrowserUrlFnParams="'syncPAGNumodes'" isMustInput="1" isSingle="false" hasInput="true"
                        completeUrl='<%=completeUrl2 %>'  width="150px" browserValue="" browserSpanValue="" />
	             </span>
	            </span>
             </span>
