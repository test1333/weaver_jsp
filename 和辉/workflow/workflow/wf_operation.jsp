...
//		          String updateDeptFieldSQL="update workflow_groupdetail set deptField ='"+deptField+"',subcompanyField='"+subcompanyField+"' where id="+maxID;
   ...
   if(isbill.equals("0")){
  	...
  	int nodeid=Util.getIntValue(Util.null2String(request.getParameter("nodeid")),0);
  	int formid=Util.getIntValue(Util.null2String(request.getParameter("formid")),0);
    String syncPAGNumodes = Util.null2String(request.getParameter("syncPAGNumodes"));
  	String isbill = Util.null2String(request.getParameter("isbill"));
    String modetype=Util.null2String(request.getParameter("modetype"));
    String _modename = Util.null2String(request.getParameter("modename"));
	int isprintpage=Util.getIntValue(Util.null2String(request.getParameter("isprintpage")),0);
    String printnum = Util.null2String(request.getParameter("printnum"));
	//======修改设置参数========
    RecordSet.executeSql("update workflow_flownode set isprintpage='"+isprintpage+"' ,printnum='"+printnum+"'  where workflowid="+wfid+" and nodeid="+nodeid);
    if(!syncPAGNumodes.equals("")){
        RecordSet.executeSql("update workflow_flownode set isprintpage='"+isprintpage+"' ,printnum='"+printnum+"'  where workflowid="+wfid+" and nodeid in ("+syncPAGNumodes+")"); 
    }
    String _sql="";
