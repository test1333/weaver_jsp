...
<%
    ...
    for (String id : ids) {
        if (!"0".equals(id)) {
            ...
            String payment = "";          
            String zjlbm = "";
            sql = " select dtId,contactReq,fkje,zjlbm from gb_noneFrameworkContactFlow where id= " + id;
            rs.executeSql(sql);
            if (rs.next()) {
            ...
            String poAmount = "";
            sql = " select * from gb_bidPurchaseView where id= " + mainId;
            rs.executeSql(sql);
            if (rs.next()) {
                ...
                cgdl = Util.null2String(rs.getString("buydl"));
                fykm = Util.null2String(rs.getString("fykm"));
            ...
            flowMap.put("HJYBJE", payment); // 付款金额
            flowMap.put("zjlbm", zjlbm); // 总经理部门
            flowMap.put("cgdl", cgdl); // 采购大类
            flowMap.put("fykm", fykm); // 费用科目
            flowMap.put("pinpai", pinpai); // 费用科目
