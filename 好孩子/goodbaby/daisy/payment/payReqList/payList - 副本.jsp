...
<%
    ...
    String late_pageId = "payList00001";
    String bzspan = "";
    String sql = "";
    if(!"".equals(bz)){
    ...
    <div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;">
        <wea:layout type="4col">
            <wea:group context="查询条件">
                ...
                <wea:item>
                    <input type="text" name="company" style='width:80%' value="<%=company%>">
                </wea:item>
...
<%
    String backfields = " * ";
    //表单中的字段
    String fromSql = " from gb_payReqView2 ";//sql查询的表
    String sqlWhere = " fkqx<= '" + fkqx + "' and bz = '" + bz + "'";//where条件
    if (!"".equals(company)) {
        sqlWhere += " and gs like '%" + company + "%'";
    }
//    out.println("select " + backfields + fromSql + "where" + sqlWhere);
    ...
    tableString = " <table tabletype=\"none\" pagesize=\"" + PageIdConst.getPageSize(late_pageId, user.getUID(), PageIdConst.HRM) + "\" pageId=\"" + late_pageId + "\">" +
            ...
            "	<head>";
    tableString += "   <col width=\"7%\" text=\"公司\" column=\"gs\" orderkey=\"gs\" />" +
...
//            "   href= \"/goodbaby/daisy/payment/payReqList/paydtList.jsp?fkqx=" + fkqx + "&amp;bz=" + bz + "\" target=\"view_window\"/>" +
            "   <col width=\"9%\" text=\"收款银行\" column=\"khh\" orderkey=\"khh\" />" +
            "   <col width=\"9%\" text=\"收款银行账号\" column=\"yhzh\" orderkey=\"yhzh\" />" +
            "   <col width=\"9%\" text=\"收款行联行号\" column=\"lhh\" orderkey=\"lhh\" />" +
            "   <col width=\"9%\" text=\"收款行机构号\" column=\"jgh\" orderkey=\"jgh\" />" +
            "   <col width=\"9%\" text=\"是否中行收款标识\" column=\"zhsk\" orderkey=\"zhsk\" />" +
            "   <col width=\"9%\" text=\"CNAPS行号\" column=\"CNAPS\" orderkey=\"CNAPS\" />" +
            "	</head>" +
            " </table>";
%>
...
<script type="text/javascript">
    ...
    function createWorkflow() {
        ...
        Dialog.confirm('确定发起流程？', function () {
            ...
            // frmmain.submit()
            var result = createPayFlow(userId, fkqx, bz, company)
            if (result === '0') {
                location.reload()
                dialog.closeByHand()
    ...
    function createPayFlow(userId, fkqx, bz, company) {
        var result = '1'
        var a = {
            url: 'createPayFlow.jsp?userId=' + userId + '&fkqx=' + fkqx + '&bz=' + bz + '&com=' + company,
