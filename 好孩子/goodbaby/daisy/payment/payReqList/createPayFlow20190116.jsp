...
<%
    ...
    sql = "select *  from gb_payReqView2 t where t.fkqx<= '" + fkqx + "' and t.bz = '" +
            bz + "'";
    if(!"".equals(com)){
        sql = sql + " and gs like '%" + com + "%'";
    }
    if(!"".equals(cgdl)){
        sql = sql + " and cgdl ='"+cgdl+"' ";
    }
    rs.executeSql(sql);
    ...
    while (rs.next()) {
        ...
        String sqldt = "select t.* from (select isnull(b.sfpk,'1') as sfpk,b.id,b.gysmc,b.fph,b.fkqx," +
                ...
                "d.selectvalue = a.ZHSK) as ZHSK from uf_payinternal b join uf_suppmessForm a" +
                ...
                "and bz = '" + bz + "' and gysmc = " + gysmc + " and company like '%" + com + "%'";
    ...
    if (!"".equals(oaid)) {
        String sqldt3 = "insert into uf_flow_payinternal (pklcid,pkzjbid,fkid,fkqx,dfje,sfje,gysmc,company,bz,jeyb,jermb,skyh,skyhzh,sfpk,fph,kprq,lsfk) " +
                "select '" + oaid + "',id, fkid,fkqx,dfje,sfje,gysmc,company,bz,jeyb,jermb,skyh,skyhzh,sfpk,fph,kprq,lsfk from uf_payinternal where id in(" + tempAll + ")";
