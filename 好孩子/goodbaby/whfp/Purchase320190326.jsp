...
%>
	...
	<FORM id=report name=report action="/goodbaby/whfp/Purchase3.jsp" method=post>
	...
	String fromSql ="";
	String sqlWhere = "";
     backfields = " tt.lbid,tt.id,tt.gysmc,tt.fphm,tt.fpze,tt.xtkh,tt.wlmc,tt.wlbm,tt.wllx1,tt.rkdh,tt.rkrq,tt.rkdw,tt.rksl ,tt.sjjg,tt.ckmc ";
	 fromSql  = " from (select (select buydl  from uf_NPP where id = (select wllx from uf_materialDatas  where WLBM =ud.wlbm))as lbid, ui.fplx,ui.id,(select GYSMC from uf_suppmessForm where id =ui.gys) as gysmc,ui.fphm,ui.fpze,ui.xtkh,(select WLMC from uf_materialDatas  where id = ud.wlmc) as wlmc,ud.wlbm,(select ub.buydl  from uf_buydl ub where ub.id =ud.wllx1) as wllx1,ud.rkdh,ud.rkrq,(select dw  from  uf_unitForms  where id=ud.rkdw) as rkdw ,ud.rksl,ud.sjjg,(select CKMC from uf_stocks  where id = ud.ckmc) as ckmc from uf_invoice ui join uf_invoice_dt1  ud on ui.id = ud.mainid  join workflow_requestbase w on w.requestid =ud.rid) tt";
	 sqlWhere = " where 1=1 and tt.fplx ='1' and tt.lbid<>'2' ";
	if(!"".equals(rkdh)){
...
tableString =" <table tabletype=\"checkbox\" pagesize=\""+ PageIdConst.getPageSize(tmc_pageId,user.getUID(),PageIdConst.HRM)+"\" pageId=\""+tmc_pageId +"\">"+
    ...
    operateString+
    "			<head>";
		tableString+="<col width=\"12%\"  text=\"物料名称\" column=\"wlmc\" orderkey=\"wlmc\"/>"+
		"<col width=\"11%\"  text=\"物料类型\" column=\"wllx1\" orderkey=\"wllx1\"/>"+
		"<col width=\"11%\"  text=\"入库单号\" column=\"rkdh\" orderkey=\"rkdh\" linkvaluecolumn=\"rkdh\" linkkey=\"rkdhcs\" href=\"/goodbaby/whfp/whfpInfoHistoryUrl.jsp?wplx=flx\" target=\"_fullwindow\" />"+
		"<col width=\"11%\"  text=\"入库日期\" column=\"rkrq\" orderkey=\"rkrq\"/>"+
		"<col width=\"11%\"  text=\"入库单位\" column=\"rkdw\" orderkey=\"rkdw\"/>"+
		"<col width=\"11%\"  text=\"入库数量\" column=\"rksl\" orderkey=\"rksl\"/>"+
		"<col width=\"11%\"  text=\"实际价格(含税)\" column=\"sjjg\" orderkey=\"sjjg\"/>"+
		"<col width=\"11%\"  text=\"开票总额(含税)\" column=\"fpze\" orderkey=\"fpze\" linkvaluecolumn=\"id\" linkkey=\"billid\" href=\"/formmode/view/AddFormMode.jsp?type=0&amp;modeId=60&amp;formId=-268&amp;opentype=0&amp;customid=54&amp;viewfrom=fromsearchlist\" target=\"_fullwindow\"/>"+		
		"<col width=\"11%\"  text=\"仓库名称\" column=\"ckmc\" orderkey=\"ckmc\"/>"+
		"<col width=\"11%\"  text=\"物料编码\" column=\"wlbm\" orderkey=\"wlbm\"/>"+
		"<col width=\"11%\"  text=\"发票号码\" column=\"fphm\" orderkey=\"fphm\"/>"+
		"<col width=\"11%\"  text=\"供应商\" column=\"gysmc\" orderkey=\"gysmc\"/>"+
