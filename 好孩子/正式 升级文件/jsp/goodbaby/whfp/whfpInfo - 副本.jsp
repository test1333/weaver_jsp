...
<input type="hidden" name="pageId" id="pageId" value="<%= PageIdConst.HRM_ONLINEUSER %>"/>
	...
	<div class="zDialog_div_content" id="differencehandle" name="differencehandle">
				...
				<wea:group context="详细数据" attributes="{'samePair':'SetInfo','groupOperDisplay':'none','itemAreaDisplay':'block'}">
					...
					<%
					String mxsql = "select a.xtgys,(select 	GYSMC from uf_suppmessForm where id =c.gys) as gysmc,c.gys from formtable_main_243 a join workflow_requestbase b on a.requestid=b.requestid join formtable_main_243_dt1 c on c.mainid = a.id where b.currentnodetype >= 3 and a.id in ("+ids+")";
					rs.executeSql(mxsql);
					//out.print(mxsql);
					if(rs.next()){
					...
					<tr class=DataDark>
						...
						<td><input type = "text" id ="fphj" name="fphj" value="<%=sums%>" readonly></td>	
						<td><input type="text" name="fphm" id="fphm" class="InputStyle" value="" onchange="jsjg()" onkeyup="value=value.replace(/[^\d{1,}\.\d{1,}|\d{1,}]/g,'')"/>
...
<%
	...
	function OnSearch2(){
		...
		if(fphm_val.length<=0){
			...
			if (null != xhr) {
				xhr.open("GET","/goodbaby/whfp/pdsf.jsp?fphm="+fphm_val+"&type="+typess, false);
		...
		if(a == 2){
			...
			if(r==true){
				...
				alert()
				if (null != xhr) {
					xhr.open("GET","/goodbaby/whfp/insert.jsp?fphm="+fphm_val+"&wplx="+wplx+"&ids="+ids_val+"&fphj="+fphj_val+"&gsyid="+gsyid_val+"&xtgysid="+xtgysid_val, false);
					xhr.onreadystatechange = function () {
						if (xhr.readyState == 4) {
							if (xhr.status == 200) {
								var text = xhr.responseText;
