...
%>
	...
	String fromSql ="";
	String sqlWhere = "";
     backfields = "b.requestname,b.requestid,b.workflowid ,(select c.name from  crm_customerinfo c where c.id = isnull(a.xtkh,0)) as xtkh,a.CGSL,(select WLname from  uf_materialDatas where id = isnull(a.WLMC,0) ) as WLMC,a.DDBH,a.YQJHRQ as shsj ,isnull((select sum(isnull(d.SJSHSL_1,0)) from  formtable_main_235 c join formtable_main_235_dt1 d on c.id =d.mainid join workflow_requestbase e on c.requestid = e.requestid where e.currentnodetype>=3 and d.wlbh_1=a.WLBM and d.cgsqd=a.requestid and e.workflowid ='291' ),0)as SJSHSL,a.SHRQ ,isnull((select  sum(isnull(g.SJSHSL_1,0)) from  formtable_main_235 f join formtable_main_235_dt1 g on f.id =g.mainid join workflow_requestbase h on f.requestid = h.requestid where h.currentnodetype &lt; 3 and g.wlbh_1=a.WLBM and g.cgsqd=a.requestid and h.workflowid ='291'),0) as chzsl  "; 
	 fromSql  = " from formtable_main_234 a, workflow_requestbase  b  ";
	 sqlWhere = " where  a.requestid=b.requestid   and  b.workflowid = 290 and b.currentnodetype >= 3 ";
