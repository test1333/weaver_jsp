...
<%
    ...
    if(!"manager".equals(type)){
        String sql = "SELECT PortalPassword,name FROM CRM_CustomerInfo WHERE id ="+ userID;

        recordGet.executeSql(sql);
        recordGet.next();
        prepassword = recordGet.getString("PortalPassword");//修改之前的旧密码
        gysmc = Util.null2String(recordGet.getString("name"));
    }
...
//已办事宜
    ...
    int no = 0;
    String gyssql = "select  count(d.id) as no from formtable_main_231 d, workflow_requestbase c  where "+
            "d.requestid=c.requestid and c.workflowid = 287 and (d.zbj>0 or d.zbj1>0) and d.xtkh= '"+userID+"'" ;
    rs.executeSql(gyssql);
    if (rs.next()) {
        no = rs.getInt("no");
    }
    int number = 0;
    String crksql ="select count(t.id) as number from (select a.id ,a.xtkh from formtable_main_234 a,"+
            "workflow_requestbase b where a.requestid=b.requestid and b.workflowid = 290 and b.currentnodetype >= 3)t where t.xtkh = '"+userID+"'" ;
...
<div id="mainDiv">
    ...
    <div style="float:right; margin-top:20px;margin-right:10px"><%=gysmc%></div>
    <div class="leftDiv" style="margin-top:110px">
    ...
    <div class="rightDiv rightDiv_1">
        ...
        <div class="rightDiv_1_2">
            <a href="/page/element/7/News.jsp?ebaseid=7&amp;eid=111&amp;styleid=1515636047766&amp;hpid=14&amp;subCompanyId=8&indie=true&needHead=true" target="_blank">公&nbsp;&nbsp;&nbsp;告</a>
        </div>
    </div>
...
<script type="text/javascript">
    jQuery(document).ready(function () {
        ...
        jQuery(".dialogInfo_content_menu li").eq(0).click(function(){
            jQuery(dialogInfoDiv).hide();
            window.open("/workflow/request/AddRequest.jsp?workflowid=268&isagent=0&beagenter=0&f_weaver_belongto_userid=");
        ...
        jQuery(".dialogInfo_content_menu li").eq(5).click(function(){
            //openFullWindowForXtable("/formmode/search/CustomSearchBySimple.jsp?customid=73&gysid="+gysid);
            var url="/formmode/search/CustomSearchBySimple.jsp?customid=73&gysid="+gysid;
            var title="价格库查询"
            var diag_vote
