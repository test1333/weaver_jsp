...
<link rel="stylesheet" type="text/css" href="/js/jquery-autocomplete/browser_wev8.css" />
    ...
    String out_pageId = "cwreportfpzs_4";
	String sql = "";
	...
	<BODY>
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;">
				...
				<wea:item>
                    ...
                    <option value="16" <%if("16".equals(fplx)){%> selected<%} %>>增值税普通发票（卷票）</option>
										<option value="1000" <%if("1000".equals(fplx)){%> selected<%} %>>非国税发票</option>
		...
		<%
		String backfields = "requestid,dtid,keyid,workflowid,(select workflowname from workflow_base where id=a.workflowid) as workflowname,createdate,requestmark,userid_new,sqr,sqryx,cbzx1,cbzxbmmc,gszt,invoicetype,km,ywlx,je,se,kdkje,bxje,sjbxje,bz,invoicenumber,nodename,bz1"+
		",case when invoicetype='1' then '增值税普通发票' when invoicetype='2' then '增值税专用发票' when invoicetype='3' then '通用机打发票' when invoicetype='4' then '卷式发票' when invoicetype='5' then '定额发票' when invoicetype='6' then '地铁票' when invoicetype='7' then '出租发票' when invoicetype='8' then '动车/高铁票' when invoicetype='9' then '过路费发票' when invoicetype='10' then '客运汽车发票' when invoicetype='11' then '二手车销售统一发票' when invoicetype='12' then '机动车销售统一发票' when invoicetype='13' then '国际小票' when invoicetype='14' then '航空运输电子客票行程单' when invoicetype='15' then '增值税电子发票' when invoicetype='16' then '增值税普通发票（卷票）' when invoicetype='1000' then '非国税发票' else '' end as fplxmc ";
		String fromSql  =  " from v_cw_fpshst_zs a";
		String sqlWhere =  " 1=1 ";
	 if(!"".equals(cbzx)){
	...
	if(!"".equals(djh)){
	 ...
	 if(!"".equals(ywlx)){
		...
		tableString +="<col width=\"220px\" text=\"流程类型\" column=\"workflowname\" orderkey=\"workflowname\"  />"+
		  "<col width=\"100px\" text=\"日期\" column=\"createdate\" orderkey=\"createdate\"  />"+
		  "<col width=\"160px\" text=\"单据号\" column=\"requestmark\" orderkey=\"requestmark\"  />"+
			"<col width=\"100px\" text=\"当前节点\" column=\"nodename\" orderkey=\"nodename\"  />"+  
			"<col width=\"100px\" text=\"发票号码\" column=\"invoicenumber\" orderkey=\"invoicenumber\"  />"+ 
			...
			"<col width=\"200px\" text=\"申请人部门\" column=\"sqryx\" orderkey=\"sqryx\" />"+ 
			...
			"<col width=\"100px\" text=\"币种\" column=\"bz1\" orderkey=\"bz1\"  />"+ 
			...
			"<col width=\"100px\" text=\"票面总额\" column=\"bxje\" orderkey=\"bxje\"  />"+ 
			"<col width=\"100px\" text=\"实际报销金额\" column=\"sjbxje\" orderkey=\"sjbxje\"  />"+ 
