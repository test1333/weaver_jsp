  ...
  String out_pageId = "gnsreportrkdmxcx_1";
	...
	<BODY>
		...
		String backfields = "a.requestid,(select (select distinct dw from uf_company where dwzt=t.ztdm) as ztmc from uf_yjcbzxdzb t where yjcbzxmc=(select yjcbzx from uf_cbzx where id=a.cbzx)) as ztmc,(select yjcbzx from uf_cbzx where id=a.cbzx) as yjcbzx,a.zjlbm,(select portalloginid from crm_customerinfo where id=a.xtgys) as gysbh,(select name from CRM_CustomerInfo where id=a.xtgys) as gysmc"+
											...
											",case when (select count(1) from "+fksqdtablename+" t,"+fksqdtablename+"_dt1 t1,workflow_requestbase t2 where t.id=t1.mainid and t.requestId=t2.requestid and t2.currentnodeid in(select nodeid from uf_fprzjdpz) and t1.fphm in(select ta1.id from  uf_invoice ta1, uf_invoice_dt1 ta2 where ta1.id=ta2.mainid and ta2.rid=a.requestid))>0 then '是' else '否'end as sfkp,(select top 1  (SUBSTRING(t.gzrq,0,5)+SUBSTRING(t.gzrq,6,2)) from formtable_main_273 t,workflow_requestbase t1,formtable_main_273_dt1 t2 where t.requestid=t1.requestid and t.id=t2.mainid and t1.currentnodeid in(select nodeid from uf_fprzjdpz) and t2.fphm in(select t5.id from uf_invoice t5, uf_invoice_dt1 t6 where t5.id=t6.mainid and t6.rid=a.requestid)) as rzrq,"+
		...
		tableString +=
		  "<col width=\"100px\" text=\"账套\" column=\"ztmc\" orderkey=\"ztmc\" />"+ 
			...
			"<col width=\"100px\" text=\"入账年月\" column=\"rzrq\" orderkey=\"rzrq\"  />"+ 
