...
<%
    String backfields = " v.id,dataId,WLMC,cgsl,gysId,GYSMC,a.requestId,a.SQRXX,needDate,flowNum,PP,XH,GG,DW,SL,YGDJ ";
    //表单中的字段
    String fromSql = " from gb_minorPurchaseView v join " +
            " (select r.requestId,cgsl,SQRXX,dtId from formtable_main_234 f left join  workflow_requestbase r " +
            " on f.requestId=r.requestid where r.currentnodetype = 3) a " +
            " on v.id = a.dtId ";//sql查询的表
    String sqlWhere = " 1=1   and cgsl >(select isnull(sum(isnull(b.sjshsl_1,0)),0) from formtable_main_235 t,formtable_main_235_dt1 b,workflow_requestbase c where t.id=b.mainid and b.cgsqd=a.requestid and t.requestId=c.requestid and c.currentnodetype>=3)";//where条件
    // PR申请人只能查看自己申请的流程
    if (userId > 1) {
