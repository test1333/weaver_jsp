...
%>
	...
	String sqlWhere = "";
	 ...
	 fromSql  = " from (select (select buydl  from uf_NPP where id = (select wllx from uf_materialDatas  where id =ud.wlmc))as lbid, ui.fplx,ui.id,(select GYSMC from uf_suppmessForm where id =ui.gys) as gysmc,ui.fphm,ui.xtkh,ui.fpze,(select WLMC from uf_materialDatas  where id = ud.wlmc) as wlmc,ud.wlbm,(select ub.buydl  from uf_buydl ub where ub.id =ud.wllx1) as wllx1,ud.rkdh,ud.rkrq,(select dw  from  uf_unitForms  where id=ud.rkdw) as rkdw ,ud.rksl,ud.sjjg,(select CKMC from uf_stocks  where id = ud.ckmc) as ckmc,a.requestid as rkrqid,b.xght,b.fkxh,b.fktj,b.fkje,(select htbh from "+tablename_ht+" where requestid=b.xght) as htbh from uf_invoice ui join uf_invoice_dt1  ud on ui.id = ud.mainid  join workflow_requestbase w on w.requestid =ud.rid join "+tablename_rk+" a on ud.rid=a.requestid join  "+tablename_rk+"_dt1 b on a.id=b.mainid) tt";
