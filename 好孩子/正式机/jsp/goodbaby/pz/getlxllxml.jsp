...
String tablename = gg.getTableName("LXLL");
String rqids = "";
String flag = "";
int count = 0;
String sql = "select a.requestid from "+tablename+" a,workflow_requestbase b "+
"where a.requestid=b.requestid  and b.currentnodetype=3 and (select yjcbzx from uf_cbzx where id=a.cbzx) = '"+yjcbzx+"' and b.lastoperatedate<='"+enddate+"' and b.lastoperatedate>='"+begindate+"'";
rs.executeSql(sql);
while(rs.next()){
	rqids = rqids+flag+Util.null2String(rs.getString("requestid"));
	flag = ",";
}
CreateLXLLXML crx = new CreateLXLLXML();
String xmlString =crx.CreateXML(begindate,enddate,yjcbzx,scr,rqids);
response.reset();
out.clear();  
//response.setContentType("application/x-download"); 
response.setContentType("application/x-msdownload");
String filedisplay = begindate.substring(0,4)+begindate.substring(5,7)+"零星材料领用财务凭证.xml";  
