...
<%
	...
	String tmc_pageId = "purchase3212";
...
%>
	...
	String sqlWhere = "";
	 ...
	 fromSql  = " from (select (select buydl  from uf_NPP where id = (select wllx from uf_materialDatas  where WLBM =ud.wlbm))as lbid, ui.fplx,ui.id,(select GYSMC from uf_suppmessForm where id =ui.gys) as gysmc,ui.fphm,ui.fpze,ui.xtkh,(select WLMC from uf_materialDatas  where id = ud.wlmc) as wlmc,ud.wlbm,(select ub.buydl  from uf_buydl ub where ub.id =ud.wllx1) as wllx1,ud.rkdh,ud.rkrq,(select dw  from  uf_unitForms  where id=ud.rkdw) as rkdw ,ud.rksl,ud.sjjg,(select CKMC from uf_stocks  where id = ud.ckmc) as ckmc,a.requestid as rkrqid,b.xght,b.fkxh,b.fktj,b.fkje,(select htbh from "+tablename_ht+" where requestid=b.xght) as htbh from uf_invoice ui join uf_invoice_dt1  ud on ui.id = ud.mainid  join workflow_requestbase w on w.requestid =ud.rid join "+tablename_rk+" a on ud.rid=a.requestid join  "+tablename_rk+"_dt1 b on a.id=b.mainid) tt";
...
tableString =" <table tabletype=\"checkbox\" pagesize=\""+ PageIdConst.getPageSize(tmc_pageId,user.getUID(),PageIdConst.HRM)+"\" pageId=\""+tmc_pageId +"\">"+
    ...
    "			<head>";
		...
		"<col width=\"8%\"  text=\"发票号码\" column=\"fphm\" orderkey=\"fphm\" linkvaluecolumn=\"rkdh\" linkkey=\"rkdhcs\" href=\"/goodbaby/whfp/whfpInfoHistoryUrl.jsp?wplx=flx\" target=\"_fullwindow\"/>"+
		...
		"<col width=\"8%\"  text=\"开票总额(含税)\" column=\"fpze\" orderkey=\"fpze\" linkvaluecolumn=\"id\" linkkey=\"billid\" href=\"/formmode/view/AddFormMode.jsp?type=0&amp;modeId=55&amp;formId=-252&amp;opentype=0&amp;viewfrom=fromsearchlist\" target=\"_fullwindow\"/>"+		
