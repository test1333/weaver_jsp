...
<%
	...
	// String sql= "select (select c.name from  crm_customerinfo c where c.id =a.gysmc ) as gymc ,a.gysmc from formtable_main_228_dt2 a join formtable_main_228 b on a.mainid =b.id where b.requestid ='"+rid+"'  ";
	
	String sql = "select (select c.name from  crm_customerinfo c where c.id =a.gysmc ) as gymc ,d.BJJE,d.HJJE,a.gysmc from formtable_main_228_dt2 a join formtable_main_228 b on a.mainid = b.id join formtable_main_227 d on d.aid = b.requestid and a.gysmc =d.GYSBM1 where b.requestid ='"+rid+"'  order by d.BJJE ";
	rs.executeSql(sql);
	int tep = 0;
	while(rs.next()){
	...
	<div class="zDialog_div_content" id="differencehandle" name="differencehandle">
				...
				<wea:group context="详细数据" attributes="{'samePair':'SetInfo','groupOperDisplay':'none','itemAreaDisplay':'block'}">
					...
					<table class=ListStyle cellspacing=1 border="1">
						...
						<%
							int fieldscount = 0;
							sql = " select a.BM_1,a.WLMC_1,a.SL_1,(select f.dw from uf_unitForms f where f.id =a.DW_1) as DW  from formtable_main_228_dt1  a  join formtable_main_228 b on a.mainid =b.id where b.requestid ='"+rid+"' ";
							rs.executeSql(sql);
							while(rs.next()){
						...
						<%if(fieldscount%2==0){%><tr><%}%>
							...
							<%	
								for(int i=0;i<list.size();i++){
								String str ="select a.DJ_1,a.JG_1 from formtable_main_227_dt1 a join formtable_main_227 b on b.id =a.mainid where b.aid = '"+rid+"'  and b.GYSBM1 ='"+list.get(i)+"' and a.BM_1 ='"+BM_1+"'";
