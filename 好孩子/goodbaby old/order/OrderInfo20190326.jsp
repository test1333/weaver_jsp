					...
					<table class=ListStyle cellspacing=1 border="1">
						...
						<%
							...
							for(int  i=0;i<rids.length;i++){
								String sql = "select a.CGSL,a.DDBH,contactReq,contactDtId,(select c.name from  crm_customerinfo c where c.id = isnull(a.xtkh,0)) as xtkh,(select WLname from  uf_materialDatas where id = isnull(a.WLMC,0) ) as WLMC,a.YQJHRQ as shsj,a.DJ,isnull((select sum(isnull(d.SJSHSL_1,0)) from  formtable_main_243 c join formtable_main_243_dt1 d on c.id =d.mainid join workflow_requestbase e on c.requestid = e.requestid where d.wlbh_1=a.WLBM and d.cgsqd=a.requestid and e.workflowid ='213' ),0) as zshsl from formtable_main_240 a where requestid ='"+rids[i]+"' ";
								//out.print(sql);
								rs.executeSql(sql);
								while(rs.next()){
									...
									str = "Select a.tablename From Workflow_bill a join workflow_base  w on a.id=w.formid where w.id =(select workflowID from  workflow_requestbase where requestid ='"+contactReq+"' )";
									RecordSet.executeSql(str);
									if(RecordSet.next()){
										tablename = Util.null2String(RecordSet.getString("tablename"));
									}
									if(tablename.equals("formtable_main_207")){
										str = "select SFRK from formtable_main_207_dt1 where id ='"+contactDtId+"'";
										RecordSet.executeSql(str);
										if(RecordSet.next()){
