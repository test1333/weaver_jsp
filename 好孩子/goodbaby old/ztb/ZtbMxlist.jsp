...
<%
	...
	// String sql = "select (select c.name from  crm_customerinfo c where c.id =a.gysmc ) as gymc ,d.zbj1,d.zbj,a.gysmc from formtable_main_200_dt2 a join formtable_main_200 b on a.mainid = b.id join formtable_main_250 d on d.zhurid = b.requestid and a.gysmc =d.xtkh where b.requestid ='"+rid+"'  ";
	...
	<div class="zDialog_div_content" id="differencehandle" name="differencehandle">
				...
				<wea:group context="详细数据" attributes="{'samePair':'SetInfo','groupOperDisplay':'none','itemAreaDisplay':'block'}">
					...
					<table class=ListStyle cellspacing=1 border="1">
						...
						<%
							int fieldscount = 0;
							sql = " select a.BM_1,a.WLMC_1,a.SHUL_1,(select f.dw from uf_unitForms f where f.id =a.DW_1) as DW  from formtable_main_200_dt1  a  join formtable_main_200 b on a.mainid =b.id where b.requestid ='"+rid+"' ";
							rs.executeSql(sql);
							while(rs.next()){
						...
						<%if(fieldscount%2==0){%><tr><%}%>
							...
							<%	
								for(int i=0;i<list.size();i++){
								// String str ="select a.BJJE_1,a.bjje from formtable_main_250_dt1 a join formtable_main_250 b on b.id =a.mainid where b.zhurid = '"+rid+"'  and b.xtkh ='"+list.get(i)+"' and a.BM_1 ='"+BM_1+"'";
