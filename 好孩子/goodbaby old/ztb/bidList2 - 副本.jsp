...
if(b.equals("1")&&bj.length()>1){
	String  mmaid = "";
	String sql = "insert into  uf_gysbjb(ZBWJBH,zhurid,zirid,zbj,WLBM,kbsj,pp,xh,gg,dw,crsj,xtkh) select  ZBWJBH,zhurid,zirid,zbj,WLBM,kbsj,pp,xh,gg,dw,crsj,xtkh from uf_gysbjb where id =(select max(id) from uf_gysbjb where kbsj ='"+kbsj+"' and zirid =(select requestid from formtable_main_250 where id  ='"+aid+"'))";
	rs.executeSql(sql);
	...
	sql = "update uf_gysbjb set zbj = '"+bj+"' ,zkzbj ='"+zkzbj+"',crsj=CONVERT(varchar(16),GETDATE(),120)  where id = '"+mmaid+"'";
	rs.executeSql(sql);
	...
	if(rs.next()){
		...
		String m2id ="";
		String sss= "update formtable_main_200_dt2 set  zbjg='"+bj+"',zkbj='"+zkzbj+"' where  id =(  select a.id from formtable_main_200_dt2 a where a.mainid = (select b.id from formtable_main_200 b where b.requestid='"+reid+"') and a.gysmc='"+xtkh+"')";
...
<input type="hidden" name="pageId" id="pageId" value="<%= PageIdConst.HRM_ONLINEUSER %>"/>
	...
	<div class="zDialog_div_content" id="differencehandle" name="differencehandle">
				...
				<wea:item>距离截止报价时间还有</wea:item>
				<wea:item><span name="time" id="time"></span></wea:item>
				<wea:item>开&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;标&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;时&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;间</wea:item>
				<wea:item><span  name="kbsj1" id="kbsj1"><%=jzsj%></span>
				<input type="hidden" name="kbsj" id = "kbsj" value="<%=jzsj%>">
				...
				<wea:item>价&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;格&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;税&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;率</wea:item>
				<wea:item>
		                <brow:browser viewType="0"  name="comzb" browserValue=""
		                browserUrl="/systeminfo/BrowserMain.jsp?url=/interface/CommonBrowser.jsp?type=browser.taxRate"
...
                </wea:item>
	...
	%>
		...
		<tr style="color:yellow" >	
			<td><a href="javascript:void(0);" onclick="MX_in(<%=uid%>)"><%=wlbm%></a></td><td><%=pp%></td><td><%=xh%></td><td><%=gg%></td><td><%=dw%></td><td><%=zbj%></td><td><%=crsj%></td>
		</tr>		
		<%
	...
	function OnSearch2(){
        ...
        var endDate=new Date(kbsj_val1.replace('-','/'));///截止时间
