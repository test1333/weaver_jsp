...
String cgsl = "";
String sqlcg = "select sl,jzbqrq+' '+jzbqsj as jzsj from formtable_main_231 where id  ='"+aid+"'";
rs.executeSql(sqlcg);
if(rs.next()){
...
if(b.equals("1")&&bj.length()>=1){
	String  mmaid = "";
	String sql = "insert into  uf_gysbjb(ZBWJBH,zhurid,zirid,zbj,WLBM,kbsj,pp,xh,gg,dw,crsj,xtkh,shuilv,jhq) select  ZBWJBH,zhurid,zirid,zbj,WLBM,kbsj,pp,xh,gg,dw,crsj,xtkh,shuilv,jhq from uf_gysbjb where id =(select max(id) from uf_gysbjb where kbsj ='"+kbsj+"' and zirid =(select requestid from formtable_main_231 where id  ='"+aid+"'))";
	rs.executeSql(sql);
	sql = "select max(id) as mmaid from uf_gysbjb where kbsj ='"+kbsj+"' and zirid =(select requestid from formtable_main_231 where id  ='"+aid+"') ";
	...
	sql = "select a.requestid, b.*  from formtable_main_231 a join formtable_main_231_dt1 b  on a.id=b.mainid   where b.mainid ='"+aid+"'";
	rs.executeSql(sql);
	int mu = 0;
	while(rs.next()){
	...
	// for(int i=0;i<fieldscountsa;i++){
		...
		//out.print("sql----"+sql);
	// }	
	sql = "select xtkh ,zhurid from formtable_main_231 where id  = '"+aid+"'";
	rs.executeSql(sql);
	if(rs.next()){
		String reid = rs.getString("zhurid");
		String xtkh = rs.getString("xtkh");
		String m2id ="";
		String sss= "update formtable_main_230_dt2 set shuilv='"+shuilv+"', zbjg='"+bj+"',zkbj='"+zkzbj+"' where  id =(  select a.id from formtable_main_230_dt2 a where a.mainid = (select b.id from formtable_main_230 b where b.requestid='"+reid+"') and a.gysmc='"+xtkh+"')";
...
<input type="hidden" name="pageId" id="pageId" value="<%= PageIdConst.HRM_ONLINEUSER %>"/>
	...
	<div class="zDialog_div_content" id="differencehandle" name="differencehandle">
				...
				<wea:group context="详细数据" attributes="{'samePair':'SetInfo','groupOperDisplay':'none','itemAreaDisplay':'block'}">
					...
					<%
					String mxsql = "select * from formtable_main_231_dt1 where mainid = '"+aid+"'";
					rs.executeSql(mxsql);
	...
	<div >
		...
		String sqt = "select zhurid ,requestid from formtable_main_231 where id ='"+aid+"'";
		rs.executeSql(sqt);
		if(rs.next()){
