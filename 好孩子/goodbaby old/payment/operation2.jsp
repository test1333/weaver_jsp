...
<%@ page import="goodbaby.util.CheckUtil"%>
...
<%
	...
	RCMenu += "{取消确认,javascript:opera(),_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
		...
		<table id="topTitle" cellpadding="0" cellspacing="0">
			<tr>
				<td>
					...
					<input type="button" id="cflc" name = "cflc"  value="取消确认" class="e8_btn_top" onClick="opera()" />
...
<wea:layout type="4col">
	<wea:group context="<%=SystemEnv.getHtmlLabelNames("15774",user.getLanguage()) %>">
		...
		<wea:item>
		 ...
		 <wea:item>出纳是否确认</wea:item>
		<wea:item>
			<select id="CHANGETYPE" name="CHANGETYPE" onchange="" >
				<option value=""></option>
				<option value="0" <% if(CHANGETYPE.equals("0")) out.print("selected");%> >否</option>
				<option value="1" <% if(CHANGETYPE.equals("1")) out.print("selected");%>  >是</option>
			</select>    
		</wea:item>
	...
	<%	
	String backfields = " uu.id,uu.gysmc,uu.pkbhs,uu.pklcid, uu.bcyf,uu.bcsf,uu.khh,uu.yhzh,uu.lhh,uu.jgh,case when uu.zhsk='0' then '是' else '否' end as zhsk,uu.cnaps,uu.remark,uu.pcno "; 
	String fromSql  = " from (select cnqr, id,flag,pklcid,(select pkbh from formtable_main_205 where requestid = pklcid) as pkbhs ,(select u.GYSMC from uf_suppmessForm u where u.id =gys ) as gysmc, bcyf,bcsf, khh,yhzh,lhh,jgh,zhsk,cnaps,remark,pcno from uf_fkzjb) uu ";
	String sqlWhere = " where  1=1 and flag = 1 ";	
	// if(userid!=1){
	...
	String tableString = "";
	if(!"".equals(pkbhs)){
		sqlWhere = sqlWhere + " and uu.pkbhs like '%"+pkbhs+"%'";
	}
	if(!"".equals(gysmcs)){
		sqlWhere = sqlWhere + " and uu.gysmc like '%"+gysmcs+"%' ";
	}
	if(!"".equals(CHANGETYPE)){
		sqlWhere = sqlWhere + " and uu.cnqr = '"+CHANGETYPE+"' ";
	}
...
tableString =" <table tabletype=\"checkbox\" pagesize=\""+ PageIdConst.getPageSize(pageID,user.getUID(),PageIdConst.HRM)+"\" pageId=\""+pageID+"\">"+
		" <checkboxpopedom  id=\"checkbox\" popedompara=\"column:id\" showmethod=\"goodbaby.util.CheckUtil.getCanCheck\" />"+

		"	   <sql backfields=\""+backfields+"\" sqlform=\""+fromSql+"\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlWhere)+"\"  sqlorderby=\""+orderby+"\"  sqlprimarykey=\"id\" sqlsortway=\"desc\" sqlisdistinct=\"false\"/>"+
    operateString+
    "			<head>";                                                                                                                                                  
	  tableString+="<col width=\"10%\" text=\"排款编号\" column=\"pkbhs\" orderkey=\"pkbhs\" linkvaluecolumn=\"pklcid\" linkkey=\"requestid\"" +
...
%>
	...
	function opera(){
		...
		if(confirm(conm)){
			...
			if (null != xhr) {
				xhr.open("GET","/goodbaby/payment/dateInfo.jsp?type=delete&ids="+ids, false);
