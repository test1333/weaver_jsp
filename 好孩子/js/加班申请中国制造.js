...
<script type="text/javascript">
    var gwzj= "#field10882";
	var sqrq="#field6860";//申请日期
	var degzr="#field10989";//第二工作日
	var syrq="#field10990";//上月日期
	
	var jbksrq_dt1="#field6866_";//加班开始日期
	var jbjsrq_dt1="#field6868_";//加班结束日期
	...
	checkCustomize = function () {
		...
		if(!checkissamemonth()){
			window.top.Dialog.alert("加班开始日期与结束日期必须在相同月，请检查");
			return false;
	    }
        if(!checkissamemonth2(sqrq_val.substring(0,7),syrq_val.substring(0,7),flag)){
			if(flag !="1"){
				window.top.Dialog.alert("加班开始日期与结束日期必须与申请日期相同月，请检查");
			   return false;					
			}else{
				window.top.Dialog.alert("加班开始日期与结束日期必须与申请日期相同月或上个月，请检查");
			   return false;	
