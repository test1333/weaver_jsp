...
<%
    ...
    String late_pageId = "adore11";

    Boolean isService = false; // 是否服务类采购员
    Boolean isInvest = false; // 是否投资了服务员
    String sql = "";
    sql = " select count(id) as num_admin from HrmRoleMembers where roleid=111 and resourceid=" + userId;
    rs.executeSql(sql);
    if (rs.next()) {
    ...
    sql = " select count(id) as num_admin from HrmRoleMembers where roleid=109 and resourceid=" + userId;
    rs.executeSql(sql);
    if (rs.next()) {
        int num_admin = rs.getInt("num_admin");
        if (num_admin > 0) {
...
<%
    String backfields = " id,dataId,WLMC,WLBM,gysId,GYSMC,a.requestId,JBRXM,needDate,flowNum,PP,XH,GG,DW,SL,YGDJ ";
    //表单中的字段
    String fromSql = " from gb_noneBidPurchaseView v join " +
            " (select r.requestId,JBRXM,dtId from formtable_main_233 f left join  workflow_requestbase r " +
            " on f.requestId=r.requestid where r.currentnodetype in(0,1,2)) a " +
            " on v.id = a.dtId ";//sql查询的表
    String sqlWhere = " 1=1 ";//where条件
    // out.println("select "+ backfields + fromSql +"where"+ sqlWhere);

    if(isService){
        sqlWhere += " and v.dataId in(select id from uf_inquiryForm where WLLX1 " +
    ...
    String orderby = " a.requestId ";//排序关键字
    String tableString = "";
    ...
    tableString += "    <col width=\"33%\" text=\"物料名称\" column=\"WLMC\" orderkey=\"WLMC\" " +
            ...
            "		    <col width=\"33%\" text=\"申请人\" column=\"JBRXM\" orderkey=\"JBRXM\" " +
