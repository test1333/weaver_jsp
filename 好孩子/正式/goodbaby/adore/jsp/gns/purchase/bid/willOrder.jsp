...
<%
    String backfields = " p.id,dataId,WLMC,isnull(p.cgsl,0) cgsl,gysId,GYSMC,contactReq,JBRXM," +
            " v.SQRXX,needDate,flowNum,PP,XH,GG,DW,SL,YGDJ,p.FKBFB,p.FKJE  ";
    //表单中的字段
    String fromSql = " from gb_bidPurchaseView v join gb_noneFrameworkContactFlow p on v.id = p.dtId ";//sql查询的表
    String sqlWhere = " p.id not in (select isnull(contactDtId,0) from formtable_main_234) ";//where条件

    // PR申请人只能查看自己申请的流程
    if (userId > 1) {
        sqlWhere += " and v.SQRXX= " + userId;
    }
    // out.println("select "+ backfields + fromSql +"where"+ sqlWhere);
    String orderby = " p.contactReq ";//排序关键字
    String tableString = "";

    tableString = " <table tabletype=\"checkbox\" pagesize=\"" + PageIdConst.getPageSize(late_pageId, user.getUID(), PageIdConst.HRM) + "\" " +
            "      pageId=\"" + late_pageId + "\">" +
            "	   <sql backfields=\"" + backfields + "\" sqlform=\"" + fromSql + "\" sqlwhere=\"" + Util.toHtmlForSplitPage(sqlWhere) + "\"  sqlorderby=\"" + orderby + "\" " +
            "      sqlprimarykey=\"p.id\" sqlsortway=\"asc\" sqlisdistinct=\"false\"/>" +
            "	<head>";
    tableString += "    <col width=\"33%\" text=\"物料名称\" column=\"WLMC\" orderkey=\"WLMC\" " +
            "           linkvaluecolumn=\"dataId\" linkkey=\"billid\" " +
            "           href=\"/formmode/view/AddFormMode.jsp?type=0&amp;modeId=42&amp;formId=-208\" target=\"_fullwindow\" />" +
            ...
            "		    <col width=\"33%\" text=\"付款条件\" column=\"FKBFB\" orderkey=\"FKBFB\" />" +
            "		    <col width=\"33%\" text=\"付款金额\" column=\"FKJE\" orderkey=\"FKJE\" />" +
            ...
            "		    <col width=\"33%\" text=\"流程\" column=\"contactReq\" orderkey=\"contactReq\" " +
            "           transmethod=\"weaver.workflow.request.RequestComInfo.getRequestname\" " +
            "           linkvaluecolumn=\"contactReq\" linkkey=\"requestid\" " +
...
<script type="text/javascript">
    ...
    function createWorkflow () {
        ...
        Dialog.confirm('确定发起流程', function () {
            report.action = 'createPOFlow.jsp?id=' + ids + '&userId=' + userId
            report.submit()
    ...
    function openDialog () {
        ...
        dlg.Width = 1080// 定义长度
        dlg.Height = 720
        dlg.URL = '/goodbaby/adore/jsp/gns/purchase/bid/editPOAmount.jsp?id=' + ids + '&userId=' + userId
        dlg.Title = '编辑采购数量'
        dlg.show()
    }
