...
String requestMethod = Util.null2String(request.getMethod());
    ...
    if (weaver_login_type == null) {
        weaver_login_type = request.getParameter("weaver_login_type");
        if (weaver_login_type != null) {
...
if(!weaverssoservice.equals("")&&!appid.equals("")){
	request.getRequestDispatcher("VerifyLoginWeaversso.jsp").forward(request,response);
	return;
}
...
if (logintype.equals("2")){
	if(!gopage.equals("")){
		...
		RedirectFile = "/supplier/gb/supplier.jsp" ;
	}
}
...
if(loginid.equals("") || userpassword.equals("") ) response.sendRedirect(loginfile + "&languageid="+islanguid+"&message=17") ;
   ...
   String needusb=Util.null2String(settings.getNeedusb());
    ...
    {
		VerifyLogin.recordFailedLogin(loginid,SystemEnv.getErrorMsgName(Util.getIntValue(usercheck),islanguid),request);
		...
		VerifyLogin.recordFailedLogin(loginid,SystemEnv.getErrorMsgName(52,islanguid),request);
		String tmploginid=(String)session.getAttribute("tmploginid");
        if(tmploginid!=null&&loginid.equals(tmploginid)) {
           session.setAttribute("tmploginid1",loginid);
        }
		...
		VerifyLogin.recordFailedLogin(loginid, SystemEnv.getHtmlLabelName(124919, islanguid),request);
		String usbMsgparam = "";
...
// response.sendRedirect(loginfile + "&languageid="+islanguid+"&message="+usercheck + usbMsgparam) ;
		response.sendRedirect(loginfile + "&languageid="+islanguid+"&message=17" + usbMsgparam) ;
	}else if(usercheck.equals("19")){
		VerifyLogin.recordFailedLogin(loginid,SystemEnv.getErrorMsgName(19,islanguid),request);
		response.sendRedirect("/system/InLicense.jsp") ;
	} else if (usercheck.equals("26")) {
		VerifyLogin.recordFailedLogin(loginid,SystemEnv.getHtmlLabelName(23656, islanguid),request);
		if (!loginPDA.equals("1")) {
			response.sendRedirect("/login/Login.jsp?logintype=1&languageid="+islanguid+"&message="+usercheck);
		} else {
	    ...
	    String desc = "";
	    if(usercheck.equals("101")){
			desc = SystemEnv.getHtmlLabelName(20289,islanguid);
		}else{
	        desc =  SystemEnv.getHtmlLabelName(84255,islanguid);
		}
		VerifyLogin.recordFailedLogin(loginid,desc,request);
	...
	{
		VerifyLogin.recordFailedLogin(loginid,usercheck,request);
		if (!loginPDA.equals("1"))
		{
			response.sendRedirect("/login/Login.jsp?logintype=1&languageid="+islanguid+"&message=" + usercheck);
		}
		...
		VerifyLogin.recordFailedLogin(loginid,SystemEnv.getHtmlLabelName(23771,islanguid),request);
		response.sendRedirect("/login/Login.jsp?logintype=1&languageid="+islanguid+"&message=" + usercheck);
	}
	else if(usercheck.equals("120")){    //未绑定动态令牌
		VerifyLogin.recordFailedLogin(loginid,"未绑定动态令牌",request);
		response.sendRedirect("/login/bindTokenKey.jsp?languageid="+islanguid);
	}
	else if(usercheck.equals("88")){     //Added by wcd 2014-01-04
		VerifyLogin.recordFailedLogin(loginid,SystemEnv.getHtmlLabelName(81628,islanguid),request);
		%>
		<script>
	...
	else if(usercheck.equals("184")){
	    VerifyLogin.recordFailedLogin(loginid,SystemEnv.getHtmlLabelName(131434,islanguid),request);
		if (!loginPDA.equals("1")) {
			response.sendRedirect("/login/Login.jsp?logintype=1&languageid="+islanguid+"&message="+usercheck);
		} else {
			response.sendRedirect("/login/LoginPDA.jsp?logintype=1&languageid="+islanguid+"&message="+usercheck);
            ...
            session.setAttribute("reloginfrom","pc");
