...
<TABLE style="display:none;" cellspacing=0  id="mytbl">
    <TR CLASS=DataLight id="total_tr" style="margin-left:50px!important;">
	 ...
	 <%-- 
	  ...
	  --%>
	 <td nowrap><%=ProjectTransUtil.getPrjTask2Date(totalbegindate, totalbegintime+"+"+totalactualbegindate+" + "+totalactualbegintime+" + ") %></td>
	  <td nowrap><%=ProjectTransUtil.getPrjTask2Date(totalenddate, totalendtime+"+"+totalactualenddate+" + "+totalactualendtime+" + ") %></td>
	  <td nowrap><%=KnowledgeTransMethod.getPercent(prjfinish, ProjectTransUtil.getPrjTaskProgressbar(prjfinish , "" )) %></td>
	  <td>&nbsp;</td>
    </TR>
</TABLE>	    
...
function afterDoWhenLoaded(){
	...
	if(totalCount>0){
		...
		firsttr.find("td").eq(1).html(totaltr.find("td").eq(0).html())
		.next().html(totaltr.find("td").eq(1).html())
		.next().html(totaltr.find("td").eq(2).html())
		.next().html(totaltr.find("td").eq(3).html())
		.next().html(totaltr.find("td").eq(4).html())
		.next().html(totaltr.find("td").eq(5).html())
		;
	}
	//initProgressbar();
}
