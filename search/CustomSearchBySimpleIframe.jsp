...
if (RecordSet.next()) { 
	...
	createname = Util.null2String(RecordSet.getString("listbatchname"));
	...
	<table id="topTitle" cellpadding="0" cellspacing="0">
		<tr>
			...
			<td class="rightSearchSpan" style="text-align:right;">
				<%if(isbatchsetnew){ %>
				<%if(isCreateBox && "1".equals(isShowCreateBtn)){%><!-- 新建 -->
				<input type="button" class="e8_btn_top" value="<%=createname %>" onclick="javascript:Add()"/>				
				<%}}else if(isshowshortcutbutton){%>
...
%>
<select class=inputstyle  name="con<%=id%>_opt" style="<%=display%>width:90" >
<option value="1" <%if(tmpopt==1){%>selected<%}%>><%=SystemEnv.getHtmlLabelName(327,user.getLanguage())%></option><!-- 绛変簬 -->
<option value="2" <%if(tmpopt==2){%>selected<%}%>><%=SystemEnv.getHtmlLabelName(15506,user.getLanguage())%></option><!-- 涓嶇瓑浜?-->
</select>
<brow:browser viewType="0" name="<%=namestr%>" browserValue="<%=tmpvalue %>" 
	browserUrl="<%=urls %>" 
...
<%}%>
	...
	<wea:group context="">
		<wea:item type="toolbar"><!-- 搜索 -->
			...
			<!-- 重置 -->
			<input type="button" value="<%=SystemEnv.getHtmlLabelName(2022,user.getLanguage())%>" class="e8_btn_cancel" onclick="clearSearchName();resetSearch();resetMultiselect();"/>
			<% if (templateid > 0) { %>
				<!-- 保存 -->
...
sql = "select isorder,ColWidth,workflow_billfield.id as id,workflow_billfield.fieldname as name,workflow_billfield.fieldlabel as label"+
		...
		",viewtype,workflow_billfield.detailtable" +
		" from workflow_billfield,Mode_CustomDspField,Mode_CustomSearch "+
...
while (RecordSet.next()) {
	...
	if(RecordSet.getString("id").equals("-1")){
		...
		String para3="";
		if(viewtype==3){
			para3="column:"+(isVirtualForm?vprimarykey:"id")+"+"+id+"+"+htmltype+"+"+type+"+"+user.getLanguage()+"+"+isbill+"+"+dbtype+"+"+istitle+"+"+formmodeid+"+"+formID+"+"+viewtype+"+"+opentype+"+"+customid+"+fromsearchlist"+"+"+showmethod;
		}else{
			para3="column:"+(isVirtualForm?vprimarykey:"id")+"+"+id+"+"+htmltype+"+"+type+"+"+user.getLanguage()+"+"+isbill+"+"+dbtype+"+"+istitle+"+"+formmodeid+"+"+formID+"+"+viewtype+"+"+opentype+"+"+customid+"+fromsearchlist"+"+"+showmethod+"+"+user.getUID()+"+"+enabled;
		}
		label = SystemEnv.getHtmlLabelName(Util.getIntValue(label),user.getLanguage());
