...
String modeid = "0";
rs.executeSql("select a.modeid,a.customname,a.customdesc,b.modename,b.formid from mode_customsearch a,modeinfo b where a.modeid = b.id and a.id="+customid);
if(rs.next()){
		...
		if((htmltype.equals("1")&& type.equals("1"))||htmltype.equals("2")){  //文本框
			if(!tmpvalue.equals(""))
			{
				...
				if(RecordSet.getDBType().equalsIgnoreCase("oracle"))
				      ...
				      sqlwhere += "and (','+CONVERT(varchar,t."+tmpcolname+")+',' ";
...
FormModeSearchClause.setTablename(tablename);
FormModeSearchClause.setCustomid(customid);
FormModeSearchClause.setFormmodeid(modeid);
FormModeSearchClause.setWhereclause(whereclause);
FormModeSearchClause.setFromdate(fromdate);
FormModeSearchClause.setTodate(todate);
FormModeSearchClause.setModedatacreatertype(creatertype);
FormModeSearchClause.setModedatacreater(createrid);
response.sendRedirect("CustomSearchResult.jsp?query=1&pagenum=1&customid="+customid);
%>