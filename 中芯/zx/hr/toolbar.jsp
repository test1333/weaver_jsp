		...
		<td width="100%">
    	    <TABLE style="height:100%;" cellpadding="0px" cellspacing="0px"  width="100%" align="right">
				...
				<%if(PortalUtil.isShowToMsgPage()) {%>
			        ...
			        <!-- emessage -->
			        <td onClick="window.open('https://emobile.weaver.com.cn/customerproduce.do?serverVersion=<%=version %>')" title="E-Mobile"  class="emobile"  style="width:40px;background:url(/wui/theme/ecology8/page/images/info_wev8.png) no-repeat scroll center 50%;"  class="backgroundPosition"  >
			        	<span style="display:block;" class="tbItm"></span>
			        </td>
...
<%
//信息确认
String checkdate = "";//人员信息确认起始日期
String xxsql = "select qssj from uf_qrsjwh";
//RecordSet.execute(xxsql);
//if(RecordSet.next()){
//	checkdate = Util.null2String(RecordSet.getString("qssj"));
//}
String needxxqr = "0";
if(userId == 1){
	needxxqr = "0";
	...
	int countxx =0;
	xxsql = "select count(1) as count from hrmresource where id="+userId+" and id>1700 and id not in(select ryid from uf_ryxxqr)";
	RecordSet.execute(xxsql);
	if(RecordSet.next()){
		countxx = RecordSet.getInt("count");
	}
	if(countxx>0){
		needxxqr = "1";
	}
...
<script language=javascript>  
function showxxqr(){
	var needxxqr = "<%=needxxqr%>";
	if(needxxqr == "1"){
		var diag_vote = new Dialog();
		diag_vote.Width = 600;
		diag_vote.Height = 400;
		diag_vote.Title = "人员信息确认";
		diag_vote.URL = "/zx/hr/person-info-confirm.jsp";
