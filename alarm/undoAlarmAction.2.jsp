...
<HTML>
	...
	String sql2="select area||province||sales_grp from uf_alarm_info where creat_ind ='Y' order by id desc";
	  rs.execute(sql2);
	  if(rs.next()){
	...
	<BODY>
		...
		<tr style="width:100%;">
			<td style=" width:1%"></td>
			<td  style="text-align:left; width:25%;font-size: 10pt;height:40px;">组织单位：<%=zuzhi%></td>
			<td></td>
			<td   style="text-align:left; width:25%;font-size: 10pt;height:40px;">时间区间：<%=alarmtime%></td>
			</tr>
		</table>
		<%
		String backfields = "id,alarm_id,alarm_id as alarmid,kfcode,kfname,duration,target_val,actual_val,decode(approvestatus,0,'待创建',1,'立项待提交',2,'立项提交审批',3,'立项审批拒绝',4,'立项审批通过',5,'结项提交审批',6,'结项审批拒绝',7,'结项审批通过','待创建') approvestatus,is_plan,Receipt_NO,";
		       backfields = backfields+ " case when (select count(1) from uf_action_plan where alarmid=t.alarm_id)>0  then '编辑' else '新建'end as xianshi,R_Level";
