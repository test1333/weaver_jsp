...
<HTML>
	...
	<BODY>
		...
		<%
		String backfields = "id,alarm_id,alarm_id as alarmid,kfcode,kfname,duration,target_val,actual_val,decode(approvestatus,1,'已审批','未审批') approvestatus,is_plan,Receipt_NO,";
		       backfields = backfields+ " case when (select count(1) from uf_action_plan where alarmid=t.alarm_id)>0  then '编辑' else '新建'end as xianshi,";
			   backfields = backfields+ " case when sales_grp is not null then sales_grp when province is not null  then province else area  end as province ";
		String fromSql  = " uf_alarm_info t";
		String sqlWhere = " where 1=1 and creat_ind ='Y' ";
		...
		if(!"".equals(alarmlevel)){
		  sqlWhere +=" and (sales_grp like '%"+alarmlevel+"%' or (sales_grp is null and province like '%"+alarmlevel+"%') or (sales_grp is null and province is null and area like '%"+alarmlevel+"%'))";
		}
		...
		"			<head>";
			...
			"               <col width=\"15%\" text=\"Alarm级别\" column=\"province\" orderkey=\"province\"/>"+		
			"               <col width=\"10%\" text=\"Action Plan\" column=\"xianshi\" orderkey=\"xianshi\" linkkey=\"alarmid\" linkvaluecolumn=\"alarmid\"  href=\"/alarm/getActionPlanUrl.jsp\" target=\"_fullwindow\"   />"+
		"			</head>"+
	" </table>";
	%>
