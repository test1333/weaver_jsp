				...
				<tr dbvalue="<%=valuedate %>">
					...
					<td width="8%" >
						<select style=" width:48px; height:26px;border:0px; ">
							<%=jspUtil.getOption("1,2","是,否",rs.getString("isstop")) %>
						</select>
					</td>
					<td width="20%" >
						<%
...
<script type="text/javascript">
	...
	function saveBoardDirectors() 
	{
		...
		jQuery("#saveBoardDirectorsBtnQ").attr("href","javascript:void(0);");	//不让保存重复点击
		...
		if(checkForm()){
			if (obj==undefined||obj!=2) {
				...
				if(truth4Told){
					...
					jQuery("#saveBoardDirectorsBtnQ").attr("href","javascript:saveDate(2);");	//恢复保存按钮
				}
			}
		}
		else
	...
	function viewVersion(){
		...
		if(versionids.length == 0){
			...
			jQuery.post("/cpcompanyinfo/action/CPBoardDirectorsOperate.jsp",o4params,function(data){
				...
				jQuery("#saveBoardDirectorsBtnQ").attr("href","javascript:nosaveAgain(2);");	//恢复保存按钮
