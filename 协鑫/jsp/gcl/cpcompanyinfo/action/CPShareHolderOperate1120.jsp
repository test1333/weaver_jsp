...
//		System.out.println("sql:"+strSql.toString());
		...
		pro.writeCompanylog("3",companyid,"1",user.getUID()+"","股东会");
		out.clear();
		out.print("0");
		...
		if(!ProManageUtil.checkEdition("share", "", companyid, versionnum)){
				...
				boolean ched=false;
				rs.writeLog("oldcompanyid = "+oldcompanyid+" newcompanyid="+newcompanyid);
				if(!jspUtil.CheckStr(oldcompanyid, newcompanyid)){
					//说明股东关系发生了变化，需要更新A-B公司的关系
					rs.execute("update CPCOMPANYINFOAB set status='u' ");
					//更改标志位，用于线程扫描更新数据
				...
				if(isaddversion.equals("add")){
					...
					if(maxnum.equals("") || maxnum.compareTo(versionnum)<0){
                        ...
                        while (rs.next()) {
                            old_gdgx+="股东名称:"+rs.getString("officername");
                            old_gdgx+=",法定投资比例:"+rs.getString("INVESTMENT")+"%"+"<br/>";
                        }
						...
						if (!"".equals(listgdcy)) {
							...
							for (int i = 0; i < o4JsonArr2gd.length(); i++) {
                                ...
                                new_gdgx+="股东名称:"+officername;
                                new_gdgx+=",法定投资比例:"+investment+"%"+"<br/>";
					...
					if(ched){//说明股东关系发生变化，并且产生了新的版本
						...
						rs02.execute(" insert into cpcchangenotice (c_type,c_companyid,c_year,c_month,c_time,c_desc) values(3,'"+companyid+"','"+year+"','"+month+"','"+TimeUtil.getCurrentTimeString()+"','股东版本["+versionnum+"]') ");	
					}
					if (!"".equals(listgdcy)) {
					/* 	listgdcy = URLDecoder.decode(listgdcy, "utf-8");
						org.json.JSONObject o4JsonArr2gd = new org.json.JSONObject(
