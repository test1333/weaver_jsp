...
//    System.out.println("myfrom:"+myfrom);
	...
	if("add".equals(method)||("implicenseBtn".equals( myfrom)&&"edit".equals(method))){
		...
		pro.writeCompanylog("3",companyid,"1",user.getUID()+"","新建证照" );
		out.clear();
		out.print("0");
	...
	**/
	if(!ProManageUtil.checkEdition("license", licenseid, companyid, versionnum)){
		...
		pro.writeCompanylog("3",companyid,"2",user.getUID()+"","编辑证照" );
		out.clear();
		out.print("0");
	}else{
		//版本号重复
		...
		rs.execute(strSql.toString());
		pro.writeCompanylog("3",companyid,"3",user.getUID()+"","删除证照" );
		...
		out.clear();
		out.print(jsa);
		pro.writeCompanylog("3",companyid,"4",user.getUID()+"","查看证照" );
