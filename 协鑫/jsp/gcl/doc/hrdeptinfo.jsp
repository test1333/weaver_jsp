...
<%
	...
	if(depids.length()>0){
		...
		for(int i=0;i<depidss.length;i++){
			String str = "select count(id) as con from uf_hrdept where departmentid in ("+depidss[i]+") and rid = '"+rid+"'";
			rs1.executeSql(str);
			int a = 0;
			int b = 0;//有效人数
			int d = 0;//删除后存在人数
			...
			str = "select count(id) as con from uf_hrdept where departmentid in ("+depidss[i]+") and rid = '"+rid+"' and bs = '1'";
			rs1.executeSql(str);
			if(rs1.next()){
				b = rs1.getInt("con");
			}
			if(a>0){
				str = "select count(id) as con  from hrmresource where departmentid = '"+ depidss[i] +"' and status <4  ";
				rs1.executeSql(str);
				int c = 0;//bumen 人数
				if(rs1.next()){
					c = rs1.getInt("con");
				}
				str =  "select id from uf_hrdept where departmentid in ("+depidss[i]+") and rid = '"+rid+"'  and hid  not in ( select id from hrmresource where departmentid = '"+ depidss[i] +"' and status<4  )";
				rs1.executeSql(str);
				while(rs1.next()){
				...
				str = "select count(id) as con from uf_hrdept where departmentid in ("+depidss[i]+") and rid = '"+rid+"'";
				rs1.executeSql(str);
				if(rs1.next()){
					d = rs1.getInt("con");
				}
				if(c>d){
					str = " select id from hrmresource where departmentid = '"+ depidss[i] +"' and status<4 and id  not in (select hid from uf_hrdept where departmentid in ("+depidss[i]+") and rid = '"+rid+"') ";
					rs1.executeSql(str);
					while(rs1.next()){
			...
			if(a>0 && b<1){
				str = " update uf_hrdept set bs = '1' where departmentid in ("+depidss[i]+") and rid = '"+rid+"' and bs = '0' ";
				rs1.executeSql(str);
			}
			
			
			if(a<1){
				str = "select id,lastname,workcode ,jobtitle,departmentid,subcompanyid1 from hrmresource where id in ( select id from hrmresource where departmentid = '"+ depidss[i] +"' and status<4 ) ";
    ...
    <tr>
        <td valign="top">
            <%
				...
				String str = "select id from uf_hrdept where  departmentid = '"+ depids +"' and rid = '"+rid+"'  and bs ='1'";
				String strs = "";
				rs1.executeSql(str);
				int st = 0;
				while(rs1.next()){
                ...
                String fromSql = " from uf_hrdept ";
                String sqlWhere = "  where departmentid = '"+ depids +"' and rid = '"+rid+"' ";
...
<script type="text/javascript">
	...
	function tosave(){
		...
		if(ids!=null && ids!=""){
			...
			$.ajax({
				type: "POST",
				url: "/gcl/doc/changeufinfo.jsp",
				data: {'idds': idds,'depid':<%=depids%>,'rid':<%=rid%>,'type':'0'},
				dataType: "text",
