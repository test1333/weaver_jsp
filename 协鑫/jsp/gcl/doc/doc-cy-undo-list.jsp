...
//    out.print("empid = " + emp_id+ "no="+no + " num = " + num);
    ...
    sqlqx = "select count(1) as no from hrmrolemembers where roleid in " +
            "(select roleid from systemrightroles where rightid=2019040201) and RESOURCEID" +
...
//    out.println("no = " + no);
    if (!HrmUserVarify.checkUserRight("OfficialDoc:Oper", user) &&
            no==0){
        ...
        <td valign="top">
            <div id="list1">
                <%
                    ...
                    String backfields = " v.*";
                        String fromSql = " from ( select id,"+emp_id+" as ryid,BCJZZCYFB,cllx,requestid,(select requestname from workflow_requestbase where requestid=a.requestid) as " +
                            "requestname,gwbh,fwbh,fwrq,wjbt,fwzt,tjbm,tjrfb,tjren,tjrq,(select mj from " + tablenamemj +
                            " where id=a.mj) as mjname,mj,mjts,swlcid,(select requestname from workflow_requestbase where requestid=swlcid) as requestname1," +
                                ...
                                "GWLX,(select GWLX from " + swtable + " where id=a.GWLX) as gwlxname," +
                                " (select jjcd from " + tablenamejjcd + " where id=a.jjcd) " +
                            "as jjcdname,jjcd,jjcdts,sjzzcyr,sjzzcyrq,sjzzjsrq,bcjzzcyrq,bcjzzjsrq,sjzzcyrq||' '||sjzzcysj as sjzccysj,sjzzjsrq||' '||sjzzjssj as sjzcjssj,bcjzzcyr,bcjzzjsrq||' " +
                            "'||bcjzzjssj as bcjzzjssj, bcjzzcyrq||' '||bcjzzcysj as bcjzzcysj,(select t.requestid from " +
                            wftablename + " t,workflow_requestbase t1 where t1.requestid = t.requestid and t1.currentnodetype=0" +
                            " and t.sjcyid=a.id and rownum=1) as wtjcyzlc ,(select t1.requestname from " + wftablename +
                            " t,workflow_requestbase t1 where t1.requestid = t.requestid and t1.currentnodetype=0 and " +
                            "t.sjcyid=a.id and rownum=1) as requestname2 from  " + tablename + " a) v ";
                    String sqlWhere = " cllx='0' ";
//                    if(!"".equals(wjbt)){
...
//                    out.println("select "+ backfields + fromSql + " where " + sqlWhere);
                    ...
                    tableString = " <table  tabletype=\"none\" pagesize=\"" + perpage + "\" >" +
                            ...
                            " 	<col width=\"4%\" text=\"公文标题\" column=\"wjbt\" orderkey=\"wjbt\" linkvaluecolumn=\"id\" linkkey=\"billid\" href=\"/formmode/view/AddFormMode.jsp?type=0&amp;modeId=" + modeid + "&amp;formId=" + formid + "&amp;opentype=0\" target=\"_fullwindow\"/>" +
                            ...
                            " 	<col width=\"5%\" text=\"发文提交日期\" column=\"tjrq\" orderkey=\"tjrq\" />" +
                            ...
                            " 	<col width=\"6%\" text=\"上级组织接收时间\" column=\"sjzcjssj\" orderkey=\"sjzcjssj\" />" +
                            ...
                            " 	<col width=\"7%\" text=\"未提交传阅子流程\" column=\"requestname2\" orderkey=\"requestname2\" linkvaluecolumn=\"wtjcyzlc\" linkkey=\"requestid\" href=\"/workflow/request/ViewRequest.jsp\" target=\"_fullwindow\" />" +
                            ...
                            " 	<col width=\"4%\" text=\"操作\" column=\"requestid\" orderkey=\"requestid\"  otherpara=\"column:id+column:ryid\" transmethod=\"gcl.doc.workflow.DocUtil.getCYButton\" hide=\"true\" />" +
