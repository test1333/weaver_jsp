...
<%
    ...
    RCMenuHeight += RCMenuHeightStep;
    RCMenu += "{导出报表,javascript:dc(),_self} " ;
    RCMenuHeight += RCMenuHeightStep ;
...
<FORM id=weaver name=weaver action="company-info-list.jsp" method=post >
    ...
    <button onclick="toggle(this)" id="ishideBtn"><%="1".equals(ishide)?"显示条件":"隐藏条件" %></button>
                ...
                <%
                   ...
                   // out.println("select "+ backfields + fromSql + " where " + sqlWhere);
                    ...
                    String paramWhere = URLEncoder.encode(URLEncoder.encode(sqlWhere));
...
function dc(){
    var sqlwhere = "<%=paramWhere%>";
     $.ajax({
             type: "POST",
             url: "/gcl/company/company-info-list-dc.jsp",
             data: {'sqlwhere':sqlwhere},
             dataType: "text",
             async:false,//同步   true异步
             success: function(data){
                        data=data.replace(/^(\s|\xA0)+|(\s|\xA0)+$/g, '');
                        window.open("/weaver/weaver.file.ExcelOut","_self");
                      }
         });
