...
<script type="text/javascript">
    ...
    var sfyys="#field13182";//是否有预算
    var fknd="#field13181";//付款年度
    var dqnd="#field13183";//当前年度

    var htbz = "#field10524";//合同币种
    var bcsm = "field10521";//补充说明
    var needcheck = document.all("needcheck");
    var htbz_val = "";

    var zhu_ys = "#field13381"; //付款类别 主表
    var mingx_ys = "field10537_"; //付款类别 明细表
        ...
        function cal_check(){
	      ...
	      for(var i=0;i<100;i++){
	       ...
	       else if (flag1==0 && flag2==0)
	       {
		     ...
		     jQuery(zhu_ys).val("0");
	       }else if(flag1==0&&flag0>0&&flag2>0){
		     //jQuery(zhu_ys+"span").val("付款且报销");
                      jQuery(zhu_ys).val("1");  
               }
	       else
             ...
             //  }
        	  ...
        	  if(sfyyjssplc_val == '0'){      	  	 
        	        ...
        	        if(Number(sjjsje_val) >Number(jsje_val)){
        	  	  	  Dialog.alert("实际结算金额不大于结算金额，请检查");
        	  	  	  return false;
        	         }
        	   }
...
jQuery(htbz).bindPropertyChange(function(){
	var needcheck = document.all("needcheck");
        htbz_val = jQuery(htbz).val();
		if(Number(htbz_val)==1){
			
		}else {
			jQuery("#"+bcsm+"span").html("<img src='/images/BacoError_wev8.gif' align=absmiddle>");				
			if(needcheck.value.indexOf(bcsm)<0)
			{  //如果非必填项，则加入必填项判断
				if(needcheck.value!='')
				needcheck.value += ",";
				needcheck.value += bcsm;
			}	
		}
    });
</script>
