...
<%@ page import="gvo.hr.sysn.service.SysnHrOrgWebserviceImpl"%>
...
<script language=javascript src="/js/ecology8/hrm/HrmSearchInit_wev8.js"></script>
	<script type="text/javascript">	
		...
		function onCheckup1(){
			if(window.confirm('你是否同步全量人员职务数据 pc=0？')){
...
<%
	...
	RCMenu += "{同步全量人员职务数据,javascript:onCheckup1(),_self} " ;
	RCMenuHeight += RCMenuHeightStep ;

%>
	<%@ include file="/systeminfo/RightClickMenu_wev8.jsp" %>
	<FORM id=report name=report action=/gvo/SysnJobPerson.jsp method=post>
		...
		if("1".equals(multiIds)){
			...
			log.writeLog("同步全量数据 SysnJobPerson start");
			JSONArray  resultArr = new JSONArray();
			  SysnHrOrgWebserviceImpl aa = new SysnHrOrgWebserviceImpl();
			  doSysPersonJobRelation(resultArr, "1","0");
		      doSysPersonJobRelation(resultArr, "2","0");
			log.writeLog("同步全量数据 SysnJobPerson end:"+resultArr.toString());
			boolean isOver = true;
			if(isOver){
...
<wea:group context="同步矩阵表" attributes="test">
