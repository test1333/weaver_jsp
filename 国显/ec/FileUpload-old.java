...
//        return Util.null2String(mpdata.getParameter(key)) ;
        ...
        try {
            return new String ( Util.null2String(mpdata.getParameter(key)).getBytes("ISO8859_1") , "GBK");
    ...
    public String[] getParameters(String key) {
        ...
        return mpdata.getParameterValues(key);
    }
    ...
    public String[] getParameterValues(String name) {
        ...
        if (mpdata == null) return null;
        return mpdata.getParameterValues(name);      
    }
    ...
    private synchronized String saveFileToEmail(String uploadname , MultipartRequest mpdata)  {
        ...
        String para = "" + separator + originalfilename + separator
                      + contenttype + separator  + filerealpath + separator
                      + iszip + separator  + isencrypt + separator + isfileattrachment + separator 
                      + fileContentId + separator +isEncoded + separator 
					  + String.valueOf(filesize)+separator+timeMillis;

        rs.executeProc("MailResourceFile_Insert",para);
     
        rs.execute("select id from MailResourceFile where timeMillis = '"+timeMillis+"'");
        if(rs.next()){
...
//for license upload by chenyingjie 2003-06-26
    ...
    private synchronized String saveFile(String uploadname , MultipartRequest mpdata)  {
        ...
        // 更新加密信息
        rs.execute("update imagefile set isaesencrypt="+isaesencrypt+", aescode='"+aescode+"' where imagefileid="+imageid);
