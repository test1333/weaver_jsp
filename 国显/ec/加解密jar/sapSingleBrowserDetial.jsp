								...
								<form action="/integration/sapSingleBrowserDetial.jsp" name="weaver" method="post" >
										...
										if(listAllSerch!=null){//输出隐藏的输入参数或者查询条件。 解决当输入参数和输入结构不勾选显示的时候，点击搜索无法出数据，以及无法获取下一页的数据问题。qc:201915
											for(int i=0;i<listAllSerch.size();i++){
												...
												if(s.getSearch()==0){
													//隐藏的搜索条件
													%>
													<input type='hidden' name='<%=s.getSapfield()%>' value='<%=s.getSapfieldvalue()%>' id="<%=s.getFromfieldid()%>" class='styled input'   saptype='<%=s.getSerchtype()%>'>
													<%
												}
											}
										%>
										...
										<%	
											for(int i=0;i<listAllSerch.size();i++){
												...
												<wea:item><input type='text' name='<%=s.getSapfield() %>' value='<%=s.getSapfieldvalue() %>'id='<%=s.getFromfieldid()%>' class='styled input'   saptype='<%=s.getSerchtype()%>'></wea:item>
												<%
