...
function getajaxurl(typeId,inputIdOrName,sqlwhere,fieldbodyid,roleid){
		...
		if(typeId==22){
			...
			if(window._FnaSubmitRequestJsFlag||true){
				if(window.dt1_subject && __fieldId==window.dt1_subject
					...
					var __orgType = jQuery("#field"+window.dt1_organizationtype2+"_"+__dtIdx).val();
					if(jQuery("#field"+window.dt1_organizationtype2+"_"+__dtIdx).length==0){
						__orgType = jQuery("#disfield"+window.dt1_organizationtype2+"_"+__dtIdx).val();
					}
					var __orgId = jQuery("#field"+window.dt1_organizationid2+"_"+__dtIdx).val();
	...
	if(typeId==161||typeId==162){
		return getUrlPara(url, inputIdOrName, fieldbodyid);
	}else{
		return url;
	}
}

function getUrlPara(url, fielddbtype, fieldid) {
		               ...
		               if (type1 == 162) {
							...
							for (var _i=0; _i<idArray.length; _i++) {
								...
								if(href==''){
									...
									sHtml += wrapshowhtml0($G(_fieldStr+id).getAttribute("viewtype"), "<a title='" + curdesc + "' href='" + href + curid + "' target='_blank'>" + curname + "</a>&nbsp", curid);
								}
				...
				for ( var _i = 0; _i < shareTypeValueArray.length; _i++) {
					...
					if (shareTypeValue == "1") {
						...
						sHtml = sHtml
								...
								+ relatedShareNames
								+ ")"
								+ SystemEnv.getHtmlNoteName(4090,languageid)+"="
								+ rolelevelText
								+ SystemEnv.getHtmlNoteName(4095,languageid)
								+ secLevelValue
