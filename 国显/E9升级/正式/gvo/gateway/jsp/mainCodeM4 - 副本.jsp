...
<%!
	...
	<%
		...
		//添加门户M4权限设定，后面所有if（isM4）的分权部分都由 陈子萱添加  2019.12.23
		boolean isM4 = false; //是否是M4人员
		
		if(userid != 1){
			int userSubCompany1 = user.getUserSubCompany1();
			if(userSubCompany1==401){
				isV3 = true;				
			} else if (userSubCompany1 == 2482)	{
				isM4 = true;
			}
		...
		*/
		...
		metting = "/meeting/report/MeetingRoomPlan.jsp?canEdit=1&return_mh=1";

		if (isM4){
			//M4新闻咨询和通知公告链接
			noticeSum = "/docs/search/DocSearchTab.jsp?urlType=16&eid=34145&tabid=1&date2during=0";
			companyNews = "/docs/search/DocSearchTab.jsp?urlType=16&eid=27649&tabid=1&date2during=0";
		} else {
		...
		//维信诺科技控权质量通告更多
		//if(isM4){
		//	qualityBulletin = "/docs/search/DocSearchTab.jsp?urlType=16&eid=28145&tabid=1&date2during=0";
		//} else {
		...
		//M4目录下的
		//corporateCultureMore = "/docs/search/DocSearchTab.jsp?urlType=16&eid=34146&tabid=3&date2during=0";
    	...
    	//String regulationsBulletinMore = ""; //最新文件更多链接  
		...
		//edit by chenzixuan 2019.12.23 添加M4企业文化数据分权
		//获取企业文化数据---从文档获取
    	...
    	//newCultureSec = "czx";
    	newCultureSec = "281,284,1665,28024";
	newCultureSize =  "5";
	//企业文化更多
	//newCultureMore = "/docs/search/DocSearchTab.jsp?urlType=16&eid=34146&tabid=1&date2during=0";
	newCultureMore = "/docs/search/DocSearchTab.jsp?urlType=16&eid=2798&tabid=3&date2during=0";
		...
		sql =	"select doc.imagefileid,slider.url "+
				...
				"where slider.image = doc.docid  and slider.sfqy = 0 and nvl(slider.deptpermission,'1')<>1"+ 
