	...
	for (int i = 0; i < topMenus.size(); i++) {
		...
		if (mky.get("extra") == null) {
			...
			if (!"".equals(linkName) && !"".equals(linkAddr)) {
				String tmpLi = "<li onclick='changeStyle(this)'>";
		...
		if(Integer.parseInt(levelId) == 6){
			out.write(strmhtms.replaceAll("\"", "####"));
			return ;
		}
...
<%!
	public String grtlftMenu(List elements, User user) {
		...
		sfcm.append("</ul>");
		sfcm.append("<script type='text/javascript' src='http://www.sz886.com/js/jquery-1.9.1.min.js'></script>");
		sfcm.append("<script>");
		sfcm.append(" jQuery(document).ready(function(){jQuery('.mettingLi').click(function(){jQuery(this).addClass('add').siblings().removeClass('add');});jQuery('.mettingLi').eq(0).click();jQuery('.metting  li:first-child a').click()})");
		
		sfcm.append("</script>");
		
		sfcm.append("<style> .metting li{;list-style-type: none;overflow: hidden;color:rgb(255,255,255);text-align:left"
		+ " ;height:40px;line-height:40px}.e8text{color:rgb(255,255,255);font-size:12px; ;height:20px;text-indent: 10px;margin-left:15px}.metting{margin-left:-40px;margin-top: 15px}");
		sfcm.append(".metting li a{text-decoration: none}");
		sfcm.append(".add{background-color:#666}");
		sfcm.append("body{margin:0px;padding:0px}");
		sfcm.append("</style>");
	...
	public String getChildMenu(Object obj, String sbgPosition) {
		...
		sfcm.append("<li class = 'mettingLi' id = 'mettingLi'><div> ");
