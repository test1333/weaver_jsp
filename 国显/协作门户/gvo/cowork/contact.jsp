...
String accsize ="8";
String accsec="19024";//目录测14024 正5801
String attach="";
			...
			var oUpload;
				...
				window.onload = function() {
				  var settings = {
						...
						button_image_url : "/gvo/cowork/img/sc3.png",	// Relative to the SWF file
...
<body>
			...
			<div class = "contact_info">
				...
				<div  class = "relative">
					...
					<%
						String address = "";
						String ms = "";
						sql = "select address,ms from uf__co_contact_mt where type=0 order by xh asc";
						rs.execute(sql);
						if(rs.next()){
							address = Util.null2String(rs.getString("address"));
							ms = Util.null2String(rs.getString("ms"));
					%>
						<a style='color:#0000EE' href = "mailto:<%=address%>" class = "contactus" id = "contactus"><%=address%></a>
				...
				<div class = "relative">
					联系方式：
						<%
						String mtid = "";
						sql = "select id,address,ms from uf__co_contact_mt where type=1 order by xh asc";
						rs.execute(sql);
						if(rs.next()){
				...
				<%
						...
						while(rs.next()){
							...
							ms = Util.null2String(rs.getString("ms"));
					%>
						<div class = "contactus it relative">
							<span class = "contactus"><%=address%></span>
							<span class= "contactus_info"><%=ms%></span>
						</div>
					<%
	...
	<input type="hidden" name="attach" id="attach" value="<%=attach %>">
					<div class = "operation">
						...
						<div class = "submit" style="width:60px;">提交</div>	
					</div>	
					...
					<div style="margin-left: 0px;margin-top: 20px">
									...
									<span style="color:#262626;cursor:hand;TEXT-DECORATION:none" disabled onclick="oUpload.cancelQueue();" id="btnCancel1">
										...
										<span><font style="color:#808080;font-size:10pt">&nbsp;&nbsp;最大8M/个</font></span>
									</span>
								</div>
...
<script type="text/javascript">
	...
	if(Number(jQuery("#contactus").width()) >100){
			jQuery(".contactus_info").css("right",Number(jQuery(window).width()-650-jQuery("#contactus").width())+"px");
		}else{
			jQuery(".contactus_info").css("right",jQuery(window).width()-750+"px");
		}
