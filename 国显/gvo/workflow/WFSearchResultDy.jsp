...
<FORM id=weaver name=frmmain method=post action="WFSearchResultDy.jsp">
...
<%}%>
  ...
  <tr>
    ...
    <td class=field>
	  <input type="text" name="request_id_tmp" value="<%=request_id_tmp%>">
    </td></TD>
    	<td>&nbsp;</td> 
    	 <td><%=SystemEnv.getHtmlLabelName(1334,user.getLanguage())%></td>
    <td class=field>
	  <input type="text" name="request_name_tmp" value="<%=request_name_tmp%>">
    </td></TD>
    	<td>&nbsp;</td> 
	    </TR>
  </tbody>
...
<FORM id=weaver1 name=frmmain1 method=post action="/gvo/workflow/WFSearchResultDy.jsp">
                      ...
                      <td valign="top">                                                                                    
                          <%
                           ...
                           // sqlWhere += " and t1.requestid not in (select requestid from workflow_requestbase wr where wr.requestname like '会议%' and wr.workflowid=1) "
                            ...
                            // 设置打印节点信息
                            sqlWhere += " and t2.nodeid in(select id from workflow_nodebase where nodename in(select czjd from formtable_main_189 ) ";
                            sqlWhere += "and id in(select nodeid from workflow_flownode  where workflowid in(select dylcmc from formtable_main_189)))";
...
//	alert("批量打开，打开请求为 "+_xtable_CheckedCheckboxId());
	...
	//	div_str = "http://222.92.108.195:8082/workflow/request/ViewRequest.jsp?requestid="+all_reqs[i];
		div_str = div_str + " <a id=\""+tmp_id+"\" href=\"/workflow/request/ViewRequest.jsp?requestid="+all_reqs[i]+"\" target=\"_blank\">a1</a>";
	}
