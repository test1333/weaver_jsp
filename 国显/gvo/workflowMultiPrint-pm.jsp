...
<HTML>
	...
	String lastoperator = Util.null2String(request.getParameter("lastoperator"));
	...
	String lastbegindate = Util.null2String(request.getParameter("lastbegindate"));
	String lastenddate = Util.null2String(request.getParameter("lastenddate"));
	...
	<BODY>
		...
		<FORM id=report name=report action="/gvo/workflowMultiPrint-pm.jsp" method=post>
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;">
				...
				<wea:item>节点处理人</wea:item>
				<wea:item>
				<brow:browser viewType="0" name="lastoperator" browserValue='<%=lastoperator+""%>' 
										browserUrl="/systeminfo/BrowserMain.jsp?url=/hrm/resource/ResourceBrowser.jsp"
										hasInput="true" isSingle="true" hasBrowser = "true" isMustInput='1'
										completeUrl="/data.jsp" linkUrl="javascript:openhrm($id$)" 
										browserSpanValue='<%=ResourceComInfo.getResourcename(lastoperator+"")%>'></brow:browser>
				</wea:item>		
			...
			<wea:item>最后修改日期</wea:item>
			<wea:item>
				 <span>从 </span>
				 <span id="wpbegindate1"  >
					<button type="button" class="Calendar" id="SelectBeginDate1" onclick="getDate(begindatespan1,lastbegindate)"></BUTTON> 
					<SPAN id="begindatespan1"><%=lastbegindate%></SPAN> 
					<INPUT type="hidden" name="lastbegindate" value="<%=lastbegindate%>">  				  		
				</span>
				<span> 到 </span>
				<span id="wpenddate1"  >
					<button type="button" class="Calendar" id="SelectEndDate1" onclick="getDate(enddatespan1,lastenddate)"></BUTTON> 
					<SPAN id="enddatespan1"><%=lastenddate%></SPAN> 
					<INPUT type="hidden" name="lastenddate" value="<%=lastenddate%>">  				  		
				</span>
			</wea:item>	
		...
		<%
		String backfields = "a.requestid,b.creatertype,b.requestname,b.workflowid,b.lastoperator,b.createdate, b.createtime, a.bxdh ,a.sqrq, a.sqr, case when b.currentnodetype >=3 then '归档' else '未归档' end as isdone ,b.lastoperatedate ";
			  
		String fromSql  = " v_kvi_print a join workflow_requestbase b on a.requestid=b.requestid ";
		String sqlWhere = " where 1=1 ";
		String orderby = " b.lastoperatedate,b.lastoperatetime desc";
		if(!"".equals(bxdh)){
		...
		sqlWhere += " and b.lastoperator = '"+lastoperator+"' ";
		}
		if(!"".equals(begindate)){
		...
		sqlWhere += " and b.workflowid = '"+workflowid+"' ";
		}
		
		if(!"".equals(lastbegindate)){
			sqlWhere +=" and b.lastoperatedate>='"+lastbegindate+"' ";
			if(!"".equals(lastenddate)){
				sqlWhere +=" and b.lastoperatedate<='"+lastenddate+"' ";
			}
		}else{
			if(!"".equals(lastenddate)){
				sqlWhere +=" and b.lastoperatedate<='"+lastenddate+"' ";
...
//out.print("select " +backfields +" from "+fromSql+sqlWhere+" order by"+orderby );
		...
		"			<head>";
			...
			"               <col width=\"8%\" text=\"节点处理人\" column=\"lastoperator\" orderkey=\"lastoperator\" otherpara=\"column:creatertype\" transmethod=\"weaver.general.WorkFlowTransMethod.getWFSearchResultName\" />"+
