...
<html>
	...
	int c_num = 0;
	int v_num = 0;
	String tmp_sql1 = " select count(id) as v_num  from formtable_main_258 where xm = "+emp_id;
	rs.executeSql(tmp_sql1);
	new BaseBean().writeLog("sql---ispxadmin------" + tmp_sql1);
	...
	String tmp_sql2 = " select count(id) as c_num  from formtable_main_257 where glyxm = " + emp_id;
	rs1.executeSql(tmp_sql2);
	new BaseBean().writeLog("sql---isExist------" + tmp_sql2);
	//out.println("emp_id="+ emp_id);
	if(rs1.next()){
	c_num = rs1.getInt("c_num");
										...
										<tr>
											<td valign="top">
												...
												tableString =" <table  tabletype=\"none\" pagesize=\""+perpage+"\" >"+
													...
													"			<head>"+
														"				<col width=\"38%\" text=\"课程名称\" column=\"kc_name\" orderkey=\"kc_name\"  />"+
