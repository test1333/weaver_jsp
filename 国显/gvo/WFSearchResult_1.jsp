...
String tmp_userid = Util.null2String(request.getParameter("userid"));
//out.println("tmp_userid = " + tmp_userid + ";fromself = " + fromself);


//START 判断帐号是否有主次帐号 by alan
String accounts = "";
String accountSql = "select id from hrmresource ";
	accountSql += "where id="+tmp_userid; 
	accountSql += " or id=(select belongto from hrmresource where id="+tmp_userid+") ";
	accountSql += " or (belongto="+tmp_userid+" and status in(0,1,2,3)) ";
	accountSql += " or (belongto=(select belongto from hrmresource where id="+tmp_userid+" and belongto>0) and status in(0,1,2,3))";
...
if(isovertime==1){
    ...
    int perpage=10;
    RecordSet.executeProc("workflow_RUserDefault_Select",""+tmp_userid);
    if(RecordSet.next()){
...
if(!"".equals(tmp_userid)&&!"1".equals(tmp_userid)){
