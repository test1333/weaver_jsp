...
//保安登录界面
int num_1 = 0;
String sql = " select count(id) as num_1 from formtable_main_127 where dlzh='"+loginid+"' and dlmm='"+userpassword+"'";
sql = " select count(id) as num_1 from formtable_main_127 where dlzh='"+loginid+"'";
rs.executeSql(sql);

if(rs.next()){
	num_1 = rs.getInt("num_1");
	if(num_1>0){
		sql = " select count(id) as num_1 from formtable_main_127 where dlzh='"+loginid+"' and dlmm='"+userpassword+"'";
		rs.executeSql(sql);
		if(rs.next()){
			num_1 = rs.getInt("num_1");
			if(num_1>0) {
				response.sendRedirect("menweilogin.jsp");
				return;
			}
		}
	}
}
String tmp_info  = "('taoyuan','xuly')";
// 部分人员之间跳转
sql = " select count(id) as num_1 from hrmresource where status in(0,1,2,3) and loginid='"+loginid+"' and loginid in"+tmp_info;
rs.executeSql(sql);
if(rs.next()){
	num_1 = rs.getInt("num_1");
	if(num_1>0){
		String tmp_passwd = Util.getEncrypt(userpassword);
		sql = " select id from hrmresource where status in(0,1,2,3) and loginid='"+loginid+"' and password='"+tmp_passwd+"'";
		rs.executeSql(sql);
		if(rs.next()){
			num_1 = rs.getInt("id");
			if(num_1>0) {
				response.sendRedirect("menweilogin_1.jsp?uid="+num_1);
				return;
			}
		}
	}
...
try {
	if(document.cookie.indexOf('logincookiecheck=<%=testcookie%>')==-1) {
		...
		String comid = "";
		if(ryid != 1){
			sqlry = "select id,supsubcomid from hrmsubcompany where id=(select subcompanyid1 from hrmresource where id="+ryid+")";
			rs.executeSql(sqlry);
			if(rs.next()){
		...
		%>
		var comid = "<%=comid%>";
		if(comid == "301" || comid == "302" || comid == "401"){
			 document.location.href = '<%=tourl%>';
		}else{
			document.location.href='/login/RemindLogin.jsp?RedirectFile=/gvo/gateway/main.jsp';
		}
		//
	}
}
