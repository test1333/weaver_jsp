...
if(!"0".equals(checkstatus)){
RCMenu += "{退回,javascript:backRecord(),_self} " ;
RCMenuHeight += RCMenuHeightStep ;
}
if("1".equals(checkstatus)){
...
%>
	...
	String sqlWhere = "";
     ...
     backfields = " id ,ygbm,ygz,khny,gh,ygxm,jxfs,case when jxdj='0' then 'A' when jxdj='1' then 'B' else '' end jxdj,jxje,bzsm ,bmzg,bmjl,seqno,(select jobtitlename from hrmjobtitles a, hrmresource b where a.id=b.jobtitle and b.id=ygxm) as zwmc "; 
...
tableString =" <table tabletype=\"checkbox\" pagesize=\""+ PageIdConst.getPageSize(tmc_pageId,user.getUID(),PageIdConst.HRM)+"\" >"+
    ...
    operateString+
    "			<head>";
		tableString+="<col width=\"10%\"  text=\"ID\" column=\"id\" orderkey=\"id\"  hide=\"true\" />"+
		"<col width=\"10%\"  text=\"员工部门\" column=\"ygbm\" orderkey=\"ygbm\"  transmethod=\"weaver.hrm.company.DepartmentComInfo.getDepartmentname\"/>"+
		...
		"<col width=\"5%\"  text=\"职位名称\" column=\"zwmc\" orderkey=\"zwmc\" hide=\"true\"/>"+
...
%>
	<wea:SplitPageTag isShowTopInfo="false" tableString="<%=tableString%>" mode="run" showExpExcel="<%=flag11%>" />
	...
	function daoru() {
		var title = "导入";
		var url = "/systeminfo/BrowserMain.jsp?url=/gvo/check/ModeForward.jsp?typex=daoru";
		// var url = "/systeminfo/BrowserMain.jsp?url=/seahonor/crm/ModeForward.jsp?typex=custom,,";
	...
	function commitALL(){
		...
		var checkresult=submitcheck(status,userid,dqny,checkbmzg);
		if(checkresult !=''){
		alert(checkresult);
		 return;
		}
	...
	function submitcheck(status,userid,dqny,checkbmzg){
			...
			//  alert(status);
				...
				if (null != xhr) {
					xhr.open("GET","/gvo/check/submitcheck.jsp?dqny="+dqny+"&status="+status+"&userid="+userid+"&checkbmzg="+checkbmzg, false);
					xhr.onreadystatechange = function () {
						if (xhr.readyState == 4) {
							if (xhr.status == 200) {
								var text = xhr.responseText;
