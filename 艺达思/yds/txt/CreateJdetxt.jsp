...
<%
	...
	RCMenu += "{生成JDE文件,javascript:onCheckup1(),_self} " ;
	RCMenuHeight += RCMenuHeightStep ;

%>
	<%@ include file="/systeminfo/RightClickMenu_wev8.jsp" %>
	<FORM id=report name=report action=/yds/txt/CreateJdetxt.jsp method=post>
		...
		if("1".equals(multiIds)){
			CreateJDEFileAction aa = new CreateJDEFileAction();
			 int count=aa.ceateJDEFile()-1;
			boolean isOver = true;
			if(isOver){
				out.println("<font size=\"5px\" color=\"red\">执行完成，创建成功! 行数:"+count+"行 </font>");
			}else{
				out.println("<font size=\"5px\" color=\"red\">执行完成，创建失败，请联系管理员</font>");
			}
		}
...
<wea:group context="创建JDE文件" attributes="test">

<wea:item>&nbsp;&nbsp;</wea:item><wea:item>&nbsp;&nbsp;</wea:item><wea:item>&nbsp;&nbsp;</wea:item>
		<wea:item>
		<table width="30%">
			<tr>
				<td><input name="hi1" type="button" value="创建JDE文件" class="e8_btn_top_first" style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis;  max-width: 100px;width:100px;height:30px;font-size:15px;" onClick="onCheckup()"></td>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
