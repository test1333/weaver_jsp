	    ...
	    <wea:group context='<%=SystemEnv.getHtmlLabelName(32905,user.getLanguage())%>'><!-- 常用条件 -->
		    ...
		    <wea:item>
		    	<span id="spanMQ" style="display: none;">
			    	<input id="mQ1" name="mQ1" value="1" <%="1".equals(mQArray[0])?"checked":"" %> type="checkbox" /><%=SystemEnv.getHtmlLabelName(1492,user.getLanguage())%>
			    	<input id="mQ2" name="mQ2" value="1" <%="1".equals(mQArray[1])?"checked":"" %> type="checkbox" /><%=SystemEnv.getHtmlLabelName(1493,user.getLanguage())%>
			    	<input id="mQ3" name="mQ3" value="1" <%="1".equals(mQArray[2])?"checked":"" %> type="checkbox" /><%=SystemEnv.getHtmlLabelName(1494,user.getLanguage())%>
			    	<input id="mQ4" name="mQ4" value="1" <%="1".equals(mQArray[3])?"checked":"" %> type="checkbox" /><%=SystemEnv.getHtmlLabelName(1495,user.getLanguage())%>
			    	<input id="mQ5" name="mQ5" value="1" <%="1".equals(mQArray[4])?"checked":"" %> type="checkbox" /><%=SystemEnv.getHtmlLabelName(1496,user.getLanguage())%>
			    	<input id="mQ6" name="mQ6" value="1" <%="1".equals(mQArray[5])?"checked":"" %> type="checkbox" /><%=SystemEnv.getHtmlLabelName(1497,user.getLanguage())%>
			    	<br />
			    	<input id="mQ7" name="mQ7" value="1" <%="1".equals(mQArray[6])?"checked":"" %> type="checkbox" /><%=SystemEnv.getHtmlLabelName(1498,user.getLanguage())%>
			    	<input id="mQ8" name="mQ8" value="1" <%="1".equals(mQArray[7])?"checked":"" %> type="checkbox" /><%=SystemEnv.getHtmlLabelName(1499,user.getLanguage())%>
			    	<input id="mQ9" name="mQ9" value="1" <%="1".equals(mQArray[8])?"checked":"" %> type="checkbox" /><%=SystemEnv.getHtmlLabelName(1800,user.getLanguage())%>
			    	<input id="mQ10" name="mQ10" value="1" <%="1".equals(mQArray[9])?"checked":"" %> type="checkbox" /><%=SystemEnv.getHtmlLabelName(1801,user.getLanguage())%>
			    	<input id="mQ11" name="mQ11" value="1" <%="1".equals(mQArray[10])?"checked":"" %> type="checkbox" /><%=SystemEnv.getHtmlLabelName(1802,user.getLanguage())%>
			    	<input id="mQ12" name="mQ12" value="1" <%="1".equals(mQArray[11])?"checked":"" %> type="checkbox" /><%=SystemEnv.getHtmlLabelName(1803,user.getLanguage())%>
		    	</span>
...
<%
	...
	//设置好搜索条件
	String backFields =" *,approvalAmt+actualAmt sqje ";
	...
	tableString=""+
       ...
       " />"+
       "<head>"+
			"<col width=\"13%\"  text=\""+SystemEnv.getHtmlLabelName(18748,user.getLanguage())+"\" column=\"orgId\" "+//预算单位
					" transmethod=\"weaver.fna.general.FnaSplitPageTransmethod.getOrgName\" otherpara=\"column:orgType\" "+
					" />"+
			"<col width=\"10%\"  text=\""+SystemEnv.getHtmlLabelName(18648,user.getLanguage())+"\" column=\"feeperiod\" "+//预算期间
					...
					" />"+
			subjectId_column+			
			"<col width=\"9%\"  text=\""+SystemEnv.getHtmlLabelName(82644,user.getLanguage())+"\" column=\"budgetAmt1\" "+//上期结余预算数
					" transmethod=\"weaver.fna.general.FnaSplitPageTransmethod.fmtAmount\" "+
					" />"+
			"<col width=\"9%\"  text=\""+StringEscapeUtils.escapeXml(titleBudgetAmt2)+"\" column=\"budgetAmt2\" "+//本期原预算数
					" transmethod=\"weaver.fna.general.FnaSplitPageTransmethod.fmtAmount\" "+
					" />"+
			"<col width=\"10%\"  text=\""+SystemEnv.getHtmlLabelName(82616,user.getLanguage())+"\" column=\"budgetAmt\" "+//本期预算总数
					" transmethod=\"weaver.fna.general.FnaSplitPageTransmethod.fmtAmount\" "+
					" />"+
			"<col width=\"9%\"  text=\""+SystemEnv.getHtmlLabelName(18769,user.getLanguage())+"\" column=\"approvalAmt\" "+//审批中费用
					" transmethod=\"weaver.fna.general.FnaSplitPageTransmethod.fmtAmount\" "+
					" />"+
			"<col width=\"9%\"  text=\""+SystemEnv.getHtmlLabelName(82510,user.getLanguage())+"\" column=\"actualAmt\" "+//实际发生数
					" transmethod=\"weaver.fna.general.FnaSplitPageTransmethod.fmtAmount\" "+
					" />"+
			"<col width=\"9%\"  text=\""+SystemEnv.getHtmlLabelName(82511,user.getLanguage())+"\" column=\"availableAmt\" "+//可用预算数
					" transmethod=\"weaver.fna.general.FnaSplitPageTransmethod.fmtAmount\" "+
					" />"+
"<col width=\"9%\"  text=\"申请金额\" column=\"sqje\" "+//实际发生数
					" transmethod=\"weaver.fna.general.FnaSplitPageTransmethod.fmtAmount\" "+
					" />"+
       "</head>"+
       "</table>";
...
function getErrorInfoAjax(_guid1){
	if(_getErrorInfoAjax){
		...
		jQuery.ajax({
			...
			cache : false,
			processData : false,
			data : "_guid1="+_guid1+"&r="+$System.Math.IntUtil.genGUIDV4(),
			dataType : "html",
			success: function do4Success(_html){
	    		if(_html!=""){
    				_guid1 = $System.Math.IntUtil.genGUIDV4();
    				jQuery("#_guid1").val(_guid1);
	    			top.Dialog.alert(_html, null, 400, 400);
	    		}else{
	    			jQuery("#qryFunctionType").val("1");
    				jQuery("#_guid1").val(_guid1);
	    			form2.submit();
