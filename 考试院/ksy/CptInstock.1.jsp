...
<wea:layout attributes="{'expandAllGroup':'true'}">
		...
		<% 
		 ...
		 String sql="select a.requestid,b.id,a.id as mainid ,a.sqr,a.tbrq,a.bh,b.hpmc,(select name from cptcapital where id=b.hpmc) as name,b.ggxh,b.dw ,(select unitname from LgcAssetUnit where id= b.dw) as dwname,b.sgsl,b.dj,b.je "+
		 ...
		 RecordSet rs1 = new RecordSet();
		  ...
		  while(rs1.next()){
	        ...
	        sgsl = Util.null2String(rs1.getString("sgsl"));
			hpmc = Util.null2String(rs1.getString("hpmc"));
			cptid = Util.null2String(rs1.getString("id"));
			 num++;
			  %>
			   <wea:item><%=name%> <input name="name_<%=num%>" id="name_<%=num%>"  type="hidden" value="<%=hpmc%>" /><input name="cptid_<%=num%>" id="cptid_<%=num%>"  type="hidden" value="<%=cptid%>" /></wea:item>			
			   <wea:item><%=ggxh%><input name="ggxh_<%=num%>" id="ggxh_<%=num%>" type="hidden" value="<%=ggxh%>" /></wea:item>
			   <wea:item><%=dj%><input name="dj_<%=num%>" id="dj_<%=num%>" type="hidden" value="<%=dj%>" /></wea:item>			    
			   <wea:item><%=sgsl%><input name="sgsl_<%=num%>" id="sgsl_<%=num%>" type="hidden" value="<%=sgsl%>" /></wea:item>
