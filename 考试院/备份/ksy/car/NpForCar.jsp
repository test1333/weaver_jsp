...
<wea:layout type="4col">
	...
	String t_sql = "";
	if("a".equals(type)){
		t_sql = "select (select carno from CarInfo where id=carid) as name,count(id)  as count_cc,'"+begindate+"' as bd,'"+enddate+"' as ed "
			+" from CarUseApprove m where "+date_sql+" group by carid ";
		...
		t_sql = "select (select departmentname from hrmdepartment where id=h.departmentid) as name ,count(m.id) as count_cc,'"+begindate+"' as bd,'"+enddate+"' as ed "
			+" from CarUseApprove m join hrmresource h on h.id=m.userid "
...
tableString =" <table tabletype=\"checkbox\" pagesize=\""+ PageIdConst.getPageSize(PageIdConst.HRM_ONLINEUSER,user.getUID(),PageIdConst.HRM)+"\" pageId=\""+PageIdConst.HRM_ONLINEUSER+"\">"+
    ...
    "			<head>";                                                                                                                                                  
		tableString+="";
	if("a".equals(type)){
	  tableString+="				<col width=\"30%\" text=\"车辆\" column=\"name\" orderkey=\"name\"  />";
	}else if("d".equals(type)){
