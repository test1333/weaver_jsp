...
<HTML>
	...
	String Application=Util.null2String(request.getParameter("Application"));//申请人
	String ApplicationDate=Util.null2String(request.getParameter("ApplicationDate"));//申请日期
	int userid = Util.getIntValue(request.getParameter("userid"));//登录人用户名	
	...
	<BODY>
		...
		<FORM id=report name=report action=UsingBadge.jsp method=post>
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;">
				...
				<wea:item>
				<brow:browser viewType="0"  name="Application" browserValue="<%=Application %>" 
				browserOnClick=""
				browserUrl="/systeminfo/BrowserMain.jsp?url=/hrm/resource/ResourceBrowser.jsp?selectedids="
				hasInput="true" isSingle="false" hasBrowser = "true" isMustInput='1'
				completeUrl="/data.jsp" width="165px"
				browserSpanValue="<%=Util.toScreen(ResourceComInfo.getResourcename(Application),user.getLanguage())%>">
				</brow:browser>
				</wea:item>
				</wea:group>
				<wea:group context="">
		...
		String backfields = " id as idkey,requestid,(select requestname from workflow_requestbase wr where wr.requestid=f.requestId) as request_name,Application,ApplicationDate,ChapterType,SealNumber,UsingDocument,(len(convert(varchar(800),UsingDocument)) - len(replace(convert(varchar(800),UsingDocument), ',', '')) + 1)as len ";
		String fromSql  = " from  formtable_main_82 f ";
		//String sqlWhere = " 1=1 ";
		String sqlWhere = " Application="+userid;
		...
		String orderby = " ApplicationDate " ;
		String tableString = "";

		if(!Application.equals("")) 
		sqlWhere+=" and Application="+Application;

		//这是一个时间段日期的比较。effeStartDate是选择的开始日期  invalidDate选择的结束日期 DueDate需要比较的收件日期
		if(!"".equals(effeStartDate)){
			sqlWhere +=" and ApplicationDate>='"+effeStartDate+"' ";
			if(!"".equals(invalidDate)){
				sqlWhere +=" and ApplicationDate<='"+invalidDate+"' ";
			}
		}else{
			if(!"".equals(invalidDate)){
				sqlWhere +=" and ApplicationDate<='"+invalidDate+"' ";
		...
		String  operateString=  "<operates width=\"20%\">"+
		                    ...
		                    "     <operate isalwaysshow=\"true\" href=\"javascript:differenceHandle();\" linkkey=\"idkey\" linkvaluecolumn=\"idkey\" text=\"用印文档\" index=\"0\"/>"+
		                    "</operates>";
		...
		tableString+="<col width=\"25%\" labelid=\"18104\" text=\"流程名称\" column=\"request_name\" orderkey=\"requestid\" linkvaluecolumn=\"requestid\" linkkey=\"requestid\" href=\"/workflow/request/ViewRequest.jsp\" target=\"_fullwindow\" />"+	
			"		<col width=\"10%\" labelid=\"368\" text=\"申请人\" column=\"Application\" orderkey=\"Application\" transmethod=\"weaver.hrm.resource.ResourceComInfo.getResourcename\" />"+
			"		<col width=\"10%\" labelid=\"855\" text=\"申请日期\" column=\"ApplicationDate\" orderkey=\"ApplicationDate\" />"+	
			"		<col width=\"10%\" labelid=\"-9642\" text=\"用印类型\" column=\"ChapterType\" orderkey=\"ChapterType\" transmethod=\"seahonor.info.SealInfo.getTreeSealName\"  />"+
			"		<col width=\"10%\" labelid=\"-9641\" text=\"盖章份数\" column=\"SealNumber\" orderkey=\"SealNumber\" />"+
			"		<col width=\"10%\" labelid=\"33343\" text=\"用印文档数量\" column=\"len\" orderkey=\"len\" />"+
		"			</head>"+
	" </table>";
	%>
	...
	function onShowSubcompanyid1(){
	 ...
	 //差异处理
		...
		var url = "";
		title = "用印文档";
		url="/seahonor/badge/jsp/UsingBadgeDocumentAttend.jsp?idkey="+idkey+"";
