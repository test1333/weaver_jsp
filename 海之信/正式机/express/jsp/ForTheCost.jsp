...
<HTML>
	...
	<BODY>
		...
		<!--增加一个隐藏的input，好传参数给另外一个页面和本地页面获取值，一般通过jsp传参数action是通过name字段，而在本页面获取值是通过jQuery获取id -->
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;">
				...
				<option value="" <%if("".equals(month)){%> selected<%} %>>
					<%=""%></option>
				<option value="01" <%if("01".equals(month)){%> selected<%} %>>
					<%="1"%></option>
				<option value="02" <%if("02".equals(month)){%> selected<%} %>>
					<%="2"%></option>
				<option value="03" <%if("03".equals(month)){%> selected<%} %>>
					<%="3"%></option>
				<option value="04" <%if("04".equals(month)){%> selected<%} %>>
					<%="4"%></option>
				<option value="05" <%if("05".equals(month)){%> selected<%} %>>
					<%="5"%></option>
				<option value="06" <%if("06".equals(month)){%> selected<%} %>>
					<%="6"%></option>
		    	<option value="07" <%if("07".equals(month)){%> selected<%} %>>
					<%="7"%></option>
				<option value="08" <%if("08".equals(month)){%> selected<%} %>>
					<%="8"%></option>
				<option value="09" <%if("09".equals(month)){%> selected<%} %>>
					<%="9"%></option>
		...
		String sqlWhere = " f.id=fd.mainid and fd.PayMethod=0 and fd.Money &gt;0  and (fd.state is null or fd.state='0' ) "; 
		...
		if(!"".equals(month)){
		 sqlWhere +=" and SUBSTRING(f.SendDate,6,2) = '"+month+"' ";
