	...
	<wea:group context="<%=SystemEnv.getHtmlLabelName(15596,user.getLanguage())%>" attributes="{'groupOperDisplay':'none'}">
		...
		<wea:item attributes="{'isTableList':'true'}">
			<table class="ListStyle" cellspacing=0  cols=4 id="oTableAddwfnode">
				...
				%>
				<tr>
					<td  height="23"><input type='checkbox' name='check_node' value="0" >&nbsp;<img moveimg src='/proj/img/move_wev8.png' title='拖动' /></td>
					<td  height="23">
...
function selectall(){
    ...
    try{
	    ...
	    $("input[name^='node_order_']").each(function(i) {
  		...
  		top.Dialog.alert('错误' + e.message + '发生在' +   e.lineNumber + '行');
  	}
    ...
    if (<%=wfversion.isActive() %> && delids != null && delids != "" && '1' != '<%=isTemplate%>') {
    	confirmContent = "您对在用流程版本做了删除节点的操作，可能影响正在流转的流程，是否保存一个新版本，并在新版本中进行本次更改？";
    	window.top.Dialog.confirm(
			...
			}, 520, 90, true, {name:'存为新版并修改', click:saveAsNewVersionAndEdit},
			'修改当前版本','取消', window);
    } else {
