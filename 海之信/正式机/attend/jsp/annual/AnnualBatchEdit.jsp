...
String sql = "select * from HrmAnnualBatchProcess where id = " + id;
RecordSet.executeSql(sql);
int workingage = 0;
float annualdays = 0;
if(RecordSet.next()){
   workingage = (int)RecordSet.getFloat("workingage");
   annualdays = RecordSet.getFloat("annualdays");
}

String imagefilename = "/images/hdMaintenance_wev8.gif" ; 
String titlename = SystemEnv.getHtmlLabelName(21599,user.getLanguage()) + " : "+SystemEnv.getHtmlLabelName(93 , user.getLanguage()) ; 
String needfav = "1" ; 
String needhelp = "" ; 
%>
<BODY>
...
<wea:layout>
    ...
    <wea:item><%=SystemEnv.getHtmlLabelName(21811,user.getLanguage())%></wea:item>
    <wea:item>
       <input class=inputstyle size=4 type="text" id="annualdays" name="annualdays" maxlength="5" value="<%=annualdays%>" onKeyPress="IsNumber_KeyPress()" onBlur='checkisnumber("annualdays");checkinput("annualdays","annualdaysimage")'> 
       <SPAN id=annualdaysimage></SPAN>               
    </wea:item>
	</wea:group>
</wea:layout> 
</FORM>
...
function ondelete(){
  if(confirm("<%=SystemEnv.getHtmlNoteName(7,user.getLanguage())%>")){
    location="AnnualBatchOperation.jsp?operation=delete&id=<%=id%>&subcompanyid=<%=subcompanyid%>";
  }
}
