...
<%
	...
	if(agented.equals("0"))
	{
	    RCMenu += "{"+SystemEnv.getHtmlLabelName(-10999,user.getLanguage())+",javascript:popSetAgentWin(),_self}" ;
		...
		<table id="topTitle" cellpadding="0" cellspacing="0" style="width:100%">
			<tr>
				...
				<td class="rightSearchSpan">
					<%if(agented.equals("0")) {%>
					<input type="button" value="<%=SystemEnv.getHtmlLabelName(-10999, user.getLanguage())%>" class="e8_btn_top middle" onclick="popSetAgentWin()">
						<%if ("0".equals(agentFlag) || haveAgentAllRight) {%>
   ...
   <wea:layout type="4col">
   	<%if(agented.equals("0")){%>
	    <wea:group context='<%=SystemEnv.getHtmlLabelName(32905,user.getLanguage())%>'>
    		<wea:item><%=SystemEnv.getHtmlLabelName(-9845, user.getLanguage())%></wea:item>
    		<wea:item>
    		...
    		<wea:item><%=SystemEnv.getHtmlLabelName(-11002, user.getLanguage())%></wea:item>
    		<wea:item>
    			<span id="agenterspan">
					...
					<brow:browser viewType="0" name="agenterId"   browserValue="<%=agenterid%>" browserOnClick="" browserUrl="/systeminfo/BrowserMain.jsp?url=/interface/CommonBrowser.jsp?type=browser.sh_agentor" hasInput="true"  width="185px" isSingle="true" hasBrowser = "true" isMustInput='2' completeUrl="/data.jsp"  browserSpanValue="<%=agentername %>"></brow:browser>
				</span>
    		</wea:item>
	    	<wea:item><%=SystemEnv.getHtmlLabelName(-11050, user.getLanguage())%></wea:item>
		...
		<wea:group context='<%=SystemEnv.getHtmlLabelName(32843,user.getLanguage())%>'  attributes="{'itemAreaDisplay':'none'}">
			<%if(agentFlag.equals("1")){ %>
				<wea:item><%=SystemEnv.getHtmlLabelName(-9845, user.getLanguage())%><%=SystemEnv.getHtmlLabelName(27511, user.getLanguage())%></wea:item>
		    	<wea:item>
		    	...
		    	<wea:item><%=SystemEnv.getHtmlLabelName(-9845, user.getLanguage())%><%=SystemEnv.getHtmlLabelName(141, user.getLanguage())%></wea:item>
		    	<wea:item>
			...
			<%} %>
	    	<wea:item><%=SystemEnv.getHtmlLabelName(-11002, user.getLanguage())%><%=SystemEnv.getHtmlLabelName(27511, user.getLanguage())%></wea:item>
	    	<wea:item>
	    	...
	    	<wea:item><%=SystemEnv.getHtmlLabelName(-11002, user.getLanguage())%><%=SystemEnv.getHtmlLabelName(141, user.getLanguage())%></wea:item>
	    	<wea:item>
...
//for(int k=0;k<userlist.size();k++){
	   ...
	   //System.out.println("--675555555555-beagenterid----"+beagenterid);
            	...
            	if(agented.equals("0"))
            	{
            		...
            		sqlWhere = " t1.workflowid = t2.id and ( t2.isvalid='1' or t2.isvalid='3') ";
            		...
            		if(!agenterid.equals("")){
            			sqlCondition += " and exists(select 1 from workflow_agentConditionSet t2 where t1.agentId=t2.agentid and t2.agenttype='1' and t2.agentuid='"+agenterid+"')   ";
            		}
            		...
            		if(selectBeginDate.equals("1"))
          			...
          			if (!"".equals(aiddepartmentid)) {
          				sqlCondition += " and exists(select 1 from workflow_agentConditionSet t2 where t1.agentId=t2.agentid and t2.agenttype='1' and t2.agentuid IN (SELECT id FROM hrmresource WHERE departmentid='"+aiddepartmentid+"') )  ";
          			}
          			if (!"".equals(aidsubcompanyid)) {
          				sqlCondition += " and exists(select 1 from workflow_agentConditionSet t2 where t1.agentId=t2.agentid and t2.agenttype='1' and t2.agentuid IN (SELECT id FROM hrmresource WHERE subcompanyid1='"+aidsubcompanyid+"') )   ";
          			}
             ...
             //out.println("select "+backfields+" from "+fromSql+" where "+sqlWhere);
            	...
            	//System.out.println(agentFlag+"++++"+agented);
            		...
            		if ("0".equals(agentFlag) || haveAgentAllRight) {//
    					...
    					operateString +="     <operate  	href=\"javascript:countermand();\" text=\"" + SystemEnv.getHtmlLabelName(-11005, user.getLanguage()) + "\" otherpara=\"it\" index=\"2\"/>";
    					operateString +="     <operate  	href=\"javascript:countermand();\" text=\"" + SystemEnv.getHtmlLabelName(-11006, user.getLanguage()) + "\" otherpara=\"pt+column:agenterId+column:beagenterid\" index=\"3\"/>";
    					operateString +=" </operates>";
                    ...
                    tableString += " <head>";
                    if(agentFlag.equals("1")){
                    	tableString+="<col width=\"10%\"  text=\""+SystemEnv.getHtmlLabelName(-9845,user.getLanguage())+"\" column=\"beagenterid\" orderkey=\"t1.beagenterid\" transmethod=\"weaver.hrm.resource.ResourceComInfo.getMulResourcename1\" />";
					}
                    tableString+="<col width=\"10%\"  text=\""+SystemEnv.getHtmlLabelName(-11002,user.getLanguage())+"\" column=\"agenterId\" otherpara=\"column:agentid\" transmethod=\"weaver.workflow.request.wfAgentCondition.getMulResourcename1\" />";
                    ...
                    tableString+="<col width=\"20%\"  text=\""+SystemEnv.getHtmlLabelName(-11050,user.getLanguage())+"\" column=\"workflowtype\"  transmethod=\"weaver.workflow.workflow.WFAgentTransMethod.getWFtype\" />";
			       	tableString+="<col width=\"20%\"  text=\""+SystemEnv.getHtmlLabelName(-11012,user.getLanguage())+"\" column=\"workflowname\"  />";
	  ...
	  int beagenterid2 = Util.getIntValue((String)userlist.get(k));
	   userID = Util.null2String((String)userlist.get(k));
			...
			<%
			System.out.println("----0000------"+PageIdConst.WF_REQUEST_WFAGENTLIST);
			%>
...
<SCRIPT language="javascript">
	...
	function trunAgented(){
		...
		leftframe.attr("src","/seahonor/attend/agent/wfAgentAllTreeList.jsp?agentFlag=0");
		var contentframe=$("#contentframe",parent.parent.document);
        contentframe.attr("src","/seahonor/attend/agent/wfAgentAll.jsp?agented=1&agentFlag=0");
   	}
   //跳转到设置代理页面
   	...
   	function trunSetAgent()
   	{
		...
		$("#oTd1",parent.parent.document).css("display","none");
   		var contentframe=$("#contentframe",parent.parent.document);
        contentframe.attr("src","/seahonor/attend/agent/wfAgentAll.jsp?agented=0&agentFlag=0");
   	}
   	//跳转到日志
   	...
   	function trunLog() {
   		var contentframe=$("#contentframe",parent.parent.document);
        contentframe.attr("src","/seahonor/attend/agent/agentLogList.jsp");
   	}
   	...
   	function popSetAgentWin(id)
   	{
		...
		var url = "/seahonor/attend/agent/wfAgentAdd.jsp?isdialog=1&f_weaver_belongto_userid=<%=user.getUID()%>";
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(82,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(18599,user.getLanguage()) %>";
   	...
   	function countermand(agentid,type)
	{
		...
		if(type === "it" || type=== "mt"){
			url = "/seahonor/attend/agent/wfAgentGetBackConfirm.jsp?agentid="+agentid+"&type="+type;
		}else
		{
			...
			url = "/seahonor/attend/agent/wfAgentGetBackConfirm.jsp?agentid="+agentid+"&type=pt&agenterid="+agenterid+"&beagenterid="+beagenterid;
		}
...
function onagentcondition(agentid,type){
		...
		var url = "/seahonor/attend/agent/wfAgentCondition.jsp?agentid="+agentid+"&type=pt&agenterid="+agenterid+"&beagenterid="+beagenterid+"&workflowid="+agentworkflowid;
...
function onagentedit(agentid,type){
		...
		var beagenterid = type.split("+")[2];
	 	var agentworkflowid = type.split("+")[3];
		var url = "/seahonor/attend/agent/wfAgentEditCondition.jsp?agentid="+agentid+"&type=pt&agenterid="+agenterid+"&beagenterid="+beagenterid+"&workflowid="+agentworkflowid;
		dialog.Title = "<%=SystemEnv.getHtmlLabelName(93,user.getLanguage())+SystemEnv.getHtmlLabelName(-11003,user.getLanguage())%>";
		dialog.Width = 600;
		dialog.Height =550;
		dialog.Drag = true;
