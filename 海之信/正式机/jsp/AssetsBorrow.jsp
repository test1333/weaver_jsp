...
<HTML>
		...
		<div class="zDialog_div_content" id="differencehandle" name="differencehandle">
				...
				<% if(canedit) {%>
         		...
         		<wea:item><BUTTON class=Calendar type="button" id=selectcontractdate onclick="getchangedate()"></BUTTON>
           		...
           		<input class=inputstyle type="hidden" name="changedate" 'checkinput("changedate","changedatespan")'>
         		</wea:item>
	...
	jQuery(differencerecord).html("<td class='interval'><span class='groupbg'></span><span class='e8_grouptitle'>处理记录</span></td><td class='interval' colspan='2' style='text-align:right;'><span class='toolbar'></span></td>");
	   ...
	   diag_vote.close();
        ...
        function save(){
        	if(check_form(report,'applicant,num,changedate')){
        	var idkeyx = <%=idkey%>;
        	document.report.idkey.value = idkeyx ;
       		document.report.action = "DifferHandleRecord.jsp?type=1";
        ...
        function deletex(){
        	...
        	document.report.handleRecord_main.value = handleRecord_main;
       		document.report.action = "DifferHandleDelete.jsp?type=1";
       		document.report.submit();
        }
