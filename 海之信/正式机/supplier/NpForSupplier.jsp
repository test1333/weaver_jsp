...
String supplierName = Util.null2String(request.getParameter("supplierName"));
String supplierCode = Util.null2String(request.getParameter("supplierCode"));
...
<wea:layout type="4col">
	<wea:group context="<%=SystemEnv.getHtmlLabelNames("15774",user.getLanguage()) %>">
		...
		<wea:item>
			<input type="text" name="supplierName" id="supplierName" class="InputStyle" value="<%=supplierName%>"/>
		</wea:item>
		<wea:item>客户代码</wea:item>
		<wea:item>
			<input type="text" name="supplierCode" id="supplierCode" class="InputStyle" value="<%=supplierCode%>" />
		</wea:item>
...
//	out.prinln(tmp_name_x);	
	...
	String backfields = " id,name,SysNo,(select countryName from uf_country where id=Country) as countryname,"
	+" (select TypeName from uf_enterprise_type where id=enterprise) as enterpriseName,enQuery,jpQuery,"
	+" (select TypeName from uf_industryType where id=industry) as industryName,telephone,webSite,'未校对' as status "; 
	String fromSql  = " from uf_supplier ";
	String sqlWhere = " where SuperID is null and status=0 ";
	String orderby = " id " ;
	String tableString = "";
...
if(!"".equals(supplierName)){
	sqlWhere = sqlWhere + " and name like '%"+supplierName+"%'";
}
if(!"".equals(supplierCode)){
	sqlWhere = sqlWhere + " and SysNo like '%"+supplierCode+"%'";
}
if(!"".equals(enLook)){
	sqlWhere = sqlWhere + " and enQuery like '%"+enLook+"%'";
}
if(!"".equals(jpLook)){
	sqlWhere = sqlWhere + " and jpQuery like '%"+jpLook+"%'";
}
if(!"".equals(telephone)){
...
tableString =" <table tabletype=\"checkbox\" pagesize=\""+ PageIdConst.getPageSize(tmc_pageId,user.getUID(),PageIdConst.HRM)+"\" pageId=\""+tmc_pageId +"\">"+
    ...
    operateString+
    "			<head>";                                                                                                                                                 
		tableString+="<col width=\"10%\" labelid=\"17401\" text=\"供应商名称\" column=\"name\" orderkey=\"name\" linkvaluecolumn=\"id\" linkkey=\"billid\" href=\"/formmode/view/ViewMode.jsp?type=0&amp;modeId=50&amp;formId=-57&amp;opentype=0&amp;customid=77&amp;viewfrom=fromsearchlist\" target=\"_fullwindow\"/>"+
	 "				<col width=\"10%\" labelid=\"590\" text=\"代码\" column=\"SysNo\" orderkey=\"SysNo\"   linkvaluecolumn=\"id\" linkkey=\"billid\" href=\"/formmode/view/ViewMode.jsp?type=0&amp;modeId=50&amp;formId=-57&amp;opentype=0&amp;customid=77&amp;viewfrom=fromsearchlist\" target=\"_fullwindow\"/>"+
	...
	function onCheckup(){
		if(window.confirm('你是否确认校队吗？')){
			...
			if(ids != ""){
				...
				document.report.action = "/seahonor/supplier/NpForSupplierOper.jsp";
			//	obj.disabled = true;
