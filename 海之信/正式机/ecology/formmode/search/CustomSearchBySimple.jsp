...
String url = "/formmode/search/CustomSearchBySimpleIframe.jsp?"+customidStr+tempquerystring+paramStr+"&mainid="+mainid;
if(url.indexOf("+")!=-1){
	url = url.replace("+", "%2B");
}
					...
					<%}else {%>
						...
						<%	
							...
							if(fieldhtmltype.equals("5")){
								sql = "select * from workflow_SelectItem where fieldid="+groupid+" and cancel<>1 order by listorder";
							}else{
							...
							while(rs.next()){
								...
								String tempurl = url+"&groupby="+groupname+"$"+selvalue;
						...
						<li>
							<a href="<%=url+"&groupby="+groupname+"$-1" %>" target="tabcontentframe">
