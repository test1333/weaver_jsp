...
<script type="text/javascript">
		...
		//	var url = "/systeminfo/BrowserMain.jsp?url=/seahonor/crm/ModeForward.jsp?typex=custom,<%=customID%>,"+id;
			...
			var url = "/systeminfo/BrowserMain.jsp?url=/seahonor/crm/ModeForward_1.jsp?typex=dyMe2,0,"+id;			
		//	alert("url = " + url);
...
<script type="text/javascript">
	...
	//	openFullWindowForXtable("CustomGroupDynamicDetail.jsp?customID="+id);
		var title = "";
		var url = "/seahonor/remind/RemindJobBase_contact.jsp?customID="+id+"&customID1=<%=customID%>";
	//	alert(url)
		var diag_vote = new window.top.Dialog();
		diag_vote.currentWindow = window;
...
//RCMenu += "{新增联系人动态,javascript:openDyInfo(),_blank} " ;
//RCMenuHeight += RCMenuHeightStep ;
%>
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;">
				...
				<wea:item>
				 <input name="biaoti" id="biaoti" class="InputStyle" type="text" value="<%=biaoti%>"/>
					...
					<wea:item>重要程度</wea:item>
				<wea:item>
				<select class="e8_btn_top middle" name="zycd" id="zycd"> 
				<option value="" <%if("".equals(zycd)){%> selected<%} %>>
					<%=""%></option>
				<option value="0" <%if("0".equals(zycd)){%> selected<%} %>>
					<%="普通"%></option>
				<option value="1" <%if("1".equals(zycd)){%> selected<%} %>>
					<%="重要"%></option>
				...
				<wea:item>关键字</wea:item>
				<wea:item>
				 <input name="gjz" id="gjz" class="InputStyle" type="text" value="<%=gjz %>"/>
				</wea:item>	
				<wea:item>发布类型</wea:item>
				<wea:item>
				<select class="e8_btn_top middle" name="fblx" id="fblx"> 
				<option value="" <%if("".equals(fblx)){%> selected<%} %>>
					<%=""%></option>
				<option value="0" <%if("0".equals(fblx)){%> selected<%} %>>
					<%="所有人"%></option>
				<option value="1" <%if("1".equals(fblx)){%> selected<%} %>>
					<%="部分人"%></option>
				
				</select>
				</wea:item>
				<wea:item>发布人</wea:item>
				<wea:item>
				<brow:browser viewType="0"  name="fbr" browserValue="<%=fbr %>" 
				browserOnClick=""
				browserUrl="/systeminfo/BrowserMain.jsp?url=/hrm/resource/ResourceBrowser.jsp?selectedids="
				hasInput="true" isSingle="false" hasBrowser = "true" isMustInput='1'
				completeUrl="/data.jsp" width="165px"
				browserSpanValue="<%=Util.toScreen(ResourceComInfo.getResourcename(fbr),user.getLanguage())%>">
				</brow:browser>
				</wea:item>
				<wea:item>发布日期</wea:item>
		<wea:item>
	...
	String backfields = " id,bt,content,gjz,fbr,modedatacreater,fbrq+' '+fbsj as createTime "; 
	String fromSql  = " from uf_ContactsDy ";
	String sqlWhere = " where contactsName = " + customID 
		+ " and (fbr="+user_id+" or Area=0 or ','+CAST(AreaType as varchar(6000))+','  like '%,"+user_id+",%') " ;
	String orderby = " modedatacreatedate,modedatacreatetime " ;
	String tableString = "";
	...
	if(!"".equals(beginDate)){
		sqlWhere +=  " and fbrq >='"+beginDate+"'";
	}
	if(!"".equals(endDate)){
		sqlWhere +=  " and fbrq <='"+endDate+"'";
	}
	if(!"".equals(zycd)){
	...
	if(!"".equals(fblx)){
		sqlWhere +=  " and Area ='"+fblx+"'";
	}
	if(!"".equals(fbr)){
		sqlWhere +=  " and fbr ='"+fbr+"'";
	}
...
operateString = "<operates width=\"20%\">"+
 	        	    " <popedom transmethod=\"weaver.hrm.common.SplitPageTagOperate.getBasicOperate\" otherpara=\""+String.valueOf(user.isAdmin())+":true:true:true:true\"></popedom> "+
						 ...
						 "     <operate href=\"javascript:openNewInfo1();\" linkkey=\"id\" linkvaluecolumn=\"id\" text=\"回复\"  target=\"_fullwindow\" isalwaysshow='true' index=\"0\"/>"+
...
tableString =" <table tabletype=\"checkbox\" pagesize=\""+ PageIdConst.getPageSize(tmc_pageId,user.getUID(),PageIdConst.HRM)+"\" pageId=\""+tmc_pageId +"\">"+
    ...
    "			<head>";
		...
		"<col width=\"35%\" labelid=\"-9403\" text=\"动态内容\" column=\"content\" orderkey=\"id\"/>"+
		"<col width=\"15%\"  text=\"关键字\" column=\"gjz\" orderkey=\"gjz\"/>"+
	  "				<col width=\"15%\" labelid=\"882\" text=\"发布人\" column=\"fbr\" orderkey=\"fbr\" transmethod=\"weaver.hrm.resource.ResourceComInfo.getResourcename\"  />"+
...
%>
	...
	function openDyInfo() {
		var title = "";
		var url = "/systeminfo/BrowserMain.jsp?url=/seahonor/crm/ModeForward_1.jsp?typex=dyMe,0,<%=customID%>";
	...
	function editInfo(id) {
		//	openFullWindowForXtable("/seahonor/crm/ModeForward.jsp?type=custom&customID=<%=customID%>&empID="+id);
			var title = "";
			var url = "/systeminfo/BrowserMain.jsp?url=/seahonor/crm/ModeForward.jsp?typex=editcontactD,"+id;
