...
<HTML>
	...
	<BODY>
		...
		String backfields = " fd.id,fd.kdCode,DateTime,SendDate,SenderDepartment,Sender,case fd.ExpressCompany when '1' then '顺丰快递' when '2' then '申通快递' when '3' then 'DHL' when '4' then 'EMS' end as ExpressCompany,fd.ReceiverAdd,case fd.PayMethod when '0' then '月结' when '1' then '现结' else '到付' end as PayMethod,fd.Money ,case fd.State when '0' then '待结算' else '已结算' end as State ,f.requestid,(select requestname from workflow_requestbase wr where wr.requestid=f.requestId) as request_name";
		String fromSql  = " from formtable_main_76 f,formtable_main_76_dt1 fd  ";
		...
		String sqlWhere = " f.id=fd.mainid and fd.PayMethod=0 and fd.State =1 "; 
		...
		sqlWhere+=" and SenderDepartment in("+department+") ";
		}
		...
		"	   <sql backfields=\""+backfields+"\" sqlform=\""+fromSql+"\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlWhere)+"\"  sqlorderby=\""+orderby+"\"  sqlprimarykey=\"fd.id\" sqlsortway=\"desc\" sqlisdistinct=\"false\"/>"+
		operateString+
		"			<head>";
