...
<%
    ...
    if(!"".equals(isSetAgent)){
        ...
        if("0".equals(isSetAgent)){  
            ...
            if("".equals(tmp_agent)){
                //response.sendRedirect("/seahonor/jsp/SH_Set_Agent_info.jsp?infoKey=1&isSetAgent=1");
                return;  
            }else{//代理成功
                //response.sendRedirect("/seahonor/jsp/SH_Set_Agent_info.jsp?infoKey=2&isSetAgent=0");
                return;      
            }   
            //shaw over
        }else{
            ...
            try{
                ...
                if("y".equals(isCountermandRunning)){
                    rs.executeSql("select agentuid from workflow_agentConditionSet where bagentuid='"+empid+"'  and  agentuid='"+tmp_agent+"' and agenttype=1 ");
                //将所有收回

                 while(rs.next()){
                    tmp_agent=Util.getIntValues(rs.getString("agentuid"));
                    String updateSQL = "";
                    rs1.executeSql("select * from workflow_currentoperator where isremark in ('0','1','5','7','8','9')  and userid = " + tmp_agent + " and agentorbyagentid = " + empid + " and agenttype = '2'");//td2302 xwj
                    out.print("/************rs1**********/");
                    log.writeLog("/*********rs1**********/");
                        while(rs1.next()){
                            ...
                            if(rs2.next()){
                                ...
                                //失效的代理人删除
                                rs3.executeSql("delete workflow_currentoperator where id="+wfcoid);//td2302 xwj
                            ...
                            rs2.executeSql("select id from workflow_currentoperator where requestid ="+tmprequestid+" and userid="+tmpuserid+" and usertype="+usertype+" order by id desc ");
                            out.print("/************rs3**********/");
                            log.writeLog("/*********rs3**********/");
                            if(rs2.next()){
                                rs3.executeSql("update workflow_currentoperator set islasttimes=1 where requestid=" +tmprequestid + " and userid=" + tmpuserid + " and id = " + rs3.getString("id"));
                            }
                            //add by fanggsh 20060519 TD4346 end
            
                            //回收代理人文档权限

                            rs2.executeSql("select distinct docid,sharelevel from Workflow_DocShareInfo where requestid="+tmprequestid+" and userid="+tmp_agent+" and beAgentid="+empid);
                            out.print("/************rs1**********/");
                           log.writeLog("/*********rs1**********/");
                            ...
                            if(hasrow){
                                rs2.executeSql("delete Workflow_DocShareInfo where requestid="+tmprequestid+" and userid="+tmp_agent+" and beAgentid="+empid);
                            }
                            
                            for(int j=0;j<docslist.size();j++){
                                rs3.executeSql("select Max(sharelevel) sharelevel from Workflow_DocShareInfo where docid="+docslist.get(j)+" and userid="+tmp_agent);
                                if(rs3.next()){
                                    int sharelevel=Util.getIntValue(rs3.getString("sharelevel"),0);
                                    if(sharelevel>0){
                                        rs4.executeSql("update DocShare set sharelevel="+sharelevel+" where sharesource=1 and docid="+docslist.get(j)+" and userid="+tmp_agent+" and sharelevel>"+sharelevel);
                                    }else{
                                        rs4.executeSql("delete DocShare where sharesource=1 and docid="+docslist.get(j)+" and userid="+tmp_agent);
                                    }
                                }else{
                                    rs4.executeSql("delete DocShare where sharesource=1 and docid="+docslist.get(j)+" and userid="+tmp_agent);
                                }
                                //重新赋予被代理人文档权限
                                rs4.executeSql("update DocShare set sharelevel="+sharlevellist.get(j)+" where sharesource=1 and docid="+docslist.get(j)+" and userid="+empid);
                ...
                //设置无效状态
                wfAgentCondition.SetbackAgent(""+empid,""+tmp_agent);
                //通过默认的工作流提醒 
                //TODO
            }catch(Exception e){
                flag = false;
