...
<%!
public String getEmpAttenStatus(String emp_id,String atten_day){
	String sql = "";
	String atten_type = "";
	RecordSet rs = new RecordSet();
	if(!"".equals(emp_id)&&!"".equals(atten_day)){
		sql = " select atten_type from dbo.sh_leave_arrange_new("+emp_id+",'"+atten_day+"') ";
		rs.executeSql(sql);
		if(rs.next()) {
			atten_type = Util.null2String(rs.getString("atten_type"));
		}
	}
	return atten_type;
}
...
String sql = " select atten_start_time,atten_end_time,leave_type,FLOOR(late_times) as late_times,FLOOR(early_leave_times) as early_leave_times,leave_start_time,leave_end_time from uf_all_attend_info "
+" where emp_id = "+emp_id+" and  atten_day = '"+atten_day+"' and isnull(isEffective,0)=0 ";
...
String temp1 = "<span class=\"sp1\">  ";
	String temp2 = "<span class=\"sp2\" >  ";
		String temp3 = "<span class=\"sp3\" > ";
			...
			String l_start = Util.null2String(recordSet.getString("leave_start_time"));
			String l_end = Util.null2String(recordSet.getString("leave_end_time"));
			...
			String atten_type = "";
			if(!"".equals(leave_type)){
				atten_type = getEmpAttenStatus(emp_id,atten_day);
				if(!"".equals(atten_type)){
					if("0".equals(atten_type)||"2".equals(atten_type)){
						buff.append(temp2);	
					}else{
		...
		if(!"".equals(leave_type)){
				atten_type = getEmpAttenStatus(emp_id,atten_day);
				if(!"".equals(atten_type)){
					if("1".equals(atten_type)||"2".equals(atten_type)){
						buff.append(temp2);	
					}else{
						if(!"".equals(end_time)){
