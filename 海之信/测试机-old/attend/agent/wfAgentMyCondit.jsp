...
function attachUrl(){
	...
	$("a[target='tabcontentframe']").bind("click",function(){
		...
		var params = "?agented=<%=agented %>&overlapagentstrid=<%=overlapagentstrid%>";
		if($(this).attr("agtcd"))
...
<body scroll="no">
	<div class="e8_box demo2">
	 <div class="e8_boxhead">
		...
		<iframe onload="update()"  id="tabcontentframe" name="tabcontentframe"  src='/workflow/request/wfAgentConditList.jsp?agentFlag=<%=Util.null2String(request.getParameter("agentFlag"))%>&menutype=<%=menutype%>&menuid=<%=menuid%>&agented=<%=agented %>&overlapagentstrid=<%=overlapagentstrid%> ' class="flowFrame" frameborder="0" width="100%;"></iframe>
