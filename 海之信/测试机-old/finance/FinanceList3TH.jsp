...
// 出纳
String sql_1 = "select count(id) as count_cc from HrmRoleMembers where roleid=14 and resourceid="+curr_user;
...
if("four".equals(val_id)){
	sql_x_1 = "update uf_tmp_financeInfo set ReqSataus=3 where tmpReqID in("+multiIds+"0)";
...
if(isShow){
	RCMenu += "{确认付款,javascript:onCheckup(),_self} " ;
	RCMenuHeight += RCMenuHeightStep ;
}
%>
...
//	out.prinln(tmp_name_x);	
	...
	while(RecordSet.next()){
		...
		buff_sql.append(" and f.requestid in (select tmpReqID from uf_tmp_financeInfo where ReqSataus=2)");
...
%>
	...
	function onCheckup(){
		if(window.confirm('你是否确认已付款吗？')){
			var ids = _xtable_CheckedCheckboxId();
			if(ids != ""){
				document.report.multiIds.value = ids;
				document.report.val_id.value = "four";
				document.report.action = "FinanceList3TH.jsp";
