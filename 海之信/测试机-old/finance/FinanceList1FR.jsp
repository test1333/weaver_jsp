...
// 公司会计权限
String sql_1 = "select count(id) as count_cc from HrmRoleMembers where roleid=47 and resourceid="+curr_user;
...
String apply = Util.null2String(request.getParameter("apply"));
String deptApply = Util.null2String(request.getParameter("deptApply"));
...
if("second".equals(val_id)){
	sql_x_1 = "update uf_tmp_financeInfo set ReqSataus=1 where tmpReqID in("+multiIds+"0)";
	RecordSet.executeSql(sql_x_1);
	sql_x_2 = "insert into uf_tmp_financeInfo(tmpReqID,ReqSataus,createTime,updateTime) select requestid,'1' as st,convert(varchar(40),getdate(),121) as cd,"
		+"convert(varchar(40),getdate(),121) as ud from workflow_requestbase where requestid in("+multiIds
		+"0) and requestid not in(select tmpReqID from uf_tmp_financeInfo)";
...
<wea:layout type="4col">
	<wea:group context="<%=SystemEnv.getHtmlLabelNames("15774",user.getLanguage()) %>">
		...
		<wea:item>
			<brow:browser viewType="0"  name="apply" browserValue="<%=apply %>"
				browserOnClick=""
				browserUrl="/systeminfo/BrowserMain.jsp?url=/hrm/resource/ResourceBrowser.jsp?selectedids="
				hasInput="true" isSingle="false" hasBrowser = "true" isMustInput='1'
				completeUrl="/data.jsp" width="165px"
				browserSpanValue="<%=Util.toScreen(ResourceComInfo.getResourcename(apply),user.getLanguage())%>">
			</brow:browser>
		</wea:item>
		<wea:item>申请人部门</wea:item>
		<wea:item>
			<brow:browser viewType="0"  name="deptApply" browserValue="<%=deptApply %>"
				browserOnClick=""
				browserUrl="/systeminfo/BrowserMain.jsp?url=/hrm/company/DepartmentBrowser.jsp?selectedids="
				hasInput="true" isSingle="false" hasBrowser = "true" isMustInput='1'
				completeUrl="/data.jsp" width="165px"
				browserSpanValue="<%=Util.toScreen(DepartmentComInfo.getDepartmentname(deptApply),user.getLanguage())%>">
			</brow:browser>
		</wea:item>
		<wea:item>时间段</wea:item>
		<wea:item>
...
String tmp_name_x = map.get(tmp_id_x);
	...
	while(RecordSet.next()){
		...
		buff_sql.append(" and f.requestid not in (select tmpReqID from uf_tmp_financeInfo where ReqSataus!=0)");
...
//	out.print("buff_sql = " + buff_sql.toString());
	...
	String wk_ids = "";
//	out.println("apply = " + apply +" ; deptApply = " + deptApply + " ; fromDate = " + fromDate + " ; toDate = " + toDate );	
	String backfields = " fh.* "; 
	String fromSql  = " from ("+buff_sql.toString()+") fh ";
	String sqlWhere = " where 1=1 ";
	if(!"".equals(apply)){
		sqlWhere = sqlWhere + " and val_1="+apply;
	}
	
	if(!"".equals(deptApply)){
		sqlWhere = sqlWhere + " and val_1 in(select id from HrmResource where departmentid="+deptApply+")";
	}
	
	if(!"".equals(fromDate)){
		sqlWhere = sqlWhere + " and val_2>='"+fromDate+"' ";
	}
	if(!"".equals(toDate)){
		sqlWhere = sqlWhere + " and val_2<='"+toDate+"' ";
	}
...
%>
	...
	function onCheckup(){
		if(window.confirm('你是否确认审核吗？')){
			var ids = _xtable_CheckedCheckboxId();
			if(ids != ""){
				document.report.multiIds.value = ids;
				document.report.val_id.value = "second";
				document.report.action = "FinanceList1FR.jsp";
