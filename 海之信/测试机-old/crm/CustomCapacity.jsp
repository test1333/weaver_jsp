...
%>
	...
	String backfields = " id,name,remark,period,uploader,uploadDate+' '+uploadTime as createTime ,(select customName from uf_custom where id=a.custom) as customName"; 
	String fromSql  = " from uf_CustomCapacity a ";
	String sqlWhere = " where (superid is null or superid ='') and custom = " + customID;
	String orderby = " uploadDate,uploadTime " ;
	String tableString = "";
	if(!"".equals(biaoti)){
		sqlWhere +=  " and UPPER(name) like upper('%"+biaoti+"%') ";
	}
	
	if(!"".equals(beginDate)){
		sqlWhere +=  " and uploadDate >='"+beginDate+"'";
	}
	if(!"".equals(endDate)){
		sqlWhere +=  " and uploadDate <='"+endDate+"'";
	}
...
tableString =" <table tabletype=\"checkbox\" pagesize=\""+ PageIdConst.getPageSize(tmc_pageId,user.getUID(),PageIdConst.HRM)+"\" pageId=\""+tmc_pageId +"\">"+
    ...
    operateString+
    "			<head>";
		tableString+="<col width=\"15%\"  text=\"公司名称\" column=\"customName\" orderkey=\"customName\" />"+
		"<col width=\"15%\"  text=\"资质名称\" column=\"name\" orderkey=\"name\" linkvaluecolumn=\"id\" linkkey=\"billid\" href=\"/formmode/view/ViewMode.jsp?type=0&amp;modeId=60&amp;formId=-67&amp;opentype=0&amp;viewfrom=fromsearchlist\" target=\"_fullwindow\"  />"+
		"<col width=\"35%\"  text=\"说明\" column=\"remark\" orderkey=\"remark\" />"+
	  ...
	  "				<col width=\"15%\"  text=\"上传时间\" column=\"createTime\" orderkey=\"createTime\"  />"+
    "			</head>"+
    " </table>";
%>
	...
	//	var url = "/systeminfo/BrowserMain.jsp?url=/formmode/view/AddFormModeIframe.jsp?modeId=7&formId=-8&type=1&customid=0&field5846=<%=customID%>";
		...
		var url = "/systeminfo/BrowserMain.jsp?url=/seahonor/crm/ModeForward.jsp?typex=customzz,<%=customID%>";
