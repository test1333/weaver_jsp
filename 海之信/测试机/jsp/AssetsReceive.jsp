...
<HTML>
    ...
    String lastname = "";
    String applicant = Util.null2String(request.getParameter("applicant"));
	...
	if(!applicant.equals("")){
    String sql_hrm = "select lastname from HrmResource where id = "+applicant+"";
    rs.executeSql(sql_hrm);
    if(rs.next()){
	...
	<BODY>
		...
		<div class="zDialog_div_content" id="differencehandle" name="differencehandle">
				...
				<wea:item>
				<brow:browser viewType="0"  id ="applicant" name="applicant" browserValue="<%=applicant %>" 
				...
				<wea:item>
				<brow:browser viewType="0"  name="project" browserValue="<%=project %>" 
				browserUrl="/systeminfo/BrowserMain.jsp?url=/interface/CommonBrowser.jsp?type=browser.project"
				hasInput="true" isSingle="true" hasBrowser = "true" isMustInput='2'
				completeUrl="/data.jsp" width="120px"
			    linkUrl=""
			    browserSpanValue="">
	...
	jQuery(differencerecord).html("<td class='interval'><span class='groupbg'></span><span class='e8_grouptitle'>处理记录</span></td><td class='interval' colspan='2' style='text-align:right;'><span class='toolbar'></span></td>");
	   ...
	   diag_vote.close();
        ...
        function save(){
        if(check_form(report,'applicant,num,project')){
        	var idkeyx = <%=idkey%>;
        	document.report.idkey.value = idkeyx ;
       		document.report.action = "DifferHandleRecord.jsp?type=3";
        ...
        function deletex(){
        	...
        	document.report.handleRecord_main.value = handleRecord_main;
       		document.report.action = "DifferHandleDelete.jsp?type=3";
       		document.report.submit();
        }
