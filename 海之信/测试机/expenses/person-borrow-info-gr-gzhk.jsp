...
<link rel="stylesheet" type="text/css" href="/js/jquery-autocomplete/browser_wev8.css" />
	...
	<BODY>
		...
		<FORM id=report name=report action="/seahonor/expenses/person-borrow-info-yq-gzhk.jsp" method=post>
			<input type="hidden" name="requestid" value="">
			<input type="hidden" name="bz" id="bz" value="<%=bz%>">
			<input type="hidden" name="jkrid" id="jkrid" value="<%=jkrid%>">
		...
		<%
		String backfields = " convert(varchar(20),jkr)+'_'+convert(varchar(20),bz) as keyid,jkr,bz as bzid,(select c.selectname from workflow_billfield a, workflow_bill b,workflow_selectitem c where a.billid=b.id and c.fieldid=a.id and b.tablename='uf_personal_borrow' and a.fieldname= 'bz' and c.selectvalue=bz) as bz,SUM(ISNULL(jkye,0)) as jkye,SUM(ISNULL(yqje,0)) as yqje  ";                 
		String fromSql  = " (select t.jkr,t.id,t.bz,t.jkye,isnull((select jkye from uf_personal_borrow where sfyq='0' and id=t.id),0) as yqje from uf_personal_borrow t";
		String sqlWhere = " where  ISNULL(t.jkye,0)>0 and jkr='"+jkrid+"' ) a group by jkr,bz ";
		...
		operateString+
		"			<head>";
			tableString+= " <col width=\"20%\" text=\"借款币种\" column=\"bz\"/>"+
			"               <col width=\"20%\" text=\"原币余额\" column=\"jkye\" transmethod=\"seahonor.action.expenses.TransMethod.getTransNum\" linkkey=\"keyid\" linkvaluecolumn=\"keyid\"  href=\"/seahonor/expenses/gr-borrow-info-gzhk-Url.jsp\"/>"+
			"               <col width=\"20%\" text=\"逾期余额\" column=\"yqje\" transmethod=\"seahonor.action.expenses.TransMethod.getTransNum\" linkkey=\"keyid\" linkvaluecolumn=\"keyid\"  href=\"/seahonor/expenses/gr-borrow-info-Url-yq.jsp\"/>"+
			
		"			</head>"+
	" </table>";
	//showExpExcel="false"
	%>
	<h3 align="center"><%=jkrname%>借款明细</h3>
