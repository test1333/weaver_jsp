...
<HTML>
	...
	String VerCodeRegister = Util.null2String(request.getParameter("VerCodeRegister"));//验证码（登记人输入）
	...
	<BODY>
		...
		String backfields = " id,requestid,(select requestname from workflow_requestbase wr where wr.requestid=f.requestId) as request_name,DueDate,receiveDepartment,Random,(case isnull(VerCodeRegister,'未领取') when '未领取' then '未领取' else '已领取' end) as zt,MailOrder,Content,Recipient,VerCode ";
		...
		sqlWhere+=" and (case isnull(VerCodeRegister,'未领取') when '未领取' then '未领取' else '已领取' end ) like '%"+ State + "%'";
