...
<%
	...
	for(int index=0;index<resArr.length;index++){
		...
		// 更新状态为校对，版本更新为1
		String sql = "UPDATE  uf_custom set CutomStatus=1,Version=1,ModifyTime=CONVERT(varchar(30),getdate(),21),Modifier="+userid+"  where id="+id;
		rs.executeSql(sql);
		
		// 存储目前版本
		gni.insertNow("-59", "uf_custom", "id", id);
	}
	
	response.sendRedirect("/seahonor/crm/NpForCustom.jsp");
%>