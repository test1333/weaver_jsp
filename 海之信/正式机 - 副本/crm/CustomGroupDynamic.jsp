...
function openDyInfo() {
//	openFullWindowForXtable("/formmode/view/AddFormMode.jsp?modeId=11&formId=-8&type=1");
	
	var title = "";
	//var url = "/formmode/view/AddFormMode.jsp?modeId=11&formId=-8&type=1";
	
	var url = "/systeminfo/BrowserMain.jsp?url=/seahonor/crm/ModeForward.jsp?typex=group,<%=customID%>";
...
function openNewInfo(id) {
	//	openFullWindowForXtable("CustomGroupDynamicDetail.jsp?customID="+id);
		var title = "";
		var url = "/seahonor/remind/RemindJobBase_group.jsp?customID="+id+"&customID1=<%=customID%>";
		//alert(url)
		var diag_vote = new window.top.Dialog();
		diag_vote.currentWindow = window;
...
//	openFullWindowForXtable("/seahonor/crm/ModeForward.jsp?type=group&customID=<%=customID%>&empID="+id);
	
	var title = "";
	var url = "/systeminfo/BrowserMain.jsp?url=/seahonor/crm/ModeForward.jsp?typex=group,<%=customID%>,"+id;
...
<%
	...
	//RCMenu += "{新增集团动态,javascript:openDyInfo(),_blank} " ;
	//RCMenuHeight += RCMenuHeightStep ;
%>
	...
	String sqlWhere = " where customGroup = " + customID 
...
tableString =" <table tabletype=\"checkbox\" pagesize=\""+ PageIdConst.getPageSize(tmc_pageId,user.getUID(),PageIdConst.HRM)+"\" pageId=\""+tmc_pageId +"\">"+
    ...
    "			<head>";
		...
		"<col width=\"50%\" labelid=\"-9403\" text=\"动态内容\" column=\"content\" orderkey=\"content\"/>"+
	  ...
	  "				<col width=\"15%\" labelid=\"1339\" text=\"创建时间\" column=\"createTime\" orderkey=\"createTime\"     target=\"_fullwindow\"/>"+
    "			</head>"+
    " </table>";
%>
