...
<%
	...
	for(int index=0;index<resArr.length;index++){
		...
		// 更新状态为校对，版本更新为1
		String sql = "UPDATE  uf_custom_group set GroupStatus=1,Version=1,ModifyTime=CONVERT(varchar(30),getdate(),21),Modifier="+userid+" where id="+id;
		rs.executeSql(sql);
		
		// 存储目前版本
		gni.insertNow("-68", "uf_custom_group", "id", id);
	}
	
	response.sendRedirect("/seahonor/crm/NpForGroup.jsp");
%>