...
String customID_x = Util.null2String(request.getParameter("customID_x"));
...
String superID = "";
String superName = "";
String sql = "select GroupName,id from uf_custom_group where id = " + customID;
...
<wea:group context="集团关系" attributes="test">
	...
	<wea:item>
		<table>	
			...
			%>
			<a href="CustomGroupJoin_all.jsp?lookGID=<%=superID%>&customID=<%=customID%>">
			<font  size="6px">&bull;</font>
	...
	<%
			sql = "select id,customName from uf_custom where superid is null and CutomStatus=1  and CustomGroup="+customID+" ";
		//	out.println("sql = " + sql);
			RecordSet.executeSql(sql);
			while(RecordSet.next()){
				...
				out.println("<font  size=\"4px\">&nbsp;&nbsp;&nbsp;<a href=\"CustomGroupJoin_all.jsp?lookID="+tmp_customID+"&customID="+customID+"\">");
	...
	<wea:item>
		<input type=button class="e8_btn_top" onclick="openNewInfo(<%=customID%>);" value="添加非系统关联客户"></input>
			...
			while(RecordSet.next()){
			   ...
			   if(otherID.equals(tmp_id)){		
				...
				out.println("<font  size=\"4px\">&nbsp;&nbsp;&nbsp;"
					+"<a href=\"CustomGroupJoin_all.jsp?lookGID="+superID+"&otherID="+tmp_id+"&customID="+customID+"\">");
