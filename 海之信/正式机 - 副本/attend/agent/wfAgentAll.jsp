...
<%
	...
	if(agented.equals("0"))
		navName = SystemEnv.getHtmlLabelName(-11004,user.getLanguage());
...
function attachUrl(){
	...
	$("a[target='tabcontentframe']").bind("click",function(){
		var url = "/seahonor/attend/agent/wfAgentList.jsp";
	 ...
	 <div class="e8_boxhead">
		...
		<ul class="tab_menu">
			...
			<li <%=agentFlag.equals("0")?"class='current'":"" %>>
				<a agtcd="0" name='0' target="tabcontentframe"><%=SystemEnv.getHtmlLabelName(-11000,user.getLanguage()) %></a>
			</li>
			<li <%=agentFlag.equals("1")?"class='current'":"" %>>
				<a agtcd="1" name='1' target="tabcontentframe"><%=SystemEnv.getHtmlLabelName(-11001,user.getLanguage()) %></a>
			</li>
			<%}else if(agented.equals("1")){ %>
			<li class="e8_tree">
		...
		<iframe onload="update()"  id="tabcontentframe" name="tabcontentframe"  src='/seahonor/attend/agent/wfAgentList.jsp?agentFlag=<%=Util.null2String(request.getParameter("agentFlag"))%>&menutype=<%=menutype%>&menuid=<%=menuid%>&agented=<%=agented %> ' class="flowFrame" frameborder="0" width="100%;"></iframe>
