...
<%@ include file="/systeminfo/RightClickMenu_wev8.jsp" %>
	<jsp:include page="/systeminfo/commonTabHead.jsp">
	   ...
	   <jsp:param name="navName" value="<%=SystemEnv.getHtmlLabelName(-11003, user.getLanguage()) %>"/>
	</jsp:include>
...
<FORM name="frmmain" action="/seahonor/attend/agent/wfAgentOperatorNew.jsp" method="post">
	...
	<wea:layout  >
		...
		<wea:item attributes="{'isTableList':'true'}">	
			<table  id="table" class="ListStyle"  width="100%">
		     <tr >
		       <td height="44" class="fieldName" width="160px"><!--被代理人  -->
		       <%=SystemEnv.getHtmlLabelName(-9845,user.getLanguage())%>
		       </td>
			 ...
			 <%=SystemEnv.getHtmlLabelName(-11002,user.getLanguage())%>
			 </td>
			 <td	class="field"  colspan="3">
			 <!--<brow:browser viewType="0" name="agenterId"   browserValue="" browserOnClick="" browserUrl="/systeminfo/BrowserMain.jsp?url=/hrm/resource/ResourceBrowser.jsp" hasInput="true"  width="185px" isSingle="true" hasBrowser = "true" isMustInput='2' completeUrl="/data.jsp"  browserSpanValue=""></brow:browser>-->
			 <brow:browser viewType="0" name="agenterId"   browserValue="" browserOnClick="" browserUrl="/systeminfo/BrowserMain.jsp?url=/interface/CommonBrowser.jsp?type=browser.sh_agentor" hasInput="true"  width="185px" isSingle="true" hasBrowser = "true" isMustInput='2' completeUrl="/data.jsp"  browserSpanValue=""></brow:browser>
			 </td>
		     </tr>
		     <tr class="Spacing" style="height: 1px !important;" jQuery1425871165066="63">
				<td class="paddingLeft0" colSpan="4" jQuery1425871165066="64">
	        ...
	        <tr >
			 <td  style="display:none" height="44" class="fieldName"	width="160px"><!-- 代理流程处理 -->
			 	<%=SystemEnv.getHtmlLabelName(-11007,user.getLanguage())%>
			 </td>
			 ...
			 <td height="44"   class="fieldName"	width="160px"  ><!-- 代理已有待办事宜 -->
			 <%=SystemEnv.getHtmlLabelName(-11008,user.getLanguage())%>
			 <div id="isPendThingtr" style="display:none">
			  <%=SystemEnv.getHtmlLabelName(-11008,user.getLanguage())%>
			 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			 <INPUT class=InputStyle checked tzCheckbox="true" type=checkbox    id="isPendThing"  name='isPendThing' >
			 </div>
			 <td	class="field" colspan="3">
			  <INPUT class=InputStyle checked tzCheckbox="true" type=checkbox    id="isPendThing"  name='isPendThing' >
			 </td>
		     ...
		     <tr class="Spacing" style="height: 1px !important;" jQuery1425871165066="63">
			  ...
			  <tr >
			 <td height="44" class="fieldName"	width="160px"><!-- 下啦项 -->
			 	<%=SystemEnv.getHtmlLabelName(-11009,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(19467,user.getLanguage())%>
			 </td>
			 ...
			 <td height="44" class="field"	 colspan="2"  ><!-- 选择流程 -->
			 <span id="rangeSpan" >
...
function changemainsub(obj)
{
	...
	var beagenterid = $(obj).find("option:selected").val();
	if(beagenterid==0){
...
function doSave(obj){
	...
	if(beginDateTime.valueOf()>endDateTime.valueOf()) {
		Dialog.alert("<%=SystemEnv.getHtmlLabelName(16722,user.getLanguage())%>");
		return;
	}
...
function goBack() {
	document.location.href="/seahonor/attend/agent/wfAgentStatistic.jsp" //xwj for td3218 20051201
}
...
function wfoverlapAgent(){
		...
		jQuery.ajax({
			 type: "POST",
			 url: "/seahonor/attend/agent/WFAgentConditeAjax.jsp",
...
function onShowHrmBeAgent(spanname,inputename){
  ...
  if (results){
	  if (results.id!="") {
	    ...
	    location.href = "/seahonor/attend/agent/wfAgentAdd.jsp?changeBeagentId="+results.id;
	  }else{ 
