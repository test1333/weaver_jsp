...
<HTML>
    ...
    String isindate = Util.null2String(request.getParameter("isindate"));
	...
	<BODY>
		...
		<div class="zDialog_div_content" id="differencehandle" name="differencehandle">
				...
				<wea:item>是否在保质期内</wea:item>
				<wea:item>
				<select class="e8_btn_top middle" name="isindate" id="isindate"> 
				<option value="" <%if("".equals(isindate)){%> selected<%} %>>
					<%=""%></option>
				<option value="0" <%if("0".equals(isindate)){%> selected<%} %>>
					<%="是"%></option>
				<option value="1" <%if("1".equals(isindate)){%> selected<%} %>>
					<%="否"%></option>
				</select>
				</wea:item>

				<wea:item>维修估价</wea:item>
				<wea:item>
				<wea:required id="nameimage2" required="true" value="">
				<% if(canedit) {%>
					<INPUT  maxLength=50 size=45 name="currency" style="width: 90px;" id = "currency"
						value="<%=currency%>" 
				...
				<wea:item>维修天数</wea:item>
				<wea:item>
				<wea:required id="nameimage3" required="true" value="">
				<% if(canedit) {%>
					<INPUT  maxLength=50 size=45 name="days" style="width: 90px;" id = "days"
						value="<%=days%>" 
						onchange='checkinput("days","nameimage3")'
						onblur="noNumbers(event,2)">
					<%} else {%><%=Util.toScreen(RecordSet.getString(2),user.getLanguage())%><%}%>
				</wea:required>
				</wea:item>

				<wea:item>维修内容</wea:item>
				<wea:item>
				<wea:required id="nameimage4" required="true" value="">
				<% if(canedit) {%>
				<TEXTAREA class=InputStyle ROWS="4" STYLE="width:65%" name="content" id="content" value="<%=content%>" onchange='checkinput("content","nameimage4")'></TEXTAREA>
				<%} else {%><%=Util.toScreen(RecordSet.getString(2),user.getLanguage())%><%}%>
			    </wea:required>
				</wea:item>
		...
		String sqlWhere = " where notinstock is null and IsReject is null";
		String orderby = "" ;
		String tableString = "";
	...
	jQuery(differencerecord).html("<td class='interval'><span class='groupbg'></span><span class='e8_grouptitle'>处理记录</span></td><td class='interval' colspan='2' style='text-align:right;'><span class='toolbar'></span></td>");
	   ...
	   diag_vote.close();
        ...
        function save(){
        if(check_form(report,'currency,days,content')){
        	var idkeyx = <%=idkey%>;
        	document.report.idkey.value = idkeyx ;
       		document.report.action = "DifferHandleRecord.jsp?type=7";
        ...
        function deletex(){
        	...
        	document.report.handleRecord_main.value = handleRecord_main;
       		document.report.action = "DifferHandleDelete.jsp?type=7";
       		document.report.submit();
        }
