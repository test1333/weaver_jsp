...
<HTML>
	...
	<BODY>
		...
		while(rs.next()){
				 ...
				 tmp_goodsname = Util.null2String(rs.getString("gname"));
		...
		<%
		sql_2 = "select * from formtable_main_45 where requestid="+requestid;	
		rs.executeSql(sql_2);
			while(rs.next()){
		...
		" formtable_main_45_dt1 f2 left join  formtable_main_45 f1 on f1.id = f2.mainid where f2.mainid = "+mainID+" and charindex(f2.name,'"+tmp_goodsname+"') >0 ";	
		rs.executeSql(sql_3);
		log.writeLog("sql_3="+sql_3); 
			while(rs.next()){
				 ...
				 String sql = " select * from formtable_main_50 where requestId = "+requestid+"and supplier ="+supplierID+" and IndexNum = "+index+" ";
				 rs1.executeSql(sql);
				 if(rs1.next()){
