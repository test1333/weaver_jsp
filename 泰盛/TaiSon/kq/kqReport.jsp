...
<html>
	<head>
		...
		<script language="javascript">
			...
			function showdata(){
				var ajax=ajaxinit();
				ajax.open("POST", "kqReportResultData.jsp", true);
			...
			function exportExcel(){
				if(document.frmMain.fromDate.value !="" && document.frmMain.fromDate.value !=""){
					if(document.frmMain.fromDate.value > document.frmMain.toDate.value) {
						window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(16721,user.getLanguage())%>") ;
					}else if("<%=hasViewAllRight%>"=="true"&&$GetEle("subCompanyId").value==""&&$GetEle("departmentId").value==""&&$GetEle("resourceId").value==""){
						window.top.Dialog.alert("<%=SystemEnv.getHtmlLabelName(18214,user.getLanguage())+SystemEnv.getHtmlLabelName(15774,user.getLanguage())%>");
					}else{
						document.getElementById("excels").src = "kqReportExcel.jsp?cmd=HrmScheduleDiffReport&tnum=20078&fromDate="+document.frmMain.fromDate.value+"&toDate="+document.frmMain.toDate.value+"&status=8&resourceId="+$GetEle("resourceId").value+"&departmentId="+$GetEle("departmentId").value+"&subCompanyId="+$GetEle("subCompanyId").value;
					}
				}else{
					window.top.Dialog.alert("请选择日期范围");
				}
			}
		</script>
	</head>
	<body onload="<%=isself.equals("1")?"showdata()":""%>">
		...
		<form id=frmMain name=frmMain method=post action="/TaiSon/kq/kqReport.jsp">
