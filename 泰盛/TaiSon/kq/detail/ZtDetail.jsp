...
<link rel="stylesheet" type="text/css" href="/js/jquery-autocomplete/browser_wev8.css" />
	...
	<BODY>
		...
		<FORM id=report name=report action="/TaiSon/kq/detail/ZtDetail.jsp" method=post>
		...
		<%
		String backfields = "a.id,b.lastname as resourceName,c.departmentName,case when b.status='1' then '正式' when b.status='0' then '试用' when b.status='5' then '离职' else '正式' end as statusName,atten_day as signDate,am_begin_time||'-'||pm_end_time as scheduleName,atten_end_time as signTime,'' as clientAddress,'' as addr ";
		String fromSql  =  " from  uf_all_atten_info a ,hrmresource b,hrmdepartment c ";
		String sqlWhere =  " 1=1 and a.emp_id=b.id and b.departmentid=c.id and  atten_day <='"+toDate+"' and atten_day>='"+fromDate+"' and nvl(early_leave_times,0)>0";
		...
		operateString+
		"			<head>";
				tableString +="<col width=\"13%\" text=\""+SystemEnv.getHtmlLabelName(124,user.getLanguage())+"\" column=\"departmentName\" orderkey=\"departmentName\"  />"+ 
							"<col width=\"8%\" text=\""+SystemEnv.getHtmlLabelName(413,user.getLanguage())+"\" column=\"resourceName\" orderkey=\"resourceName\"  />"+ 
							"<col width=\"8%\" text=\""+SystemEnv.getHtmlLabelName(602,user.getLanguage())+"\" column=\"statusName\" orderkey=\"statusName\"  />"+ 
							"<col width=\"12%\" text=\""+SystemEnv.getHtmlLabelName(97,user.getLanguage())+"\" column=\"signDate\" orderkey=\"signDate\"  />"+ 
							"<col width=\"20%\" text=\""+SystemEnv.getHtmlLabelName(125799,user.getLanguage())+"\" column=\"scheduleName\" orderkey=\"scheduleName\" />"+ 
							"<col width=\"10%\" text=\""+SystemEnv.getHtmlLabelName(20039,user.getLanguage())+"\" column=\"signTime\" orderkey=\"signTime\"  />"+ 
							"<col width=\"12%\" text=\""+SystemEnv.getHtmlLabelName(32531,user.getLanguage())+"\" column=\"clientAddress\" orderkey=\"clientAddress\"  />"+ 
							"<col width=\"16%\" text=\""+SystemEnv.getHtmlLabelName(81524,user.getLanguage())+"\" column=\"addr\" orderkey=\"addr\"  />"+ 
	...
	<script type="text/javascript">
		...
		function exportExcel(){
				document.getElementById("excels").src = "/TaiSon/kq/kqReportExcel.jsp?cmd=HrmScheduleDiffLeaveEarlyDetail&tnum=20089&fromDate=<%=fromDate%>&toDate=<%=toDate%>&resourceId=<%=resourceId%>&departmentId=<%=departmentId%>&subCompanyId=<%=subCompanyId%>";
		}
