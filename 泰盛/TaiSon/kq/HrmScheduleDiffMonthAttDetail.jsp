...
<html>
	<head>
		...
		<script type="text/javascript"> 
			...
			function getParam(){
				return "currentdate="+document.frmMain.currentdate.value+"&status=8&resourceId="+$GetEle("resourceId").value+"&departmentId="+$GetEle("departmentId").value+"&rstr="+randomString(10)+"&subCompanyId="+$GetEle("subCompanyId").value;
			}
			...
			function exportExcel(){
				...
				document.getElementById("excels").src = "/TaiSon/kq/kqReportExcel.jsp?cmd=HrmScheduleDiffMonthAttDetail&tnum=82801&"+getParam();
			}
			
			function showdata() {
				common.ajax("/TaiSon/kq//HrmScheduleDiffMonthAttData.jsp?"+getParam(), false, function(result){
					document.all("showdatadiv").innerHTML = result;
				});
			}
			
			function showWindow(rId, curDate){
				common.showDialog("/TaiSon/kq/detail/HrmDialogTab.jsp?_fromURL=hrmReport&cmd=HrmScheduleDiffMonthAttDateDetail&resourceId="+rId+"&curDate="+curDate, "<%=SystemEnv.getHtmlLabelNames("15880,17463",user.getLanguage())%>");
			}
		</script>
	</head>
