...
var startTime = "#field16245";
var endTime = "#field16247"; 
jQuery(document).ready(function(){ 
	jQuery(endData).attr('disabled','disabled'); 
    getJsrq();
	//alert(111);
    ...
    checkCustomize = function () {
       ...
       // jQuery(endData).attr('disabled',false); 
		jQuery(endData).removeAttr('disabled');
        //开始日期校验
        if(getDetailDate(jQuery(startData).val())){
            alert("温馨提示：外出申请单开始日期不能晚于 2019-06-01");
            return false;
        }
        //结束日期校验
        if(getDetailDate(jQuery(endData).val())){
            alert("温馨提示：外出申请单结束日期不能晚于 2019-06-01");
            return false;
        }
        return true;
    }
